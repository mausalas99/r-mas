...
test('curated 7.1.0 highlights are current default', async () => {
  const mod = await import(
    path.join(ROOT, 'public/js/features/settings-help/release-notes-curated.mjs')
  );
  const highlights = mod.RELEASE_NOTES_HIGHLIGHTS['7.1.0'];
  ...
  assert.ok(joined.includes('Guardia') || joined.includes('entrega') || joined.includes('delta'));
  assert.equal(mod.RELEASE_NOTES_HIGHLIGHTS_DEFAULT, mod.RELEASE_NOTES_HIGHLIGHTS['7.1.0']);
});
