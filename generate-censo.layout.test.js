...
test('4 pacientes densos caben en una página sin columnas vacías', async () => {
  var rows = [
    makeGuardiaRow(1, {
      ...
      pacienteMeta: '8100018-6\n83 años',
    }),
    makeGuardiaRow(2, {
      pacienteNombre: 'NOMBRE SINTETICO UNO',
      pacienteMeta: '8100019-0\n33 años',
      labs:
        makeGuardiaRow(2).labs +
        '\nGASES pH 7.36 pCO2 35* pO2 51* Lactato 0.6 Bica 19.8* AG 8.3 AGc 12.8* Delta-Delta 0.2',
    }),
    makeGuardiaRow(3, {
      pacienteNombre: 'FICTICIO SINTETICO EJEMPLO CASOZa',
      pacienteMeta: '9000104-0\n29 años',
    }),
    makeGuardiaRow(4, {
      pacienteNombre: 'VERONICA HERNANDEZ HILARIA',
      pacienteMeta: '8100021-5\n48 años',
      labs:
...
test('nombre de paciente se envuelve sin elipsis', async () => {
  ...
  var lines = wrapPacienteCellLines(
    font,
    fontBold,
    'NOMBRE SINTETICO UNO\n8100019-0\n33 años',
    innerW,
    0
  );
  assert.ok(lines.length >= 2);
  assert.match(lines.join(' '), /NOMBRE SINTETICO UNO/);
  lines.forEach(function (ln) {
    assert.doesNotMatch(ln, /…$/);
  });
});
