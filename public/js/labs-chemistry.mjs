...
function findTroponinaAnchor_(tUp) {
  var patterns = [
  ...
  for (var i = 0; i < patterns.length; i++) {
    var idx = tUp.indexOf(patterns[i]);
    if (idx !== -1) return idx;
  }
  return -1;
}
...
export function parseTroponina_(textoBruto) {
  ...
  if (troData.valor === '---') return '';

  var idx = findTroponinaAnchor_(tUp);
  var sub = idx >= 0 ? texto.substring(idx, idx + 320) : '';
  var qual = troponinaQualFromSub_(sub);
  var v = parseFloat(String(troData.valor).replace(',', '.'));
  var maxRef =
    troData.max != null && troData.min != null && troData.max > troData.min
      ? troData.max
      : TROPONINA_HS_NORMAL_MAX_NG_L;
  var minRef = troData.min != null ? troData.min : 0;
  var flagged =
    qual === 'indet' ||
    qual === 'pos' ||
    (isFinite(v) && (v > maxRef || v < minRef));

  var out = fmt(marcarSegunRango(troData.valor, minRef, maxRef));
