...
async function refreshOnboardingHost() {
  const { refreshMainClinicalOnboardingIfNeeded } = await import("/js/chunks/clinical-onboarding-main-FDFCLHPQ.js");
...
async function handleUsernameStepSubmit(ev) {
  ...
  if (needsClaim) {
    const { assertLanRoomForUsernameRegister } = await import("/js/chunks/clinical-profile-lan-sync-XZ5LLGM2.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  if (shiftPin && !isClinicalLocalOnlyMode()) {
    const { connectLanWithShiftPin } = await import("/js/chunks/lan-shift-pin-connect-TRVOZWZF.js");
  ...
  } = await import("/js/chunks/clinical-profile-lan-sync-XZ5LLGM2.js");
  ...
  if (!localOnly && !lanPush.ok && isLanProfileNeedsConnectCode(lanPush.code)) {
    ...
    const rot = await import("/js/chunks/clinical-rotation-entry-F5PLFRT4.js");
    rot.syncClinicalRotationEntryChrome();
    const { refreshMainClinicalOnboardingIfNeeded: refreshMainClinicalOnboardingIfNeeded2 } = await import("/js/chunks/clinical-onboarding-main-FDFCLHPQ.js");
...
async function renderOnboardingPanelInto(host) {
  ...
  if (!userId) {
    ...
    const { buildOnboardingSessionBlockHtml } = await import("/js/chunks/clinical-onboarding-main-FDFCLHPQ.js");
  ...
  if (!needsProfileOnboarding()) {
    const { hideMainClinicalOnboarding } = await import("/js/chunks/clinical-onboarding-main-FDFCLHPQ.js");
    hideMainClinicalOnboarding();
    if (host.closest("#clinical-teams-panel-body")) {
      const { renderClinicalTeamsPanel } = await import("/js/chunks/clinical-teams-DTE66OAL.js");
...
//# sourceMappingURL=/js/chunks/chunk-AMN7F4XV.js.map
