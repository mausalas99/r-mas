...
import {
  lanMutationRegistry
} from "/js/chunks/chunk-TNTHAQJD.js";
import {
...
} from "/js/chunks/chunk-MSBFOYVD.js";
import {
  bundledWardShiftPin,
...
import {
  LanClient
} from "/js/chunks/chunk-LX374JRN.js";
import {
...
} from "/js/chunks/chunk-QKS27SZP.js";
import {
  RoomSyncPhase,
  clearRoomSyncPhase,
  getRoomSyncPhase,
...
} from "/js/chunks/chunk-BCNABZWJ.js";
...
function rebuildGasesFromResults_(rows) {
  ...
  var esc5 = pickBestSectionLine_(rows, "ESC");
  var pfhs = pickBestSectionLine_(rows, "PFHS");
  var na = valueFromSectionLine_(qs, "Na") || valueFromSectionLine_(esc5, "Na") || values.Na;
  var cl = valueFromSectionLine_(qs, "Cl") || valueFromSectionLine_(esc5, "Cl");
...
function procesarLabs(textoBruto) {
  ...
  if (!esSoloGaso) {
    ...
    var esc5 = parseESC_(textoQS);
    if (esc5) resLabs.push(esc5);
...
function normalizeFechaLabHistory(fechaRaw) {
  ...
  var t2 = String(fechaRaw).trim();
  var mEn = t2.match(/([A-Za-z]{3})\s+(\d{1,2})\s+(\d{4})/i);
  if (mEn) {
    var mon = TEND_MESES_MAP[mEn[1].toLowerCase().slice(0, 3)];
    if (mon) return mEn[2].padStart(2, "0") + "/" + mon + "/" + mEn[3];
  }
  var mNum = t2.match(/^(\d{1,2})[\/\-](\d{1,2})(?:[\/\-](\d{2,4}))?$/);
  if (mNum) {
...
function normalizeHoraLabHistory(horaRaw) {
  ...
  var t2 = String(horaRaw).trim();
  if (!t2) return "";
  var m = t2.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?/);
  if (!m) return "";
...
function parseFechaLabToMs(fechaStr, horaStr) {
  ...
  var t2 = String(fechaStr).trim();
  if (t2 === "Anterior") return null;
  var mEn = t2.match(/([A-Za-z]{3})\s+(\d{1,2})\s+(\d{4})/i);
  if (mEn) {
...
function liveSyncDeletePatchesFromEntityMap(entityMap) {
  const patches = [];
...
function attachTodosMapToPatientEntries(entries, todosMap, todoTouchedPatientIds) {
  ...
  for (const entry of entries) {
    ...
    if (Object.prototype.hasOwnProperty.call(map, id)) {
      const list = map[id];
      entry.todos = Array.isArray(list) ? list.map((t2) => ({ ...t2 })) : [];
...
function adherenceStats(days, notAdmin) {
  const indicated = [];
  const missed = [];
  const dmap = days || {};
  const na = notAdmin || {};
...
function formatFreqShort(raw) {
  const t2 = trimStr(raw).toUpperCase();
  if (!t2) return "\u2014";
...
function isDayHeaderToken(cell) {
  const t2 = trimStr(cell);
  if (!/^\d{1,2}$/.test(t2)) return false;
...
function parseDayCell(raw) {
  const t2 = trimStr(raw);
  if (!t2 || t2 === "-" || t2 === "0") return 0;
  const n = parseInt(t2, 10);
  if (n === 2) return 2;
  if (n >= 1) return 1;
  return 0;
...
function mergePatientEntry(a, b) {
  ...
  const medPharmProfile = compareIso(medPharmTimestamp(a.medPharmProfile), medPharmTimestamp(b.medPharmProfile)) >= 0 ? a.medPharmProfile ? structuredClone(a.medPharmProfile) : null : b.medPharmProfile ? structuredClone(b.medPharmProfile) : null;
...
function enqueueLocal(roomId, item) {
  ...
  const kind = item.kind === "delta" ? "delta" : item.kind === "patch" ? "patch" : item.kind === "clinical_ops" ? "clinical_ops" : "bundle";
  list.push({
    kind,
...
async function enqueueOutbox(roomId, item) {
  const api2 = getApi();
  if (api2?.dbLanOutboxEnqueue) {
    return api2.dbLanOutboxEnqueue({
      roomId,
      kind: item.kind,
      payload: item.payload
    });
...
async function drainOutbox(roomId) {
  const api2 = getApi();
  if (api2?.dbLanOutboxDrain) {
    const res = await api2.dbLanOutboxDrain({ roomId });
...
async function outboxSize(roomId) {
  const api2 = getApi();
  if (api2?.dbLanOutboxCount) {
    const res = await api2.dbLanOutboxCount({ roomId });
...
function buildPermanentMobileJoinUrl(hostUrl, teamCode) {
  const base = String(hostUrl || "").trim().replace(/\/+$/, "");
...
function buildMobilePairingFromSearch(search, origin) {
  ...
  const sharer = user || name || rank || salaParam ? { user, name, rank, sala: salaParam || void 0 } : void 0;
...
function isMobileWeb() {
  var g2 = mobileRuntimeGlobal();
  if (!g2) return false;
  return !!(g2.__RPC_MOBILE_WEB__ || typeof document !== "undefined" && document.documentElement && document.documentElement.classList.contains("rpc-mobile-web") || isMobileWebModePersisted());
}
...
function syncGuardiaModeHeaderChip() {
  ...
  chip.title = isGuardiaMode() ? "Salir de Guardia" : "Abrir Guardia \u2014 censo, entrega y turno activo";
  chip.setAttribute(
    "aria-label",
...
function toggleGuardiaMode() {
  if (isGuardiaMode()) {
    void import("/js/chunks/entrega-roster-panel-GFKCIADI.js").then(({ deactivateTurnoActivo: deactivateTurnoActivo2, closeEntregaRosterPanel: closeEntregaRosterPanel2 }) => {
      deactivateTurnoActivo2();
      closeEntregaRosterPanel2();
    });
    void import("/js/chunks/guardia-phase-bar-IOMYUBWQ.js").then(({ teardownGuardiaPhaseBar: teardownGuardiaPhaseBar2 }) => {
      teardownGuardiaPhaseBar2();
    });
    setUiDensity("normal");
    return;
  }
...
function setUiDensity(mode) {
  ...
  void import("/js/chunks/clinical-rotation-entry-F5PLFRT4.js").then((mod) => {
...
var GUARDIA_MODE_LABEL_OFF = "Alcance completo";
var GUARDIA_MODE_LABEL_ON = "Solo mis entregas";
var GUARDIA_MODE_TOGGLE_TITLE = "Filtro de la grilla: pacientes en tu alcance o solo los que te entregaron (independiente del bot\xF3n Entrega).";
...
function normalizeVasopressor(vas) {
  const active = vas != null && "active" in vas ? !!vas.active : !!(vas?.agent || vas?.dose || vas?.rate);
...
var UnifiedPatientGridBoard = class {
  ...
  drawCensusGrid(patients2, guardiasMap, userRank = "R1") {
    ...
    if (userRank === "R4") {
      ...
      for (const sector of R4_GUARDIA_SECTOR_ORDER) {
        const sectorPatients = patients2.filter((p) => {
      ...
      const otherPatients = patients2.filter((p) => p?.id && !assignedIds.has(p.id));
  ...
  compileChip(p, g2) {
    ...
    const vitalsSpec = meta?.pendientes_json ? normalizePendientesJson(meta.pendientes_json).vitalsPlan?.frequency ?? meta?.vitals_frequency ?? null : meta?.vitals_frequency ?? null;
...
var LAN_PROFILE_NEEDS_CONNECT_MSG = "Perfil guardado en esta Mac. Para aparecer en la guardia, con\xE9ctate a la sala del equipo: pega el enlace de invitaci\xF3n o escanea el anfitri\xF3n.";
function isLanProfileNeedsConnectCode(code) {
  const c = String(code || "");
...
async function ensureLiveSyncRoomForUsernameRegister(opts = {}) {
  const lan = await import("/js/chunks/lan-sync-KAQL434D.js");
...
async function seedDevPeerLanConfigIfNeeded() {
...
async function flushClinicalProfileToLan(opts = {}) {
  const { isClinicalLocalOnlyMode: isClinicalLocalOnlyMode2 } = await import("/js/chunks/clinical-settings-CMYQ2OQU.js");
  if (isClinicalLocalOnlyMode2()) {
  ...
  const pushMod = await import("/js/chunks/push-2R3WG4YZ.js");
  ...
  try {
    const { lanClient: lanClient2 } = await import("/js/chunks/lan-client-FNTPRIGU.js");
    if (lanClient2 && !lanClient2.connected) lanClient2.connectSyncChannel();
  } catch (_e) {
  }
...
async function ensureClinicalPanelSession() {
  ...
  try {
    const { applyClinicalDbUnlockCompletion } = await import("/js/chunks/db-unlock-OFFY7CYM.js");
...
async function publishClinicalTeamsToLan() {
  try {
    const mod = await import("/js/chunks/lan-sync-KAQL434D.js");
...
function renderLanUsersDirectoryEntryHtml(user) {
  ...
  return `
    <div class="clinical-teams-lan-users-entry">
      ...
      <p class="clinical-teams-section-desc">Crea equipos vac\xEDos y asigna integrantes desde aqu\xED (ventana aparte).</p>
    </div>`;
}
...
var lanDirectoryCollapsedRanks = /* @__PURE__ */ new Set();
var LAN_DIRECTORY_LAN_PULL_MIN_MS = 3e4;
var LAN_DIRECTORY_IPC_MIN_MS = 4e3;
var lastLanDirectoryFingerprint = "";
var lanDirectoryLanPullLastAt = 0;
var lanDirectoryIpcLastAt = 0;
var lanDirectoryFreezeAutoRefresh = false;
...
function renderLanUsersModalBodyHtml(users, teams, opts = {}) {
  ...
  if (!list.length) {
    return `<p class="clinical-teams-empty">A\xFAn no hay otros @usuario en esta Mac. Cada residente debe conectarse a tu LAN, abrir <strong>\u21C4 \u2192 Unirse</strong> en la misma sala y guardar <strong>Mi rotaci\xF3n \u2192 Guardar perfil</strong>. Luego los asignas al equipo desde aqu\xED (no hace falta que ya tengan equipo).</p>`;
  }
  ...
  const rankSections = LAN_USER_RANK_ORDER.map((rank) => {
    ...
    return `<details class="clinical-lan-rank-group"${openAttr} data-lan-rank-group="${escapeAttr2(rank)}">
  ...
  const otherSection = other.length ? `<details class="clinical-lan-rank-group"${isLanRankGroupCollapsed("Otros") ? "" : " open"} data-lan-rank-group="Otros">
        <summary class="clinical-lan-rank-group-summary">
  ...
  return `
    <p class="clinical-lan-users-modal-lead">${list.length} usuario${list.length === 1 ? "" : "s"} \xB7 <strong>todas las salas</strong> en esta Mac (no filtra por tu sala). Asigna a cualquier equipo activo.
...
Desaparecer\xE1 del directorio LAN. Las dem\xE1s R+ en la misma sala \u21C4 lo quitar\xE1n al sincronizar.`
  ...
  const { isBenignLanPushSkipCode: isBenignLanPushSkipCode2 } = await import("/js/chunks/clinical-profile-lan-sync-XZ5LLGM2.js");
...
function buildLanDirectoryFingerprint(users, teams) {
  const userPart = (users || []).map(
    (u) => `${String(u.user_id || "")}	${String(u.username || "")}	${String(u.rank || "")}	${String(u.clinical_name || "")}	${String(u.sala || "")}`
  ).sort().join("\n");
  const teamPart = (teams || []).map((t2) => {
    const members = (t2.members || []).map((m) => `${String(m.user_id || "")}:${String(m.sub_area_fraction || "")}`).sort().join(",");
    return `${String(t2.team_id || "")}	${members}`;
  }).sort().join("\n");
...
async function loadLanUsersDirectoryIntoHost(host, options = {}) {
  const now = Date.now();
  if (!options.forceIpc && lanDirectoryFreezeAutoRefresh && now - lanDirectoryIpcLastAt < LAN_DIRECTORY_IPC_MIN_MS && host.querySelector(".clinical-lan-rank-groups")) {
    return;
  }
  const api2 = dbApi2();
  ...
  if (!options.forceRender && fingerprint === lastLanDirectoryFingerprint && host.querySelector(".clinical-lan-rank-groups")) {
    const title2 = document.getElementById("clinical-lan-users-title");
    if (title2) title2.textContent = `Directorio de usuarios LAN (${users.length})`;
    return;
  }
...
async function pullLanDirectoryFromHostIfDue(options = {}) {
  ...
  try {
    const lanMod = await import("/js/chunks/lan-sync-KAQL434D.js");
    if (typeof lanMod.refreshLanClinicalDirectoryFromRoom !== "function") return false;
    return !!await lanMod.refreshLanClinicalDirectoryFromRoom({ timeoutMs: 6e3 });
  } catch (_e) {
    return false;
  }
}
...
function renderCreateTeamFormStandard() {
  ...
  const serviceOptions = CLINICAL_TEAM_SERVICES.map(
    (svc) => `<option value="${escapeAttr2(svc)}" ${svc === defaultService ? "selected" : ""}>${escapeHtml2(svc)}</option>`
  ).join("");
  const rank = effectiveClinicalRank(clinicalSessionContext.user);
    ...
    <form id="clinical-team-create-form" class="clinical-teams-create-form">
      <div class="field-group" id="clinical-team-sala-group">
        ...
        <select id="clinical-team-create-sala" class="profile-input">
          <option value="">\u2014 Seleccionar sala \u2014</option>
          ${CLINICAL_SALAS.map(
    (s) => `<option value="${escapeAttr2(s)}" ${s === userSala ? "selected" : ""}>${escapeHtml2(s)}</option>`
  ).join("")}
        </select>
      </div>
      <div class="field-group">
      ...
      <p id="clinical-team-create-day-hint" class="clinical-teams-hint clinical-teams-create-cycle-hint">${escapeHtml2(defaultMeta.hint)}</p>
...
function renderTeamPatientCountLine(team) {
  ...
  if (onDevice <= 0 && assignedLan > 0) {
    const waiting = assignedLan === 1 ? "1 asignado en la red \u2014 sincronizando expediente\u2026" : `${assignedLan} asignados en la red \u2014 sincronizando expedientes\u2026`;
    return `<p class="clinical-teams-card-meta clinical-teams-card-patients">${escapeHtml2(waiting)}</p>`;
  }
  if (assignedLan > onDevice && assignedLan > 0) {
    ...
    return `<p class="clinical-teams-card-meta clinical-teams-card-patients">${escapeHtml2(visible)} \xB7 ${escapeHtml2(waiting)}</p>`;
  }
  const label = onDevice === 1 ? "1 paciente en censo" : `${onDevice} pacientes en censo`;
  return `<p class="clinical-teams-card-meta clinical-teams-card-patients">${escapeHtml2(label)}</p>`;
}
...
function renderDirectoryTeamCard(team, opts = {}) {
  ...
  return `
    <article class="clinical-teams-card clinical-teams-card--directory" data-team-id="${escapeAttr2(teamId)}">
      ...
      ${joinHint ? `<p class="clinical-teams-card-join-reason">${escapeHtml2(joinHint)}</p>` : ""}
      ${editPanel}
...
async function renderClinicalTeamsPanelInto(host, opts = {}) {
  ...
  if (!opts.skipLanPull) {
    ...
    if (!pulled) {
      void pullClinicalOpsFromLanRoom({ timeoutMs: 12e3 }).then((ok) => {
        const bd = document.getElementById("clinical-teams-backdrop");
  ...
  const { needsClinicalLanProfileGate: needsClinicalLanProfileGate2, ensureLanProfileGateDeviceReset } = await import("/js/chunks/clinical-settings-CMYQ2OQU.js");
  settings = ensureLanProfileGateDeviceReset(settings);
  const profileGatePending = needsClinicalLanProfileGate2(settings);
  ...
  const clinicalName = profileGatePending ? "" : escapeHtml2(user.clinical_name || "");
    ...
    <div class="clinical-teams-profile-panel clinical-teams-rank-section">
      ...
      <form id="clinical-profile-form" class="clinical-teams-create-form" novalidate>
        <div class="field-group">
          ...
          ${hintHtml("Usuario LAN (@usuario): min\xFAsculas, sin espacios \u2014 p. ej. drmendoza. No es tu nombre en guardia.")}
        </div>
        <div class="field-group">
    ...
    <section class="clinical-teams-section clinical-teams-section--more">
      ...
      <details class="clinical-teams-advanced-rotation">
        ...
        <div class="clinical-teams-advanced-rotation-body">
          <p class="clinical-teams-advanced-rotation-hint">Solo R4/Admin. Configura el calendario del ciclo o inicia una rotaci\xF3n nueva (archiva equipos y guardias del d\xEDa).</p>
          <div class="clinical-teams-advanced-rotation-actions">
            <button type="button" id="btn-rotation-config-open" class="btn-med-secondary" hidden>Configuraci\xF3n rotaci\xF3n\u2026</button>
  ...
  const { wireRenderedClinicalTeamsPanel: wireRenderedClinicalTeamsPanel2 } = await import("/js/chunks/teams-roster-interactions-N3JPMNL7.js");
...
function renderCreateTeamSectionHtml() {
  ...
  return `
    <section class="clinical-teams-section clinical-teams-section--create">
      <button type="button" id="btn-clinical-team-create-open" class="btn-save clinical-teams-create-open-btn">${escapeHtml2(openLabel)}</button>
...
async function resolveLanTeamMemberHintHtml(joinedTeams) {
  ...
  try {
    ...
    const roomId = typeof lan.getActiveLiveSyncRoomId === "function" ? String(lan.getActiveLiveSyncRoomId() || "").trim() : "";
    if (!roomId) {
    ...
    if (canDir) {
      return `<p class="clinical-teams-section-desc clinical-teams-lan-member-hint">Est\xE1s en sala \u21C4 pero el directorio a\xFAn no lista a otros. Cada Mac debe usar tu enlace/c\xF3digo LAN, <strong>Unirse</strong> en la misma sala y <strong>Guardar perfil</strong> con @usuario; despu\xE9s aparecen aqu\xED y t\xFA los asignas al equipo (no al rev\xE9s).</p>`;
    }
    return `<p class="clinical-teams-section-desc clinical-teams-lan-member-hint">En <strong>Integrantes</strong> ver\xE1s compa\xF1eros cuando el admin te asigne a un equipo desde el directorio LAN. Mientras tanto: \u21C4 \u2192 misma sala, @usuario guardado.</p>`;
  } catch (_e) {
    return "";
  }
}
...
async function renderDirectorySectionHtml(opts) {
  ...
  const cards = directory.map((team) => {
    ...
    if (team.joinEligible) {
      joinBtn = `<button type="button" class="btn-med-secondary clinical-teams-join-btn" data-team-id="${escapeAttr2(teamId)}">Unirme</button>`;
      if (team.joinWarning) {
...
function wireJoinButtons() {
  document.querySelectorAll(".clinical-teams-join-btn").forEach((btn) => {
    ...
    btn.addEventListener("click", async () => {
      ...
      if (slotWarn) {
        toast2(slotWarn, "warn");
      } else if (team?.joinWarning) {
        toast2(String(team.joinWarning), "warn");
      }
      const res = await api2.dbClinicalTeamsJoin({
        teamId,
        userId,
...
async function refreshTeamsUiAfterChange() {
  const { isLanDirectoryModalOpen: isLanDirectoryModalOpen2 } = await import("/js/chunks/teams-roster-lan-3UVJORKX.js");
  if (isLanDirectoryModalOpen2()) return;
  const { refreshClinicalPatientListForScope: refreshClinicalPatientListForScope2 } = await import("/js/chunks/clinical-access-runtime-LDG3LE6E.js");
  await refreshClinicalPatientListForScope2({ allowLanPull: false });
  import("/js/chunks/clinical-rotation-entry-F5PLFRT4.js").then((m) => m.syncClinicalRotationEntryChrome());
...
async function openClinicalTeamsPanel(opts = {}) {
  ...
  if (!sessionOk) {
    const mainMod = await import("/js/chunks/clinical-onboarding-main-FDFCLHPQ.js");
  ...
  try {
    if (!opts.skipProfileGate) {
      const { needsClinicalOnboarding } = await import("/js/chunks/clinical-onboarding-VL57BWTH.js");
      if (needsClinicalOnboarding()) {
        closeClinicalTeamsPanel();
        const { openMiRotacion } = await import("/js/chunks/clinical-rotation-entry-F5PLFRT4.js");
  ...
  try {
    const { stopLanAutoDiscovery: stopLanAutoDiscovery2 } = await import("/js/chunks/panel-D42ZCYTB.js");
    stopLanAutoDiscovery2();
  } catch (_disc) {
  }
  try {
...
function closeClinicalTeamsPanel() {
  ...
  import("/js/chunks/panel-D42ZCYTB.js").then((m) => {
...
async function handleProfileFormSubmit(ev) {
  ...
  if (usernameWillChange) {
    const { assertLanRoomForUsernameRegister: assertLanRoomForUsernameRegister2 } = await import("/js/chunks/clinical-profile-lan-sync-XZ5LLGM2.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  const { flushClinicalProfileToLan: flushClinicalProfileToLan2, LAN_PROFILE_PUSH_FAILED_MSG: LAN_PROFILE_PUSH_FAILED_MSG2, isBenignLanPushSkipCode: isBenignLanPushSkipCode2 } = await import("/js/chunks/clinical-profile-lan-sync-XZ5LLGM2.js");
  const lanPush = await flushClinicalProfileToLan2({ sala });
  if (!lanPush.ok && !isBenignLanPushSkipCode2(lanPush.code)) {
  ...
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
  await refreshTeamsUiAfterChange();
  void import("/js/chunks/lan-sync-KAQL434D.js").then((mod) => {
    if (typeof mod.pushClinicalOpsLanNow === "function") void mod.pushClinicalOpsLanNow();
  }).catch(() => {
  });
  void import("/js/chunks/patients-BOB2VPYS.js").then((m) => m.renderPatientList()).catch(() => {
  });
}
...
async function handleCreateTeamSubmit(ev) {
  ...
  if (elevated) {
    const res2 = await api2.dbClinicalTeamsCreate({
      name,
  ...
  if (teamId && typeof api2.dbClinicalTeamsMemberAdd === "function") {
...
async function resolveTeamIdForInviteInput(codeOrId) {
  ...
  const api2 = dbApi2();
  if (!teamId && api2 && typeof api2.dbClinicalTeamResolveCode === "function") {
...
async function joinTeamById(teamId, subAreaFraction) {
  ...
  if (filterJoinedTeams(clinicalSessionContext.teams, clinicalSessionContext.user).some(
    ...
    const { openClinicalTeamsPanel: openClinicalTeamsPanel2 } = await import("/js/chunks/teams-roster-GP2U75AQ.js");
  ...
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
  await publishClinicalTeamsToLan();
  const { refreshTeamsUiAfterChange: refreshTeamsUiAfterChange2 } = await import("/js/chunks/teams-roster-GP2U75AQ.js");
...
async function redirectLanInviteFromTeamJoinField(raw) {
  const text = String(raw || "").trim();
  if (!text) return false;
  const { openConnectionDropdown: openConnectionDropdown2, joinLanFromInviteUi: joinLanFromInviteUi2 } = await import("/js/chunks/lan-sync-KAQL434D.js");
  if (typeof openConnectionDropdown2 === "function") openConnectionDropdown2();
  ...
  toast2(
    "Ese enlace es de conexi\xF3n LAN (\u21C4), no de equipo. Conectando desde Conexi\xF3n guardia\u2026",
    "info"
  );
  if (typeof joinLanFromInviteUi2 === "function") joinLanFromInviteUi2();
  return true;
}
...
function loadRoster() {
  return import("/js/chunks/teams-roster-GP2U75AQ.js");
}
...
function wireClinicalTeamsControls() {
  void import("/js/chunks/teams-roster-modal-chrome-UC3CQPCF.js").then((m) => m.wireClinicalTeamsModalChrome());
  if (teamsControlsWired) return;
  teamsControlsWired = true;
  import("/js/chunks/clinical-rotation-entry-F5PLFRT4.js").then((mod) => {
...
function buildHandoffPanelMarkup(ctx, isCritical) {
  ...
  return `
    ...
    <div class="field-group entrega-handoff-notes">
      ...
      <textarea id="entrega-handoff-notes" class="profile-input entrega-handoff-notes-input" maxlength="240" rows="2" placeholder="Antecedentes relevantes para la guardia\u2026">${escapeHtml4(norm.notes)}</textarea>
    </div>`;
}
...
function mountEntregaHandoffPanel(handoffContext, opts = {}) {
  ...
  const statusSlot = document.getElementById("entrega-clinical-status-slot");
  if (statusSlot) statusSlot.innerHTML = buildClinicalStatusMarkup(draftHandoffContext);
...
var LAN_ON_CALL_HOST_TIER = 1e6;
function resolveLocalProgramAdmin(settings = readRpcSettings()) {
...
function buildLocalLanHostMeta(settings = readRpcSettings()) {
...
function isClinicalRankConfiguredForLan(settings = readRpcSettings()) {
  const rank = String(settings?.clinicalRank || "").trim();
  if (!rank) return false;
  if (needsClinicalLanProfileGate(settings)) return false;
  return true;
}
function canLocalMacBeLanHost(meta) {
...
function lanHostRankPriority(meta) {
  if (!meta) return 0;
...
var _diskHostMeta = null;
function resolveLocalOnCallGuardia() {
  try {
    const user = clinicalSessionContext?.user;
    ...
    return userIsOnCallForLanHost(uid, rank, teams, /* @__PURE__ */ new Date(), salaGuardiaToday);
  } catch (_e) {
    return false;
  }
}
...
function mergeSalaGuardiaTodayRows(teams, salaGuardiaToday) {
  ...
  for (const t2 of teams || []) {
    const tid = String(t2.team_id || "");
    if (!tid || seen.has(tid)) continue;
    const uid = t2?.guardia_today?.user_id;
    if (!uid) continue;
    rows.push({
      team_id: tid,
      user_id: String(uid),
      declared_at: t2.guardia_today.declared_at
    });
    seen.add(tid);
  }
  return rows;
...
function shouldPromptGuardiaHoy(ctx) {
  ...
  if (rank === "R1") {
    const joined = getJoinedTeams(teams, userId).filter((t2) => String(t2.sala || "") === sala);
    const isCovering = onCall.some((r) => String(r.user_id) === userId);
    const hasR1Team = joined.some(
      (t2) => (t2.members || []).some((m) => String(m.user_id) === userId && m.rank === "R1")
    );
...
function openGuardiaHoyModal(ctx) {
  ...
  const rows = salaTeams.map((team) => {
    ...
    const defaultTeam = rank === "R1" && r1Members.some((m) => String(m.user_id) === userId);
    return `
        <div class="guardia-hoy-team-row" data-team-id="${String(team.team_id)}">
          ...
          ${defaultTeam ? `<button type="button" class="btn-med-secondary guardia-hoy-self-btn" data-team-id="${String(
      team.team_id
    )}">Activar guardia hoy (yo)</button>` : ""}
        </div>`;
  }).filter(Boolean).join("");
  body.innerHTML = rows || '<p class="guardia-hoy-empty">No hay equipos R1 en esta sala. Puedes continuar y elegir R1 en cada paciente.</p>';
  bd.classList.add("open");
  bd.setAttribute("aria-hidden", "false");
  return new Promise((resolve) => {
    let selfBusy = false;
    ...
    async function persistGuardiaSelections(selects) {
      ...
      if (!api2?.dbClinicalTeamsGuardiaSet) {
        toast4("Base cl\xEDnica no disponible.", "error");
      ...
      for (const sel of list) {
        ...
        if (!res?.ok) {
          toast4(res?.error || "No se guard\xF3 la guardia.", "error");
          return { ok: false, activated: false };
        }
        activated = true;
      }
    ...
    body.querySelectorAll(".guardia-hoy-self-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        void (async () => {
          ...
          if (!sel) {
            toast4("No se encontr\xF3 el equipo.", "error");
            return;
          }
          sel.value = userId;
          selfBusy = true;
...
function rowContextSummary(g2) {
  if (!g2?.pendientes_json) return null;
  const doc = normalizePendientesJson(g2.pendientes_json);
  const ctx = normalizeHandoffContext(doc.handoffContext);
  const summary = handoffContextSummary(ctx);
  return summary === "Sin resumen cl\xEDnico" ? null : summary;
}
function rowIcons(g2) {
  if (!g2?.pendientes_json) return "";
  const doc = normalizePendientesJson(g2.pendientes_json);
  const ctx = normalizeHandoffContext(doc.handoffContext);
  const flags = [];
...
function rowStatus(g2) {
  if (!g2?.pendientes_json) return "";
  const doc = normalizePendientesJson(g2.pendientes_json);
  const ctx = normalizeHandoffContext(doc.handoffContext);
  return ctx.clinicalStatus || "";
}
function rowIsCriticalOrUnstable(g2) {
  const status = rowStatus(g2);
  return status === "critical" || status === "unstable" || !!g2?.is_critical;
}
...
function openEntregaRosterPanel(settings) {
  ...
  const censusPatients = patients.filter((p) => p && p.id && !p.isDemo && !p.archived).map((p) => ({ ...mapPatientForGuardiaGrid(p), _raw: p }));
  ...
  function renderRow(p) {
    const g2 = guardiasMap.get(p.id);
    const summary = rowContextSummary(g2);
    const icons = rowIcons(g2);
    const status = rowStatus(g2);
  ...
  host.querySelectorAll(".roster-row").forEach((row) => {
    ...
    const open = () => {
      const g2 = guardiasMap.get(patientId);
      ...
      openModal({
        patientId,
        guardiaId: g2?.guardia_id ? String(g2.guardia_id) : void 0,
  ...
  document.getElementById("roster-btn-cancel")?.addEventListener("click", () => {
    void (async () => {
      ...
      const { endEntregaPhase: endEntregaPhase2 } = await import("/js/chunks/clinical-entrega-2D6GLICJ.js");
      endEntregaPhase2();
...
function openEntregaModal(opts) {
  ...
  if (select) {
    ...
    const coverHint2 = document.getElementById("entrega-covering-hint");
    if (hideR1Picker) {
      select.disabled = true;
      select.removeAttribute("required");
      if (coverHint2) {
        coverHint2.textContent = "";
        coverHint2.classList.add("hidden");
      }
    } else {
      select.disabled = false;
      select.setAttribute("required", "");
      if (coverHint2) {
        coverHint2.textContent = "Residente de guardia que asumir\xE1 la cobertura nocturna de este paciente.";
      }
    }
  }
  if (teamSelect) {
...
function resolveR1GuardiaCovering(teams, users, sala, now = /* @__PURE__ */ new Date(), salaGuardiaToday = []) {
...
async function beginEntregaPhaseFlow(opts = {}) {
  ...
  const users = collectEntregaScopeUsers(ctx, teams, clinicalSessionContext.user);
  const covering = resolveR1GuardiaCovering(teams, users, sala, /* @__PURE__ */ new Date(), salaGuardiaToday);
  startEntregaPhase(
    covering || {
      coveringUserId: "",
      teamId: "",
...
function isInTurnoSession(ts, turnoStart) {
  ...
  return Number.isFinite(t2) && t2 >= turnoStart.getTime();
}
  ...
  if (!items.length) {
    host.innerHTML = `
      ...
      <div class="vfeed-empty">
        ...
        ${censusCount ? `<span class="vfeed-empty-sub">${censusCount} paciente${censusCount === 1 ? "" : "s"} en censo \u2014 los chips de abajo muestran cu\xE1ndo toca tomar signos.</span>` : ""}
      </div>`;
    return;
  }
  ...
  const cards = list.map((row) => {
    ...
    return `<li class="guardia-orphan-row">
        <div class="guardia-orphan-row-main">
          <span class="guardia-orphan-id" title="${escapeAttr3(patientId)}">${escapeHtml5(shortId)}</span>
          <span class="guardia-orphan-vitals">${escapeHtml5(vitalsHint)}</span>
        </div>
        <div class="guardia-orphan-row-actions">
          <button type="button" class="btn-med-secondary guardia-orphan-open-btn"
            data-patient-id="${escapeAttr3(patientId)}"
            data-guardia-id="${escapeAttr3(guardiaId)}">Abrir</button>
          <button type="button" class="btn-med-secondary guardia-orphan-delete-btn"
            data-patient-id="${escapeAttr3(patientId)}"
            data-guardia-id="${escapeAttr3(guardiaId)}">Eliminar del servidor</button>
        </div>
      </li>`;
  }).join("");
  host.hidden = false;
  ...
  try {
    if (!patientInLocalCensus(patientId)) {
      ...
      if (typeof lan.getActiveLiveSyncRoomId === "function" && lan.getActiveLiveSyncRoomId()) {
        ...
        if (restored?.ok) {
          toast6("Expediente recuperado del anfitri\xF3n.", "success");
        } else if (restored?.error === "patient_not_on_host") {
          toast6("El anfitri\xF3n no tiene este expediente; se abre la entrega con datos limitados.", "warn");
        } else if (restored && !restored.ok) {
          toast6("No se pudo recuperar el expediente del anfitri\xF3n.", "warn");
        }
      }
    }
    const entrega = await import("/js/chunks/clinical-entrega-2D6GLICJ.js");
...
async function deleteOrphanFromServer(btn, settings) {
  ...
  try {
    ...
    if (!res?.ok || !res.resolved) {
      toast6(res?.error || "No se liber\xF3 la entrega.", "error");
      return;
    }
    const lan = await import("/js/chunks/lan-sync-KAQL434D.js");
    ...
    if (onLan && typeof lan.purgeLanPatientFromHost === "function") {
      ...
      if (purge?.ok) {
        hostPurged = true;
        if (!purge.hadHostRow) {
          toast6("No hab\xEDa expediente en el anfitri\xF3n; se liberar\xE1 solo la entrega.", "info");
        }
      } else if (purge?.error === "not_configured") {
        toast6("Sin conexi\xF3n \u21C4 activa; solo se liberar\xE1 la entrega local.", "warn");
      } else {
        toast6("No se pudo borrar del anfitri\xF3n; se liberar\xE1 la entrega local.", "warn");
      }
      if (typeof lan.pushClinicalOpsLanNow === "function") {
        await lan.pushClinicalOpsLanNow();
      }
    } else {
      toast6("Sin conexi\xF3n \u21C4; solo se liberar\xE1 la entrega en esta Mac.", "info");
    }
    if (patientInLocalCensus(patientId)) {
      const lanMod = await import("/js/chunks/lan-sync-KAQL434D.js");
...
function createDeltaMutationBuilder(entityType, entityId) {
  const pathValues = {};
  const pathMeta = {};
  return {
...
function renderCalendarBody(year, month, selectedIso) {
  ...
  for (var day = 1; day <= days; day++) {
    ...
    cells += '<button type="button" class="' + cls + '" data-iso="' + esc(iso) + '">' + day + "</button>";
  }
  return cells;
}
function openRpcDatePicker(anchor, opts) {
  ...
  closePopover();
  function paint() {
    pop.innerHTML = '<div class="rpc-date-popover__head"><button type="button" class="rpc-date-popover__nav" data-nav="-1" aria-label="Mes anterior">\u2039</button><span class="rpc-date-popover__title">' + esc(MONTHS_ES[viewMonth] + " " + viewYear) + '</span><button type="button" class="rpc-date-popover__nav" data-nav="1" aria-label="Mes siguiente">\u203A</button></div><div class="rpc-date-popover__weekdays">' + WEEKDAYS_ES.map(function(w) {
      return "<span>" + w + "</span>";
...
function getLanScanIntervalMs() {
  return lanNetworkProfile.getNetworkProfile() === "slow" ? 6e4 : 5e3;
}
...
// public/js/lan-delta-client.mjs
var LABELS = {
  historiaClinica: {
...
function applyDeltaPathValues(target, pathValues) {
...
async function withRemoteDeltaApply(fn) {
  remoteApplyDepth += 1;
  try {
    return await fn();
  } finally {
...
function isTeamIdInCensusCatalog(teamId, teams) {
  ...
  return (teams || []).some((t2) => String(t2.team_id || "") === String(teamId));
}
...
function patientMatchesCensusTeamFilter(patient, teamId, teams, assignments, now) {
  ...
  const team = (teams || []).find((t2) => String(t2.team_id || "") === tid);
...
// public/js/lan-patient-team-scope.mjs
...
// public/js/lan-command-client.mjs
function trim(value) {
...
function normalizeCommandPushResponse(result) {
...
function appendLanHubStatusCard(root, opts) {
  ...
  const line = String(opts.statusLine || "").trim() || (connected ? "Conectado a la red del hospital" : "Sin red \u2014 buscando anfitri\xF3n en la Wi\u2011Fi del hospital\u2026");
  statusCard.innerHTML = '<div class="lan-hub-status-line">' + (connected ? '<span class="lan-hub-status-dot lan-hub-status-dot--online"></span> ' : '<span class="lan-hub-status-dot lan-hub-status-dot--offline"></span> ') + line + "</div>";
  ...
  if (opts.showInvitePaste) {
    ...
    inviteHint.innerHTML = "Pega aqu\xED el enlace <strong>Otra Mac del equipo</strong> del anfitri\xF3n (<code>http://\u2026/join/req_\u2026</code>).";
...
async function copyInternoQrImage(url, showToast) {
  const toast8 = typeof showToast === "function" ? showToast : (msg, kind) => {
  ...
  try {
    ...
    if (navigator.clipboard && typeof ClipboardItem !== "undefined") {
      ...
      toast8("QR copiado \u2014 p\xE9galo en WhatsApp o imprime", "success");
      return;
    }
    const dataUrl = canvas.toDataURL("image/png");
    await navigator.clipboard.writeText(dataUrl);
    toast8("QR copiado como imagen (data URL)", "info");
  } catch (_e) {
    toast8("No se pudo copiar el QR", "error");
  }
}

// public/js/features/interno-qr-panel.mjs
var SALA_DEFS = CLINICAL_SALA_VALUES.map((key) => ({
  key,
...
async function appendInternoQrPanel(root, opts = {}) {
  ...
  if (!api2 || !userId || typeof api2.dbInternoAccessList !== "function") return;
  const details = document.createElement("details");
  details.className = "rpc-disclosure lan-connect-card lan-hub-interno-details";
  const summary = document.createElement("summary");
  summary.className = "rpc-disclosure__summary rpc-disclosure__summary--stacked lan-hub-interno-summary";
  ...
  async function loadRows() {
    const res = await api2.dbInternoAccessList({ userId });
    ...
    for (const def of SALA_DEFS) {
      ...
      btnRow.appendChild(
        mkBtn(active ? "Desactivar" : "Activar", async () => {
          const r = await api2.dbInternoAccessSetActive({
            userId,
            sala: def.key,
            active: !active
          });
      ...
      btnRow.appendChild(
        mkBtn("Regenerar token", async () => {
          ...
          const r = await api2.dbInternoAccessRotate({ userId, sala: def.key });
          if (r?.ok) {
...
var LAN_PEER_OPS_PULL_MIN_MS = 3e4;
var _lastSubnetLanScanAt = 0;
var _lanPeerOpsPullLastAt = /* @__PURE__ */ new Map();
...
var LAN_SYNC_DIAG_OPEN_KEY = "rpc-lan-sync-diagnostics-open";
var LAN_INVITE_MOBILE_OPEN_KEY = "rpc-lan-invite-mobile-open";
var LAN_INVITE_SALA_OPEN_KEY = "rpc-lan-invite-sala-open";
...
function appendLanMobileSharerCard(root) {
  if (!root || !runtime2().isMobileWeb()) return;
  var card = document.createElement("div");
  card.className = "lan-connect-card lan-mobile-sharer-card";
  var who = esc2(mobileSharerDisplayLabel());
  card.innerHTML = '<div class="lan-connect-card-title">Turno compartido</div><p class="lan-connect-card-hint">Sincronizando con <strong>' + who + "</strong>. Este enlace copia su identidad en la red; no uses \xABUnirse\xBB a sala como en el escritorio.</p>";
...
function appendLanMobileJoinSection(root) {
  ...
  card.innerHTML = '<div class="lan-connect-card-title">Conectar al turno</div><p class="lan-connect-card-hint">Abre el <strong>enlace m\xF3vil</strong> que te compartieron (\u21C4 \u2192 invitaci\xF3n al turno) o p\xE9galo aqu\xED. Debe incluir el anfitri\xF3n y la identidad de quien lo comparti\xF3.</p>';
...
function appendLanJoinOtherMacSection(root, opts) {
  ...
  if (prominent) {
    ...
    container.className = "lan-connect-card lan-connect-other-mac lan-connect-other-mac--prominent";
    var title = document.createElement("div");
    title.className = "lan-connect-card-title";
    title.textContent = "Conectar al anfitri\xF3n del turno";
  ...
  hint.innerHTML = prominent ? "Pega el enlace <strong>Otra Mac del equipo</strong> que copiaste del anfitri\xF3n (<code>http://\u2026/join/req_\u2026</code>), luego pulsa <strong>Unirse con enlace</strong>." : "Pega el enlace que te compartieron. Esta R+ dejar\xE1 de usar el servidor de <strong>esta</strong> Mac y se conectar\xE1 a la otra.";
  ...
  inputInvite.rows = 2;
  inputInvite.autocomplete = "off";
  inputInvite.placeholder = "http://\u2026/join/req_\u2026 o PIN del turno (6 d\xEDgitos)";
...
function lanPanelNeedsFullRebuild(root) {
  ...
  if (!runtime2().isMobileWeb() && !isLanSessionConfiguredForRest() && !root.querySelector("#lan-input-invite-link")) {
    return true;
  }
  return false;
}
async function refreshLanPanelChromeInPlace() {
  ...
  if (statusCard) {
    ...
    var line = String(hubStatus.line || "").trim() || (connected ? "Conectado a la red del hospital" : "Sin red \u2014 buscando anfitri\xF3n en la Wi\u2011Fi del hospital\u2026");
    var lineEl = statusCard.querySelector(".lan-hub-status-line");
    if (lineEl) {
      lineEl.innerHTML = (connected ? '<span class="lan-hub-status-dot lan-hub-status-dot--online"></span> ' : '<span class="lan-hub-status-dot lan-hub-status-dot--offline"></span> ') + esc2(line);
    }
...
function appendLanInviteCollapsible(root, opts) {
  ...
  details.className = "rpc-disclosure lan-connect-card lan-invite-collapsible lan-hub-invite-card " + String(opts.extraClass || "");
  try {
...
function appendLanMobileInviteCard(root) {
  appendLanInviteCollapsible(root, {
    ...
    fill: function(body) {
      ...
      hint.innerHTML = "Para que alguien use <strong>tu turno</strong> en el iPad (tu @usuario y pacientes). Enlace <strong>permanente</strong> para favoritos en Safari. <strong>No</strong> lo uses si debe entrar con su propia cuenta.";
      body.appendChild(hint);
      var copyBtn = document.createElement("button");
      copyBtn.type = "button";
      copyBtn.className = "btn-lan-primary";
      copyBtn.style.width = "100%";
      copyBtn.textContent = "Copiar enlace m\xF3vil";
...
function appendLanSalaInviteCard(root) {
  appendLanInviteCollapsible(root, {
    ...
    fill: function(body) {
      ...
      hint.className = "lan-connect-card-hint";
      hint.style.margin = "0 0 8px";
      hint.innerHTML = "Para que un compa\xF1ero se conecte a la red y entre a la sala con <strong>su</strong> @usuario. El enlace <strong>no</strong> lleva tu identidad; despu\xE9s debe pulsar \xABUnirse\xBB en su sala.";
      body.appendChild(hint);
      var copyBtn = document.createElement("button");
      copyBtn.type = "button";
...
async function mintMobileLanPairingFromUi() {
  var link = await resolvePermanentMobileShareUrl();
  if (!link) {
    runtime2().showToast(
      "No hay anfitri\xF3n o c\xF3digo del equipo. Con\xE9ctate a la red del turno e int\xE9ntalo de nuevo.",
      "error"
    );
    return;
  }
  ...
  runtime2().showToast(
    "Enlace listo abajo. En el iPad \xE1brelo en Safari (ver\xE1s la app cargar) y luego A\xF1adir a pantalla de inicio. Si el acceso directo no sincroniza, borra el icono y repite desde este enlace.",
    "success"
  );
}
function mintSalaLanPairingFromUi() {
  return mintLanPairingFromUi({
    ...
    toastMsg: "Enlace de sala listo abajo. Tambi\xE9n puedes usar \xABCopiar enlace de sala\xBB."
  });
}
function lanHostUrl() {
...
async function renderLanPanelOnce() {
  ...
  if (registered && !rankConfigured) {
    ...
    needRankCard.innerHTML = '<p class="lan-connect-card-hint">Primero completa <strong>Configura tu rotaci\xF3n</strong> (rango y sala). Despu\xE9s R+ buscar\xE1 al anfitri\xF3n del turno en la Wi\u2011Fi.</p>';
  ...
  if (lanNetworkProfile.getNetworkProfile() === "offline") {
    ...
    offlineBanner.innerHTML = [
      ...
      " Sin conexi\xF3n al anfitri\xF3n \xB7 LiveSync en pausa",
      "</div>",
      '<div class="lan-offline-banner__hint">',
      "Los cambios se guardan localmente y se sincronizar\xE1n al reconectar.",
      "</div>",
  ...
  var hubStatus = lanHubStatusCopy();
  var needsInvitePaste = !runtime2().isMobileWeb() && !isLanSessionConfiguredForRest();
  appendLanHubStatusCard(root, {
    ...
    showInvitePaste: needsInvitePaste && runtime2().isMobileWeb(),
  ...
  if (lanPanelRenderStale(gen)) return;
  if (runtime2().isMobileWeb() && !hubStatus.connected) {
  ...
  var appendConflictDrafts = runtime2().appendLanConflictDraftsSection;
...
function appendLanHostPinSection(root) {
  ...
  void resolveOwnLanBaseForPin().then(function(resolvedOwn) {
    ...
    cb.checked = !!pinned && (pinned === String(hostUrl || "").replace(/\/+$/, "") || isPinnedHostLocal(ownForPin) || ownForPin && pinned === ownForPin);
    cb.disabled = false;
    cb.onchange = function() {
      if (cb.checked) {
        void resolveLanShareBaseUrl().then(function(shareUrl) {
          ...
          void applyPinnedHostOverride(getLanTeamCodeFromConfig(), {}).then(function(ok) {
            if (ok) {
              runtime2().showToast(
                "Anfitri\xF3n fijado: esta Mac asume el servidor del turno.",
                "success"
              );
            }
            renderLanPanel2({ force: true });
        ...
        clearPinnedHostUrl();
        runtime2().showToast(
          "Anfitri\xF3n ya no est\xE1 fijado; la red puede sugerir otro servidor.",
          "info"
        );
        renderLanPanel2({ force: true });
      }
  ...
  label.appendChild(cb);
  label.appendChild(document.createTextNode(" Fijar anfitri\xF3n del turno (solo en la Mac servidor)"));
  wrap.appendChild(label);
  var hint = document.createElement("p");
  hint.className = "lan-connect-card-hint";
  hint.style.marginTop = "6px";
  if (isLanRemoteJoinMode()) {
    hint.textContent = "Marca la casilla para forzar esta Mac como anfitri\xF3n (anula modo cliente y elecci\xF3n autom\xE1tica).";
  } else {
    hint.textContent = "Override del turno: esta Mac ser\xE1 el servidor aunque haya otros en la red. Desmarca para volver a elecci\xF3n autom\xE1tica.";
  }
  ...
  if (pinned) {
    void resolveOwnLanBaseForPin().then(function(resolvedOwn) {
      ...
      if (!isPinnedHostLocal(ownResolved) && isLanRemoteJoinMode()) {
        ...
        remoteHint.textContent = "Conectando al anfitri\xF3n fijado: " + pinned;
        ...
        localHint.textContent = "Fijado en esta Mac (" + pinned + "). La casilla fuerza servidor local (override).";
...
async function appendLanTurnResetSection(root, gen) {
  ...
  var title = document.createElement("div");
  title.className = "lan-connect-card-title";
  title.textContent = ownHost ? "Dos servidores en la misma sala" : "Restablecer conexi\xF3n \u21C4";
  card.appendChild(title);
  var hint = document.createElement("p");
  hint.className = "lan-connect-card-hint";
  hint.textContent = ownHost ? "Esta Mac est\xE1 usando su propio servidor. Para ver el mismo directorio que el turno, restablece y con\xE9ctate al anfitri\xF3n con el PIN o el enlace \u21C4." : "Si el directorio no coincide entre Macs, sal de la sala, quita el anfitri\xF3n fijado y vuelve a conectar.";
  card.appendChild(hint);
  var btn = document.createElement("button");
  btn.type = "button";
  btn.className = ownHost ? "btn-lan-primary" : "btn-lan-secondary";
  btn.style.width = "100%";
  btn.textContent = "Restablecer conexi\xF3n al turno";
  ...
  if (canLocalMacBeLanHost()) {
    ...
    hostHint.innerHTML = "Si <strong>t\xFA</strong> eres el \xFAnico R4 anfitri\xF3n, en Ajustes usa \xABLAN \xB7 servidor en esta computadora\xBB \u2192 Restablecer estado del host.";
    card.appendChild(hostHint);
  }
  root.appendChild(card);
}
async function resetLanTurnConnectionFromUi() {
  if (!isLanElectronDesktop()) {
    runtime2().showToast("Solo disponible en la app de escritorio.", "error");
    return;
  }
  var resetMod = await import("/js/chunks/lan-turn-reset-GFBNGO3T.js");
  ...
  runtime2().showToast(
    "Conexi\xF3n restablecida. Ingresa el PIN del turno o pega el enlace del anfitri\xF3n.",
    "success"
  );
  renderLanPanel2({ force: true });
...
function appendLanShiftPinClientConnectSection(root, gen) {
  ...
  void shouldShowLanShiftPinClientConnect().then(function(offer) {
    ...
    lead.textContent = "Pide los 6 d\xEDgitos al anfitri\xF3n (R4 en \u21C4). R+ encuentra la sala en la red del hospital.";
    ...
    btn.addEventListener("click", function() {
      ...
      if (!/^\d{6}$/.test(pin)) {
        runtime2().showToast("Ingresa los 6 d\xEDgitos del PIN.", "error");
        return;
      }
      btn.disabled = true;
      void import("/js/chunks/lan-shift-pin-connect-TRVOZWZF.js").then(function(m) {
        ...
        runtime2().showToast(
          "No encontramos el turno con ese PIN. Revisa el Wi\u2011Fi cl\xEDnico o pide otro PIN.",
          "error"
        );
      }).finally(function() {
        btn.disabled = false;
...
async function appendLanShiftPinSection(root, gen) {
  ...
  try {
    ...
    lead.textContent = "Dilo en voz alta al equipo (6 d\xEDgitos). Sirve al registrar @usuario o si cambian de Wi\u2011Fi.";
    wrap.appendChild(lead);
    ...
    copyBtn.addEventListener("click", function() {
      ...
      runtime2().showToast("PIN del turno copiado.", "success");
    });
    ...
    regenBtn.addEventListener("click", function() {
      void lanFetchAuthed("/api/lan/v1/auth/shift-pin/regenerate", { method: "POST" }).then(
        function(r) {
          if (r && r.ok) {
            runtime2().showToast("PIN del turno renovado.", "success");
            renderLanPanel2({ force: true });
          } else {
            runtime2().showToast("No se pudo renovar el PIN.", "error");
          }
        }
      );
    });
    ...
    if (body.expiresAt) {
      ...
      try {
        exp.textContent = "V\xE1lido hasta " + new Date(body.expiresAt).toLocaleString("es-MX", {
          hour: "2-digit",
          minute: "2-digit",
          day: "numeric",
          month: "short"
        ...
        exp.textContent = "V\xE1lido hasta " + String(body.expiresAt);
      }
...
async function appendLanSyncDiagnosticsSection(root) {
  ...
  if (existing) {
    ...
    var showSplit = diag.phase === "live" && diag.wsLive && Number(diag.peerHostCount || 0) === 0 && isLanElectronDesktop() && !isLanRemoteJoinMode();
  ...
  copyBtn.onclick = function() {
    var report2 = formatDiagnosticsReport(getLanSyncDiagnostics(deps2));
    void copyToClipboardSafe(report2).then(function(ok) {
      runtime2().showToast(
  ...
  retryBtn.onclick = function() {
    ...
    if (!rid) {
      runtime2().showToast("No hay sala activa para reintentar.", "warn");
      return;
    }
    void flushLiveSyncOutbox(rid).then(function() {
      runtime2().showToast("Cola reintentada. Revisa el informe abajo.", "info");
      renderLanPanel2({ force: true });
    });
  };
...
async function openR4TeamCreationModal() {
  try {
    var mod = await import("/js/chunks/clinical-teams-DTE66OAL.js");
...
async function handleFinalizarRotacion() {
  var api2 = typeof window !== "undefined" ? window.rplusDb || window.electronAPI : null;
  if (!api2 || typeof api2.dbRotationNueva !== "function") {
    runtime2().showToast("Operaci\xF3n no disponible.", "error");
    return;
  }
  var user = typeof clinicalSessionContext !== "undefined" ? clinicalSessionContext.user : null;
  var userId = user ? String(user.user_id || "") : "";
  var res = await api2.dbRotationNueva({ userId });
  if (res && res.ok) {
...
function buildR4Section(root) {
  ...
  var salas = LIVE_SYNC_SALA_DEFS.map(function(d) {
    return d.key;
  });
...
async function joinClinicalTeam(teamId) {
  ...
  var addRes = await api2.dbClinicalTeamsMemberAdd({ teamId, userId });
  if (!addRes || addRes.ok === false) {
    runtime2().showToast(addRes?.error || "No se pudo unir al equipo.", "error");
    return;
  }
  var rank = getClinicalRank();
  if (rank === "R2" && api2 && typeof api2.dbClinicalTeamsPromoteLeader === "function") {
    var promoteRes = await api2.dbClinicalTeamsPromoteLeader({ teamId, userId });
...
async function refreshClinicalSessionTeams() {
  var api2 = typeof window !== "undefined" ? window.rplusDb || window.electronAPI : null;
  if (!api2) return;
  if (typeof api2.dbClinicalScopeContext === "function") {
    var userId = getClinicalUserUserId();
    var res = await api2.dbClinicalScopeContext({ userId });
  ...
  if (typeof api2.dbClinicalTeamsList === "function") {
    var listRes = await api2.dbClinicalTeamsList();
...
function lanHubPausedCopy() {
  return {
    connected: false,
    line: "Sin anfitri\xF3n detectado",
    hint: "B\xFAsqueda autom\xE1tica en pausa (5 intentos). Usa PIN, Restablecer conexi\xF3n \u21C4 o vuelve a abrir este panel."
  };
}
function lanHubStatusCopy() {
  if (!lanClient.connected) {
    ...
    if (!isClinicalRankConfiguredForLan()) {
      return {
        connected: false,
        line: "Configura tu rotaci\xF3n para usar la red del turno",
        hint: "Abre \xABConfigura tu rotaci\xF3n\xBB y confirma rango y sala; despu\xE9s buscaremos al anfitri\xF3n en la Wi\u2011Fi."
      };
    }
    if (!canLocalMacBeLanHost()) {
      var esc5 = getHostEscalationStatus();
      var nextRank = ["R3", "R2", "R1"][esc5.tier] || "";
      var escHint = esc5.tier < 3 && esc5.msUntilNext > 0 ? "Sin R4/admin en la red: en " + formatEscalationCountdown(esc5.msUntilNext) + " podr\xE1 anfitrionar " + nextRank + " (escalada autom\xE1tica 10+10+10 min)." : "R1\u2013R3 esperan anfitri\xF3n R4 o escalada. Pide enlace (\u21C4) o p\xE9galo abajo.";
      return {
        connected: false,
        line: "Sin conexi\xF3n al turno",
        hint: "Pide el PIN de 6 d\xEDgitos al anfitri\xF3n (\u21C4) o pulsa Conectar al turno arriba. " + escHint
      };
    }
    var onCallHost = resolveLocalOnCallGuardia();
    return {
      connected: false,
      line: onCallHost ? "De guardia hoy \u2014 listo para anfitrionar" : "Sin conexi\xF3n al turno",
      hint: onCallHost ? "Esta Mac puede ser el servidor del turno en tu Wi\u2011Fi. Pulsa Conectar al turno o abre \u21C4." : "Pide el PIN de 6 d\xEDgitos al anfitri\xF3n (\u21C4) o con\xE9ctate abajo."
    };
  }
  if (isLanRemoteJoinMode()) {
  ...
  return {
    ...
    hint: "Comparte el enlace de sala (\u21C4 \u2192 Copiar enlace de sala). Para iPad usa \xABCopiar enlace m\xF3vil\xBB. No activen otro servidor salvo suplente."
  };
}
  ...
  try {
    ...
    if (now - _lastSubnetLanScanAt >= SUBNET_LAN_SCAN_MIN_MS) {
      ...
      if (wardHosts.length && !wsPeers.length && !sessionStorage.getItem(LAN_SPLIT_BRAIN_HINT_KEY)) {
        ...
        runtime2().showToast(
          "Otra R+ en la red (" + wardHosts[0] + "). Para ver el directorio juntos, una Mac debe ser anfitri\xF3n.",
          "warning"
        );
      }
    ...
    var roomIdForPeerOps = typeof ensureEffectiveLiveSyncRoomId === "function" ? ensureEffectiveLiveSyncRoomId() : "";
...
async function copyMobileLanLinkFromUi(opts) {
  ...
  if (!link) {
    if (!silent) {
      runtime2().showToast(
        "Falta la direcci\xF3n del anfitri\xF3n o el c\xF3digo del equipo. Revisa \u21C4 y que esta Mac sea anfitriona.",
        "error"
      );
    }
    return false;
  ...
  if (copied) {
    if (!silent) {
      ...
      var roomMsg = roomHint ? "" : " Primero \xFAnete t\xFA a una sala en \u21C4 (Unirse) para que el iPad reciba pacientes.";
      runtime2().showToast(
        "Enlace m\xF3vil copiado. Abre /mobile/?token=\u2026 en Safari y luego A\xF1adir a pantalla de inicio (as\xED no se pierde el token)." + roomMsg,
...
async function copyLanInviteLinkFromUi(opts) {
  ...
  if (copied) {
    if (!silent) {
      ...
      runtime2().showToast(
        "Enlace de sala copiado (sin tu identidad)." + pinHint + (inviteExpiry ? " V\xE1lido hasta " + inviteExpiry + "." : ""),
        "success"
      );
    }
    return true;
...
function joinLanFromInviteUi(fromBtn) {
  ...
  runtime2().showToast(
    "No reconocimos un enlace v\xE1lido. Pide al anfitri\xF3n un enlace /join/req_\u2026 o el PIN del turno.",
    "error"
  );
}
async function saveLanSettingsFromUi(opts) {
  ...
  if (pingOk) {
    ...
    void import("/js/chunks/historia-clinica-lan-sync-INLQPSSK.js").then(function(m) {
      return m.scheduleFlushAllPendingHistoriaClinicaLanSync();
    });
    void maybeShowLanMigrationNotice();
    if (copyInviteAfter) {
      runtime2().showToast(
        copiedOk ? "Anfitri\xF3n listo. La invitaci\xF3n ya est\xE1 en el portapapeles; comp\xE1rtela por WhatsApp o correo." : "Anfitri\xF3n listo, pero no se pudo copiar solo. Pulsa \xABCopiar enlace de sala\xBB o \xABGenerar y mostrar\xBB en \u21C4.",
...
async function reconnectFromOfflineUi() {
  ...
  if (!pingOk || newProfile === "offline") {
    ...
    errEl.textContent = "No se encontr\xF3 el anfitri\xF3n.";
    return;
  }
  try {
    var rid = String(activeLiveSyncRoomId || "").trim() || (typeof ensureEffectiveLiveSyncRoomId === "function" ? ensureEffectiveLiveSyncRoomId() : "");
    if (rid) {
...
var deltaEchoTracker = createDeltaEchoTracker(getLanClientId2());
...
function renderPatientListLanSilent() {
  runtime3.renderPatientList({ silent: true });
}
var runtime3 = {
  showToast() {
  },
...
var _lanNetworkRefreshWired = false;
function wireLanNetworkRefresh() {
  ...
  window.addEventListener("online", function() {
    void (async function() {
      ...
      var m = await import("/js/chunks/lan-network-change-K45O2VEA.js");
  ...
  if (window.electronAPI && typeof window.electronAPI.onLanNetworkChanged === "function") {
    window.electronAPI.onLanNetworkChanged(function(payload) {
      void import("/js/chunks/lan-network-change-K45O2VEA.js").then(function(m) {
        if (typeof m.handleLanNetworkChanged === "function") {
...
function rememberPatientDeleteTombstone(patient) {
  ...
  function transmit() {
    ...
    void guardAndSignLiveSyncMutation(mutation, envelope).then(function() {
      ...
      if (err && err.code === "CLINICAL_ACCESS_DENIED") {
        runtime3.showToast(String(err.message || "Acceso cl\xEDnico denegado"), "error");
      }
    });
    return true;
  }
...
async function applyConflictUseServer(payload) {
  ...
  if (server && server.data) {
    if (payload.entityType === "historiaClinica" && payload.patientId) {
      ...
      if (hcRow) {
        var mod = await import("/js/chunks/historia-clinica-lan-sync-INLQPSSK.js");
...
async function appendLanConflictDraftsSection(root) {
  ...
  bulkBtn.onclick = function() {
    ...
    void clearAllDraftConflicts().then(function(cleared) {
      ...
      runtime3.showToast("No se pudieron descartar los borradores.", "error");
    }).finally(function() {
...
async function applyLwwConflictLocally(payload) {
  ...
  if (shouldAutoResolveTodoConflict(payload) && tryAutoResolveTodoConflict(payload)) {
    ...
    if (!localDelete) {
      runtime3.showToast("Pendiente alineado con la sala", "info");
    }
    return;
  }
...
async function restoreLanPatientFromHost(patientId) {
  var pid = String(patientId || "").trim();
  var rid = String(activeLiveSyncRoomId || "").trim();
...
async function purgeLanPatientFromHost(patientId) {
  var pid = String(patientId || "").trim();
...
async function lanPushHistoriaClinicaDelta(patientId, delta) {
...
function collectPatientEntriesForLanSync() {
  ...
  patients.forEach(function(p) {
    ...
    var entry = runtime3.buildPatientEntry(p.id);
...
function applyLanPatientEntries(entries, opts) {
  ...
  for (var i = 0; i < scopedEntries.length; i += 1) {
    ...
    var existing = reg ? runtime3.findPatientByRegistro(reg) : null;
    ...
    if (existing) {
      ...
      if (remoteId && !idTaken) {
        var newPat = {
          id: remoteId,
          nombre: runtime3.ensureUniquePatientName(entry.patient.nombre || "PACIENTE SIN NOMBRE"),
        ...
        newId = runtime3.applyImportEntry(entry, "duplicate", null);
      }
  ...
  if (added || updated) {
    ...
    if (runtime3.getActiveId()) {
      try {
        runtime3.renderNoteForm();
      } catch (_e) {
      }
      try {
        runtime3.renderLabHistoryPanel();
      } catch (_e2) {
      }
    }
  }
...
function removePatientLocally(patientId) {
  ...
  if (runtime3.getActiveId() === pid) runtime3.setActiveId(patients.length ? patients[0].id : null);
  return true;
}
function applyLiveSyncPatientDeletes(deletes, idMap) {
  ...
  for (var i = 0; i < deletes.length; i += 1) {
    ...
    if (reg) {
      var existing = runtime3.findPatientByRegistro(reg);
...
async function applyLiveSyncMerged(merged) {
  ...
  if (patientRemoved) {
    renderPatientListLanSilent();
    if (runtime3.getActiveId()) runtime3.selectPatient(runtime3.getActiveId());
    else {
      var pv = document.getElementById("patient-view");
      var es = document.getElementById("empty-state");
      if (pv) pv.style.display = "none";
      if (es) es.style.display = "flex";
      runtime3.syncWorkContextChrome();
    }
  }
  if (runtime3.getActiveAppTab() === "agenda" || runtime3.isMobileWeb()) {
    runtime3.renderProcedureAgendaPanel();
  }
  if (todosChanged) runtime3.refreshAllTodoUIs();
  if (patientsChanged && runtime3.getActiveId()) {
    try {
      runtime3.renderNoteForm();
    } catch (_eNote) {
    }
...
async function applyLiveSyncDeltaApplied(msg) {
  ...
  if (partial) {
    ...
    runtime3.showToast('Tu cambio en "' + labels.join(", ") + '" fue reemplazado por una edici\xF3n m\xE1s reciente en la sala.', "warn");
  }
  syncHostBundleEntityFromApplied(msg);
}
...
function applyLiveSyncApplied(msg) {
  ...
  if (entityType === "agenda") {
    ...
    if (row && !entityData._deleted) {
      ...
      if (runtime3.getActiveId() === entityId) {
        try {
          runtime3.renderNoteForm();
        } catch (_eNote) {
        }
  ...
  if (msg.lwwApplied) {
    ...
    runtime3.showToast("Cambios fusionados autom\xE1ticamente con el servidor.", "success");
  }
}
...
function emitLiveSyncTodoDelete(patientId, todoRef, updatedAt) {
  ...
  rememberLiveSyncEntity("todo", eid, patientId, tombVer, {
    ...
    patientId,
    _deleted: true,
    updatedAt: String(todo && todo.updatedAt || updatedAt || (/* @__PURE__ */ new Date()).toISOString())
  });
  sendLiveSyncMutation(mutation);
}
...
wireLanSyncBridges();
var _lanRuntimeStarted = false;
function ensureLanSyncRuntimeStarted() {
  if (typeof document === "undefined") return;
  if (isClinicalLocalOnlyMode(readRpcSettings())) return;
...
function buildEstadoActualCommand(opts) {
...
function buildEventualidadAddCommand(opts) {
  return buildLanCommand({
    ...opts,
...
function buildPendienteCommand(opts) {
...
function renderEntryCard(entry, editingId) {
  ...
  return '<article class="ev-card' + (isEditing ? " ev-card--editing" : "") + '" data-entry-id="' + esc3(entry.id) + '"><p class="ev-card__text">' + esc3(normalizeEventualidadText(entry.text)) + '</p><footer class="ev-card__foot"><div class="ev-card__actions"><button type="button" class="ev-card__edit" data-ev-edit="' + esc3(entry.id) + '" aria-label="Editar eventualidad">Editar</button><button type="button" class="ev-card__delete" data-ev-delete="' + esc3(entry.id) + '" aria-label="Eliminar eventualidad">Eliminar</button></div></footer></article>';
}
function renderDaySection(dayGroup, editingId, now) {
  ...
  return '<details class="ev-day' + todayClass + '"' + (isOpen ? " open" : "") + ' data-day="' + esc3(dayGroup.day) + '"><summary class="ev-day__summary"><span class="ev-day__chevron" aria-hidden="true"></span><div class="ev-day__titles"><span class="ev-day__pill">' + esc3(dayGroup.label) + "</span>" + (subLabel ? '<span class="ev-day__date">' + esc3(subLabel) + "</span>" : "") + '</div><span class="ev-day__count">' + esc3(countLabel) + '</span></summary><div class="ev-day__panel">' + dayGroup.entries.map(function(e) {
...
function renderComposeBlock(editingEntry) {
  ...
  return '<footer class="ev-compose"><div class="ev-compose__card' + (isEdit ? " ev-compose__card--edit" : "") + '"><div class="ev-compose__top"><label class="ev-compose__label" for="eventualidades-input">' + (isEdit ? "Editar eventualidad" : "Nueva eventualidad") + '</label><div class="ev-compose__date-slot"><input type="date" id="eventualidades-at" class="rpc-date-input" value="' + esc3(atValue) + '" title="Fecha de la eventualidad" aria-label="Fecha de la eventualidad"></div></div><textarea id="eventualidades-input" class="ev-compose__input" rows="2" placeholder="Describe lo ocurrido\u2026">' + esc3(textValue) + '</textarea><div class="ev-compose__actions"><span class="ev-compose__hint">' + (isEdit ? "Puedes cambiar la fecha y el texto" : "Elige una fecha anterior si aplica") + '</span><div class="ev-compose__btns">' + (isEdit ? '<button type="button" class="ea-btn ea-btn--ghost ev-compose__cancel" id="eventualidades-cancel">Cancelar</button>' : "") + '<button type="button" class="ea-btn ea-btn--primary ev-compose__submit" id="eventualidades-add">' + (isEdit ? "Guardar" : "Agregar") + "</button></div></div></div></footer>";
}
function activePatient() {
  const id = rt2.getActiveId();
  if (!id) return null;
  ...
  void (async function() {
    try {
      ...
      if (out && !out.ok && !out.conflict) {
        const msg = out.status === 401 || out.status === 403 ? "No se pudo autenticar con el host LAN. Revisa el c\xF3digo de equipo." : "No se pudo sincronizar la eventualidad con el host LAN.";
        rt2.showToast(msg, "error");
      }
    } catch (_e) {
    }
  })();
...
function renderEventualidadesPanel(mountEl) {
  ...
  async function submitEntry() {
    ...
    if (out && out.ok) {
      ...
      rt2.showToast(wasEdit ? "Eventualidad actualizada." : "Eventualidad guardada.", "success");
    ...
    timeline.addEventListener("click", function(ev) {
      ...
      if (delBtn) {
        ...
        void (async function() {
          ...
          const out = await persistEventualidades(patient, next);
          if (out && out.ok) {
            rt2.showToast("Eventualidad eliminada.", "success");
            renderEventualidadesPanel(mountEl);
          }
        })();
        return;
...
async function applyDriveImportEventualidades(patient, incoming) {
  ...
  void driveImportLanTimeout(lanPushPatientVersioned(patient.id, mutation), DRIVE_IMPORT_LAN_MS).then(function(out) {
    if (out && out.ok) {
      if (out.data) Object.assign(patient, out.data);
      saveState();
    }
...
async function submitEventualidad(patientId, patientLabel, rawText) {
  ...
  if (!text) {
    toast7("Escribe la eventualidad antes de guardar.", "error");
    return;
  }
  const patient = findPatient(patientId);
  if (!patient) {
    toast7("Paciente no encontrado.", "error");
    closeGuardiaPatientActionSheet();
    return;
  }
  ...
  try {
    ...
    if (!out?.ok) {
      toast7("No se pudo guardar la eventualidad.", "error");
      return;
    }
    toast7("Eventualidad guardada.", "success");
    closeGuardiaPatientActionSheet();
  } finally {
...
var entregaControlsInstalled = false;
var guardiaViewBootstrapped = false;
var elevatedFullWardPullScheduled = false;
...
function syncEntregaPhaseChrome(opts = {}) {
  ...
  if (btn) {
    ...
    btn.title = active ? "Continuar entrega \u2014 listado de pacientes" : "Iniciar entrega al R1 de guardia de tu sala";
  }
  if (status) {
...
function enrichPatientForGuardiaCard(p, guardiasMap) {
  ...
  const isCritical = isGuardiaChipCritical(g2);
...
function renderGuardiaSummaryTiles(summary, opts = {}) {
  ...
  if (!host) return;
  const vitalsTitle = summary.vitalsOverdue > 0 ? `${summary.vitalsOverdue} signo${summary.vitalsOverdue === 1 ? "" : "s"} vencido${summary.vitalsOverdue === 1 ? "" : "s"}` : summary.vitalsDueSoon > 0 ? `${summary.vitalsDueSoon} signo${summary.vitalsDueSoon === 1 ? "" : "s"} pronto` : "Signos al d\xEDa";
  const stats = [
    {
      value: summary.total,
      label: "censo",
...
function renderGuardiaCensusHead(count, state) {
  ...
  const sortHint = parts.length ? `${parts.join(" \xB7 ")} arriba` : "Sin alertas urgentes";
  host.innerHTML = `
...
function renderGuardiaBoard(settings) {
  ...
  if (!guardiaViewBootstrapped) {
    ...
    void ensureTeamAssignedPatientsOnDevice({ allowLanPull: true, lanPullDelayMs: 3e3 }).then(() => {
  ...
  if (hasElevatedTeamPrivileges(clinicalSessionContext.user) && !elevatedPatientFilters.teamId && !elevatedFullWardPullScheduled) {
...
function prunePatientsOutsideClinicalScope() {
  return 0;
}
...
async function applyBootstrapResult(res) {
  ...
  if (api2 && typeof api2.dbClinicalProfileGet === "function") {
    try {
      const profileRes = await api2.dbClinicalProfileGet({ userId: res.user.userId });
  ...
  if (typeof document !== "undefined") {
    void import("/js/chunks/clinical-profile-lan-sync-XZ5LLGM2.js").then((mod) => mod.flushClinicalProfileToLan()).catch(() => {
    });
  }
...
function migrateLocalPatientsClinicalSala() {
  ...
  if (migrated > 0) {
    ...
    if (typeof document !== "undefined") {
      void import("/js/chunks/patients-BOB2VPYS.js").then((mod) => mod.renderPatientList({ silent: true })).catch(() => {
      });
    }
  }
  return migrated;
...
async function resumeClinicalIdentityByUsername(username, settings, clientId) {
  ...
  if (typeof api2.dbClinicalIdentityResume === "function") {
    ...
    const res2 = await api2.dbClinicalIdentityResume({
      username: handle,
      fromUserId
    });
    if (!res2 || res2.ok === false) {
...
async function refreshClinicalUserProfile() {
  const { ensureLanProfileGateDeviceReset } = await import("/js/chunks/clinical-settings-CMYQ2OQU.js");
...
async function scheduleLanPatientReconcile(reason, delayMs2) {
  try {
    const lan = await import("/js/chunks/lan-sync-KAQL434D.js");
    if (typeof lan.isLanSessionConfiguredForRest !== "function" || !lan.isLanSessionConfiguredForRest()) {
      return;
    }
    const rid = typeof lan.getActiveLiveSyncRoomId === "function" ? String(lan.getActiveLiveSyncRoomId() || "").trim() : "";
    if (!rid) return;
    const push = await import("/js/chunks/push-2R3WG4YZ.js");
    if (typeof push.scheduleReconcileLiveSyncRoom === "function") {
      push.scheduleReconcileLiveSyncRoom(rid, { reason, delayMs: delayMs2 });
    }
  } catch (_e) {
  }
}
async function ensureTeamAssignedPatientsOnDevice2(options) {
  const user = clinicalSessionContext.user;
  if (!user?.user_id) return;
  const ctx = getClinicalScopeContextForEvaluate();
  const teams = Array.isArray(ctx.teams) ? ctx.teams : [];
  ...
  if (elevated) {
    ...
    const { filterJoinedTeams: filterJoinedTeams2 } = await import("/js/chunks/shared-LEEJXXTE.js");
    const { resolvePatientTeamIdFromAssignments: resolvePatientTeamIdFromAssignments2 } = await import("/js/chunks/clinico-access-FEQPVL2K.js");
    const joined = filterJoinedTeams2(teams, user);
    const teamIds = new Set(joined.map((t2) => String(t2.team_id || "")));
...
async function ensureElevatedWardCensusOnDevice(options = {}) {
  ...
  await ensureTeamAssignedPatientsOnDevice2(options);
...
async function refreshClinicalPatientListForScope(options) {
  ...
  refreshClinicalPatientListForScopeInFlight = (async function() {
    await fetchClinicalTeamsFromDb();
    await fetchClinicalScopeContextFromDb();
    await ensureTeamAssignedPatientsOnDevice2({ allowLanPull: !!opts.allowLanPull });
    if (typeof document === "undefined") return;
    try {
      const mod = await import("/js/chunks/patients-BOB2VPYS.js");
...
async function pullHostPatientsAfterAssignmentMerge(event) {
  ...
  try {
    ...
    const push = await import("/js/chunks/push-2R3WG4YZ.js");
    if (typeof push.scheduleReconcileLiveSyncRoom === "function") {
      push.scheduleReconcileLiveSyncRoom(rid, { reason: "assignment-merge", delayMs: 2e3 });
...
// public/js/mobile-sharer-sync.mjs
function resolveMobilePairingRoomId(activeRoomId) {
...
function appendMobileSharerParamsToJoinUrl(url, activeRoomId) {
  const s = readRpcSettings();
  try {
    const u = new URL(url);
...
function hydrateMobileSharerSessionFromSettings() {
...
async function runConsolidateIntoHost(opts, deps2) {
  ...
  if (!roomId) {
    deps2.showToast?.("\xDAnete primero a una sala \u21C4 para combinar con el anfitri\xF3n.", "warn");
    return false;
  }
  _consolidating = true;
  try {
...
function ensureLanSyncTransportDepsWired() {
  ...
  if (!transportDepsWirePromise) {
    transportDepsWirePromise = import("/js/chunks/orchestrator-GPN4H2L4.js").then(function() {
      if (!transportDeps && typeof globalThis !== "undefined") {
        var g2 = lanSyncTransportDepsGlobal();
        if (g2 && typeof g2 === "object") transportDeps = g2;
      }
    });
  }
...
function isLanSessionConfiguredForRest() {
  try {
    ...
    return !!(c && String(c.hostUrl || "").trim() && trimStoredLanBearer(c.teamCode));
  } catch (_e) {
    return false;
  }
}
async function isLanRestHostOwnMachine() {
...
async function shouldShowLanShiftPinClientConnect() {
...
function shouldShowLanShiftPinHostDisplay() {
  if (!isLanElectronDesktop()) return false;
  if (!canLocalMacBeLanHost() || isLanRemoteJoinMode()) return false;
  return true;
}
...
function fixMobileLanHostUrl(hostUrl) {
  var raw = String(hostUrl || "").trim().replace(/\/+$/, "");
...
async function exchangeLanJoinFromInvite(hostUrl, ticketId, roomId) {
  ...
  if (!base || !tid) {
    runtime4().showToast("Falta la direcci\xF3n del servidor o el ticket de invitaci\xF3n.", "error");
    return;
  }
  ...
  try {
    ...
    runtime4().showToast("Error de red al unirse. Revisa Wi\u2011Fi y que R+ siga abierto en el anfitri\xF3n.", "error");
    return;
  } finally {
    clearTimeout(timer);
  }
  ...
  try {
    ...
    runtime4().showToast("Respuesta inv\xE1lida del servidor.", "error");
    return;
  }
  await persistGuestBearerFromExchange(data);
  configureLanFromMobileJoin2(String(data.hostUrl || base), data.token, roomId);
}
...
async function refreshElectronLanCandidateUrl(opts) {
  ...
  const delayMs2 = Math.max(50, Number(opts.delayMs) || 300);
  for (let i = 0; i < tries; i += 1) {
    ...
    try {
      const url = String(await window.electronAPI.getLanCandidateBaseUrl() || "").trim().replace(/\/+$/, "");
      if (url && !isLocalLoopbackLanUrl(url)) return url;
    } catch (_e) {
    }
    if (i < tries - 1) {
      await new Promise(function(resolve) {
        setTimeout(resolve, delayMs2);
      });
    }
  }
  return "";
...
async function resolveOwnLanBaseForPin() {
...
async function applyPinnedHostOverride(teamCode, opts) {
  opts = opts || {};
  ...
  if (pinTargetsThisMac(pinned, ownUrl)) {
    if (!canLocalMacBeLanHost()) {
      if (!opts.quiet) {
        runtime4().showToast(
          "No puedes fijar esta Mac como anfitri\xF3n con tu rango todav\xEDa (escalada o R4/admin).",
          "info"
        );
      }
      return false;
    ...
    const alreadyHost = !isLanRemoteJoinMode() && current && (pinTargetsThisMac(pinned, current) || lanHostBasesSameMachine(pinned, current));
    if (alreadyHost) {
...
async function consolidateIntoHost(winnerUrl, teamCode, opts) {
  opts = opts || {};
  const room = await import("/js/chunks/room-VGV6EQKR.js");
  ...
  return runConsolidateIntoHost(
    { winnerUrl, teamCode, requireConfirm: !!opts.requireConfirm },
    {
      ...
      showToast: (msg, kind) => runtime4().showToast(msg, kind)
    }
  );
}
...
async function promoteThisMacToLanHost(opts) {
  opts = opts || {};
  if (!isLanElectronDesktop()) {
    runtime4().showToast("Solo disponible en la app de escritorio.", "info");
    return false;
  }
  if (!isClinicalRankConfiguredForLan()) {
    runtime4().showToast(
      "Completa \xABConfigura tu rotaci\xF3n\xBB (rango y sala) antes de usar la red del turno.",
      "info"
    );
    return false;
  }
  if (!canLocalMacBeLanHost()) {
    const { getHostEscalationStatus: getHostEscalationStatus2, formatEscalationCountdown: formatEscalationCountdown2 } = await import("/js/chunks/lan-host-escalation-3AFJQV4S.js");
    const esc5 = getHostEscalationStatus2();
    const nextRank = ["R3", "R2", "R1"][esc5.tier] || "R1";
    const msg = esc5.tier < 3 && esc5.msUntilNext > 0 ? "Sin R4 en la red: en " + formatEscalationCountdown2(esc5.msUntilNext) + " podr\xE1 anfitrionar " + nextRank + " (escalada 10 min por nivel)." : "A\xFAn no puedes ser anfitri\xF3n en esta Mac. Busca al R4 o espera la escalada autom\xE1tica.";
    runtime4().showToast(msg, "info");
    return false;
  }
  ...
  if (ok && !opts.skipToast) {
    runtime4().showToast("Esta Mac ahora es el servidor del turno.", "success");
  }
  if (!ok) {
    runtime4().showToast("No se pudo activar el servidor local. Reinicia R+ e int\xE9ntalo de nuevo.", "error");
  }
  return ok;
}
async function tryConnectToPinnedHost(teamCode, opts) {
  opts = opts || {};
  const pinned = getPinnedHostUrl();
  const code = String(teamCode || "").trim();
  if (!pinned || !code) return false;
  ...
  if (!alive) {
    if (!opts.quiet && !opts.boot) {
      runtime4().showToast(
        "Anfitri\xF3n fijado no responde (" + target + "). Verifica la red o el enlace.",
        "warning"
      );
    }
    return false;
...
async function initLanHostPlugAndPlay() {
  ...
  if (!isClinicalRankConfiguredForLan()) return;
  try {
    const pinMod = await import("/js/chunks/lan-shift-pin-connect-TRVOZWZF.js");
...
function maybeApplyLanHostUrlSwitch(hostUrl, teamCode, opts) {
  ...
  if (pinned) {
    ...
    runtime4().showToast("Anfitri\xF3n fijado: " + pinned + ".", "info");
    return false;
  }
...
async function syncMobileWithSharedInvite(hintRoomId) {
  if (!isMobileWeb()) return false;
  if (!isLanSessionConfiguredForRest()) return false;
  ...
  if (!rid) {
    runtime4().showToast(
      "Conectado al anfitri\xF3n. Pide a quien comparti\xF3 el enlace que est\xE9 en una sala \u21C4 activa antes de abrir R+ M\xF3vil.",
      "warn"
    );
    d.renderLanPanel?.();
    return false;
  ...
  d.renderLanPanel?.();
  void d.joinLanRoom(rid, liveSyncRoomLabel(rid), { silent: true, mobileSharerSync: true }).catch(function() {
  });
  return true;
}
async function resumeMobileLanRoomJoin(hintRoomId) {
  return syncMobileWithSharedInvite(hintRoomId);
}
function configureLanFromMobileJoin2(hostUrl, teamCode, roomId) {
  var resolvedHost = fixMobileLanHostUrl(hostUrl);
  if (!resolvedHost) {
    resolvedHost = resolveLanJoinHostUrl(hostUrl, typeof location !== "undefined" ? location.origin : "") || String(hostUrl || "").trim().replace(/\/+$/, "");
  }
  ...
  var pingMs = isMobileWeb() ? 3e3 : 5e3;
  var pingCtrl = new AbortController();
  var pingTimer = setTimeout(function() {
    pingCtrl.abort();
  }, pingMs);
  lanClient.fetch("/api/lan/v1/ping", { signal: pingCtrl.signal, cache: "no-store" }).then(function(r) {
    ...
    deps().renderLanPanel?.();
    if (isLanElectronDesktop()) {
      runtime4().showToast("Conectado al anfitri\xF3n del turno.", "success");
    }
  }).catch(function() {
    clearTimeout(pingTimer);
    ...
    runtime4().showToast("Error de red al conectar con el anfitri\xF3n", "error");
    deps().renderLanPanel?.();
  });
}
...
function getLastLanPairing() {
  return _lastLanPairing;
}
function updateLanPairingDisplay(root, opts) {
  opts = opts || {};
  ...
  box.hidden = false;
  var joinLine = displayUrl ? '<div><strong>Enlace:</strong> <code style="word-break:break-all;">' + esc4(displayUrl) + "</code></div>" : "";
  var lead;
  var pinLine = "";
  var expiryLine = "";
  if (opts.permanent) {
    lead = "Enlace permanente para Safari (favoritos). Incluye tu identidad; no caduca mientras el c\xF3digo del equipo no cambie. No lo compartas fuera del turno.";
  } else if (opts.mobileHints) {
    lead = "Enlace m\xF3vil (incluye tu identidad). Un solo uso por ticket:";
  } else {
    lead = "Enlace de sala (sin tu identidad). Un solo uso por ticket:";
  }
  if (!opts.permanent && pairing) {
    pinLine = "<div><strong>PIN:</strong> <code>" + esc4(pairing.pin) + "</code></div>";
    var expiryLabel = formatLanTicketExpiryLabel(pairing.expiresAt);
    var expirySoon = lanTicketExpirySoon(pairing.expiresAt);
    if (expiryLabel) {
      expiryLine = '<p class="lan-pairing-expiry' + (expirySoon ? " lan-pairing-expiry--soon" : "") + '" style="margin:8px 0 0;font-size:12px;">V\xE1lido hasta <strong>' + esc4(expiryLabel) + "</strong></p>";
    }
  }
  box.innerHTML = '<p style="margin:0 0 6px;font-size:12px;color:var(--text-muted);">' + esc4(lead) + "</p>" + pinLine + joinLine + expiryLine;
}
async function mintLanPairingFromUi(opts) {
  opts = opts || {};
  try {
    await mintLanPairingTicket();
    var root = document.getElementById("lan-connection-panel-root");
    ...
    runtime4().showToast(
      opts.toastMsg || (opts.mobileHints ? "Enlace m\xF3vil generado. C\xF3pialo abajo o usa \xABCopiar enlace m\xF3vil\xBB." : "Enlace de sala generado. C\xF3pialo abajo o usa \xABCopiar enlace de sala\xBB."),
      "success"
    );
  } catch (e) {
    ...
    if (e && e.status === 401) {
      runtime4().showToast("No autorizado para generar invitaci\xF3n. Revisa el token del anfitri\xF3n.", "error");
      return;
    }
    runtime4().showToast("No se pudo generar enlace / PIN. Intenta de nuevo.", "error");
  }
}
...
// public/js/lan-safety-bundle-builder.mjs
var TYPED_ENTRY_FIELDS = /* @__PURE__ */ new Set([
  "note",
  "indicaciones",
  "labHistory",
  "todos"
]);
function buildSafetyBundleEntries(allEntries, dirtyPatientIds) {
...
function ensureLanSyncPushBridgeWired() {
  ...
  if (!pushBridgeWirePromise) {
    pushBridgeWirePromise = import("/js/chunks/orchestrator-GPN4H2L4.js").then(function() {
      if (!pushBridge && typeof globalThis !== "undefined") {
  ...
  return drainOutbox(rid).then(function(items) {
    ...
    function pushLiveSyncPatchOutbox(envelope) {
      ...
      return trySend().then(function(sent) {
        ...
        return import("/js/chunks/room-VGV6EQKR.js").then(function(mod) {
    ...
    function pushOutboxItem(item) {
      ...
      if (item.kind === "nota_replace") {
        return pushTypedMutationToHost("/patients/" + encodeURIComponent(item.payload.patientId) + "/nota", item.payload.data !== void 0 ? item.payload : { data: item.payload });
      }
      if (item.kind === "indicaciones_replace") {
        return pushTypedMutationToHost("/patients/" + encodeURIComponent(item.payload.patientId) + "/indicaciones", item.payload.data !== void 0 ? item.payload : { data: item.payload });
      }
    ...
    function outboxItemSucceeded(result) {
      return result === true || result === BUNDLE_PUSH_HANDLED || result === CLINICAL_OPS_HANDLED || !!(result && result.removeOutbox);
    }
...
var UNTYPED_SAFETY_BUNDLE_DEBOUNCE_MS = 3e4;
var _dirtyUntypedByDomain = /* @__PURE__ */ new Map();
var untypedSafetyBundleTimer = null;
...
async function buildUntypedSafetyBundleEnvelope(roomId) {
  ...
  return {
    ...
    roomId: rid,
    clientId: full.clientId || getLanClientId2(),
    entries,
    entriesPartial: true
  };
}
...
function scheduleUntypedSafetyBundle() {
...
function finishReconcilePhase(rid, b) {
  ...
  void import("/js/chunks/room-VGV6EQKR.js").then(function(mod) {
...
async function reconcileLiveSyncRoomBody(roomId) {
  ...
  try {
    ...
    try {
      ...
      const fetchMs = isMobileWeb() ? window.__RPC_MOBILE_SYNC_BUNDLE_DONE__ ? 2e4 : 12e3 : 8e3;
      ...
      if (resp && resp.status === 404) {
        ...
        if (isMobileWeb() && typeof b.showToast === "function") {
          b.showToast(
            "El anfitri\xF3n a\xFAn no comparti\xF3 pacientes en esta sala. En la Mac anfitri\xF3n: abre \u21C4 y pulsa \xABUnirse\xBB en la misma sala; luego en el iPad \u21C4 \u2192 Unirse otra vez.",
            "warn"
          );
        }
    ...
    if (sources.length) {
      ...
      try {
        if (!bundleHadClinicalOps && isClinicalOpsLanAvailable() && typeof b.fetchAndApplyClinicalOpsFromHost === "function") {
          const opsApplied = await b.fetchAndApplyClinicalOpsFromHost(rid, { skipGossipPush: true });
          if (opsApplied && merged && merged.entries && merged.entries.length && typeof b.reapplyLanPatientEntries === "function") {
            await b.reapplyLanPatientEntries(merged.entries);
          }
        }
      } catch (_eOps) {
      }
      if (isMobileWeb() && hostBundleLoaded && (!merged || !merged.entries || !merged.entries.length) && (!patients || !patients.length)) {
        if (typeof b.showToast === "function") {
          b.showToast(
            "La sala est\xE1 vac\xEDa en el anfitri\xF3n. Confirma que la Mac anfitri\xF3n tiene pacientes y est\xE1 unida a esta sala.",
            "info"
          );
        }
      }
...
async function reconcileLiveSyncRoom(roomId, options) {
  var rid = String(roomId || ensureEffectiveLiveSyncRoomId() || "").trim();
  if (!rid) return false;
  var opts = options || {};
  ...
  var now = Date.now();
  if (!opts.force && opts.reason !== "revision-hint" && now - reconcileLastFinishedAt < getReconcileCooldownMs()) {
  ...
  reconcileInFlight = reconcileLiveSyncRoomBody(rid).finally(function() {
...
// public/js/lan-command-room-order.mjs
function shouldApplyCommandBroadcast(state, msg) {
...
function updateCommandSeqState(state, msg) {
  return {
    ...state || {},
...
function ensureLanSyncRoomBridgeWired() {
  ...
  if (!roomBridgeWirePromise) {
    roomBridgeWirePromise = import("/js/chunks/orchestrator-GPN4H2L4.js").then(function() {
      if (!roomBridge && typeof globalThis !== "undefined") {
...
function liveSyncStatusChromeDetail(roomLabel, phase) {
  var prefix = roomLabel ? "Sala: " + roomLabel + " \xB7 " : "";
  ...
  return "Conexi\xF3n LAN / LiveSync";
}
function syncLiveSyncStatusChrome() {
  var btn = document.getElementById("btn-header-team-sync");
  if (!btn) return;
  var roomLabel = activeLiveSyncRoomId ? activeLiveSyncRoomLabel || activeLiveSyncRoomId : "";
  var phase = activeLiveSyncRoomId ? getRoomSyncPhase(activeLiveSyncRoomId) : RoomSyncPhase.offline;
  var chromeClass = liveSyncStatusChromeClass(phase);
  var detail = liveSyncStatusChromeDetail(roomLabel, phase);
  btn.className = "btn-header-icon btn-livesync-header btn-livesync-header--" + chromeClass;
  btn.title = detail;
...
function resumeAutoHostDetectAndReconnect() {
  resumeAutoHostDetect();
...
var _shiftPinRediscoverInFlight = false;
function startLiveSyncReconnectLoop() {
  ...
  function tick() {
    ...
    if (_liveSyncReconnectAttempt >= 2 && !_shiftPinRediscoverInFlight && canAttemptAutoHostDetect()) {
      ...
      void import("/js/chunks/lan-shift-pin-connect-TRVOZWZF.js").then(function(mod) {
...
function bootLanRoomMembership() {
  ...
  void (async function() {
    var rid = m.roomId;
    try {
      const accessMod = await import("/js/chunks/clinical-access-runtime-LDG3LE6E.js");
      ...
      var liveOpen = await waitForLiveChannelOpen(rid, 8e3);
      if (!liveOpen) {
        recordLanSyncError({
          op: "live-ws",
          code: "TIMEOUT",
          message: "Canal live no conect\xF3 en 8s; sync HTTP sigue activo"
        });
        try {
          const pinMod = await import("/js/chunks/lan-shift-pin-connect-TRVOZWZF.js");
          if (typeof pinMod.tryEasyLanShiftPinConnect === "function") {
...
function onLiveSyncWireMessageBody(data) {
  ...
  if (data.type === "livesync:leave" && data.bundle && data.clientId !== myId) {
    void bridge2().applyLiveSyncMerged(
      mergeLiveSyncFullBundles([data.bundle, bridge2().buildLiveSyncLocalMergeSource()])
    );
    return;
  }
  ...
  if (data.type === "livesync:bundle") {
    var mergedBundle = mergeLiveSyncFullBundles([data, bridge2().buildLiveSyncLocalMergeSource()]);
    void bridge2().applyLiveSyncMerged(mergedBundle);
    return;
  }
  if (data.type === "livesync:delta:applied") {
    bridge2().applyLiveSyncDeltaApplied(data);
    return;
  }
...
async function fetchClinicalOpsFromAlternateHost(hostUrl, roomId, options = {}) {
  const url = String(hostUrl || "").trim().replace(/\/+$/, "");
  ...
  const cfg = typeof storage.getLanConfig === "function" ? storage.getLanConfig() || {} : {};
  const ownUrl = String(cfg.hostUrl || "").trim().replace(/\/+$/, "");
...
async function fetchAndApplyClinicalOpsFromHost(roomId, options = {}) {
  const rid = String(roomId || "").trim();
  ...
  try {
    ...
    recordClinicalOpsTrace("get", {
      ...
      incomingUsers: Array.isArray(body?.snapshot?.clinical_users) ? body.snapshot.clinical_users.length : 0,
      ok: true
    });
    if (body && body.revision != null) {
...
function syncLiveSyncAfterRoomJoinBody(rid) {
  ...
  if (isClinicalOpsLanAvailable()) {
    chain = chain.then(function() {
      return import("/js/chunks/clinical-ops-lan-EIR2EKRI.js").then(function(mod) {
  ...
  return chain.then(function() {
    ...
    void import("/js/chunks/clinical-access-runtime-LDG3LE6E.js").then(function(accessMod) {
      ...
      runtime5().renderPatientList({ silent: true });
    });
    void import("/js/chunks/historia-clinica-lan-sync-INLQPSSK.js").then(function(m) {
      return m.scheduleFlushAllPendingHistoriaClinicaLanSync();
    });
...
async function joinLanRoom(roomId, displayName, opts) {
  opts = opts || {};
  ...
  if (activeLiveSyncRoomId === id && String(lanClient.liveRoomId || "") === id && lanClient.liveConnected) {
    ...
    syncLiveSyncStatusChrome();
    bridge2().patchLanPanelJoinButtons();
    if (!silent) runtime5().showToast("Ya est\xE1s en esta sala", "success");
    return;
  }
...
export {
  ...
  maybeShowLanMigrationNotice2 as maybeShowLanMigrationNotice,
  ...
  pushBundleToHostUrl2 as pushBundleToHostUrl,
  consolidateIntoHost,
  ...
  configureLanFromMobileJoin2 as configureLanFromMobileJoin,
  ...
  windowHandlers3 as windowHandlers2,
  ...
  ensureTeamAssignedPatientsOnDevice2 as ensureTeamAssignedPatientsOnDevice,
...
//# sourceMappingURL=/js/chunks/chunk-INNDSZUX.js.map
