...
async function openLanConnectPanelForPin() {
  try {
    const { openConnectionDropdown, focusLanShiftPinInput } = await import("/js/chunks/lan-sync-L5G3XZWH.js");
...
async function handleLanConnectCtaClick() {
  ...
  if (/^\d{6}$/.test(savedPin)) {
    try {
      const { tryEasyLanShiftPinConnect } = await import("/js/chunks/lan-shift-pin-connect-ABKWS4UN.js");
...
async function isLanConnectCtaVisible() {
  ...
  try {
    const lan = await import("/js/chunks/lan-sync-L5G3XZWH.js");
...
async function openMiRotacion() {
  ...
  const { ensureClinicalPanelSession } = await import("/js/chunks/clinical-panel-host-DZNH7ZRD.js");
  const sessionOk = await ensureClinicalPanelSession();
  if (!sessionOk) {
    const mainMod = await import("/js/chunks/clinical-onboarding-main-5LBHTTT5.js");
  ...
  const { wireClinicalTeamsModalChrome } = await import("/js/chunks/teams-roster-modal-chrome-R767WB7P.js");
  wireClinicalTeamsModalChrome();
  const { openClinicalTeamsPanel } = await import("/js/chunks/teams-roster-QGFQRNWU.js");
...
//# sourceMappingURL=/js/chunks/chunk-GXOE2WNM.js.map
