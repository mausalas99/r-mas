...
} from "/js/chunks/chunk-6SG46GPE.js";
import {
  clinicalSessionContext,
  isDbMode
} from "/js/chunks/chunk-TWXFHMQR.js";
import {
...
async function refreshOnboardingHost() {
  const { refreshMainClinicalOnboardingIfNeeded } = await import("/js/chunks/clinical-onboarding-main-M6XLNQK2.js");
...
async function handleUsernameStepSubmit(ev) {
  ...
  if (!sala) {
    if (errEl) {
      errEl.textContent = "Selecciona tu rotaci\xF3n.";
      errEl.hidden = false;
    }
    return;
  }
  ...
  if (needsClaim) {
    const { assertLanRoomForUsernameRegister } = await import("/js/chunks/clinical-profile-lan-sync-QDSTEDTY.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  if (shiftPin && !isClinicalLocalOnlyMode()) {
    const { connectLanWithShiftPin } = await import("/js/chunks/lan-shift-pin-connect-Q4YIPJN4.js");
  ...
  } = await import("/js/chunks/clinical-profile-lan-sync-QDSTEDTY.js");
  ...
  if (!localOnly && !lanPush.ok && isLanProfileNeedsConnectCode(lanPush.code)) {
    ...
    const rot = await import("/js/chunks/clinical-rotation-entry-CBKBATQU.js");
    rot.syncClinicalRotationEntryChrome();
    const { refreshMainClinicalOnboardingIfNeeded: refreshMainClinicalOnboardingIfNeeded2 } = await import("/js/chunks/clinical-onboarding-main-M6XLNQK2.js");
...
async function handleResumeIdentityClick() {
  ...
  if (!existing?.user_id) {
    if (errEl) {
      errEl.textContent = `No encontramos @${username} en esta base de datos. Para registrarte, completa el formulario y pulsa Guardar perfil.`;
      errEl.hidden = false;
    }
    return;
  }
  ...
  try {
    ...
    if (needsProfileOnboarding()) {
      toast2("Completa tu perfil y pulsa Guardar perfil.", "info");
    }
  } finally {
    if (resumeBtn instanceof HTMLButtonElement) {
...
async function renderOnboardingPanelInto(host) {
  ...
  if (!userId) {
    ...
    const { buildOnboardingSessionBlockHtml } = await import("/js/chunks/clinical-onboarding-main-M6XLNQK2.js");
  ...
  if (!needsProfileOnboarding()) {
    const { hideMainClinicalOnboarding } = await import("/js/chunks/clinical-onboarding-main-M6XLNQK2.js");
    hideMainClinicalOnboarding();
    if (host.closest("#clinical-teams-panel-body")) {
      const { renderClinicalTeamsPanel } = await import("/js/chunks/clinical-teams-W3LNBDO3.js");
...
//# sourceMappingURL=/js/chunks/chunk-GISUJOQD.js.map
