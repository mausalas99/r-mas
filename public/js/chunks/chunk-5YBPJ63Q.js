...
} from "/js/chunks/chunk-7CKRCX6S.js";
import {
  RoomSyncPhase,
  clearRoomSyncPhase,
  getRoomSyncPhase,
...
function toggleGuardiaMode() {
  if (isGuardiaMode()) {
    void import("/js/chunks/entrega-roster-panel-CE35XDUX.js").then(({ deactivateTurnoActivo: deactivateTurnoActivo2, closeEntregaRosterPanel: closeEntregaRosterPanel2 }) => {
      deactivateTurnoActivo2();
      closeEntregaRosterPanel2();
    });
    void import("/js/chunks/guardia-phase-bar-7O3ZOF3W.js").then(({ teardownGuardiaPhaseBar: teardownGuardiaPhaseBar2 }) => {
...
function setUiDensity(mode) {
  ...
  void import("/js/chunks/clinical-rotation-entry-YCQF34NF.js").then((mod) => {
...
async function ensureClinicalPanelSession() {
  ...
  try {
    const { applyClinicalDbUnlockCompletion } = await import("/js/chunks/db-unlock-UMDAF5PX.js");
...
async function publishClinicalTeamsToLan() {
  try {
    const mod = await import("/js/chunks/lan-sync-2N45OU3K.js");
...
async function pullClinicalOpsFromLanRoom(options = {}) {
  ...
  lanClinicalOpsPullInFlight = (async () => {
    try {
      const lan = await import("/js/chunks/lan-sync-2N45OU3K.js");
...
Desaparecer\xE1 del directorio LAN. Las dem\xE1s R+ en la misma sala \u21C4 lo quitar\xE1n al sincronizar.`
  ...
  const { isBenignLanPushSkipCode: isBenignLanPushSkipCode2 } = await import("/js/chunks/clinical-profile-lan-sync-2FCQKDYH.js");
...
async function pullLanDirectoryFromHostIfDue(options = {}) {
  ...
  try {
    const lanMod = await import("/js/chunks/lan-sync-2N45OU3K.js");
...
async function renderClinicalTeamsPanelInto(host, opts = {}) {
  ...
  const { wireRenderedClinicalTeamsPanel: wireRenderedClinicalTeamsPanel2 } = await import("/js/chunks/teams-roster-interactions-4PK6J27R.js");
...
async function refreshTeamsUiAfterChange() {
  const { isLanDirectoryModalOpen: isLanDirectoryModalOpen2 } = await import("/js/chunks/teams-roster-lan-LSIFT3MI.js");
  if (isLanDirectoryModalOpen2()) return;
  const { refreshClinicalPatientListForScope: refreshClinicalPatientListForScope2 } = await import("/js/chunks/clinical-access-runtime-WPSDP3GD.js");
  await refreshClinicalPatientListForScope2({ allowLanPull: false });
  import("/js/chunks/clinical-rotation-entry-YCQF34NF.js").then((m) => m.syncClinicalRotationEntryChrome());
...
async function openClinicalTeamsPanel(opts = {}) {
  ...
  if (!sessionOk) {
    const mainMod = await import("/js/chunks/clinical-onboarding-main-6BAEIL4V.js");
  ...
  try {
    if (!opts.skipProfileGate) {
      const { needsClinicalOnboarding } = await import("/js/chunks/clinical-onboarding-DIOOMC3I.js");
      if (needsClinicalOnboarding()) {
        closeClinicalTeamsPanel();
        const { openMiRotacion } = await import("/js/chunks/clinical-rotation-entry-YCQF34NF.js");
  ...
  try {
    const { stopLanAutoDiscovery: stopLanAutoDiscovery2 } = await import("/js/chunks/panel-63CSSLDF.js");
...
function closeClinicalTeamsPanel() {
  ...
  import("/js/chunks/panel-63CSSLDF.js").then((m) => {
...
async function handleProfileFormSubmit(ev) {
  ...
  if (usernameWillChange) {
    const { assertLanRoomForUsernameRegister: assertLanRoomForUsernameRegister2 } = await import("/js/chunks/clinical-profile-lan-sync-2FCQKDYH.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  const { flushClinicalProfileToLan: flushClinicalProfileToLan2, LAN_PROFILE_PUSH_FAILED_MSG: LAN_PROFILE_PUSH_FAILED_MSG2, isBenignLanPushSkipCode: isBenignLanPushSkipCode2 } = await import("/js/chunks/clinical-profile-lan-sync-2FCQKDYH.js");
  ...
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
  await refreshTeamsUiAfterChange();
  void import("/js/chunks/lan-sync-2N45OU3K.js").then((mod) => {
    if (typeof mod.scheduleLiveSyncPush === "function") mod.scheduleLiveSyncPush();
  }).catch(() => {
  });
  void import("/js/chunks/patients-BOT4PUMU.js").then((m) => m.renderPatientList()).catch(() => {
  });
}
...
async function joinTeamById(teamId, subAreaFraction) {
  ...
  if (filterJoinedTeams(clinicalSessionContext.teams, clinicalSessionContext.user).some(
    ...
    const { openClinicalTeamsPanel: openClinicalTeamsPanel2 } = await import("/js/chunks/teams-roster-OKF4CZGY.js");
  ...
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
  await publishClinicalTeamsToLan();
  const { refreshTeamsUiAfterChange: refreshTeamsUiAfterChange2 } = await import("/js/chunks/teams-roster-OKF4CZGY.js");
...
async function redirectLanInviteFromTeamJoinField(raw) {
  const text = String(raw || "").trim();
  if (!text) return false;
  const { openConnectionDropdown: openConnectionDropdown2, joinLanFromInviteUi: joinLanFromInviteUi2 } = await import("/js/chunks/lan-sync-2N45OU3K.js");
...
function loadRoster() {
  return import("/js/chunks/teams-roster-OKF4CZGY.js");
}
...
function wireClinicalTeamsControls() {
  void import("/js/chunks/teams-roster-modal-chrome-ZJEU6WTT.js").then((m) => m.wireClinicalTeamsModalChrome());
  if (teamsControlsWired) return;
  teamsControlsWired = true;
  import("/js/chunks/clinical-rotation-entry-YCQF34NF.js").then((mod) => {
  ...
  return new Promise((resolve) => {
    ...
    async function persistGuardiaSelections(selects) {
      ...
      if (!api2?.dbClinicalTeamsGuardiaSet) {
        toast3("Base cl\xEDnica no disponible.", "error");
      ...
      for (const sel of list) {
        ...
        if (!res?.ok) {
          toast3(res?.error || "No se guard\xF3 la guardia.", "error");
          return { ok: false, activated: false };
        }
        activated = true;
      }
    ...
    body.querySelectorAll(".guardia-hoy-self-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        void (async () => {
          ...
          if (!sel) {
            toast3("No se encontr\xF3 el equipo.", "error");
            return;
          }
          sel.value = userId;
          selfBusy = true;
...
async function consolidateIntoHost(winnerUrl, teamCode, opts) {
  opts = opts || {};
  const room = await import("/js/chunks/room-2LDQFCPD.js");
...
async function initLanHostPlugAndPlay() {
  ...
  try {
    const pinMod = await import("/js/chunks/lan-shift-pin-connect-DGNBC2A3.js");
...
function updateLanPairingDisplay(root, opts) {
  ...
  box.hidden = false;
  var joinLine = displayUrl ? '<div><strong>Enlace:</strong> <code style="word-break:break-all;">' + esc(displayUrl) + "</code></div>" : "";
  var lead;
  var pinLine = "";
  var expiryLine = "";
  if (opts.permanent) {
  ...
  if (!opts.permanent && pairing) {
    ...
    if (expiryLabel) {
      expiryLine = '<p class="lan-pairing-expiry' + (expirySoon ? " lan-pairing-expiry--soon" : "") + '" style="margin:8px 0 0;font-size:12px;">V\xE1lido hasta <strong>' + esc(expiryLabel) + "</strong></p>";
    }
  }
  box.innerHTML = '<p style="margin:0 0 6px;font-size:12px;color:var(--text-muted);">' + esc(lead) + "</p>" + pinLine + joinLine + expiryLine;
}
...
function ensureLanSyncPushBridgeWired() {
  ...
  if (!pushBridgeWirePromise) {
    pushBridgeWirePromise = import("/js/chunks/orchestrator-BZD3YGSI.js").then(function() {
      if (!pushBridge && typeof globalThis !== "undefined") {
  ...
  return drainOutbox(rid).then(function(items) {
    ...
    function pushLiveSyncPatchOutbox(envelope) {
      ...
      return trySend().then(function(sent) {
        ...
        return import("/js/chunks/room-2LDQFCPD.js").then(function(mod) {
...
function finishReconcilePhase(rid, b) {
  ...
  void import("/js/chunks/room-2LDQFCPD.js").then(function(mod) {
...
async function reconcileLiveSyncRoom(roomId, options) {
  ...
  var now = Date.now();
  if (!opts.force && opts.reason !== "revision-hint" && now - reconcileLastFinishedAt < RECONCILE_COOLDOWN_MS) {
...
function ensureLanSyncRoomBridgeWired() {
  ...
  if (!roomBridgeWirePromise) {
    roomBridgeWirePromise = import("/js/chunks/orchestrator-BZD3YGSI.js").then(function() {
      if (!roomBridge && typeof globalThis !== "undefined") {
...
async function maybeRevertSurrogateToPrimary() {
  ...
  runtime3().showToast("El anfitri\xF3n original volvi\xF3: esta Mac dej\xF3 de ser servidor temporal.", "success");
  bridge2().renderLanPanel();
  return true;
}
  ...
  for (var i = 0; i < targets.length; i += 1) {
    if (await tryReconnectLanToHostUrl(targets[i], teamCode)) {
      if (targets[i] !== primary) {
        runtime3().showToast("Reconectado al nuevo anfitri\xF3n de la sala.", "success");
      } else if (!isSurrogateHostActive()) {
        runtime3().showToast("Anfitri\xF3n original de vuelta.", "success");
      }
      return;
    }
  }
...
function applyRoomSyncPhaseAfterReconcile(roomId) {
  ...
  if (lanClient.liveConnected && (liveRid === rid || !liveRid)) {
    ...
    if (canAttemptAutoHostDetect()) {
      void import("/js/chunks/lan-shift-pin-connect-DGNBC2A3.js").then(function(m) {
...
function startLiveSyncReconnectLoop() {
  ...
  function tick() {
    ...
    if (_liveSyncReconnectAttempt >= 2 && !_shiftPinRediscoverInFlight && canAttemptAutoHostDetect()) {
      ...
      void import("/js/chunks/lan-shift-pin-connect-DGNBC2A3.js").then(function(mod) {
...
function bootLanRoomMembership() {
  ...
  void (async function() {
    var rid = m.roomId;
    try {
      const accessMod = await import("/js/chunks/clinical-access-runtime-WPSDP3GD.js");
...
function syncLiveSyncAfterRoomJoinBody(rid) {
  ...
  if (isClinicalOpsLanAvailable()) {
    chain = chain.then(function() {
      return import("/js/chunks/clinical-ops-lan-QVYORILC.js").then(function(mod) {
  ...
  return chain.then(function() {
    ...
    void import("/js/chunks/clinical-access-runtime-WPSDP3GD.js").then(function(accessMod) {
    ...
    void import("/js/chunks/historia-clinica-lan-sync-DUNIBK5E.js").then(function(m) {
...
async function joinLanRoom(roomId, displayName, opts) {
  ...
  if (!id) {
    runtime3().showToast("No se pudo identificar la sala. Vuelve a abrir \u21C4 e int\xE9ntalo.", "error");
    return;
  }
  if (!isLanSessionConfiguredForRest()) {
    runtime3().showToast(
  ...
  if (!lanClient.baseUrl()) {
    runtime3().showToast("Falta la direcci\xF3n del servidor LAN. Config\xFArala en \u21C4 antes de unirte.", "error");
    return;
  }
  if (activeLiveSyncRoomId === id && String(lanClient.liveRoomId || "") === id && lanClient.liveConnected) {
    ...
    syncLiveSyncStatusChrome();
    bridge2().patchLanPanelJoinButtons();
    if (!silent) runtime3().showToast("Ya est\xE1s en esta sala", "success");
    return;
  }
  ...
  try {
    ...
    runtime3().showToast("No se pudo activar relay de sala", "error");
    return;
  }
  if (!silent) {
    runtime3().showToast("Sala: sincronizando expediente, agenda y pendientes", "success");
  }
  syncLiveSyncStatusChrome();
  bridge2().patchLanPanelJoinButtons();
...
function appendLanMobileSharerCard(root) {
  if (!root || !runtime4().isMobileWeb()) return;
...
function lanPanelNeedsFullRebuild(root) {
  ...
  if (!runtime4().isMobileWeb() && !isLanSessionConfiguredForRest() && !root.querySelector("#lan-input-invite-link")) {
    return true;
  }
  return false;
}
...
async function renderLanPanelOnce() {
  ...
  var hubStatus = lanHubStatusCopy();
  var needsInvitePaste = !runtime4().isMobileWeb() && !isLanSessionConfiguredForRest();
  ...
  if (lanPanelRenderStale(gen)) return;
  if (runtime4().isMobileWeb() && !hubStatus.connected) {
  ...
  var appendConflictDrafts = runtime4().appendLanConflictDraftsSection;
...
async function resetLanTurnConnectionFromUi() {
  if (!isLanElectronDesktop()) {
    runtime4().showToast("Solo disponible en la app de escritorio.", "error");
    return;
  }
  var resetMod = await import("/js/chunks/lan-turn-reset-GQBPACHT.js");
...
function appendLanShiftPinClientConnectSection(root, gen) {
  ...
  void shouldShowLanShiftPinClientConnect().then(function(offer) {
    ...
    btn.addEventListener("click", function() {
      ...
      if (!/^\d{6}$/.test(pin)) {
        runtime4().showToast("Ingresa los 6 d\xEDgitos del PIN.", "error");
        return;
      }
      btn.disabled = true;
      void import("/js/chunks/lan-shift-pin-connect-DGNBC2A3.js").then(function(m) {
...
async function appendLanShiftPinSection(root, gen) {
  ...
  try {
    ...
    copyBtn.addEventListener("click", function() {
      ...
      runtime4().showToast("PIN del turno copiado.", "success");
    });
    ...
    regenBtn.addEventListener("click", function() {
      void lanFetchAuthed("/api/lan/v1/auth/shift-pin/regenerate", { method: "POST" }).then(
        function(r) {
          if (r && r.ok) {
            runtime4().showToast("PIN del turno renovado.", "success");
            renderLanPanel2({ force: true });
          } else {
            runtime4().showToast("No se pudo renovar el PIN.", "error");
          }
        }
      );
    });
...
async function appendLanSyncDiagnosticsSection(root) {
  ...
  retryBtn.onclick = function() {
    ...
    if (!rid) {
      runtime4().showToast("No hay sala activa para reintentar.", "warn");
      return;
    }
    void flushLiveSyncOutbox(rid).then(function() {
      runtime4().showToast("Cola reintentada. Revisa el informe abajo.", "info");
      renderLanPanel2({ force: true });
    });
  };
...
async function openR4TeamCreationModal() {
  try {
    var mod = await import("/js/chunks/clinical-teams-ID3YITEV.js");
    if (typeof mod.openClinicalTeamsPanel === "function") {
      mod.openClinicalTeamsPanel();
    } else {
      runtime4().showToast("Panel de equipos no disponible.", "error");
    }
  } catch (_e) {
    runtime4().showToast("Panel de equipos no disponible.", "error");
  }
...
async function handleFinalizarRotacion() {
  ...
  if (!api2 || typeof api2.dbRotationNueva !== "function") {
    runtime4().showToast("Operaci\xF3n no disponible.", "error");
    return;
  }
  var user = typeof clinicalSessionContext !== "undefined" ? clinicalSessionContext.user : null;
  ...
  if (res && res.ok) {
    runtime4().showToast("Rotaci\xF3n finalizada. Crea nuevos equipos para el siguiente mes.", "success");
    document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
    renderLanPanel2({ force: true });
  } else {
    runtime4().showToast(res && res.error || "No se pudo finalizar la rotaci\xF3n.", "error");
  }
}
function buildR4Section(root) {
  ...
  viewBtn.onclick = function() {
    ...
    document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
    if (typeof runtime4().renderPatientList === "function") runtime4().renderPatientList();
    runtime4().showToast("Censo global \u2014 usa los filtros en la lista de pacientes.", "info");
  };
...
async function joinClinicalTeam(teamId) {
  var api2 = typeof window !== "undefined" ? window.rplusDb || window.electronAPI : null;
  if (!api2 || typeof api2.dbClinicalTeamsMemberAdd !== "function") {
    runtime4().showToast("Base de datos no disponible.", "error");
    return;
  }
  var userId = getClinicalUserUserId();
  if (!userId) {
    runtime4().showToast("No hay sesi\xF3n cl\xEDnica activa.", "error");
    return;
  }
  var addRes = await api2.dbClinicalTeamsMemberAdd({ teamId, userId });
  if (!addRes || addRes.ok === false) {
    runtime4().showToast(addRes?.error || "No se pudo unir al equipo.", "error");
    return;
  }
  var rank = getClinicalRank();
  if (rank === "R2" && api2 && typeof api2.dbClinicalTeamsPromoteLeader === "function") {
    ...
    if (!promoteRes || promoteRes.ok === false) {
      runtime4().showToast("Unido al equipo pero no se pudo asignar como l\xEDder.", "warn");
    }
  }
  runtime4().showToast("Unido al equipo.", "success");
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
...
async function saveLanHostTeamCodeFromUi() {
  if (!window.electronAPI || typeof window.electronAPI.writeLanHostTeamCode !== "function") {
    runtime4().showToast("Solo disponible en la app Electron", "error");
    return;
  }
  ...
  try {
    ...
    runtime4().showToast(e && e.message ? e.message : "Error al guardar", "error");
    return;
  }
  if (res && res.ok) {
    var plainTrim = String(plain || "").trim();
    if (!plainTrim) {
      runtime4().showToast("Escribe un token de al menos 32 caracteres.", "error");
      return;
    }
    var cfg = typeof storage.getLanConfig === "function" ? storage.getLanConfig() || {} : {};
    var hostUrl = String(cfg.hostUrl || "").trim().replace(/\/+$/, "");
    ...
    runtime4().showToast("Guardado. Reinicia R+ para que el proceso del servidor relea el archivo.", "success");
  } else {
    runtime4().showToast(res && res.error ? res.error : "Error al guardar", "error");
  }
}
async function resetLanSquadHostStateFromUi() {
  ...
  try {
    ...
    runtime4().showToast(e && e.message ? e.message : "Error al restablecer", "error");
    return;
  }
  if (res && res.ok) {
    ...
    runtime4().showToast(res && res.error ? res.error : "No se pudo borrar el archivo.", "error");
  }
}
async function copyMobileLanLinkFromUi(opts) {
  ...
  if (!silent) runtime4().showToast("No se pudo copiar al portapapeles.", "error");
  return false;
}
  ...
  try {
    ...
    if (!silent) {
      if (e && e.code === "no_host_url") {
        ...
        runtime4().showToast("Genera primero un enlace / PIN o revisa el token del anfitri\xF3n.", "error");
      }
    }
    return false;
  }
...
function joinLanFromInviteUi() {
  ...
  if (!raw) {
    runtime4().showToast("Pega el enlace de invitaci\xF3n que te envi\xF3 el anfitri\xF3n.", "error");
    return;
  }
...
async function saveLanSettingsFromUi(opts) {
  ...
  if (pingOk) {
    ...
    if (copyInviteAfter) {
      ...
      runtime4().showToast("Listo: ya iniciaste sesi\xF3n en la sala del equipo.", "success");
    }
  } else if (pingStatus === 401) {
    runtime4().showToast("El c\xF3digo no coincide con el del servidor. Pide el c\xF3digo correcto a quien tiene la computadora anfitriona.", "error");
  } else {
...
async function createLanRoomFromUi() {
  if (!isLanSessionConfiguredForRest()) {
    runtime4().showToast("Falta la direcci\xF3n LAN. Configura la conexi\xF3n en \u21C4 y vuelve a intentar.", "error");
    return;
  }
  await ensureLanClientTeamCodeAligned();
  ...
  if (!displayName) {
    runtime4().showToast("Escribe un nombre de sala", "error");
    return;
  }
  var resp;
  try {
    ...
    runtime4().showToast("No se pudo crear la sala", "error");
    return;
  }
  if (!resp.ok) {
    if (resp.status === 401) {
...
async function deleteLanRoom(roomId) {
  if (!isLanSessionConfiguredForRest()) {
    runtime4().showToast("Falta configuraci\xF3n LAN para eliminar salas.", "error");
    return;
  }
  await ensureLanClientTeamCodeAligned();
  var id = String(roomId || "").trim();
  ...
  try {
    ...
    runtime4().showToast("No se pudo eliminar la sala", "error");
    return;
  }
  if (!resp.ok) {
    if (resp.status === 401) {
      runtime4().showToast("El c\xF3digo del equipo no coincide con el servidor; no se pudo eliminar la sala.", "error");
    } else {
      runtime4().showToast("No se pudo eliminar la sala", "error");
    }
    return;
  }
  runtime4().showToast("Sala eliminada", "success");
  renderLanPanel2({ force: true });
}
...
function openConnectionDropdown() {
  runtime4().closeSettingsDropdown();
  var dd = document.getElementById("connection-dropdown");
  var bg = document.getElementById("connection-dropdown-backdrop");
  if (!dd) return;
...
function wireLanNetworkRefresh() {
  ...
  window.addEventListener("online", function() {
    void import("/js/chunks/room-2LDQFCPD.js").then(function(m) {
      if (typeof m.resumeAutoHostDetectAndReconnect === "function") {
        m.resumeAutoHostDetectAndReconnect();
      }
    });
    void initLanHostPlugAndPlay();
    if (typeof renderLanPanel2 === "function") void renderLanPanel2();
  });
}
function registerLanRuntime(ctx) {
  if (!ctx || typeof ctx !== "object") return;
  Object.assign(runtime5, ctx);
  ...
  function transmit() {
    ...
    void guardAndSignLiveSyncMutation(mutation, envelope).then(function() {
      ...
      if (err && err.code === "CLINICAL_ACCESS_DENIED") {
        runtime5.showToast(String(err.message || "Acceso cl\xEDnico denegado"), "error");
      }
    });
    return true;
  }
...
async function applyConflictUseServer(payload) {
  ...
  if (server && server.data) {
    if (payload.entityType === "historiaClinica" && payload.patientId) {
      ...
      if (hcRow) {
        var mod = await import("/js/chunks/historia-clinica-lan-sync-DUNIBK5E.js");
...
async function appendLanConflictDraftsSection(root) {
  ...
  bulkBtn.onclick = function() {
    ...
    void clearAllDraftConflicts().then(function(cleared) {
      ...
      runtime5.showToast("No se pudieron descartar los borradores.", "error");
    }).finally(function() {
...
async function applyLwwConflictLocally(payload) {
  ...
  if (shouldAutoResolveTodoConflict(payload) && tryAutoResolveTodoConflict(payload)) {
    ...
    if (!localDelete) {
      runtime5.showToast("Pendiente alineado con la sala", "info");
    }
    return;
  }
...
function collectPatientEntriesForLanSync() {
  ...
  patients.forEach(function(p) {
    ...
    var entry = runtime5.buildPatientEntry(p.id);
...
function applyLanPatientEntries(entries, opts) {
  ...
  for (var i = 0; i < scopedEntries.length; i += 1) {
    ...
    var existing = reg ? runtime5.findPatientByRegistro(reg) : null;
    ...
    if (existing) {
      ...
      if (remoteId && !idTaken) {
        var newPat = {
          id: remoteId,
          nombre: runtime5.ensureUniquePatientName(entry.patient.nombre || "PACIENTE SIN NOMBRE"),
        ...
        newId = runtime5.applyImportEntry(entry, "duplicate", null);
      }
  ...
  if (added || updated) {
    ...
    if (runtime5.getActiveId()) {
      try {
        runtime5.renderNoteForm();
      } catch (_e) {
      }
      try {
        runtime5.renderLabHistoryPanel();
      } catch (_e2) {
      }
    }
  }
...
function removePatientLocally(patientId) {
  ...
  if (runtime5.getActiveId() === pid) runtime5.setActiveId(patients.length ? patients[0].id : null);
  return true;
}
function applyLiveSyncPatientDeletes(deletes, idMap) {
  ...
  for (var i = 0; i < deletes.length; i += 1) {
    ...
    if (reg) {
      var existing = runtime5.findPatientByRegistro(reg);
...
async function applyLiveSyncMerged(merged) {
  ...
  if (patientRemoved) {
    renderPatientListLanSilent();
    if (runtime5.getActiveId()) runtime5.selectPatient(runtime5.getActiveId());
    else {
      var pv = document.getElementById("patient-view");
      var es = document.getElementById("empty-state");
      if (pv) pv.style.display = "none";
      if (es) es.style.display = "flex";
      runtime5.syncWorkContextChrome();
    }
  }
  if (runtime5.getActiveAppTab() === "agenda" || runtime5.isMobileWeb()) {
    runtime5.renderProcedureAgendaPanel();
  }
  if (todosChanged) runtime5.refreshAllTodoUIs();
  if (patientsChanged && runtime5.getActiveId()) {
    try {
      runtime5.renderNoteForm();
    } catch (_eNote) {
    }
...
async function applyLiveSyncDeltaApplied(msg) {
  ...
  if (partial) {
    ...
    runtime5.showToast('Tu cambio en "' + labels.join(", ") + '" fue reemplazado por una edici\xF3n m\xE1s reciente en la sala.', "warn");
  }
  syncHostBundleEntityFromApplied(msg);
}
function applyLiveSyncApplied(msg) {
  ...
  if (entityType === "agenda") {
    ...
    if (row && !entityData._deleted) {
      ...
      if (runtime5.getActiveId() === entityId) {
        try {
          runtime5.renderNoteForm();
        } catch (_eNote) {
        }
  ...
  if (msg.lwwApplied) {
    ...
    runtime5.showToast("Cambios fusionados autom\xE1ticamente con el servidor.", "success");
  }
}
...
function addItemFromForm(formEl) {
  if (!draftActor) {
    toast4("No se pudo agregar el procedimiento. Cierra y vuelve a abrir la entrega.", "error");
    return;
  }
...
function openEntregaRosterPanel(settings) {
  ...
  document.getElementById("roster-btn-cancel")?.addEventListener("click", () => {
    void (async () => {
      ...
      const { endEntregaPhase: endEntregaPhase2 } = await import("/js/chunks/clinical-entrega-S7Q23BHS.js");
      endEntregaPhase2();
...
async function openOrphanEntrega(btn, settings) {
  ...
  try {
    ...
    const entrega = await import("/js/chunks/clinical-entrega-S7Q23BHS.js");
...
function renderCalendarBody(year, month, selectedIso) {
  ...
  for (var day = 1; day <= days; day++) {
    ...
    cells += '<button type="button" class="' + cls + '" data-iso="' + esc3(iso) + '">' + day + "</button>";
  }
  return cells;
}
function openRpcDatePicker(anchor, opts) {
  ...
  closePopover();
  function paint() {
    pop.innerHTML = '<div class="rpc-date-popover__head"><button type="button" class="rpc-date-popover__nav" data-nav="-1" aria-label="Mes anterior">\u2039</button><span class="rpc-date-popover__title">' + esc3(MONTHS_ES[viewMonth] + " " + viewYear) + '</span><button type="button" class="rpc-date-popover__nav" data-nav="1" aria-label="Mes siguiente">\u203A</button></div><div class="rpc-date-popover__weekdays">' + WEEKDAYS_ES.map(function(w) {
      return "<span>" + w + "</span>";
...
function renderEntryCard(entry, editingId) {
  ...
  return '<article class="ev-card' + (isEditing ? " ev-card--editing" : "") + '" data-entry-id="' + esc4(entry.id) + '"><p class="ev-card__text">' + esc4(normalizeEventualidadText(entry.text)) + '</p><footer class="ev-card__foot"><div class="ev-card__actions"><button type="button" class="ev-card__edit" data-ev-edit="' + esc4(entry.id) + '" aria-label="Editar eventualidad">Editar</button><button type="button" class="ev-card__delete" data-ev-delete="' + esc4(entry.id) + '" aria-label="Eliminar eventualidad">Eliminar</button></div></footer></article>';
}
function renderDaySection(dayGroup, editingId, now) {
  ...
  return '<details class="ev-day' + todayClass + '"' + (isOpen ? " open" : "") + ' data-day="' + esc4(dayGroup.day) + '"><summary class="ev-day__summary"><span class="ev-day__chevron" aria-hidden="true"></span><div class="ev-day__titles"><span class="ev-day__pill">' + esc4(dayGroup.label) + "</span>" + (subLabel ? '<span class="ev-day__date">' + esc4(subLabel) + "</span>" : "") + '</div><span class="ev-day__count">' + esc4(countLabel) + '</span></summary><div class="ev-day__panel">' + dayGroup.entries.map(function(e) {
...
async function applyBootstrapResult(res) {
  ...
  await fetchClinicalScopeContextFromDb();
  if (typeof document !== "undefined") {
    void import("/js/chunks/clinical-profile-lan-sync-2FCQKDYH.js").then((mod) => mod.flushClinicalProfileToLan()).catch(() => {
    });
  }
...
function migrateLocalPatientsClinicalSala() {
  ...
  if (migrated > 0) {
    ...
    if (typeof document !== "undefined") {
      void import("/js/chunks/patients-BOT4PUMU.js").then((mod) => mod.renderPatientList({ silent: true })).catch(() => {
      });
    }
  }
  return migrated;
...
async function ensureTeamAssignedPatientsOnDevice(options) {
  ...
  if (elevated) {
    ...
    const { filterJoinedTeams: filterJoinedTeams2 } = await import("/js/chunks/shared-DKRPSVUL.js");
  ...
  try {
    ...
    if (!rid) return;
    const push = await import("/js/chunks/push-AI7THDR6.js");
...
async function refreshClinicalPatientListForScope(options) {
  ...
  refreshClinicalPatientListForScopeInFlight = (async function() {
    ...
    if (typeof document === "undefined") return;
    try {
      const mod = await import("/js/chunks/patients-BOT4PUMU.js");
...
async function flushClinicalProfileToLan(opts = {}) {
  ...
  const pushMod = await import("/js/chunks/push-AI7THDR6.js");
...
//# sourceMappingURL=/js/chunks/chunk-5YBPJ63Q.js.map
