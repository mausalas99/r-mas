...
} from "/js/chunks/chunk-YY7S5Z4B.js";
...
async function ensureLiveSyncRoomForUsernameRegister(opts = {}) {
  const lan = await import("/js/chunks/lan-sync-46H6GQXG.js");
...
async function ensureClinicalPanelSession() {
  ...
  try {
    const { applyClinicalDbUnlockCompletion } = await import("/js/chunks/db-unlock-Q3VDLHQR.js");
...
async function publishClinicalTeamsToLan() {
  try {
    const mod = await import("/js/chunks/lan-sync-46H6GQXG.js");
...
Desaparecer\xE1 del directorio LAN. Las dem\xE1s R+ en la misma sala \u21C4 lo quitar\xE1n al sincronizar.`
  ...
  const { flushClinicalProfileToLan: flushClinicalProfileToLan2, isBenignLanPushSkipCode: isBenignLanPushSkipCode2 } = await import("/js/chunks/clinical-profile-lan-sync-TLB25IZI.js");
...
function backgroundRefreshLanUsersDirectory() {
  void import("/js/chunks/lan-sync-46H6GQXG.js").then((lanMod) => {
...
async function renderClinicalTeamsPanelInto(host, opts = {}) {
  ...
  const { wireRenderedClinicalTeamsPanel: wireRenderedClinicalTeamsPanel2 } = await import("/js/chunks/teams-roster-interactions-UY2YSBEV.js");
...
async function refreshTeamsUiAfterChange() {
  await fetchClinicalTeamsFromDb();
  import("/js/chunks/clinical-rotation-entry-RR57DVPP.js").then((m) => m.syncClinicalRotationEntryChrome());
...
async function openClinicalTeamsPanel() {
  ...
  try {
    const { needsClinicalOnboarding } = await import("/js/chunks/clinical-onboarding-TMCZ5CZH.js");
    if (needsClinicalOnboarding()) {
      closeClinicalTeamsPanel();
      const { openMiRotacion } = await import("/js/chunks/clinical-rotation-entry-RR57DVPP.js");
...
async function handleProfileFormSubmit(ev) {
  ...
  if (usernameWillChange) {
    const { assertLanRoomForUsernameRegister: assertLanRoomForUsernameRegister2 } = await import("/js/chunks/clinical-profile-lan-sync-TLB25IZI.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  const { flushClinicalProfileToLan: flushClinicalProfileToLan2, LAN_PROFILE_PUSH_FAILED_MSG: LAN_PROFILE_PUSH_FAILED_MSG2, isBenignLanPushSkipCode: isBenignLanPushSkipCode2 } = await import("/js/chunks/clinical-profile-lan-sync-TLB25IZI.js");
  const lanPush = await flushClinicalProfileToLan2();
  if (!lanPush.ok && !isBenignLanPushSkipCode2(lanPush.code)) {
  ...
  syncRotationConfigButton();
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
  void import("/js/chunks/lan-sync-46H6GQXG.js").then((mod) => {
    if (typeof mod.scheduleLiveSyncPush === "function") mod.scheduleLiveSyncPush();
  }).catch(() => {
  });
  void import("/js/chunks/patients-3WYULQQJ.js").then((m) => m.renderPatientList()).catch(() => {
  });
}
...
async function joinTeamById(teamId, subAreaFraction) {
  ...
  if (filterJoinedTeams(clinicalSessionContext.teams, clinicalSessionContext.user).some(
    ...
    const { openClinicalTeamsPanel: openClinicalTeamsPanel2 } = await import("/js/chunks/teams-roster-M5WXUCT3.js");
  ...
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
  await publishClinicalTeamsToLan();
  const { refreshTeamsUiAfterChange: refreshTeamsUiAfterChange2 } = await import("/js/chunks/teams-roster-M5WXUCT3.js");
...
function loadRoster() {
  return import("/js/chunks/teams-roster-M5WXUCT3.js");
}
...
function wireClinicalTeamsControls() {
  void import("/js/chunks/teams-roster-modal-chrome-A74JAVKG.js").then((m) => m.wireClinicalTeamsModalChrome());
  if (teamsControlsWired) return;
  teamsControlsWired = true;
  import("/js/chunks/clinical-rotation-entry-RR57DVPP.js").then((mod) => {
...
async function advertiseHostClinicalRank() {
  if (!isLanElectronDesktop() || isLanRemoteJoinMode()) return;
  var rank = "R1";
  try {
...
async function promoteThisMacToLanHost(opts) {
  ...
  if (!opts.skipOtherHostCheck) {
    ...
    if (teamCode) {
      try {
        var { discoverOtherLanHostOnSubnet: discoverOtherLanHostOnSubnet2 } = await import("/js/chunks/lan-host-discovery-OIH424R7.js");
        var localUrl = await resolveLanShareBaseUrl();
        var other = await discoverOtherLanHostOnSubnet2({
          teamCode,
          localBaseUrl: localUrl || lanClient.baseUrl()
        });
        if (other) {
          var warnMsg = "Ya hay un servidor R+ activo en " + other + ". Si t\xFA eres el anfitri\xF3n del turno, pide al equipo que se unan con tu enlace. \xBFCrear otro servidor en esta Mac?";
...
async function initLanHostPlugAndPlay() {
  ...
  if (teamCode && !getPinnedHostUrl()) {
    try {
      var discovery = await import("/js/chunks/lan-host-discovery-OIH424R7.js");
      var other = await discovery.discoverOtherLanHostOnSubnet({
        teamCode,
        localBaseUrl: ""
      });
      if (other) {
        var bootMsg = "Ya hay un servidor R+ del turno en " + other + ". \xBFConectarte a esa Mac como cliente? (Recomendado si no eres el anfitri\xF3n.)";
...
function appendLanHubStatusCard(root, opts) {
  ...
  const defaultLine = connected2 ? "Conectado a la red del hospital" : "Sin red \u2014 buscando\u2026";
...
var _lanOtherHostWarnedUrls = /* @__PURE__ */ new Set();
...
async function openR4TeamCreationModal() {
  try {
    var mod = await import("/js/chunks/clinical-teams-6ZFPEIYW.js");
...
async function fetchPeerClinicalRank(peerUrl, teamCode) {
  try {
    ...
    return String(data.rank || "").trim();
  } catch (_e) {
    return "";
  }
}
async function considerDiscoveredLanHost(peerUrl, teamCode, currentRank) {
  var url = String(peerUrl || "").trim().replace(/\/+$/, "");
  if (!url || !teamCode) return false;
  var cfg = typeof storage.getLanConfig === "function" ? storage.getLanConfig() || {} : {};
  var currentUrl = String(cfg.hostUrl || "").trim().replace(/\/+$/, "");
  if (url === currentUrl || lanHostBasesSameMachine(url, currentUrl)) return false;
  var peerRank = await fetchPeerClinicalRank(url, teamCode);
  var pinnedScan = getPinnedHostUrl();
  if (shouldSupersede(peerRank, currentRank)) {
    if (pinnedScan && pinnedScan !== url) {
      runtime3().showToast(
        "Anfitri\xF3n fijado: " + pinnedScan + ". No se cambi\xF3 a " + url + " (" + peerRank + ").",
        "info"
      );
      return false;
    }
    var supersedeMsg = "Un anfitri\xF3n de mayor rango (" + peerRank + ") est\xE1 en " + url + ". \xBFConectar como cliente?";
    if (maybeApplyLanHostUrlSwitch(url, teamCode, {
  ...
  if (!_lanOtherHostWarnedUrls.has(url)) {
    _lanOtherHostWarnedUrls.add(url);
    runtime3().showToast(
      "Otro servidor R+ activo en " + url + (peerRank ? " (" + peerRank + ")" : "") + ". Solo debe haber un anfitri\xF3n: pide al equipo tu enlace de \u21C4 o \xFAnete al otro con \xABUnirme a otra computadora\xBB.",
      "warn"
    );
  }
  return false;
...
async function scanLanHosts() {
  ...
  try {
    ...
    for (var i = 0; i < peers.length; i += 1) {
      var peerUrl = peers[i];
      if (!peerUrl || seen.has(peerUrl)) continue;
      seen.add(peerUrl);
      var alive = typeof pingLanHostUrl === "function" ? await pingLanHostUrl(peerUrl, teamCode) : false;
      if (!alive) continue;
      if (await considerDiscoveredLanHost(peerUrl, teamCode, currentRank)) return;
    }
    var now = Date.now();
    if (now - _lastSubnetLanScanAt >= SUBNET_LAN_SCAN_MIN_MS) {
      _lastSubnetLanScanAt = now;
      var localUrl = String(lanClient.baseUrl() || "").trim();
      var subnetHit = await discoverOtherLanHostOnSubnet({
        teamCode,
        localBaseUrl: localUrl
      });
      if (subnetHit && !seen.has(subnetHit)) {
        await considerDiscoveredLanHost(subnetHit, teamCode, currentRank);
      }
    }
  } catch (_scanErr) {
  }
...
async function saveLanSettingsFromUi(opts) {
  ...
  if (pingOk) {
    ...
    void import("/js/chunks/historia-clinica-lan-sync-PFCNXAC7.js").then(function(m) {
...
async function applyConflictUseServer(payload) {
  ...
  if (server && server.data) {
    if (payload.entityType === "historiaClinica" && payload.patientId) {
      ...
      if (hcRow) {
        var mod = await import("/js/chunks/historia-clinica-lan-sync-PFCNXAC7.js");
...
async function applyBootstrapResult(res) {
  ...
  if (typeof document !== "undefined") {
    void import("/js/chunks/clinical-profile-lan-sync-TLB25IZI.js").then((mod) => mod.flushClinicalProfileToLan()).catch(() => {
    });
  }
  migrateLocalPatientsClinicalSala();
}
function migrateLocalPatientsClinicalSala() {
  ...
  if (migrated > 0) {
    ...
    if (typeof document !== "undefined") {
      void import("/js/chunks/patients-3WYULQQJ.js").then((mod) => mod.renderPatientList()).catch(() => {
      });
    }
  }
  return migrated;
...
function ensureLanSyncPushBridgeWired() {
  ...
  if (!pushBridgeWirePromise) {
    pushBridgeWirePromise = import("/js/chunks/orchestrator-2L7VVYYP.js").then(function() {
      if (!pushBridge && typeof globalThis !== "undefined") {
  ...
  return drainOutbox(rid).then(function(items) {
    ...
    function pushLiveSyncPatchOutbox(envelope) {
      ...
      return trySend().then(function(sent) {
        ...
        return import("/js/chunks/room-43IBDA2C.js").then(function(mod) {
...
function finishReconcilePhase(rid, b) {
  ...
  void import("/js/chunks/room-43IBDA2C.js").then(function(mod) {
...
function ensureLanSyncRoomBridgeWired() {
  ...
  if (!roomBridgeWirePromise) {
    roomBridgeWirePromise = import("/js/chunks/orchestrator-2L7VVYYP.js").then(function() {
      if (!roomBridge && typeof globalThis !== "undefined") {
...
//# sourceMappingURL=/js/chunks/chunk-EXPHMT5M.js.map
