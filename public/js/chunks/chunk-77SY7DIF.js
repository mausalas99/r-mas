...
} from "/js/chunks/chunk-653WVBJ7.js";
import {
...
} from "/js/chunks/chunk-XCZPKL2L.js";
...
async function ensureLiveSyncRoomForUsernameRegister(opts = {}) {
  const lan = await import("/js/chunks/lan-sync-IJHD2DVD.js");
...
async function ensureClinicalPanelSession() {
  ...
  try {
    const { applyClinicalDbUnlockCompletion } = await import("/js/chunks/db-unlock-6NVUCPA2.js");
...
async function publishClinicalTeamsToLan() {
  try {
    const mod = await import("/js/chunks/lan-sync-IJHD2DVD.js");
...
Desaparecer\xE1 del directorio LAN. Las dem\xE1s R+ en la misma sala \u21C4 lo quitar\xE1n al sincronizar.`
  ...
  const { flushClinicalProfileToLan: flushClinicalProfileToLan2, isBenignLanPushSkipCode: isBenignLanPushSkipCode2 } = await import("/js/chunks/clinical-profile-lan-sync-SSG5H2A3.js");
...
function backgroundRefreshLanUsersDirectory() {
  void import("/js/chunks/lan-sync-IJHD2DVD.js").then((lanMod) => {
...
async function renderClinicalTeamsPanelInto(host, opts = {}) {
  ...
  const { needsClinicalLanProfileGate: needsClinicalLanProfileGate2, ensureLanProfileGateDeviceReset } = await import("/js/chunks/clinical-settings-NLMARKG2.js");
  ...
  const { wireRenderedClinicalTeamsPanel: wireRenderedClinicalTeamsPanel2 } = await import("/js/chunks/teams-roster-interactions-4UVAMDDO.js");
...
async function refreshTeamsUiAfterChange() {
  await fetchClinicalTeamsFromDb();
  import("/js/chunks/clinical-rotation-entry-4CCHSS2H.js").then((m) => m.syncClinicalRotationEntryChrome());
...
async function openClinicalTeamsPanel() {
  ...
  if (!sessionOk) {
    const mainMod = await import("/js/chunks/clinical-onboarding-main-VSNJIPFL.js");
  ...
  try {
    const { needsClinicalOnboarding } = await import("/js/chunks/clinical-onboarding-FT4M57EM.js");
    if (needsClinicalOnboarding()) {
      closeClinicalTeamsPanel();
      const { openMiRotacion } = await import("/js/chunks/clinical-rotation-entry-4CCHSS2H.js");
...
async function handleProfileFormSubmit(ev) {
  ...
  if (usernameWillChange) {
    const { assertLanRoomForUsernameRegister: assertLanRoomForUsernameRegister2 } = await import("/js/chunks/clinical-profile-lan-sync-SSG5H2A3.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  const { flushClinicalProfileToLan: flushClinicalProfileToLan2, LAN_PROFILE_PUSH_FAILED_MSG: LAN_PROFILE_PUSH_FAILED_MSG2, isBenignLanPushSkipCode: isBenignLanPushSkipCode2 } = await import("/js/chunks/clinical-profile-lan-sync-SSG5H2A3.js");
  const lanPush = await flushClinicalProfileToLan2();
  if (!lanPush.ok && !isBenignLanPushSkipCode2(lanPush.code)) {
  ...
  syncRotationConfigButton();
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
  void import("/js/chunks/lan-sync-IJHD2DVD.js").then((mod) => {
    if (typeof mod.scheduleLiveSyncPush === "function") mod.scheduleLiveSyncPush();
  }).catch(() => {
  });
  void import("/js/chunks/patients-U2H3DFBL.js").then((m) => m.renderPatientList()).catch(() => {
  });
}
...
async function joinTeamById(teamId, subAreaFraction) {
  ...
  if (filterJoinedTeams(clinicalSessionContext.teams, clinicalSessionContext.user).some(
    ...
    const { openClinicalTeamsPanel: openClinicalTeamsPanel2 } = await import("/js/chunks/teams-roster-UD375NHB.js");
  ...
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
  await publishClinicalTeamsToLan();
  const { refreshTeamsUiAfterChange: refreshTeamsUiAfterChange2 } = await import("/js/chunks/teams-roster-UD375NHB.js");
...
function loadRoster() {
  return import("/js/chunks/teams-roster-UD375NHB.js");
}
...
function wireClinicalTeamsControls() {
  void import("/js/chunks/teams-roster-modal-chrome-HL5BZ7MS.js").then((m) => m.wireClinicalTeamsModalChrome());
  if (teamsControlsWired) return;
  teamsControlsWired = true;
  import("/js/chunks/clinical-rotation-entry-4CCHSS2H.js").then((mod) => {
...
function getLocalLanHostMeta() {
  try {
    ...
    const isProgramAdmin = settings.clinicalProgramAdmin === true || settings.clinicalIsProgramAdmin === true;
...
async function consolidateIntoHost(winnerUrl, teamCode, opts) {
  opts = opts || {};
  const room = await import("/js/chunks/room-HCOO6PHX.js");
...
async function promoteThisMacToLanHost(opts) {
  ...
  if (!canLocalMacBeLanHost()) {
    runtime2().showToast(
      "Solo R4 o administrador de programa pueden ser anfitri\xF3n del turno. Busca al anfitri\xF3n en la red.",
      "info"
    );
    return false;
  }
...
function appendLanMobileJoinSection(root) {
  ...
  card.innerHTML = '<div class="lan-connect-card-title">Unirte a la guardia</div><p class="lan-connect-card-hint">Pega el <strong>enlace de invitaci\xF3n</strong> que te comparti\xF3 el anfitri\xF3n (\u21C4 \u2192 <strong>Copiar enlace de invitaci\xF3n</strong>). Tambi\xE9n puedes abrirlo en Safari en iPad.</p>';
...
function appendLanInviteShareCard(root) {
  ...
  card.innerHTML = '<div class="lan-connect-card-title">Invitaci\xF3n al turno</div><p class="lan-connect-card-hint" style="margin:0 0 8px;">Comparte este enlace con otra Mac del equipo o con iPad (misma Wi\u2011Fi). R1\u2013R3 suelen conectarse solos; el enlace sirve si no los detecta.</p>';
  var copyBtn = document.createElement("button");
  copyBtn.type = "button";
  copyBtn.className = "btn-lan-primary";
  copyBtn.style.width = "100%";
  copyBtn.textContent = "Copiar enlace de invitaci\xF3n";
...
async function openR4TeamCreationModal() {
  try {
    var mod = await import("/js/chunks/clinical-teams-A5U2MXK2.js");
...
function lanHubStatusCopy() {
  if (!lanClient.connected) {
    ...
    if (!canLocalMacBeLanHost()) {
      return {
        ...
        hint: "R1\u2013R3 no pueden ser servidor. Pide el enlace al R4 (\u21C4) o p\xE9galo abajo si no te detecta."
      };
    }
    return {
      connected: false,
      line: "Sin red \u2014 buscando anfitri\xF3n en la Wi\u2011Fi del hospital\u2026",
      hint: "Si otra Mac ya es anfitri\xF3n, pide su enlace (\u21C4 \u2192 Copiar enlace de invitaci\xF3n) o usa la secci\xF3n de abajo para pegarlo."
    };
  }
  ...
  return {
    ...
    hint: "Comparte el enlace con \u21C4 \u2192 Copiar enlace de invitaci\xF3n. No activen otro servidor salvo suplente."
  };
}
...
async function copyMobileLanLinkFromUi(opts) {
  ...
  if (copied) {
    ...
    if (!silent) {
      ...
      runtime3().showToast(
        "Enlace m\xF3vil copiado. \xC1brelo en Safari en la misma Wi\u2011Fi; tus pacientes deber\xEDan sincronizar solos." + (expiryHint ? " V\xE1lido hasta " + expiryHint + "." : ""),
        "success"
      );
    }
    return true;
...
async function saveLanSettingsFromUi(opts) {
  ...
  if (pingOk) {
    ...
    void import("/js/chunks/historia-clinica-lan-sync-SLU7DAA3.js").then(function(m) {
...
async function applyConflictUseServer(payload) {
  ...
  if (server && server.data) {
    if (payload.entityType === "historiaClinica" && payload.patientId) {
      ...
      if (hcRow) {
        var mod = await import("/js/chunks/historia-clinica-lan-sync-SLU7DAA3.js");
...
async function applyBootstrapResult(res) {
  ...
  if (typeof document !== "undefined") {
    void import("/js/chunks/clinical-profile-lan-sync-SSG5H2A3.js").then((mod) => mod.flushClinicalProfileToLan()).catch(() => {
    });
  }
  migrateLocalPatientsClinicalSala();
}
function migrateLocalPatientsClinicalSala() {
  ...
  if (migrated > 0) {
    ...
    if (typeof document !== "undefined") {
      void import("/js/chunks/patients-U2H3DFBL.js").then((mod) => mod.renderPatientList()).catch(() => {
      });
    }
  }
  return migrated;
...
async function refreshClinicalUserProfile() {
  const { ensureLanProfileGateDeviceReset } = await import("/js/chunks/clinical-settings-NLMARKG2.js");
...
function ensureLanSyncPushBridgeWired() {
  ...
  if (!pushBridgeWirePromise) {
    pushBridgeWirePromise = import("/js/chunks/orchestrator-ZDSZCYNJ.js").then(function() {
      if (!pushBridge && typeof globalThis !== "undefined") {
  ...
  return drainOutbox(rid).then(function(items) {
    ...
    function pushLiveSyncPatchOutbox(envelope) {
      ...
      return trySend().then(function(sent) {
        ...
        return import("/js/chunks/room-HCOO6PHX.js").then(function(mod) {
...
function finishReconcilePhase(rid, b) {
  ...
  void import("/js/chunks/room-HCOO6PHX.js").then(function(mod) {
...
function ensureLanSyncRoomBridgeWired() {
  ...
  if (!roomBridgeWirePromise) {
    roomBridgeWirePromise = import("/js/chunks/orchestrator-ZDSZCYNJ.js").then(function() {
      if (!roomBridge && typeof globalThis !== "undefined") {
...
//# sourceMappingURL=/js/chunks/chunk-77SY7DIF.js.map
