...
async function openLanConnectPanelForPin() {
  try {
    const { openConnectionDropdown, focusLanShiftPinInput } = await import("/js/chunks/lan-sync-7AJDUYFL.js");
...
async function handleLanConnectCtaClick() {
  ...
  if (/^\d{6}$/.test(savedPin)) {
    try {
      const { tryEasyLanShiftPinConnect } = await import("/js/chunks/lan-shift-pin-connect-L3TATIM2.js");
...
async function isLanConnectCtaVisible() {
  ...
  try {
    const lan = await import("/js/chunks/lan-sync-7AJDUYFL.js");
...
async function openMiRotacion() {
  ...
  const { ensureClinicalPanelSession } = await import("/js/chunks/clinical-panel-host-MVZ6CZ43.js");
  const sessionOk = await ensureClinicalPanelSession();
  if (!sessionOk) {
    const mainMod = await import("/js/chunks/clinical-onboarding-main-MMFJQAUH.js");
  ...
  const { wireClinicalTeamsModalChrome } = await import("/js/chunks/teams-roster-modal-chrome-5EK3M6JV.js");
  wireClinicalTeamsModalChrome();
  const { openClinicalTeamsPanel } = await import("/js/chunks/teams-roster-D7UTK4OR.js");
...
//# sourceMappingURL=/js/chunks/chunk-RGO3SJJC.js.map
