...
async function openLanConnectPanelForPin() {
  try {
    const { openConnectionDropdown, focusLanShiftPinInput } = await import("/js/chunks/lan-sync-MSR5F36O.js");
...
async function handleLanConnectCtaClick() {
  ...
  if (/^\d{6}$/.test(savedPin)) {
    try {
      const { tryEasyLanShiftPinConnect } = await import("/js/chunks/lan-shift-pin-connect-2ODCSOAX.js");
...
async function isLanConnectCtaVisible() {
  ...
  try {
    const lan = await import("/js/chunks/lan-sync-MSR5F36O.js");
...
async function openMiRotacion() {
  ...
  const { ensureClinicalPanelSession } = await import("/js/chunks/clinical-panel-host-RCCYKCLM.js");
  const sessionOk = await ensureClinicalPanelSession();
  if (!sessionOk) {
    const mainMod = await import("/js/chunks/clinical-onboarding-main-F4DX3CBP.js");
  ...
  const { wireClinicalTeamsModalChrome } = await import("/js/chunks/teams-roster-modal-chrome-XAKVVNLO.js");
...
//# sourceMappingURL=/js/chunks/chunk-KHEHVSJL.js.map
