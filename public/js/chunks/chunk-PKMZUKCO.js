...
async function handleUsernameStepSubmit(ev) {
  ...
  if (needsClaim) {
    const { assertLanRoomForUsernameRegister } = await import("./clinical-profile-lan-sync-IHGFYEUR.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  } = await import("./clinical-profile-lan-sync-IHGFYEUR.js");
  ...
  const { refreshMainClinicalOnboardingIfNeeded } = await import("./clinical-onboarding-main-G6AMUHOI.js");
...
async function handleResumeIdentityClick() {
  ...
  try {
    ...
    if (!needsUsernameClaim()) {
      const { refreshMainClinicalOnboardingIfNeeded: refreshMainClinicalOnboardingIfNeeded2 } = await import("./clinical-onboarding-main-G6AMUHOI.js");
...
async function renderOnboardingPanelInto(host) {
  ...
  if (!needsProfileOnboarding()) {
    const { hideMainClinicalOnboarding } = await import("./clinical-onboarding-main-G6AMUHOI.js");
    hideMainClinicalOnboarding();
    if (host.closest("#clinical-teams-panel-body")) {
      const { renderClinicalTeamsPanel } = await import("./clinical-teams-37G4N2SV.js");
...
//# sourceMappingURL=chunk-PKMZUKCO.js.map
