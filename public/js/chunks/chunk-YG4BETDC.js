...
} from "/js/chunks/chunk-OPJSETWU.js";
import {
...
} from "/js/chunks/chunk-YUPUKFLS.js";
import {
  bundledWardShiftPin,
...
} from "/js/chunks/chunk-PHHXUOUN.js";
import {
  RoomSyncPhase,
  clearRoomSyncPhase,
  getRoomSyncPhase,
...
var GUARDIA_CENSUS_FILTER_HINT = "Filtrar censo en la grilla (no es la fase Entrega del turno).";
...
function syncGuardiaModeUI(opts = {}) {
  ...
  if (typeof document !== "undefined") {
    ...
    if (boardBtn) {
      ...
      boardBtn.setAttribute("aria-label", `${GUARDIA_CENSUS_FILTER_HINT} ${active ? GUARDIA_MODE_LABEL_ON : GUARDIA_MODE_LABEL_OFF}`);
    ...
    if (filterHint) filterHint.textContent = GUARDIA_CENSUS_FILTER_HINT;
  }
...
async function ensureLiveSyncRoomForUsernameRegister(opts = {}) {
  const lan = await import("/js/chunks/lan-sync-3EYV46QW.js");
...
async function flushClinicalProfileToLan(opts = {}) {
  ...
  const pushMod = await import("/js/chunks/push-NL7KBDJI.js");
...
function setClinicalTeamsPanelError(message) {
  ...
  if (!host) return;
  host.innerHTML = `
    <p class="clinical-registration-error">${escapeHtml2(message)}</p>
...
async function ensureClinicalPanelSession() {
  ...
  try {
    const { applyClinicalDbUnlockCompletion } = await import("/js/chunks/db-unlock-EE2J2TX2.js");
...
async function publishClinicalTeamsToLan() {
  try {
    const mod = await import("/js/chunks/lan-sync-3EYV46QW.js");
...
function renderLanUserRowHtml(u, teamList, opts = {}) {
  ...
  const handleCell = handleValid ? `<span class="clinical-lan-users-handle">@${escapeHtml3(rawHandle)}</span>` : `<span class="clinical-lan-users-handle clinical-lan-users-handle--pending" title="Falta registrar @usuario en Mi rotaci\xF3n">sin @usuario</span>`;
  const name = escapeHtml3(String(u.clinical_name || "").trim() || "Sin nombre");
  const rankRaw = escapeAttr2(String(u.rank || "R1"));
  const salaLabel = escapeHtml3(String(u.sala || "").trim() || "\u2014");
  const placement = resolveLanUserPlacement(u.user_id, teamList);
  const placementLabel = escapeHtml3(formatLanUserPlacementLabel(placement, String(u.rank || "R1")));
  const teamOptions = renderLanAssignTeamOptionsHtml(teamList, placement?.teamId);
  const cycleOptions = placement?.cycle ? `<option value="${escapeAttr2(placement.cycle)}" selected>${escapeHtml3(formatLanCycleOptionLabel(placement.cycle, String(u.rank || "R1")))}</option>` : '<option value="">\u2014 Ciclo \u2014</option>';
  ...
  const rankSections = LAN_USER_RANK_ORDER.map((rank) => {
    ...
    return `<details class="clinical-lan-rank-group" open>
      <summary class="clinical-lan-rank-group-summary">
        <span class="clinical-lan-rank-group-title">${escapeHtml3(rank)}</span>
...
function syncLanAssignCycleSelect(teamSelect, preferredCycle = "") {
  ...
  cycleSelect.innerHTML = letters.map((letter) => {
    ...
    return `<option value="${escapeAttr2(letter)}" ${letter === defaultCycle ? "selected" : ""}>${escapeHtml3(label)}</option>`;
  }).join("");
...
Desaparecer\xE1 del directorio LAN. Las dem\xE1s R+ en la misma sala \u21C4 lo quitar\xE1n al sincronizar.`
  ...
  const { isBenignLanPushSkipCode: isBenignLanPushSkipCode2 } = await import("/js/chunks/clinical-profile-lan-sync-X3VZ7IR3.js");
...
async function loadLanUsersDirectoryIntoHost(host) {
  ...
  if (!usersRes?.ok) {
    host.innerHTML = `<p class="clinical-teams-empty">${escapeHtml3(usersRes?.error || "No se pudo cargar el directorio.")}</p>`;
    return;
  }
...
async function pullLanDirectoryFromHostBeforeDisplay() {
  try {
    const lanMod = await import("/js/chunks/lan-sync-3EYV46QW.js");
...
async function openLanUsersDirectoryModal() {
  ...
  try {
    ...
    host.innerHTML = `<p class="clinical-teams-empty">${escapeHtml3(
...
function syncCreateTeamCycleField() {
  ...
  daySelect.innerHTML = letters.map((letter) => `<option value="${escapeAttr2(letter)}">${escapeHtml3(letter)}</option>`).join("");
    ...
    <form id="clinical-team-create-form" class="clinical-teams-create-form clinical-teams-create-form--elevated">
      ...
      <div class="field-group">
        ...
        <select id="clinical-team-create-sala" class="profile-input" required>
          ...
          ${CLINICAL_SALAS.map(
    (s) => `<option value="${escapeAttr2(s)}" ${homeSala === s ? "selected" : ""}>${escapeHtml3(s)}</option>`
  ).join("")}
        </select>
      </div>
...
function renderCreateTeamFormStandard() {
  ...
  const serviceOptions = CLINICAL_TEAM_SERVICES.map(
    (svc) => `<option value="${escapeAttr2(svc)}" ${svc === defaultService ? "selected" : ""}>${escapeHtml3(svc)}</option>`
  ).join("");
  const rank = effectiveClinicalRank(clinicalSessionContext.user);
  ...
  const letterOptions = defaultLetters.map((letter) => `<option value="${escapeAttr2(letter)}">${escapeHtml3(letter)}</option>`).join("");
    ...
    <form id="clinical-team-create-form" class="clinical-teams-create-form">
      <div class="field-group" id="clinical-team-sala-group">
        ...
        <select id="clinical-team-create-sala" class="profile-input">
          <option value="">\u2014 Seleccionar sala \u2014</option>
          ${CLINICAL_SALAS.map(
    (s) => `<option value="${escapeAttr2(s)}" ${s === userSala ? "selected" : ""}>${escapeHtml3(s)}</option>`
  ).join("")}
        </select>
      </div>
      <div class="field-group">
      ...
      <div class="clinical-teams-create-service-row">
        ...
        <div class="field-group">
          <label id="clinical-team-create-day-label" for="clinical-team-create-day">${escapeHtml3(defaultMeta.label)}</label>
          <select id="clinical-team-create-day" class="profile-input" required>${letterOptions}</select>
        </div>
      </div>
      <p id="clinical-team-create-day-hint" class="clinical-teams-hint clinical-teams-create-cycle-hint">${escapeHtml3(defaultMeta.hint)}</p>
...
function renderTeamMetaLine(team) {
  ...
  return `<p class="clinical-teams-card-meta">${parts.map((p) => escapeHtml3(p)).join(" \xB7 ")}</p>`;
}
...
function renderTeamPatientCountLine(team) {
  ...
  if (onDevice <= 0 && assignedLan > 0) {
    ...
    return `<p class="clinical-teams-card-meta clinical-teams-card-patients">${escapeHtml3(waiting)}</p>`;
  }
  if (assignedLan > onDevice && assignedLan > 0) {
    ...
    return `<p class="clinical-teams-card-meta clinical-teams-card-patients">${escapeHtml3(visible)} \xB7 ${escapeHtml3(waiting)}</p>`;
  }
  const label = onDevice === 1 ? "1 paciente en censo" : `${onDevice} pacientes en censo`;
  return `<p class="clinical-teams-card-meta clinical-teams-card-patients">${escapeHtml3(label)}</p>`;
}
function renderCycleSelectForRank(team, rank, current, selectId) {
  ...
  const opts = letters.map(
    (l) => `<option value="${escapeAttr2(l)}" ${l === cur ? "selected" : ""}>${escapeHtml3(l)}</option>`
  ).join("");
...
function renderAddMemberCycleSelect(team) {
  ...
  if (!isSalaWardService(service)) {
    ...
    return `<select id="${escapeAttr2(id)}" class="profile-input clinical-teams-add-member-cycle" required>
      ${letters.map((l) => `<option value="${escapeAttr2(l)}">${escapeHtml3(l)}</option>`).join("")}
    </select>`;
  }
  ...
  return `<select id="${escapeAttr2(id)}" class="profile-input clinical-teams-add-member-cycle" required>
    <optgroup label="R2 \xB7 A\u2013F">${r2.map((l) => `<option value="${escapeAttr2(l)}">${escapeHtml3(l)}</option>`).join("")}</optgroup>
    <optgroup label="R1 \xB7 primera l\xEDnea">${r1a.map((l) => `<option value="${escapeAttr2(l)}">${escapeHtml3(l)}</option>`).join("")}</optgroup>
    <optgroup label="R1 \xB7 segunda l\xEDnea">${r1b.map((l) => `<option value="${escapeAttr2(l)}">${escapeHtml3(l)}</option>`).join("")}</optgroup>
  </select>`;
}
function renderMemberRow(m) {
  ...
  const displayName = name ? escapeHtml3(name) : handle;
  const cycle = formatMemberCycleLabel(m);
  const meta = name ? `@${handle} \xB7 ${rank}` : rank;
  const cycleHtml = cycle ? `<span class="clinical-teams-member-cycle">${escapeHtml3(cycle)}</span>` : "";
  ...
  return `
    <div class="clinical-teams-my-cycle-box">
      <form class="clinical-teams-my-cycle-form" data-team-id="${escapeAttr2(teamId)}">
        ...
        <p class="clinical-teams-hint">${escapeHtml3(hint)}</p>
...
function renderTeamEditPanelHtml(team) {
  const teamId = escapeAttr2(String(team.team_id || ""));
  const name = escapeHtml3(String(team.name || ""));
  const sala = String(team.sala || "").trim();
  return `
      ...
      <form class="clinical-teams-edit-form" data-team-id="${teamId}">
        ...
        <div class="field-group">
          ...
          <select id="clinical-edit-sala-${teamId}" class="profile-input clinical-teams-edit-sala" required>
            ${CLINICAL_SALAS.map(
    (s) => `<option value="${escapeAttr2(s)}" ${sala === s ? "selected" : ""}>${escapeHtml3(s)}</option>`
  ).join("")}
          </select>
        </div>
  ...
  return `
    <article class="clinical-teams-card clinical-teams-card--mine" data-team-id="${escapeAttr2(teamId)}">
      ...
      <div class="clinical-teams-invite-box">
        <p class="clinical-teams-invite-code-line">C\xF3digo para invitar: <code class="clinical-teams-invite-code">${escapeHtml3(teamInviteCode(teamId))}</code></p>
...
function renderDirectoryTeamCard(team, opts = {}) {
  ...
  return `
    <article class="clinical-teams-card clinical-teams-card--directory" data-team-id="${escapeAttr2(teamId)}">
      ...
      ${joinHint ? `<p class="clinical-teams-card-join-reason">${escapeHtml3(joinHint)}</p>` : ""}
      ${editPanel}
...
async function renderClinicalTeamsPanelInto(host, opts = {}) {
  ...
  const handleHint = displayHandle ? `<p class="clinical-teams-lead clinical-teams-handle-hint">Tu usuario LAN: <strong>@${escapeHtml3(displayHandle)}</strong> \u2014 comp\xE1rtelo para que te agreguen a un equipo.${savedHandle !== displayHandle ? " Pulsa <strong>Guardar perfil</strong> para registrarlo en la red." : ""}</p>` : "";
  ...
  const clinicalName = profileGatePending ? "" : escapeHtml3(user.clinical_name || "");
  ...
  const profileHandleBanner = displayHandle ? `<p class="clinical-teams-profile-handle">Visible en la red como <strong>@${escapeHtml3(displayHandle)}</strong></p>` : "";
  ...
  const { wireRenderedClinicalTeamsPanel: wireRenderedClinicalTeamsPanel2 } = await import("/js/chunks/teams-roster-interactions-G2O3SVUK.js");
...
function renderCreateTeamSectionHtml() {
  ...
  return `
    <section class="clinical-teams-section clinical-teams-section--create">
      <button type="button" id="btn-clinical-team-create-open" class="btn-save clinical-teams-create-open-btn">${escapeHtml3(openLabel)}</button>
...
async function renderDirectorySectionHtml(opts) {
  ...
  const browseControl = elevated ? `<label class="clinical-teams-browse-label" for="clinical-browse-sala">Sala</label>
        <select id="clinical-browse-sala" class="profile-input clinical-teams-browse-select" aria-label="Explorar equipos por sala">
          ${CLINICAL_SALAS.map(
    (s) => `<option value="${escapeAttr2(s)}" ${browseSala === s ? "selected" : ""}>${escapeHtml3(s)}</option>`
  ).join("")}
          <option value="__all__" ${browseSala === "__all__" ? "selected" : ""}>Todas las salas</option>
        </select>` : "";
  const sectionTitle = elevated ? browseSala === "__all__" ? "Explorar \xB7 todas las salas" : `Explorar \xB7 ${escapeHtml3(browseSala)}` : `Otros equipos \xB7 ${escapeHtml3(browseSala || homeSala)}`;
...
async function refreshTeamsUiAfterChange() {
  const { refreshClinicalPatientListForScope: refreshClinicalPatientListForScope2 } = await import("/js/chunks/clinical-access-runtime-3TILEVDG.js");
  await refreshClinicalPatientListForScope2();
  import("/js/chunks/clinical-rotation-entry-XC3NNFHV.js").then((m) => m.syncClinicalRotationEntryChrome());
...
async function openClinicalTeamsPanel(opts = {}) {
  ...
  if (!sessionOk) {
    const mainMod = await import("/js/chunks/clinical-onboarding-main-HDOIVFAU.js");
  ...
  try {
    if (!opts.skipProfileGate) {
      const { needsClinicalOnboarding } = await import("/js/chunks/clinical-onboarding-P7XLCBT5.js");
      if (needsClinicalOnboarding()) {
        closeClinicalTeamsPanel();
        const { openMiRotacion } = await import("/js/chunks/clinical-rotation-entry-XC3NNFHV.js");
  ...
  try {
    const { stopLanAutoDiscovery: stopLanAutoDiscovery2 } = await import("/js/chunks/panel-MQ423N5X.js");
...
function closeClinicalTeamsPanel() {
  ...
  import("/js/chunks/panel-MQ423N5X.js").then((m) => {
...
async function handleProfileFormSubmit(ev) {
  ...
  if (usernameWillChange) {
    const { assertLanRoomForUsernameRegister: assertLanRoomForUsernameRegister2 } = await import("/js/chunks/clinical-profile-lan-sync-X3VZ7IR3.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  const { flushClinicalProfileToLan: flushClinicalProfileToLan2, LAN_PROFILE_PUSH_FAILED_MSG: LAN_PROFILE_PUSH_FAILED_MSG2, isBenignLanPushSkipCode: isBenignLanPushSkipCode2 } = await import("/js/chunks/clinical-profile-lan-sync-X3VZ7IR3.js");
  ...
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
  await refreshTeamsUiAfterChange();
  void import("/js/chunks/lan-sync-3EYV46QW.js").then((mod) => {
    if (typeof mod.scheduleLiveSyncPush === "function") mod.scheduleLiveSyncPush();
  }).catch(() => {
  });
  void import("/js/chunks/patients-NKQIONHY.js").then((m) => m.renderPatientList()).catch(() => {
  });
}
...
async function joinTeamById(teamId, subAreaFraction) {
  ...
  if (filterJoinedTeams(clinicalSessionContext.teams, clinicalSessionContext.user).some(
    ...
    const { openClinicalTeamsPanel: openClinicalTeamsPanel2 } = await import("/js/chunks/teams-roster-WONMJKFZ.js");
  ...
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed"));
  await publishClinicalTeamsToLan();
  const { refreshTeamsUiAfterChange: refreshTeamsUiAfterChange2 } = await import("/js/chunks/teams-roster-WONMJKFZ.js");
...
async function redirectLanInviteFromTeamJoinField(raw) {
  const text = String(raw || "").trim();
  if (!text) return false;
  const { openConnectionDropdown: openConnectionDropdown2, joinLanFromInviteUi: joinLanFromInviteUi2 } = await import("/js/chunks/lan-sync-3EYV46QW.js");
...
function loadRoster() {
  return import("/js/chunks/teams-roster-WONMJKFZ.js");
}
...
function wireClinicalTeamsControls() {
  void import("/js/chunks/teams-roster-modal-chrome-ALLMZL5L.js").then((m) => m.wireClinicalTeamsModalChrome());
  if (teamsControlsWired) return;
  teamsControlsWired = true;
  import("/js/chunks/clinical-rotation-entry-XC3NNFHV.js").then((mod) => {
  ...
  if (openBtn && !openBtn._rpcTeamsOpenWired) {
    ...
    openBtn.addEventListener("click", () => {
      void import("/js/chunks/clinical-rotation-entry-XC3NNFHV.js").then((m) => m.openMiRotacion());
    });
  }
...
function rebuildGasesFromResults_(rows) {
  ...
  var esc3 = pickBestSectionLine_(rows, "ESC");
  var pfhs = pickBestSectionLine_(rows, "PFHS");
  var na = valueFromSectionLine_(qs, "Na") || valueFromSectionLine_(esc3, "Na") || values.Na;
  var cl = valueFromSectionLine_(qs, "Cl") || valueFromSectionLine_(esc3, "Cl");
...
function procesarLabs(textoBruto) {
  ...
  if (!esSoloGaso) {
    ...
    var esc3 = parseESC_(textoQS);
    if (esc3) resLabs.push(esc3);
...
function ensureLanSyncPushBridgeWired() {
  ...
  if (!pushBridgeWirePromise) {
    pushBridgeWirePromise = import("/js/chunks/orchestrator-WPV3JC2Z.js").then(function() {
      if (!pushBridge && typeof globalThis !== "undefined") {
  ...
  return drainOutbox(rid).then(function(items) {
    ...
    function pushLiveSyncPatchOutbox(envelope) {
      ...
      return trySend().then(function(sent) {
        ...
        return import("/js/chunks/room-RUG7W7UQ.js").then(function(mod) {
...
function finishReconcilePhase(rid, b) {
  ...
  void import("/js/chunks/room-RUG7W7UQ.js").then(function(mod) {
...
async function consolidateIntoHost(winnerUrl, teamCode, opts) {
  opts = opts || {};
  const room = await import("/js/chunks/room-RUG7W7UQ.js");
...
async function promoteThisMacToLanHost(opts) {
  ...
  if (!canLocalMacBeLanHost()) {
    const { getHostEscalationStatus: getHostEscalationStatus2, formatEscalationCountdown: formatEscalationCountdown2 } = await import("/js/chunks/lan-host-escalation-QTEVRYTA.js");
    const esc3 = getHostEscalationStatus2();
    const nextRank = ["R3", "R2", "R1"][esc3.tier] || "R1";
    const msg = esc3.tier < 3 && esc3.msUntilNext > 0 ? "Sin R4 en la red: en " + formatEscalationCountdown2(esc3.msUntilNext) + " podr\xE1 anfitrionar " + nextRank + " (escalada 10 min por nivel)." : "A\xFAn no puedes ser anfitri\xF3n en esta Mac. Busca al R4 o espera la escalada autom\xE1tica.";
...
async function initLanHostPlugAndPlay() {
  ...
  try {
    const pinMod = await import("/js/chunks/lan-shift-pin-connect-MAVG5ZIR.js");
...
function ensureLanSyncRoomBridgeWired() {
  ...
  if (!roomBridgeWirePromise) {
    roomBridgeWirePromise = import("/js/chunks/orchestrator-WPV3JC2Z.js").then(function() {
      if (!roomBridge && typeof globalThis !== "undefined") {
...
function applyRoomSyncPhaseAfterReconcile(roomId) {
  ...
  if (lanClient.liveConnected && (liveRid === rid || !liveRid)) {
    ...
    void import("/js/chunks/lan-shift-pin-connect-MAVG5ZIR.js").then(function(m) {
...
function startLiveSyncReconnectLoop() {
  ...
  function tick() {
    ...
    if (_liveSyncReconnectAttempt >= 2 && !_shiftPinRediscoverInFlight) {
      ...
      void import("/js/chunks/lan-shift-pin-connect-MAVG5ZIR.js").then(function(mod) {
...
function bootLanRoomMembership() {
  ...
  void (async function() {
    var rid = m.roomId;
    try {
      const accessMod = await import("/js/chunks/clinical-access-runtime-3TILEVDG.js");
...
function syncLiveSyncAfterRoomJoinBody(rid) {
  ...
  if (isClinicalOpsLanAvailable()) {
    chain = chain.then(function() {
      return import("/js/chunks/clinical-ops-lan-YNZBIPC2.js").then(function(mod) {
  ...
  return chain.then(function() {
    ...
    void import("/js/chunks/clinical-access-runtime-3TILEVDG.js").then(function(accessMod) {
    ...
    void import("/js/chunks/historia-clinica-lan-sync-UBXQ2X4N.js").then(function(m) {
...
async function openR4TeamCreationModal() {
  try {
    var mod = await import("/js/chunks/clinical-teams-KWYW4BS2.js");
...
function lanHubStatusCopy() {
  if (!lanClient.connected) {
    ...
    if (!canLocalMacBeLanHost()) {
      var esc3 = getHostEscalationStatus();
      var nextRank = ["R3", "R2", "R1"][esc3.tier] || "";
      var escHint = esc3.tier < 3 && esc3.msUntilNext > 0 ? "Sin R4/admin en la red: en " + formatEscalationCountdown(esc3.msUntilNext) + " podr\xE1 anfitrionar " + nextRank + " (escalada autom\xE1tica 10+10+10 min)." : "R1\u2013R3 esperan anfitri\xF3n R4 o escalada. Pide enlace (\u21C4) o p\xE9galo abajo.";
      return {
        connected: false,
    ...
    return {
      connected: false,
      line: "Sin conexi\xF3n al turno",
      hint: "Pide el PIN de 6 d\xEDgitos al anfitri\xF3n (\u21C4) o con\xE9ctate abajo."
    };
  }
  if (isLanRemoteJoinMode()) {
...
async function applyConflictUseServer(payload) {
  ...
  if (server && server.data) {
    if (payload.entityType === "historiaClinica" && payload.patientId) {
      ...
      if (hcRow) {
        var mod = await import("/js/chunks/historia-clinica-lan-sync-UBXQ2X4N.js");
...
async function saveTemplateFromForm(formEl) {
  ...
  try {
    if (scope === "team") {
      ...
      if (!api2?.dbEntregaTemplateSaveTeam) throw new Error("Plantillas no disponibles");
      await api2.dbEntregaTemplateSaveTeam({
        teamId,
        createdBy: userId,
      ...
      if (!api2?.dbEntregaTemplateSaveUser) throw new Error("Plantillas no disponibles");
      await api2.dbEntregaTemplateSaveUser({
        userId,
        name: String(name).trim(),
        payload
      });
...
async function refreshTemplateCatalog(userId) {
  const api2 = dbApi4();
  if (!api2?.dbEntregaTemplateList) {
    templateCatalog = { user: [], team: [] };
    return;
  }
  const teamIds = draftSourceTeamId ? [draftSourceTeamId] : [];
  const res = await api2.dbEntregaTemplateList({ userId, teamIds });
  if (!res?.ok) {
    templateCatalog = { user: [], team: [] };
    return;
  }
...
var ACTIVE_SVG = `<svg width="7" height="7" viewBox="0 0 8 8" aria-hidden="true"><circle cx="4" cy="4" r="4" fill="#6c5ce7"/></svg>`;
var STATUS_LABELS = {
  critical: "Cr\xEDtico",
  unstable: "Inestable",
  stable: "Estable",
  ...
  host.innerHTML = `
    <div class="roster-panel">
      ...
      <div class="roster-list">
        ${critical.length ? `<div class="roster-section">Cr\xEDticos / inestables</div>${critical.map(renderRow).join("")}` : ""}
...
function openEntregaModal(opts) {
  ...
  if (select) {
    ...
    if (phase?.active && phaseCovering) {
      ...
      if (coverHint2) {
        coverHint2.textContent = `Entrega activa \u2192 ${phase.coveringLabel || "R1 de guardia"}. Este paciente se entregar\xE1 a esa persona.`;
      }
    } else {
...
async function applyBootstrapResult(res) {
  ...
  await fetchClinicalScopeContextFromDb();
  if (typeof document !== "undefined") {
    void import("/js/chunks/clinical-profile-lan-sync-X3VZ7IR3.js").then((mod) => mod.flushClinicalProfileToLan()).catch(() => {
    });
  }
...
function migrateLocalPatientsClinicalSala() {
  ...
  if (migrated > 0) {
    ...
    if (typeof document !== "undefined") {
      void import("/js/chunks/patients-NKQIONHY.js").then((mod) => mod.renderPatientList({ silent: true })).catch(() => {
      });
    }
  }
  return migrated;
...
async function ensureTeamAssignedPatientsOnDevice() {
  ...
  const { filterJoinedTeams: filterJoinedTeams2 } = await import("/js/chunks/shared-YTPMRYHZ.js");
  const { resolvePatientTeamIdFromAssignments: resolvePatientTeamIdFromAssignments2 } = await import("/js/chunks/clinico-access-ZJ5JFBJV.js");
  ...
  try {
    ...
    if (!rid) return;
    const push = await import("/js/chunks/push-NL7KBDJI.js");
...
async function refreshClinicalPatientListForScope() {
  ...
  if (typeof document === "undefined") return;
  try {
    const mod = await import("/js/chunks/patients-NKQIONHY.js");
...
function hintHtml(text) {
  return `<p class="clinical-teams-hint">${escapeHtml3(text)}</p>`;
}
...
//# sourceMappingURL=/js/chunks/chunk-YG4BETDC.js.map
