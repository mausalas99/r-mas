...
async function refreshOnboardingHost() {
  const { refreshMainClinicalOnboardingIfNeeded } = await import("/js/chunks/clinical-onboarding-main-KLHRKWDF.js");
...
async function handleUsernameStepSubmit(ev) {
  ...
  if (needsClaim) {
    const { assertLanRoomForUsernameRegister } = await import("/js/chunks/clinical-profile-lan-sync-F277ZDQ6.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  if (shiftPin && !isClinicalLocalOnlyMode()) {
    const { connectLanWithShiftPin } = await import("/js/chunks/lan-shift-pin-connect-DYGMWIV2.js");
  ...
  } = await import("/js/chunks/clinical-profile-lan-sync-F277ZDQ6.js");
  ...
  if (!localOnly && !lanPush.ok && isLanProfileNeedsConnectCode(lanPush.code)) {
    ...
    const rot = await import("/js/chunks/clinical-rotation-entry-3U2XUI3F.js");
    rot.syncClinicalRotationEntryChrome();
    const { refreshMainClinicalOnboardingIfNeeded: refreshMainClinicalOnboardingIfNeeded2 } = await import("/js/chunks/clinical-onboarding-main-KLHRKWDF.js");
...
async function renderOnboardingPanelInto(host) {
  ...
  if (!userId) {
    ...
    const { buildOnboardingSessionBlockHtml } = await import("/js/chunks/clinical-onboarding-main-KLHRKWDF.js");
  ...
  if (!needsProfileOnboarding()) {
    const { hideMainClinicalOnboarding } = await import("/js/chunks/clinical-onboarding-main-KLHRKWDF.js");
    hideMainClinicalOnboarding();
    if (host.closest("#clinical-teams-panel-body")) {
      const { renderClinicalTeamsPanel } = await import("/js/chunks/clinical-teams-LSOSCJPO.js");
...
//# sourceMappingURL=/js/chunks/chunk-YA2AMVNV.js.map
