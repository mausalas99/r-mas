import {
  ...
  needsTeamOnboarding
} from "/js/chunks/chunk-HS7I4VKW.js";
import {
  filterJoinedTeams,
...
async function openLanConnectPanelForPin() {
  try {
    const { openConnectionDropdown, focusLanShiftPinInput } = await import("/js/chunks/lan-sync-2V3COZDO.js");
...
async function handleLanConnectCtaClick() {
  ...
  if (/^\d{6}$/.test(savedPin)) {
    try {
      const { tryEasyLanShiftPinConnect } = await import("/js/chunks/lan-shift-pin-connect-BVHYOHEM.js");
...
async function openMiRotacion() {
  ...
  const { ensureClinicalPanelSession } = await import("/js/chunks/clinical-panel-host-5CSCK6TI.js");
  const sessionOk = await ensureClinicalPanelSession();
  if (!sessionOk) {
    const mainMod = await import("/js/chunks/clinical-onboarding-main-2QKKJMK5.js");
  ...
  const { wireClinicalTeamsModalChrome } = await import("/js/chunks/teams-roster-modal-chrome-CDSAHUED.js");
  wireClinicalTeamsModalChrome();
  const { openClinicalTeamsPanel } = await import("/js/chunks/teams-roster-HCVTYGTN.js");
...
//# sourceMappingURL=/js/chunks/chunk-VGVMBN4F.js.map
