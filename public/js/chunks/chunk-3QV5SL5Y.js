import {
  ...
  needsTeamOnboarding
} from "/js/chunks/chunk-AJWFP2CJ.js";
import {
  filterJoinedTeams,
...
async function openLanConnectPanelForPin() {
  try {
    const { openConnectionDropdown, focusLanShiftPinInput } = await import("/js/chunks/lan-sync-Q23EO4KP.js");
...
async function handleLanConnectCtaClick() {
  ...
  if (/^\d{6}$/.test(savedPin)) {
    try {
      const { tryEasyLanShiftPinConnect } = await import("/js/chunks/lan-shift-pin-connect-WIPJPRVW.js");
...
async function isLanConnectCtaVisible() {
  ...
  try {
    const lan = await import("/js/chunks/lan-sync-Q23EO4KP.js");
...
async function openMiRotacion() {
  ...
  const { ensureClinicalPanelSession } = await import("/js/chunks/clinical-panel-host-G5ZYAZF5.js");
  const sessionOk = await ensureClinicalPanelSession();
  if (!sessionOk) {
    const mainMod = await import("/js/chunks/clinical-onboarding-main-HPM4GR63.js");
  ...
  const { wireClinicalTeamsModalChrome } = await import("/js/chunks/teams-roster-modal-chrome-NWTR5Q6I.js");
  wireClinicalTeamsModalChrome();
  const { openClinicalTeamsPanel } = await import("/js/chunks/teams-roster-JR553UOL.js");
...
function buildEntryStatus() {
  ...
  if (hasElevatedTeamPrivileges(user)) {
    sub = name || "Supervisi\xF3n de rotaciones \u2014 sin equipo requerido";
...
//# sourceMappingURL=/js/chunks/chunk-3QV5SL5Y.js.map
