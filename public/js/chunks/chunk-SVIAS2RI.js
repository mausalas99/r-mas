...
async function refreshOnboardingHost() {
  const { refreshMainClinicalOnboardingIfNeeded } = await import("/js/chunks/clinical-onboarding-main-VV7C2WMQ.js");
...
async function handleUsernameStepSubmit(ev) {
  ...
  if (needsClaim) {
    const { assertLanRoomForUsernameRegister } = await import("/js/chunks/clinical-profile-lan-sync-DZFTXSYY.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  if (shiftPin && !isClinicalLocalOnlyMode()) {
    const { connectLanWithShiftPin } = await import("/js/chunks/lan-shift-pin-connect-XJXDTLTH.js");
  ...
  } = await import("/js/chunks/clinical-profile-lan-sync-DZFTXSYY.js");
  ...
  if (!localOnly && !lanPush.ok && isLanProfileNeedsConnectCode(lanPush.code)) {
    ...
    const rot = await import("/js/chunks/clinical-rotation-entry-N76ZO4SY.js");
    rot.syncClinicalRotationEntryChrome();
    const { refreshMainClinicalOnboardingIfNeeded: refreshMainClinicalOnboardingIfNeeded2 } = await import("/js/chunks/clinical-onboarding-main-VV7C2WMQ.js");
...
async function renderOnboardingPanelInto(host) {
  ...
  if (!userId) {
    ...
    const { buildOnboardingSessionBlockHtml } = await import("/js/chunks/clinical-onboarding-main-VV7C2WMQ.js");
  ...
  if (!needsProfileOnboarding()) {
    const { hideMainClinicalOnboarding } = await import("/js/chunks/clinical-onboarding-main-VV7C2WMQ.js");
    hideMainClinicalOnboarding();
    if (host.closest("#clinical-teams-panel-body")) {
      const { renderClinicalTeamsPanel } = await import("/js/chunks/clinical-teams-YCUK4D3W.js");
...
//# sourceMappingURL=/js/chunks/chunk-SVIAS2RI.js.map
