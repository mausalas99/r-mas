...
async function refreshOnboardingHost() {
  const { refreshMainClinicalOnboardingIfNeeded } = await import("/js/chunks/clinical-onboarding-main-BC3QP7TI.js");
...
async function handleUsernameStepSubmit(ev) {
  ...
  if (needsClaim) {
    const { assertLanRoomForUsernameRegister } = await import("/js/chunks/clinical-profile-lan-sync-HEGREZOF.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  if (shiftPin && !isClinicalLocalOnlyMode2()) {
    const { connectLanWithShiftPin } = await import("/js/chunks/lan-shift-pin-connect-PWGC5JWK.js");
  ...
  } = await import("/js/chunks/clinical-profile-lan-sync-HEGREZOF.js");
  ...
  const localOnly = isClinicalLocalOnlyMode2();
  if (!localOnly && !lanPush.ok && isLanProfileNeedsConnectCode(lanPush.code)) {
    toast2(LAN_PROFILE_NEEDS_CONNECT_MSG, "info");
    const rot = await import("/js/chunks/clinical-rotation-entry-MQKB74ZZ.js");
    rot.syncClinicalRotationEntryChrome();
    const { refreshMainClinicalOnboardingIfNeeded: refreshMainClinicalOnboardingIfNeeded2 } = await import("/js/chunks/clinical-onboarding-main-BC3QP7TI.js");
...
async function renderOnboardingPanelInto(host) {
  ...
  if (!userId) {
    ...
    const { buildOnboardingSessionBlockHtml } = await import("/js/chunks/clinical-onboarding-main-BC3QP7TI.js");
  ...
  if (!needsProfileOnboarding()) {
    const { hideMainClinicalOnboarding } = await import("/js/chunks/clinical-onboarding-main-BC3QP7TI.js");
    hideMainClinicalOnboarding();
    if (host.closest("#clinical-teams-panel-body")) {
      const { renderClinicalTeamsPanel } = await import("/js/chunks/clinical-teams-WMU6MOPN.js");
  ...
  }
  {
    ...
    const gateLead = profileGatePending ? `<p class="clinical-teams-lead clinical-onboard-gate-lead">${CLINICAL_LAN_PROFILE_GATE_LEAD_HTML}</p>` : '<p class="clinical-teams-lead">Confirma tu usuario LAN, nombre en guardia, rango y sala. Para equipos, abre <strong>Mi rotaci\xF3n</strong> despu\xE9s.</p>';
    host.innerHTML = `
      ...
      <form id="clinical-onboard-username-form" class="clinical-teams-create-form clinical-onboard-form">
        ...
        <div class="field-group">
          ...
          <p class="clinical-teams-hint">Pide el PIN del turno a quien tenga R+ como anfitri\xF3n (\u21C4). R+ buscar\xE1 el host en la red y te conectar\xE1 autom\xE1ticamente.</p>
        </div>
        <p id="onboard-error" class="clinical-registration-error" hidden></p>
        <div class="modal-actions">
...
//# sourceMappingURL=/js/chunks/chunk-67BJJSBU.js.map
