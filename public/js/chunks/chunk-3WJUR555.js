...
async function refreshOnboardingHost() {
  const { refreshMainClinicalOnboardingIfNeeded } = await import("/js/chunks/clinical-onboarding-main-MVWUHTSN.js");
...
async function handleUsernameStepSubmit(ev) {
  ...
  if (needsClaim) {
    const { assertLanRoomForUsernameRegister } = await import("/js/chunks/clinical-profile-lan-sync-IHLWREGP.js");
...
\xBFRecuperar tu cuenta en este dispositivo?`
  ...
  if (shiftPin && !isClinicalLocalOnlyMode()) {
    const { connectLanWithShiftPin } = await import("/js/chunks/lan-shift-pin-connect-FMAAUZ7T.js");
  ...
  } = await import("/js/chunks/clinical-profile-lan-sync-IHLWREGP.js");
  ...
  if (!localOnly && !lanPush.ok && isLanProfileNeedsConnectCode(lanPush.code)) {
    ...
    const rot = await import("/js/chunks/clinical-rotation-entry-5C5CBQUR.js");
    rot.syncClinicalRotationEntryChrome();
    const { refreshMainClinicalOnboardingIfNeeded: refreshMainClinicalOnboardingIfNeeded2 } = await import("/js/chunks/clinical-onboarding-main-MVWUHTSN.js");
...
async function renderOnboardingPanelInto(host) {
  ...
  if (!userId) {
    ...
    const { buildOnboardingSessionBlockHtml } = await import("/js/chunks/clinical-onboarding-main-MVWUHTSN.js");
  ...
  if (!needsProfileOnboarding()) {
    const { hideMainClinicalOnboarding } = await import("/js/chunks/clinical-onboarding-main-MVWUHTSN.js");
    hideMainClinicalOnboarding();
    if (host.closest("#clinical-teams-panel-body")) {
      const { renderClinicalTeamsPanel } = await import("/js/chunks/clinical-teams-VHZ3MQPK.js");
...
//# sourceMappingURL=/js/chunks/chunk-3WJUR555.js.map
