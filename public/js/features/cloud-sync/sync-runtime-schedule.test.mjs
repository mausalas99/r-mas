  ...
  it('schedules the next cycle within the 2-minute overload cap after repeated 503s', async () => {
    ...
    try {
      const scheduler = createCloudPollScheduler({
        ...
        pendingCount: () => 0,
        getLastLocalWriteAt: () => 0,
        drainPacer: createDrainPacer({ random: () => 0 }),
      });
      for (let i = 0; i < 6; i += 1) {
        const err = new Error('overloaded');
        err.status = 503;
      ...
      assert.ok(scheduledDelay <= CLOUD_POLL_ERROR_OVERLOAD_MAX_MS);
      scheduler.stop();
    } finally {
  ...
  it('schedules up to the full 5-minute cap for a permanent error, not the 2-minute overload cap', async () => {
    ...
    try {
      const scheduler = createCloudPollScheduler({
        ...
        pendingCount: () => 0,
        getLastLocalWriteAt: () => 0,
        drainPacer: createDrainPacer({ random: () => 1 }), // random()=1 -> jitterMs returns the full exp value
      });
      for (let i = 0; i < 8; i += 1) {
      ...
      assert.ok(scheduledDelay > CLOUD_POLL_ERROR_OVERLOAD_MAX_MS);
      assert.ok(scheduledDelay <= 5 * 60_000);
      scheduler.stop();
    } finally {
      globalThis.setTimeout = originalSetTimeout;
    }
  ...
  it('an overload cap does not stick after a success — a later permanent-error streak still reaches the 5-minute cap', async () => {
    ...
    try {
      const scheduler = createCloudPollScheduler({
        ...
        pendingCount: () => 0,
        getLastLocalWriteAt: () => 0,
        drainPacer: createDrainPacer({ random: () => 1 }),
      });
      for (let i = 0; i < 6; i += 1) {
        const err = new Error('overloaded');
        err.status = 503;
