...
function renderBucketSectionHtml(bucketKey, patients, highlight) {
  return (
    '<div class="ic-board-bucket' + (highlight ? ' ic-board-bucket--accent' : '') + '">' +
    '<div class="r4-section-divider">' + escHtml(BUCKET_LABELS[bucketKey]) + ' (' + patients.length + ')</div>' +
...
function renderActivoLaneHtml(team, patients, now) {
  const body = team
  ...
  return (
    ...
    '<div class="ic-board-lane__head">' +
    '<h3 class="ic-board-lane__title">' + escHtml(teamLabel(team)) + '</h3>' +
    '</div>' +
    '<div class="ic-board-lane__body"' + laneBodyAttr(team && team.team_id) + '>' + body + '</div>' +
    '</section>'
  );
...
function renderUnassignedLaneHtml(patients, now) {
  const body = renderActiveLaneBodyHtml(patients, ['preop', 'pendientes', 'under'], false, now);
  return (
    '<section class="ic-board-lane ic-board-lane--unassigned" data-role="sin-equipo">' +
    '<div class="ic-board-lane__head"><h3 class="ic-board-lane__title">Sin equipo</h3></div>' +
...
/**
 * Renders the 4-lane interconsulta team board as an HTML string.
 ...
 * @param {boolean} [filterGuardiaOnly] — when true, render only the guardia lane's Preop/Nuevas-hoy bucket
 * @returns {string}
 */
export function renderInterconsultaTeamBoardHtml(patients, teams, now = new Date(), filterGuardiaOnly = false) {
  ...
  // Patients whose censusTeamId is empty or doesn't match any of the 4 lanes
  // used to be silently dropped — render them in a 5th "Sin equipo" lane instead.
  const knownTeamIds = new Set(
    [roles.guardia, roles.postguardia, ...(roles.activo || [])]
      .filter(Boolean)
  ...
  const lanes = [
    ...
    ...(roles.activo || []).map((team) => renderActivoLaneHtml(team, patientsFor(team), now)),
    renderPostguardiaLaneHtml(roles.postguardia, patientsFor(roles.postguardia), now),
  ];
  if (unassignedPatients.length) {
    lanes.push(renderUnassignedLaneHtml(unassignedPatients, now));
  }
...
export function mountInterconsultaTeamBoard(container, patients, teams, opts = {}) {
  ...
  container.innerHTML = renderInterconsultaTeamBoardHtml(patients, teams, now, opts.filterGuardiaOnly);
