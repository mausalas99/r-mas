...
} from "/mobile/js/chunks/chunk-QAC2VFLD.js";
import {
...
async function applyCultivoLabRepoBatch(patientId, registro, studies, errors) {
  await import("/mobile/js/chunks/lazy-feature-routes-IJS5AV5S.js").then(function(routes) {
...
function buildCultivosNegStrip(negs) {
  ...
  var chips = negs.map(function(r) {
    ...
    var sitio = r.sitio || "\u2014";
    return '<li class="cultivos-neg-chip"><span class="cultivos-neg-chip-tipo">' + esc(r.tipoLabel || "") + "</span> \xB7 " + esc(fd) + " \xB7 " + esc(sitio) + cultivoRowRemoveBtnHtml(r) + "</li>";
  }).join("");
...
function cultivoRowRemoveBtnHtml(r) {
  ...
  return '<button type="button" class="cultivos-row-remove-btn" data-cult-set-id="' + esc(String(r.labSetId)) + '" title="Eliminar este cultivo del historial" aria-label="Eliminar este cultivo del historial">\xD7</button>';
}
function collectCultivoTableRowChunks(groups, rowFechaDisplayFn) {
  ...
  groups.forEach(function(g) {
    ...
    g.rows.forEach(function(r) {
      totalRows += 1;
      rowChunks.push(
        '<tr class="' + (r.negativo ? "cultivos-row-neg" : "") + '"><td>' + esc(rowFechaDisplayFn(r)) + "</td><td>" + esc(r.sitio) + '</td><td class="cultivos-cell-org">' + cultivoOrganismoCellHtml(r) + '</td><td class="cultivos-cell-atb">' + cultivoAntibiogramCellHtml(r) + '</td><td class="cultivos-cell-remove">' + cultivoRowRemoveBtnHtml(r) + "</td></tr>"
      );
    });
  });
...
function renderCultivosTable() {
  ...
  var thead = '<thead><tr><th>Fecha</th><th>Sitio / muestra</th><th>Organismo</th><th>Antibiograma</th><th><span class="visually-hidden">Acciones</span></th></tr></thead>';
...
function populateProfileVersionBlock() {
  ...
  window.electronAPI.getAppVersion().then(async function(v) {
    ...
    if (versionChanged || window.__RPC_RELEASE_NOTES_DEV__) {
      var notes = await import("/mobile/js/chunks/release-notes-EHC73A35.js");
...
function syncInterconsultaModeChrome() {
  import("/mobile/js/chunks/interconsulta-mode-chrome-YY536KJV.js").then(function(mod) {
...
//# sourceMappingURL=/js/chunks/chunk-VPWSQYDS.js.map
