...
function listWardHostUrlsForProbe(maxAgeMs = DEFAULT_MAX_AGE_MS, opts = {}) {
  ...
  for (const e of sorted) {
    ...
    if (localPrefixes && url !== bundled && !isHostOnCurrentSubnets(url, localPrefixes)) {
      continue;
    }
    seen.add(url);
    out.push(url);
...
async function listWardSubnetPrefixesForProbe(ownBaseUrl) {
  const { resolveLocalLanSubnetPrefixes } = await import("/mobile/js/chunks/lan-host-subnet-discovery-33IFEMEJ.js");
...
function pruneWardHostRegistry(maxAgeMs = DEFAULT_MAX_AGE_MS) {
  const cutoff = tsNow() - maxAgeMs;
  const reg = loadWardHostRegistry();
...
function seedBundledWardConnectionPoints() {
...
function syncWardHostRegistrySnapshotToMain() {
  const api = wardHostMainIpc();
  if (!api || typeof api.lanWardHostMerge !== "function") return;
  const reg = loadWardHostRegistry();
  void api.lanWardHostMerge({
...
export {
  ...
  syncWardHostRegistrySnapshotToMain,
  clearWardHostRegistry,
  summarizeWardHostRegistry
};
//# sourceMappingURL=/js/chunks/chunk-I4NFL7CB.js.map
