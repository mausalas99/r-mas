...
} from "/mobile/js/chunks/chunk-W2QML3CA.js";
import {
  getActiveInnerTab,
...
import "/mobile/js/chunks/chunk-ADCCUMNS.js";
...
import {
  LAB_INNER_SECTIONS
} from "/mobile/js/chunks/chunk-3TQ2NHWJ.js";
...
import "/mobile/js/chunks/chunk-HLOATWQV.js";
...
// public/js/features/patients-list/copy-team-estado-actual.mjs
function buildTeamEstadoActualCopyText() {
  var pinned = getPatients().filter(function(p) {
    return p && p.pinned && !p.archived;
  });
  var blocks = [];
...
function copyTeamEstadoActualForToday(showToast) {
  ...
  copyTableText(built.text, function(ok) {
    showToast(
      ok ? "Estado actual copiado (" + built.estadoCount + " de " + built.patientCount + " fijados)." : "No se pudo copiar el estado actual.",
      ok ? "success" : "error"
    );
  });
}
...
function handleShellCopyTeamLabsShortcut(e, key, showToast) {
  ...
  e.preventDefault();
  if (rt.getActiveAppTab() === "nota" && getActiveInnerTab() === "estadoActual") {
    copyTeamEstadoActualForToday(showToast);
  } else {
...
//# sourceMappingURL=/js/chunks/app-shell-keyboard-DKLMGWVE.js.map
