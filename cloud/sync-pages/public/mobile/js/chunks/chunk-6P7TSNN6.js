...
} from "/mobile/js/chunks/chunk-LN2N4VIO.js";
import {
...
} from "/mobile/js/chunks/chunk-P2TSIQM4.js";
import {
...
import {
  flushPendingClinicalOpsSnapshot,
  getClinicalOpsTrace,
  recordClinicalOpsTrace
} from "/mobile/js/chunks/chunk-G6B5EEF6.js";
import {
  showToast
...
function shouldUseCloudTeamPatientMirror(_user) {
  return false;
}
...
function shouldUseDesktopCensusWithFilters(user) {
...
function canViewUserDirectory(user) {
  return hasElevatedTeamPrivileges(user);
}
function canManageTeamRoster(user) {
  return hasElevatedTeamPrivileges(user);
}
function canDeleteDirectoryUser(user) {
...
function filterPatientsForMobileTeamMirror(patients, user, scopeContext, guardiasMap) {
...
function filterPatientsForDesktopCloudTeamScope(patients, user, scopeContext, guardiasMap) {
  if (!user?.user_id) return [];
  return (patients || []).filter((p) => {
    if (!p?.id) return false;
    const activeGuardia = guardiasMap && typeof guardiasMap.get === "function" ? guardiasMap.get(String(p.id)) || null : null;
...
function parseBedLabelSortKey(bedLabel2) {
  if (!bedLabel2 || bedLabel2 === "\u2014") return null;
  const nums = bedLabel2.match(/\d+/g);
...
function patientBedSortKey(patient) {
  ...
  const bedLabel2 = String(patient?.bed_label || patient?.bedLabel || "").trim();
  const fromLabel = parseBedLabelSortKey(bedLabel2);
...
function sortPatientsByPriorityThenBed(patients, guardiasMap) {
...
var CLINICAL_CENSUS_FILTER_SALA_LS = "rpc.clinicalCensusFilterSala";
var CENSUS_TEAM_FILTER_ALL = "__all__";
var CENSUS_TEAM_FILTER_UNASSIGNED = "__unassigned__";
var CENSUS_SALA_FILTER_ALL = "__all__";
...
function readCensusSalaFilterPreference(storage2 = globalThis.localStorage) {
  try {
    const raw = storage2?.getItem(CLINICAL_CENSUS_FILTER_SALA_LS);
...
function writeCensusSalaFilterPreference(sala, storage2 = globalThis.localStorage) {
  try {
    storage2?.setItem(
...
function resolveCensusSalaFilterId(user, storage2 = globalThis.localStorage) {
  const pref = readCensusSalaFilterPreference(storage2);
...
function readCensusFiltersCollapsed(storage2 = globalThis.localStorage) {
  try {
    const raw = storage2?.getItem(CLINICAL_CENSUS_FILTERS_COLLAPSED_LS);
...
function writeCensusFiltersCollapsed(collapsed, storage2 = globalThis.localStorage) {
  try {
    storage2?.setItem(CLINICAL_CENSUS_FILTERS_COLLAPSED_LS, collapsed ? "1" : "0");
  } catch (_e) {
    void _e;
  }
}
...
function filterPatientsForClinicalSidebar(patients, user, scopeContext, guardiasMap) {
...
function applyElevatedPatientFilters(patients, filters, ctx = {}) {
  let list = patients || [];
...
function buildGuardiaTeamCensusGroups(patients, ctx = {}) {
  const teams = ctx.teams || [];
  const assignments = ctx.assignments || [];
  const now = ctx.now || (/* @__PURE__ */ new Date()).toISOString();
...
function filterR4FollowUpPinPatients(patients) {
  return patients.filter(
    (p) => p.interconsult_type === "Follow-up" && p.interconsult_status !== "Resolved"
  );
}

// public/js/features/workbench/wb-table.mjs
function buildTableCardHeaderHtml({ title = "", actionsHtml = "" } = {}) {
  return `<div class="wb-table-card-header"><span class="wb-table-card-title">${escHtml(title)}</span>` + (actionsHtml ? `<div class="wb-table-card-actions">${actionsHtml}</div>` : "") + "</div>";
}
function buildColumnHeadHtml(columns, gridTemplate) {
  const style = gridTemplate ? ` style="grid-template-columns:${escAttr(gridTemplate)}"` : "";
  return `<div class="wb-table-colhead"${style}>` + (columns || []).map((c) => `<span>${escHtml(c)}</span>`).join("") + "</div>";
}
function buildRowHtml({ id, cellsHtml = [], alert = false, twoLine = false, gridTemplate } = {}) {
  const classes = ["wb-row"];
  if (alert) classes.push("wb-row--alert");
  if (twoLine) classes.push("wb-row--twoline");
  const style = gridTemplate ? ` style="grid-template-columns:${escAttr(gridTemplate)}"` : "";
  return `<div class="${classes.join(" ")}"${style} data-wb-row-id="${escAttr(id)}" role="button" tabindex="0">` + cellsHtml.map((h) => `<span class="wb-cell">${h}</span>`).join("") + "</div>";
}
function buildSummaryLineHtml(text) {
  return `<div class="wb-table-summary">${escHtml(text)}</div>`;
}

// public/js/features/workbench/status-label.mjs
var STATUS_META = {
...
function buildFilterChipHtml(chip, activeId, variant) {
  ...
  return `<button type="button" class="${classes.join(" ")}" data-wb-chip-id="${escAttr(chip.id)}" aria-pressed="${isActive}">${escHtml(chip.label)}</button>`;
}
function buildFilterChipsHtml(chips, activeId, opts = {}) {
...
// public/js/features/guardia-census-table.mjs
var VITAL_LABELS = { ta: "T/A", tas: "T/A", fc: "FC", fr: "FR", temp: "Temp", sat: "SatO\u2082" };
var GUARDIA_TABLE_GRID = "92px 1fr 132px 1fr 84px";
var GUARDIA_CENSUS_FILTER_DEFAULT = "todos";
...
function alteradosForPatient(p) {
  ...
  const chips = Object.keys(alt).filter((k) => vitals[k] != null).map((k) => `${vitalLabel(k)} ${vitals[k]}`);
  return { taken: true, chips };
}
function patientPendientes(patientId, now) {
...
function guardiaPatientStatus(pendientes) {
...
function admissionDateForPatient(p) {
  return isoDatetimeToLocalDateInputValue(p?.registeredAt) || accesoFechaToDateInputValue(p?.fimiFecha) || accesoFechaToDateInputValue(p?.fiuxFecha) || "";
}
...
function isPatientAdmittedToday(p) {
...
function alteradosCellHtml(p) {
  ...
  return `<span class="gct-cell-alert">${shown.map(escHtml).join(" \xB7 ")}</span>${tail}`;
}
...
function buildGuardiaCensusTableRowHtml(p) {
...
function batchRowsHtml(patients, guardiasMap) {
  return sortPatientsByPriorityThenBed(patients, guardiasMap).map(buildGuardiaCensusTableRowHtml).join("");
}
...
function guardiaCensusFilterChips(patients) {
  ...
  return [
    { id: "pendiente", label: `Con pendiente \xB7 ${pendienteCount}` },
...
function buildGuardiaCensusTableHtml(patients, guardiasMap, userRank = "R1", groupCtx = {}, activeFilter = GUARDIA_CENSUS_FILTER_DEFAULT) {
  const all = patients || [];
  const title = `Censo \xB7 ${all.length} paciente${all.length === 1 ? "" : "s"}`;
  ...
  return '<div class="wb-table-card guardia-census-table">' + header + colhead + `<div class="wb-table-body">${bodyHtml}</div>` + (summary ? buildSummaryLineHtml(summary) : "") + "</div>";
}
function mountGuardiaCensusTable(container, patients, guardiasMap, userRank, groupCtx, onRowClick) {
  if (!container) return;
  ...
  function renderAt(filterId) {
    ...
    var prevRows = /* @__PURE__ */ Object.create(null);
    container.querySelectorAll(".wb-row[data-wb-row-id]").forEach((row) => {
...
var clinicalTeamsEmbedHost = null;
function setClinicalTeamsEmbedHost(host) {
...
async function ensureClinicalPanelSession(opts = {}) {
  ...
  try {
    const { applyClinicalDbUnlockCompletion } = await import("/mobile/js/chunks/db-unlock-OHUQSED6.js");
...
// public/js/features/clinical-teams/teams-roster-directory-dom.mjs
function directoryUsersModalBackdropEl() {
  return document.getElementById("clinical-directory-users-backdrop");
}
function directoryUsersModalBodyEl() {
  return document.getElementById("clinical-directory-users-panel-body");
}
function isDirectoryModalOpen() {
  const bd = directoryUsersModalBackdropEl();
  return !!(bd && bd.classList.contains("open"));
}

// public/js/features/clinical-teams/teams-directory-filters.mjs
...
function directoryUserMatchesFilters(meta, filters) {
...
// public/js/features/clinical-teams/teams-roster-directory-state.mjs
var directoryRt = {
  teams: [],
...
var DIRECTORY_USER_RANK_ORDER = ["R1", "R2", "R3", "R4", "Admin"];

// public/js/features/clinical-teams/teams-roster-directory-filters.mjs
function lanRankGroupKey(rank) {
  return String(rank || "").trim() || "Otros";
}
function shouldRankGroupOpen(rank, userCount) {
...
function captureDirectoryCollapseState(host) {
...
function renderDirectoryToolbarHtml(users, teams) {
  const salas = lanDirectorySalaFilterOptions(users, teams);
  const salaOptions = salas.map(
    (s) => `<option value="${escapeAttr(s)}"${directoryRt.filterSala === s ? " selected" : ""}>${escapeHtml(s)}</option>`
  ).join("");
...
function applyDirectoryFilters(host) {
...
function bindDirectoryFilterControls(host) {
  if (!host) return;
...
function ensureDirectoryFilterDelegation() {
  const bd = directoryUsersModalBackdropEl();
...
// public/js/features/clinical-teams/teams-roster-directory-row-html.mjs
...
function renderLanUserHandleCell(u) {
  ...
  return handleValid ? `<span class="clinical-directory-users-handle">@${escapeHtml(rawHandle)}</span>` : `<span class="clinical-directory-users-handle clinical-directory-users-handle--pending" title="Falta registrar @usuario en Mi rotaci\xF3n">sin @usuario</span>`;
}
...
function lanUserCardAssignMeta(u, placement, teamList, userRank) {
  ...
  const cycleOptions = placement?.cycle ? `<option value="${escapeAttr(placement.cycle)}" selected>${escapeHtml(formatCycleOptionLabel(placement.cycle, userRank))}</option>` : '<option value="">\u2014 Ciclo \u2014</option>';
  ...
  return `<article class="clinical-lan-user-card clinical-lan-user-row" data-user-id="${userId}" data-user-rank="${rankRaw}" data-preferred-cycle="${escapeAttr(placement?.cycle || "")}" data-sala="${salaAttr}" data-has-team="${placement?.teamId ? "1" : "0"}" data-activity-tier="${escapeAttr(activityTier)}" data-search="${searchHaystack}">
    ...
    <div class="clinical-lan-user-card-assign">
      ...
      <span class="clinical-directory-assign-actions" role="group" aria-label="Acciones">
        ...
        <button type="button" class="btn-med-secondary clinical-directory-delete-user-btn${deleteBtnClass}" data-user-id="${userId}" data-user-label="${escapeAttr(String(u.clinical_name || normalizeUsername(u.username || "") || rawUserId))}" title="Quitar de la base cl\xEDnica (se publica por R+ Cloud)"${deleteBtnAttrs}>Quitar</button>
      </span>
    </div>
  </article>`;
}
function renderDirectoryUserRowHtml(u, teamList, opts = {}) {
...
// public/js/features/clinical-teams/teams-roster-directory-render.mjs
function renderDirectoryUsersTopButtonHtml(user) {
...
function renderDirectoryUsersEntryHtml(user) {
...
function renderAssignTeamOptionsHtml(teams, selectedTeamId) {
  const list = Array.isArray(teams) ? teams : [];
...
function resolveUserPlacement(userId, teams) {
...
function formatCycleOptionLabel(letter, userRank) {
...
function renderDirectoryUsersModalBodyHtml(users, teams, opts = {}) {
  ...
  const otherSection = other.length ? `<details class="clinical-directory-rank-group"${shouldRankGroupOpen("Otros", other.length) ? " open" : ""} data-lan-rank-group="Otros" data-lan-rank-count="${other.length}">
        <summary class="clinical-directory-rank-group-summary">
...
async function publishClinicalTeamsToSync() {
  try {
    const mod = await import("/mobile/js/chunks/mutate-bridge-clinical-ops-N3N2N3O6.js");
...
function toastTeamPublishResult(lanPush, localOkMessage) {
  if (!lanPush) {
...
async function pullClinicalOpsFromCloudRoom(options = {}) {
  ...
  cloudClinicalOpsPullInFlight = (async () => {
    try {
      ...
      const { clinicalSessionContext: clinicalSessionContext2 } = await import("/mobile/js/chunks/clinical-access-runtime-3IIQQT2K.js");
      const { getCloudSyncToken: getCloudSyncToken2 } = await import("/mobile/js/chunks/settings-JOYOHYNE.js");
      ...
      if (!salas.length) {
        const { autostartCloudSyncIfConfigured } = await import("/mobile/js/chunks/autostart-2IGWEWI2.js");
        const rtMod = await import("/mobile/js/chunks/panel-conexion-runtime-XRURMEDI.js");
      ...
      const { pullClinicalOpsForSala } = await import("/mobile/js/chunks/cloud-clinical-ops-sala-6EU24WFI.js");
...
function scheduleBackgroundClinicalOpsPush(options = {}) {
  void (async () => {
    try {
      const { pushClinicalOpsForSalas, listLocalTeamSalas } = await import("/mobile/js/chunks/cloud-clinical-ops-sala-6EU24WFI.js");
...
async function publishClinicalTeamsAfterChange(options = {}) {
  if (isCloudSyncActive()) {
    try {
      const { syncClinicalOpsForSala, pushClinicalOpsForSalas, listLocalTeamSalas } = await import("/mobile/js/chunks/cloud-clinical-ops-sala-6EU24WFI.js");
      const sala = normalizeCloudSala(options.sala || "");
      const push = sala ? await syncClinicalOpsForSala(sala) : await pushClinicalOpsForSalas(await listLocalTeamSalas());
...
async function pullClinicalOpsFromRoom(_options = {}) {
  return false;
}
async function resolveLocalUserIdByHandle(handle) {
  const api2 = dbApi2();
...
// public/js/features/clinical-teams/teams-roster-directory-assign.mjs
...
function syncAssignCycleSelect(teamSelect, preferredCycle = "") {
...
function initUserRowAssignState(row) {
...
async function handleLanDeleteDirectoryUserClick(btn) {
  ...
  if (!api2 || typeof api2.dbClinicalUserDelete !== "function") {
    toast("Eliminar usuarios requiere R+ de escritorio con base cl\xEDnica desbloqueada.", "error");
    return;
  }
  const result = await openConfirm({
    weight: "destructive",
    title: `\xBFEliminar a \xAB${label}\xBB de la base cl\xEDnica en esta Mac?`,
    message: "Desaparecer\xE1 del directorio. Las dem\xE1s R+ en la misma sala Nube lo quitar\xE1n al sincronizar.",
  ...
  const { isBenignPushSkipCode: isBenignPushSkipCode2 } = await import("/mobile/js/chunks/clinical-profile-cloud-stubs-LE2FYXR3.js");
  const lanPush = await publishClinicalTeamsToSync();
  if (!lanPush.ok && !isBenignPushSkipCode2(lanPush.code)) {
    toast(
  ...
  const { reloadDirectoryUsersAfterMutation } = await import("/mobile/js/chunks/teams-roster-directory-load-B7PT3ONO.js");
...
async function handleLanAssignButtonClick(btn) {
  ...
  try {
    const { scheduleCloudSyncPush: scheduleCloudSyncPush2 } = await import("/mobile/js/chunks/mutate-bridge-NAGIKQFO.js");
...
// public/js/features/clinical-teams/teams-roster-directory-load.mjs
...
async function reloadDirectoryUsersPreservingUi(options = {}) {
  const host = directoryUsersModalBodyEl();
...
async function fetchDirectoryLists(api2, callerUserId) {
  return Promise.all([
...
function shouldSkipDirectoryIpcRefresh(options, host, now) {
  return !options.forceIpc && directoryRt.freezeAutoRefresh && now - directoryRt.ipcLastAt < DIRECTORY_IPC_MIN_MS && host.querySelector(".clinical-directory-rank-groups");
}
...
function shouldReuseDirectoryFingerprint(host, fingerprint, options) {
  return !options.forceRender && fingerprint === directoryRt.lastFingerprint && host.querySelector(".clinical-directory-rank-groups");
}
async function loadDirectoryData_(api2, callerUserId) {
  const [usersRes, teamsRes] = await fetchDirectoryLists(api2, callerUserId);
...
async function loadDirectoryUsersIntoHost(host, options = {}) {
  const now = Date.now();
  ...
  const { usersRes, teamsRes } = await loadDirectoryData_(api2, currentUserId());
  if (!usersRes?.ok) {
...
async function refreshDirectoryFromHostUi(options = {}) {
  const host = directoryUsersModalBodyEl();
  if (!host || !isDirectoryModalOpen()) return;
...
async function pullDirectoryFromHostIfDue() {
  return false;
}

// public/js/features/clinical-teams/teams-roster-directory-modal.mjs
async function openDirectoryUsersModal() {
...
function closeDirectoryUsersModal() {
...
// public/js/features/clinical-teams/teams-roster-directory-wire.mjs
...
function wireDirectoryHostInteractions(host) {
  ...
  host.addEventListener("click", (ev) => {
    const refreshBtn = ev.target instanceof Element ? ev.target.closest(".clinical-directory-refresh-btn") : null;
    if (refreshBtn) {
      void refreshDirectoryFromHostUi({ forcePull: true });
      return;
    }
    const delBtn = ev.target instanceof Element ? ev.target.closest(".clinical-directory-delete-user-btn") : null;
    if (delBtn) {
...
function wireDirectoryUsersControls() {
...
function buildClinicalTeamInviteMessage(team) {
  ...
  const lines = [
    ...
    "",
    "El enlace web no une al equipo cl\xEDnico; Safari/iPad usan Nube (censo / MIP)."
  ];
  return lines.join("\n");
}
...
// public/js/features/clinical-teams/teams-roster-inherit-gate.mjs
function isInheritPatientsHandoffWindow(cycle, now = /* @__PURE__ */ new Date()) {
  if (isRotationRejoinPending()) return true;
  return isIncomingPreviewWindow(cycle, now);
}
function shouldShowInheritPatientsUi(now = /* @__PURE__ */ new Date()) {
...
async function resolveTeamMemberHintHtml(joinedTeams) {
  ...
  try {
    ...
    const { getCloudSyncRoomId } = await import("/mobile/js/chunks/settings-JOYOHYNE.js");
...
function buildDirectorySectionTitle(elevated, browseSala, count = 0) {
  const countLabel = count > 0 ? `${count} equipo${count === 1 ? "" : "s"} \xB7 ` : "";
  if (!elevated) {
    return `${countLabel}Equipos disponibles \xB7 ${escapeHtml(browseSala)}`;
  }
  if (browseSala === "__all__") {
    return count > 0 ? `${countLabel}Explorar \xB7 todas las salas` : "Explorar \xB7 todas las salas";
  }
  return count > 0 ? `${countLabel}Explorar \xB7 ${escapeHtml(browseSala)}` : `Explorar \xB7 ${escapeHtml(browseSala)}`;
}
function buildDirectorySectionDesc(elevated, count) {
  if (count <= 0) return "Equipos de la sala a los que puedes unirte.";
  if (elevated) {
    return "Equipos publicados en Nube \u2014 asigna residentes o \xFAnete si corresponde.";
  }
  return "Tu R2 o R4 ya public\xF3 estos equipos en Nube. Elige el tuyo y pulsa <strong>Unirme</strong>.";
}
...
async function renderDirectorySectionHtml(opts) {
  ...
  const api2 = dbApi2();
  if (!api2 || typeof api2.dbClinicalTeamsListBySala !== "function") {
    return { html: "", count: 0 };
  }
...
async function resolveClinicalTeamsPanelContext(user, joined) {
  ...
  const { needsClinicalLanProfileGate: needsClinicalLanProfileGate2, ensureLanProfileGateDeviceReset } = await import("/mobile/js/chunks/clinical-settings-7XQJIPLW.js");
...
function buildJoinedTeamsEmptyHtml(displayHandle, pickTeamMode = false) {
  if (pickTeamMode) {
    return '<p class="clinical-teams-empty clinical-teams-empty--section">A\xFAn no te has unido. Elige un equipo disponible arriba y pulsa <strong>Unirme</strong>.</p>';
  }
...
function buildPickTeamsBannerHtml(opts) {
  ...
  if (elevated) {
    const lead2 = rejoinPending ? `Nueva rotaci\xF3n: hay <strong>${countLabel}</strong> ya publicados en <strong>${salaLabel}</strong>. Asigna residentes o crea equipos adicionales si hace falta.` : `Hay <strong>${countLabel}</strong> en <strong>${salaLabel}</strong> listos para asignar residentes.`;
    return `<div class="clinical-teams-pick-banner clinical-teams-pick-banner--elevated" role="status">${lead2}</div>`;
  }
  const lead = rejoinPending ? `Nueva rotaci\xF3n: tu R2 o R4 ya public\xF3 <strong>${countLabel}</strong> en <strong>${salaLabel}</strong>. Elige el tuyo abajo \u2014 no hace falta crear uno nuevo.` : `Ya hay <strong>${countLabel}</strong> en <strong>${salaLabel}</strong>. Elige el tuyo y pulsa <strong>Unirme</strong>.`;
  return `<div class="clinical-teams-pick-banner" role="status">${lead}</div>`;
}
function shouldUsePickTeamPanelLayout(joinedCount, directoryCount, elevated) {
...
async function resolveTeamIdFromDirectory(raw) {
  try {
    const { pullClinicalOpsFromCloudRoom: pullClinicalOpsFromCloudRoom2 } = await import("/mobile/js/chunks/teams-guardia-bridge-U6MEWTW4.js");
    await pullClinicalOpsFromCloudRoom2({ timeoutMs: 8e3, force: true });
    const { fetchClinicalTeamsFromDb: fetchClinicalTeamsFromDb2 } = await import("/mobile/js/chunks/clinical-access-runtime-3IIQQT2K.js");
...
async function openTeamsPanelAfterAlreadyJoined() {
  ...
  const { openClinicalTeamsPanel: openClinicalTeamsPanel2 } = await import("/mobile/js/chunks/teams-roster-AJB4B3M2.js");
...
async function finalizeSuccessfulTeamJoin(team, teamId, cycle) {
  ...
  const { closeClinicalTeamsPanel: closeClinicalTeamsPanel2, refreshTeamsUiAfterChange: refreshTeamsUiAfterChange2 } = await import("/mobile/js/chunks/teams-roster-shell-OTAB7QTU.js");
  await fetchClinicalTeamsFromDb();
  closeClinicalTeamsPanel2();
  document.dispatchEvent(new CustomEvent("rpc-clinical-teams-changed", { detail: { sala } }));
  await publishClinicalTeamsAfterChange({ sala });
  void import("/mobile/js/chunks/ensure-turn-room-57XFBPZG.js").then(
    ({ ensureTurnRoomAfterTeamJoin }) => ensureTurnRoomAfterTeamJoin(toast)
  );
...
async function redirectLanInviteFromTeamJoinField(raw) {
  ...
  const { openConnectionDropdown } = await import("/mobile/js/chunks/panel-chrome-M4CXZ2YE.js");
  if (typeof openConnectionDropdown === "function") openConnectionDropdown();
  toast("LiveSync LAN retirado \u2014 usa Nube en \u21C4 Conexi\xF3n.", "info");
  return true;
}
...
async function consumeClinicalTeamJoinFromUrl() {
  ...
  if (!teamId) {
    toast("Pega el c\xF3digo en Mi rotaci\xF3n y pulsa Unirme.", "info");
...
// public/js/features/clinical-teams/teams-roster-form-delegation.mjs
var wiredRoots = /* @__PURE__ */ new WeakSet();
function loadRoster() {
  return import("/mobile/js/chunks/teams-roster-AJB4B3M2.js");
}
function wireClinicalTeamsFormDelegation(root) {
...
// public/js/features/cloud-sync/panel-equipo-embed.mjs
...
async function mountEquipoTeamsPanel(host, opts = {}) {
  const toast9 = typeof opts.toast === "function" ? opts.toast : function() {
  };
  if (!host) return;
  ...
  if (!sessionOk) {
    setClinicalTeamsPanelError("Activa la sesi\xF3n cl\xEDnica para gestionar equipos.");
    return;
  }
  try {
    const { needsClinicalOnboarding: needsClinicalOnboarding2 } = await import("/mobile/js/chunks/clinical-onboarding-P2ZGDIVY.js");
  ...
  try {
    const { renderClinicalTeamsPanelInto: renderClinicalTeamsPanelInto2 } = await import("/mobile/js/chunks/teams-roster-panel-3ID4S3I5.js");
    await renderClinicalTeamsPanelInto2(panelHost, { skipLanPull: false });
    ...
    toast9("No se pudo cargar equipos.", "error");
  }
}
function isEquipoEmbedActive() {
  const host = getClinicalTeamsPanelHost();
...
// public/js/features/clinical-teams/teams-roster-panel-draft.mjs
var CREATE_FIELD_IDS = [
...
var JOIN_CODE_FIELD_IDS = [
  "clinical-team-join-code-input",
  "clinical-team-join-code-cycle"
];
function resolveClinicalTeamsScrollHost(panelHost) {
...
function writeFieldValue(id, value) {
  const el = document.getElementById(id);
  if (value === void 0 || el == null) return;
...
function hasDraftTextInTeamsPanel() {
  ...
  if (createPanel instanceof HTMLElement && !createPanel.hidden && createName instanceof HTMLInputElement && createName.value.trim()) {
    return true;
  }
...
function isClinicalTeamsPanelUserInteracting() {
  ...
  if (host && active instanceof HTMLElement && host.contains(active)) {
    ...
    if (active instanceof HTMLOptionElement && active.parentElement instanceof HTMLSelectElement && host.contains(active.parentElement)) {
      return true;
    }
  }
  return hasDraftTextInTeamsPanel();
}
function captureClinicalTeamsPanelDraft(host) {
  const createPanel = document.getElementById("clinical-team-create-panel");
  ...
  return {
    ...
    profile: {
      ...
      ...adminValue !== void 0 ? { "clinical-profile-admin": adminValue } : {}
    },
...
function restoreCreateTeamPanelDraft(draft) {
  ...
  void import("/mobile/js/chunks/teams-roster-create-QYLPT65Z.js").then((m) => {
...
function restoreClinicalTeamsPanelDraft(host, draft) {
  if (!draft) return;
...
function closeCreateTeamPanelAfterSuccess() {
...
async function maybeRefreshClinicalOpsDirectory(skipPull, browseSala, homeSala) {
  ...
  const { isClinicalTeamsPanelActive: isClinicalTeamsPanelActive2 } = await import("/mobile/js/chunks/teams-roster-shell-OTAB7QTU.js");
  if (!ok || !isClinicalTeamsPanelActive2()) return;
...
async function resolveClinicalTeamsPanelSections(userId, user, joined, ctx, elevated) {
  const joinedHtml = joined.length ? joined.map((team) => renderJoinedTeamCard(team)).join("") : buildJoinedTeamsEmptyHtml(ctx.displayHandle, false);
...
async function renderClinicalTeamsPanelInto(host, opts = {}) {
  ...
  const { wireRenderedClinicalTeamsPanel: wireRenderedClinicalTeamsPanel2 } = await import("/mobile/js/chunks/teams-roster-interactions-JRKM27BH.js");
...
function isClinicalTeamsPanelActive() {
  return isClinicalTeamsPanelOpen() || isEquipoEmbedActive();
}
async function refreshTeamsUiAfterChange(opts = {}) {
  const { isDirectoryModalOpen: isDirectoryModalOpen2 } = await import("/mobile/js/chunks/teams-roster-users-3ODI5JSN.js");
  if (isDirectoryModalOpen2()) return;
  const { refreshClinicalPatientListForScope: refreshClinicalPatientListForScope2 } = await import("/mobile/js/chunks/clinical-access-runtime-3IIQQT2K.js");
  await refreshClinicalPatientListForScope2({ allowLanPull: true });
  import("/mobile/js/chunks/clinical-rotation-entry-4QY3QSUG.js").then((m) => m.syncClinicalRotationEntryChrome());
  if (isClinicalTeamsPanelActive()) {
    if (!opts.force) {
      const { isClinicalTeamsPanelUserInteracting: isClinicalTeamsPanelUserInteracting2 } = await import("/mobile/js/chunks/teams-roster-panel-draft-WHSAWK3J.js");
...
async function openClinicalTeamsPanel(opts = {}) {
  const { openConexionEquipoPanel } = await import("/mobile/js/chunks/panel-equipo-nav-AY4TPQP6.js");
...
async function openClinicalTeamsPanelModal(opts = {}) {
  const bd = teamsModalEl();
  if (!bd) return;
  showClinicalTeamsPanelShell();
  try {
    const { wireClinicalTeamsModalChrome: wireClinicalTeamsModalChrome2 } = await import("/mobile/js/chunks/teams-roster-modal-chrome-ASODRUQG.js");
...
async function handleRemoveMemberClick(btn, confirmFn = openConfirm) {
  const targetUserId = String(btn.dataset.userId || "").trim();
  const label = String(btn.dataset.userLabel || "").trim() || targetUserId;
  if (!targetUserId) return;
  const result = await confirmFn({
    weight: "destructive",
    title: `\xBFQuitar a \xAB${label}\xBB del equipo y de la base cl\xEDnica en esta Mac?`,
    message: "Desaparecer\xE1 de Integrantes al sincronizar clinicalOps con la sala.",
    confirmLabel: "Quitar",
  ...
  const api2 = dbApi2();
  if (!api2 || typeof api2.dbClinicalUserDelete !== "function") {
    toast("Quitar integrantes requiere R+ de escritorio con base cl\xEDnica desbloqueada.", "error");
    return;
  }
  btn.disabled = true;
  const res = await api2.dbClinicalUserDelete({
...
async function handleLeaveTeamClick(btn, confirmFn = openConfirm) {
  ...
  const result = await confirmFn({
    weight: "consequence",
    title: `\xBFSalir del equipo \xAB${teamName}\xBB?`,
    consequenceText: "Dejar\xE1s de ver los pacientes asignados a ese equipo en Mi rotaci\xF3n.",
    confirmLabel: "Salir",
    cancelLabel: "Cancelar"
  });
  if (result !== "confirm") return;
...
async function handleDeleteTeamClick(btn, confirmFn = openConfirm) {
  const teamId = String(btn.dataset.teamId || "").trim();
  const teamName = String(btn.dataset.teamName || "este equipo").trim();
  if (!teamId) return;
  const result = await confirmFn({
    weight: "destructive",
    title: `\xBFEliminar el equipo \xAB${teamName}\xBB?`,
    message: "Se quitar\xE1n sus integrantes. Esta acci\xF3n no se puede deshacer.",
    confirmLabel: "Eliminar",
    cancelLabel: "Cancelar"
  });
  if (result !== "confirm") return;
  ...
  btn.disabled = false;
  if (!res || res.ok === false) {
    toast(res?.error || "No se elimin\xF3 el equipo.", "error");
    return;
  }
...
async function submitTeamEdit(api2, { teamId, name, sala, userId, form }) {
  ...
  if (!res || res.ok === false) {
    toast(res?.error || "No se guard\xF3 el equipo.", "error");
    return;
  }
...
async function confirmUsernameChange(currentUsername, username, confirmFn) {
  ...
  const result = await confirmFn({
    weight: "consequence",
    title: `\xBFCambiar tu usuario de @${currentUsername} a @${username}?`,
    consequenceText: "Los equipos ver\xE1n el nuevo nombre.",
...
async function tryResumeExistingUsername(username, errMsg, confirmFn) {
  ...
  const result = await confirmFn({
    ...
    consequenceText: "\xBFRecuperar tu cuenta en este dispositivo?",
...
async function claimClinicalUsernameIfNeeded(username, sala, deps = {}) {
...
async function persistProfileFromPanel(fields) {
  ...
  if (ok && upgraded) {
    toast("Sala Nube: completa el registro de contrase\xF1a.", "info");
    try {
      const main = await import("/mobile/js/chunks/clinical-onboarding-main-RTQEC5VT.js");
...
async function submitProfileUpsert(api2, userId, fields) {
  ...
  if (!res || res.ok === false) {
    toast(res?.error || "No se guard\xF3 el perfil.", "error");
    return false;
  }
...
async function resolveProgramAdminChange(adminCb, wasProgramAdmin) {
  ...
  if (!isAdminAccessGrantedThisSession()) {
    ...
    if (!code || !verifyAdminAccessCode(code)) {
      ...
      if (code != null) toast("C\xF3digo incorrecto.", "error");
      return null;
    }
...
async function toastProfileSaveResult({ msg, usernameWillChange, sala }) {
  const { flushClinicalProfileToCloud } = await import("/mobile/js/chunks/clinical-profile-cloud-stubs-LE2FYXR3.js");
...
async function handleProfileFormSubmit(ev) {
  ...
  if (!isValidUsernameFormat(fields.username)) {
    toast("Usuario inv\xE1lido. Usa 3\u201332 caracteres en min\xFAsculas: letras, n\xFAmeros y _.", "error");
    return;
  }
  ...
  if (!currentUserId() || !dbApi2()) {
    toast("Sesi\xF3n cl\xEDnica no disponible. Desbloquea la base de datos.", "error");
    return;
  }
  ...
  void import("/mobile/js/chunks/teams-guardia-bridge-U6MEWTW4.js").then((mod) => mod.publishClinicalTeamsAfterChange({ sala: fields.sala })).catch(() => {
  });
  void import("/mobile/js/chunks/patients-GGIREUSR.js").then((m) => m.renderPatientList()).catch(() => {
  });
}
...
async function createElevatedTeam(api2, { name, sala, userId }) {
  ...
  if (!res || res.ok === false) {
    toast(res?.error || "No se cre\xF3 el equipo.", "error");
    return;
  }
...
async function autoJoinCreatorToTeam(api2, teamId, userId, cycleLetter) {
  ...
  if (!addRes || addRes.ok === false) {
    toast(addRes?.error || "Equipo creado pero no se pudo unir autom\xE1ticamente.", "error");
  }
}
...
async function handleAddMemberSubmit(ev, form) {
  ...
  if (!res || res.ok === false) {
    toast(res?.error || "No se agreg\xF3 el miembro.", "error");
    return;
  }
...
async function handleMyCycleSubmit(ev, form) {
  ...
  if (!res || res.ok === false) {
    toast(res?.error || "No se guard\xF3 el ciclo.", "error");
    return;
  }
...
function partitionNmMedsForSoap(fieldVal) {
  ...
  part.antidiabeticos.forEach(function(line) {
    if (INSULIN_NM_LINE_RE.test(line) && !RESCATE_NM_LINE_RE.test(line) && !/^INSULINA\s+PREPRANDIAL:/i.test(line) && !/BOMBA\s+DE\s+INSULINA/i.test(line)) {
      insulin.push(line);
    }
  });
  return {
...
// public/js/gaso-extended-steps.mjs
...
function buildPhStep_(pH) {
  if (pH != null) {
    if (pH < 7.35) return { label: "pH", interpretation: "Acidemia (pH < 7.35)." };
    if (pH > 7.45) return { label: "pH", interpretation: "Alcalemia (pH > 7.45)." };
    return {
      label: "pH",
      interpretation: "pH dentro del rango fisiol\xF3gico t\xEDpico (7.35\u20137.45) o apenas compensado."
    };
  }
  return { label: "pH", interpretation: "Sin dato de pH para clasificar estado \xE1cido-base." };
}
function detectMixedFromWinter_(metaLow, pCO2, winterCenter, hco3) {
...
function classifyPrimaryDisorder_(pH, metaLow, metaHigh, respLow, respHigh, mixedFromWinter) {
  if (mixedFromWinter) {
    var type = "acidosis";
...
function buildCompensation_(hco3, metaLow, pCO2, winterCenter) {
  var compensation = {
    expectedPCO2: null,
  ...
  if (pCO2 != null) {
    ...
    if (hco3 != null) {
      if (Math.abs(hco3 - compensation.expectedHCO3Acute) < 2) {
        ...
        compParts.push("El HCO\u2083\u207B coincide mejor con patr\xF3n cr\xF3nico/compensatorio (~0.4/\u0394PaCO\u2082).");
      }
    }
  }
...
function buildAnionGapStep_(agVal, agcVal) {
  ...
  if (display != null && isFinite(display)) {
    var label = agcVal != null ? "Ani\xF3n gap corregido por alb\xFAmina (cAG)" : "Ani\xF3n gap";
    if (display < 8) {
      anionGap.interpretation = label + " por debajo del rango usual (referencia habitual 8\u201312 mEq/L).";
    } else if (display > 12) {
      anionGap.interpretation = label + " elevado (>12): favor\xE9zcase gap en acidosis metab\xF3lica (lista amplia diferencial).";
    } else {
      anionGap.interpretation = label + " dentro del rango usual (aproximadamente 8\u201312).";
    }
    if (agcVal != null && agVal != null) {
      anionGap.interpretation += " AG crudo " + String(Math.round((agVal + Number.EPSILON) * 10) / 10) + " \u2192 cAG " + String(Math.round((agcVal + Number.EPSILON) * 10) / 10) + " (correcci\xF3n +2.5\xD7(4\u2212Alb)).";
    }
  } else {
...
function buildUrinaryAnionGapStep_(uagVal) {
  var urinaryAnionGap = {
  ...
  if (uagVal != null && isFinite(uagVal)) {
    if (uagVal < 0) {
      urinaryAnionGap.interpretation = "UAG negativo: favorece excreci\xF3n renal adecuada de NH\u2084\u207A (p. ej. p\xE9rdidas GI / diarrea).";
    } else {
      urinaryAnionGap.interpretation = "UAG positivo o cero: sugiere alteraci\xF3n en excreci\xF3n de NH\u2084\u207A (considerar ATR u otras causas renales).";
    }
  } else {
    urinaryAnionGap.interpretation = "No se puede calcular (faltan Na, K o Cl urinarios).";
  }
  return urinaryAnionGap;
}
function buildDeltaDeltaStep_(agVal, hco3, agcVal) {
...
function pfRatioInterpretation_(pfRatio) {
  if (pfRatio >= 400) return "P/F alta (usualmente mejor perfusi\xF3n/tejido si FiO\u2082 es confiable).";
  if (pfRatio >= 300) return "P/F discretamente alterada.";
  if (pfRatio >= 200) return "P/F compatible con d\xE9ficit leve/moderado de oxigenaci\xF3n.";
...
function appendAaGradientNotes_(oxBits, pO2, pCO2, fio2, ageMonths) {
  ...
  oxBits.push(
    "Gradiente A\u2013a simplificado (~nivel del mar; PAO\u2082 \u2248 FiO\u2082\xD7713 \u2212 PaCO\u2082/0.8): \u2248 " + aaGradient + " mmHg."
  );
  ...
  if (ageYears != null && ageYears >= 18) {
    ...
    oxBits.push("Regla pr\xE1ctica esperada en adultos (orientativa ~edad a\xF1os/4+4): \u2248 " + expAa + " mmHg.");
  }
  return aaGradient;
}
function buildOxygenationStep_(pO2, pCO2, fio2, ageMonths) {
...
function buildSummaryLines_(parts) {
  var summaryLines = [];
  if (parts.phStep.interpretation) {
    summaryLines.push(parts.phStep.label + ": " + parts.phStep.interpretation);
  }
  summaryLines.push(
    "Primario predominante inferido \u2014 " + parts.primary.disorder + (parts.primary.type !== "none" ? " (" + parts.primary.type + ")" : "")
  );
  if (parts.compensation.note) summaryLines.push("Compensaci\xF3n: " + parts.compensation.note);
  if (parts.anionGap.value != null) {
    var agLine = "Ani\xF3n gap: " + String(parts.anionGap.value);
  ...
  if (parts.oxygenation.pfRatio != null || parts.oxygenation.aaGradient != null || parts.oxygenation.note) {
    summaryLines.push("Oxigenaci\xF3n: " + parts.oxygenation.note);
  }
  return summaryLines;
}

// public/js/gaso-extended-rationale.mjs
...
function rationaleMetabolicAcidosis_(ctx) {
  ...
  lines.push("HCO\u2083\u207B " + fmtLab_(ctx.hco3, 1) + " < 22: acidosis metab\xF3lica primaria.");
  if (ctx.pCO2 != null && ctx.winterCenter != null && Math.abs(ctx.pCO2 - ctx.winterCenter) <= 2) {
    lines.push(
      "PaCO\u2082 " + fmtLab_(ctx.pCO2) + " mmHg coincide con compensaci\xF3n respiratoria esperada (Winter \u2248 " + fmtLab_(ctx.winterCenter, 1) + ")."
    );
  }
  return lines.join(" ");
}
function rationaleMetabolicAlkalosis_(ctx) {
  return phPrefix_(ctx.pH).concat(["HCO\u2083\u207B " + fmtLab_(ctx.hco3, 1) + " > 26: alcalosis metab\xF3lica primaria."]).join(" ");
}
...
function rationaleMixedAcidosis_(ctx) {
  ...
  if (ctx.metaLow) lines.push("Hay acidosis metab\xF3lica (HCO\u2083\u207B bajo).");
  if (ctx.respHigh) lines.push("Hay retenci\xF3n de CO\u2082 (PaCO\u2082 elevada).");
  return lines.join(" ");
}
function rationaleMixedAlkalosis_(ctx) {
  ...
  if (ctx.metaLow && ctx.respLow) {
    ...
    if (ctx.metaLow) lines.push("HCO\u2083\u207B bajo sin alcalosis metab\xF3lica (HCO\u2083\u207B no elevado).");
...
function rationaleCompensated_(ctx) {
  ...
  if (ctx.metaLow && ctx.respLow) lines.push("Patr\xF3n acid\xF3tico compensado (HCO\u2083\u207B bajo + PaCO\u2082 baja).");
  if (ctx.metaHigh && ctx.respHigh) lines.push("Patr\xF3n alcal\xF3tico compensado (HCO\u2083\u207B alto + PaCO\u2082 alta).");
  return lines.join(" ");
}
...
function evaluateGasoExtended(input) {
  ...
  var agcVal = computeAlbuminCorrectedAnionGapValue_(strLab_(na), strLab_(cl), strLab_(hco3), strLab_(alb)) ?? null;
...
function gasoMetricChip(label, value, tone) {
...
function gasoSubMetric(label, value) {
...
function extractGasoLabValues(latest) {
  return {
    na: serieNumFromLabSet(latest, "QS", "Na") ?? serieNumFromLabSet(latest, "ESC", "Na") ?? serieNumFromLabSet(latest, "GASES", "Na"),
    cl: serieNumFromLabSet(latest, "QS", "Cl") ?? serieNumFromLabSet(latest, "ESC", "Cl"),
...
function appendCompensationStep(steps, cmp) {
  ...
  if (cmp.expectedHCO3Chronic != null) {
    cmpGrid.appendChild(gasoSubMetric("HCO\u2083\u207B cr\xF3nico", "~" + cmp.expectedHCO3Chronic));
  }
  if (cmpGrid.childNodes.length) cmpBody.appendChild(cmpGrid);
  cmpBody.appendChild(gasoStepText(cmp.note));
  steps.appendChild(gasoStepCard(3, "Compensaci\xF3n esperada", cmpBody));
}
function appendBadgeStep(steps, num2, title, badgeText, badgeWarn, interpretation) {
  var body = document.createElement("div");
  if (badgeText != null) {
  ...
  steps.appendChild(gasoStepCard(num2, title, body));
}
function appendOxygenationStep(steps, ox) {
  ...
  steps.appendChild(gasoStepCard(7, "Oxigenaci\xF3n", oxBody));
}
function buildGasoMetricsRow(lab, ev) {
  ...
  var agDisplay = ev.steps.anionGap.corrected != null ? ev.steps.anionGap.corrected : ev.steps.anionGap.value;
  metrics.appendChild(
    gasoMetricChip(
      ev.steps.anionGap.corrected != null ? "cAG" : "Ani\xF3n gap",
...
function buildGasoSteps(ev) {
  ...
  steps.appendChild(gasoStepCard(1, "Estado \xE1cido-base", gasoStepText(ev.steps.ph.interpretation)));
  appendPrimaryStep(steps, ev);
  appendCompensationStep(steps, ev.steps.compensation);
  var ag = ev.steps.anionGap;
  var agBadge = ag.corrected != null ? "cAG " + ag.corrected + " (AG " + ag.value + ") mEq/L" : ag.value != null ? ag.value + " mEq/L" : null;
  var agHigh = (ag.corrected != null ? ag.corrected : ag.value) != null && (ag.corrected != null ? ag.corrected : ag.value) > 12;
  appendBadgeStep(steps, 4, "Ani\xF3n gap", agBadge, agHigh, ag.interpretation);
...
function renderGasoEmptySlot(slot, escHtml2, message, isError) {
  var color = isError ? "var(--error)" : "var(--text-muted)";
  slot.innerHTML = '<p class="tend-empty" style="font-size:13px;color:' + color + ';">' + escHtml2(message) + "</p>";
}
function refillGasoExtendedSlot(slot, latest, fio2, escHtml2) {
  if (!slot) return;
  slot.innerHTML = "";
  if (!latest || !latest.parsedBySection) {
    renderGasoEmptySlot(slot, escHtml2, "Sin valores recientes disponibles para gasometr\xEDa.", false);
    return;
  }
  ...
  try {
    ...
    renderGasoEmptySlot(slot, escHtml2, "No se pudo calcular la gasometr\xEDa extendida.", true);
    console.error("evaluateGasoExtended", e);
  }
}

// public/js/features/estado-actual-ventilatorio-labs.mjs
function emptyVentilatorioLabContext() {
  return { kind: null, pO2: null, pCO2: null, pH: null, sourceLabel: "", setId: null };
}
function classifyGasometryKind(sourceText, resLabs) {
...
function gasometryKindLabel(kind) {
...
function formatLabSetSourceLabel(set) {
  ...
  if (date && kindLbl) return "Gasometr\xEDa " + kindLbl + " \xB7 " + date;
  if (date) return "Gasometr\xEDa \xB7 " + date;
  if (kindLbl) return "Gasometr\xEDa " + kindLbl;
  return "Gasometr\xEDa en expediente";
}
function resolveVentilatorioLabContext(patientId, labHistoryMap) {
...
function gasometryKindSupportsPafi(kind) {
  return kind === "arterial" || kind === "capillary";
}

// public/js/features/estado-actual-ventilatorio.mjs
var SOPORTE_LITROS_TYPES = [
  "Puntillas nasales",
  "Mascarilla simple",
...
function ventilatorioLabHintsEligible(ec) {
...
function resolvePafiForHints(labHintsOn, lab, fio2) {
  ...
  if (lab && gasometryKindSupportsPafi(lab.kind)) {
    ...
    hints.push("Gaso venosa: PaFi no v\xE1lida \u2014 usar SpO\u2082/FiO\u2082");
...
function resolveDrivingPressureHints(pmeseta, peep) {
  ...
  if (pmeseta != null && pmeseta >= 30) hints.push("P meseta \u226530: ventilaci\xF3n no protectora");
  return hints;
}
function resolveRoxAndFio2HintsHfnc(sat, fio2, fr, soporte) {
  ...
  if (fio2 != null && fio2 > 60 && isHfnc) {
    hints.push("FiO\u2082 >60% en alto flujo: valorar intubaci\xF3n");
  }
  return hints;
}
function resolveVtMlKgHints(vt, peso) {
  ...
  if (mlKg != null) {
    ...
    if (mlKg > 8) hints.push("VT >8 ml/kg: revisar protecci\xF3n pulmonar");
    if (mlKg > 6 && mlKg <= 8) hints.push("VT 6\u20138 ml/kg (pulm\xF3n normal)");
...
function buildVentilatorioCalcHints(ec, ctx) {
  ctx = ctx || {};
...
function formatSoporteVmClause(ec, fio2) {
  ...
  var vm = "CON VENTILACI\xD3N MEC\xC1NICA";
...
function formatSoporteVentilatorioClause(ec) {
  ...
  if (soporte === "Traqueostom\xEDa") return formatSoporteTraqueostomiaClause(fio2);
  if (tier === "litros") {
...
function formatVentilatorioCalcClause(ec, ctx) {
...
function resolveSoporteClause(ec, ctx) {
...
function openSOAPModal() {
  ...
  var existing = getNotes()[activeId] && getNotes()[activeId].evolucion ? getNotes()[activeId].evolucion.trim() : "";
  if (existing) {
...
var SOAP_SOPORTE_MAP = {
  ...
  "Ventilaci\xF3n mec\xE1nica": "CON VENTILACI\xD3N MEC\xC1NICA",
...
function wireInheritPatientsButtons() {
  document.querySelectorAll(".clinical-teams-inherit-btn").forEach((btn) => {
    ...
    btn.addEventListener("click", async () => {
      ...
      const { openInheritPatientsModal, wireInheritPatientsModal } = await import("/mobile/js/chunks/teams-roster-inherit-patients-modal-S4LEO44U.js");
...
function wireClinicalTeamsModalChrome() {
  ...
  if (bd) {
    if (!bd._rpcTeamsBackdropClick) {
      ...
      bd.addEventListener("click", (ev) => {
        if (ev.target === bd) {
          void loadRoster2().then((m) => m.closeClinicalTeamsPanel());
        }
      });
    }
  ...
  void loadRoster2().then((m) => m.wireTeamManageModalDelegation());
}

// public/js/features/clinical-teams/index.mjs
var teamsControlsWired = false;
function wireClinicalTeamsControls() {
  void import("/mobile/js/chunks/teams-roster-modal-chrome-ASODRUQG.js").then((m) => m.wireClinicalTeamsModalChrome());
  if (teamsControlsWired) return;
  teamsControlsWired = true;
  import("/mobile/js/chunks/clinical-rotation-entry-4QY3QSUG.js").then((mod) => {
    mod.wireClinicalRotationEntryControls();
    mod.syncClinicalRotationEntryChrome();
  });
  void import("/mobile/js/chunks/clinical-rotation-AM2FB5WS.js").then((mod) => {
...
function buildRotationRejoinLeadHtml(user) {
  ...
  const sala = String(user?.sala || "").trim();
  if (elevated) {
    return "<p>Se archivaron los equipos del mes anterior" + (sala ? ` en <strong>${escapeHtml(sala)}</strong>` : "") + ". Confirma tu sala y <strong>publica los equipos nuevos</strong> en Mi rotaci\xF3n para que el resto se una.</p>";
  }
  return "<p>Hay <strong>nueva rotaci\xF3n</strong>: los equipos anteriores ya no est\xE1n activos" + (sala ? ` (sala actual: <strong>${escapeHtml(sala)}</strong>)` : "") + ". Tu R2 o R4 ya public\xF3 equipos nuevos en Nube \u2014 confirma tu sala y <strong>elige el tuyo</strong> en Mi rotaci\xF3n.</p>";
}
function updateRotationRejoinOpenButton() {
  ...
  btn.textContent = elevated ? "Confirmar sala y abrir Mi rotaci\xF3n" : "Confirmar sala y elegir equipo";
}
...
async function maybeShowRotationRejoinModal() {
...
async function promptRotationRejoinAfterNuevaRotacion() {
...
async function persistSalaFromModal() {
  ...
  const { persistProfileFromPanel: persistProfileFromPanel2 } = await import("/mobile/js/chunks/teams-roster-profile-persist-R3CIA5KT.js");
  const user = clinicalSessionContext.user || {};
  ...
  host.querySelectorAll(".guardia-incoming-chip").forEach((btn, idx) => {
    ...
    btn.addEventListener("click", (ev) => {
      ...
      if (isChartLockedForPatient(row, /* @__PURE__ */ new Date())) {
        ...
        else toast2(`Disponible el ${formatEffectiveLabel(String(row.effective_at || ""))}`, "info");
        return;
      }
    });
  });
...
function openRotationConfigModal() {
  if (!canConfigureRotation2()) {
    toast2("Solo R4 o Admin pueden configurar la rotaci\xF3n.", "error");
    return;
  }
...
function wireRotationConfigFormOnce() {
  ...
  form.addEventListener("submit", async (ev) => {
    ...
    const res = await submitRotationConfigForm(toast2);
    if (!res.ok) return;
    closeRotationConfigModal();
    toast2("Configuraci\xF3n de rotaci\xF3n guardada.", "success");
...
async function confirmNuevaRotacion() {
  ...
  if (!res || res.ok === false) {
    toast2(res?.error || "No se aplic\xF3 la nueva rotaci\xF3n.", "error");
    return { ok: false };
  }
  toast2("Nueva rotaci\xF3n aplicada. Cada residente debe unirse a su equipo nuevo.", "success");
  try {
    await fetchClinicalTeamsFromDb();
  } catch {
  }
  ...
  document.dispatchEvent(new CustomEvent("rpc-guardia-rotation-changed"));
  try {
    const { publishClinicalTeamsAfterChange: publishClinicalTeamsAfterChange2 } = await import("/mobile/js/chunks/teams-guardia-bridge-U6MEWTW4.js");
...
function wireGuardiaRotationControls() {
  ...
  void import("/mobile/js/chunks/teams-roster-inherit-patients-modal-S4LEO44U.js").then(
...
async function syncGuardiaIncomingStrip(settings) {
  ...
  renderIncomingStrip(assignments, {
    onLockedClick: (row) => {
      toast2(`Disponible el ${formatEffectiveLabel(String(row.effective_at || ""))}`, "info");
    }
  });
}
...
function resolveEntregaPatientRow(patientId) {
  ...
  if (!id) return null;
  const row = (getPatients() || []).find((p) => String(p.id) === id) || (clinicalSessionContext.scopeContext?.patients || []).find(
...
async function pushClinicalOpsAfterTeamAssign(teamSala) {
  const lan = await import("/mobile/js/chunks/mutate-bridge-NAGIKQFO.js").catch(() => null);
...
async function mirrorAssignedPatientToCloud(pid) {
  ...
  const { getClinicalScopeContextForEvaluate: getClinicalScopeContextForEvaluate2 } = await import("/mobile/js/chunks/clinical-access-runtime-3IIQQT2K.js");
  const scopeCtx = getClinicalScopeContextForEvaluate2();
  try {
    const cloud = await import("/mobile/js/chunks/mutate-bridge-NAGIKQFO.js");
    if (!patient) {
      if (typeof cloud.scheduleCloudSyncPush === "function") cloud.scheduleCloudSyncPush();
      return;
    }
    const { mirrorPatientCensusToOperationalSala, patientBelongsToActiveCloudRoom } = await import("/mobile/js/chunks/cloud-census-sala-push-FUYE5QJF.js");
    await mirrorPatientCensusToOperationalSala(patient, {
      actorId: cloud.resolveCloudActorId?.(),
      context: scopeCtx
    });
    if (patientBelongsToActiveCloudRoom(patient, scopeCtx) && typeof cloud.scheduleCloudSyncPush === "function") {
...
async function onPatientTeamAssignChange(teamId) {
  ...
  if (res.ok) {
    ...
    const patientsMod = await import("/mobile/js/chunks/patients-GGIREUSR.js").catch(() => null);
...
async function notifyAssignToast(msg, type) {
  try {
    const shell = await import("/mobile/js/chunks/app-shell-7T6KXOLW.js");
...
function hydrateElevatedPatientFiltersFromStorage(storage2, user) {
  try {
    var pref = readElevatedTeamFilterPreference(storage2);
    if (pref.pinned) {
  ...
  try {
    elevatedPatientFilters.sala = resolveCensusSalaFilterId(user || null, storage2);
  } catch (_e) {
    void _e;
  }
...
function censusFiltersAreActive(filters) {
  var f = filters || elevatedPatientFilters;
  return String(f.sala || "__all__") !== "__all__" || !!String(f.teamId || "").trim() || !!String(f.service || "").trim();
}
...
function rosterHandoffCounts(guardiasMap) {
  let totalEstudios = 0;
...
function rosterPendingTodoCount(censusPatients) {
  return censusPatients.reduce(
    (sum, p) => sum + storage.getTodos(p.id).filter((t) => !t.completed).length,
    0
  );
}
function buildEntregaConsequenceText(censusPatients, guardiasMap) {
  const { totalEstudios, patientsWithSignos } = rosterHandoffCounts(guardiasMap);
...
function wireRosterFooter(guardiasMap, censusPatients) {
  document.getElementById("roster-btn-cancel")?.addEventListener("click", () => {
    void (async () => {
      ...
      const { endEntregaPhase: endEntregaPhase2 } = await import("/mobile/js/chunks/clinical-entrega-5DW537SJ.js");
      endEntregaPhase2();
  ...
  document.getElementById("roster-btn-confirm")?.addEventListener("click", () => {
    void (async () => {
      ...
      const result = await openConfirm({
        weight: "consequence",
        title: "\xBFConfirmar entrega de guardia?",
        consequenceText: hasSaved ? consequenceText : consequenceText + " No hay entregas guardadas con signos vitales ni estudios: los internos (MIP) solo ven pacientes entregados al R1 de guardia.",
...
function collectOpenPendientesBySourceTeam(guardias, patients, opts) {
...
async function resolveGuardiasForSourceTeam(group, deps) {
  ...
  return { resolved, total: patients.length, failed };
}
...
var feedOrderState = { order: [], alertState: {} };
var pendingReorderIds = [];
var lastNaturalIds = [];
...
function commitVitalsFeedReorder() {
...
function renderGuardiaVitalsFeed(patients, censusIds = []) {
  ...
  const reorderPillHtml = pendingCount > 0 ? `<button type="button" class="vfeed-reorder-pill" data-vfeed-reorder title="Actualizar el orden por prioridad">${pendingCount} nuevo${pendingCount === 1 ? "" : "s"}</button>` : "";
  host.innerHTML = `
    <div class="vfeed-header">
      ${VITALS_SVG}
      <span class="vfeed-title">Signos en este turno</span>
...
async function tryRestoreOrphanPatientFromLan(patientId) {
  ...
  const lan = await import("/mobile/js/chunks/mutate-bridge-NAGIKQFO.js");
...
async function openEntregaForOrphan(patientId, guardiaId) {
  const entrega = await import("/mobile/js/chunks/clinical-entrega-5DW537SJ.js");
...
async function removeOrphanPatientLocally(patientId) {
  if (!patientInLocalCensus(patientId)) return;
  const lanMod = await import("/mobile/js/chunks/mutate-bridge-NAGIKQFO.js");
...
async function deleteOrphanFromServer(btn, settings) {
  ...
  await openConfirm({
    weight: "destructive",
    title: "\xBFEliminar este paciente del anfitri\xF3n \u21C4 y liberar la entrega?",
    message: "Se borrar\xE1 el expediente en la red local y dejar\xE1 de asignarte el paciente. Esta acci\xF3n no se puede deshacer.",
    confirmLabel: "Eliminar",
    cancelLabel: "Cancelar",
...
// public/js/clinical-repo-flag.mjs
var LS_KEY = "rpc-clinical-repo-eventualidades";
var LS_KEY_PROJECTOR = "rpc-clinical-repo-sync-projector";
...
function isClinicalRepoEventualidadesEnabled() {
  try {
...
function isClinicalRepoSyncProjectorEnabled() {
  try {
    if (typeof process !== "undefined" && process.env) {
...
// public/js/clinical-repo-sync-drain.mjs
async function drainClinicalSyncProjector(opts = {}) {
...
// public/js/features/tendencias-bridge.mjs
var tendenciasBridge = {
...
function removeEventualidadCardEl(mountEl, entryId) {
  ...
  if (timeline && typeof timeline.querySelector === "function" && !timeline.querySelector("[data-entry-id]")) {
    timeline.classList.add("ev-timeline--empty");
    timeline.innerHTML = '<p class="ev-empty">A\xFAn no hay eventualidades. Agr\xE9galas abajo.</p>';
  }
  return true;
}
...
async function savePatientEventualidad(patient, text, atIso, kind, transfusionProduct) {
...
function normalizeEventualidadesStore(store) {
  ...
  return { entries: [], updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
}
function refreshTendenciasAfterEventualidades() {
  if (typeof document === "undefined") return;
  const onTend = typeof rt.getActiveInner === "function" && rt.getActiveInner() === "tend";
  if (onTend && document.getElementById("tendencias-container") && document.getElementById("tendencias-container").querySelector(".tend-grid") && typeof tendenciasBridge.renderTendencias === "function") {
...
function shouldPersistViaClinicalRepo(command) {
  return !!(isClinicalRepoEventualidadesEnabled() && canExecuteClinicalCommand() && command && typeof command === "object" && command.type);
}
...
function eventualidadesMountEl(preferred) {
  ...
  return typeof document !== "undefined" ? document.getElementById("exp-pane-eventualidades") : preferred;
}
...
function invalidateEventualidadesRelatedCaches() {
  ...
  void import("/mobile/js/chunks/expediente-inner-cache-NAAXSN3P.js").then(function(m) {
...
async function persistEventualidades(patient, store, command) {
...
function eventualidadUpsertCommand(patientId, wasEdit, editId, added, text, atIso) {
  return {
    type: "eventualidad.upsert",
    patientId: String(patientId || ""),
    entry: wasEdit ? { id: editId, text, at: atIso } : {
      id: added && added.id,
      text: added && added.text,
      at: added && added.at,
      kind: added && added.kind
...
function toastEventualidadPersist(out, wasEdit) {
  ...
  rt.showToast(
    out && out.reason === "empty" ? "Escribe la eventualidad antes de agregar." : "No se pudo guardar la eventualidad. Intenta de nuevo.",
    "error"
  );
  return false;
}
async function submitEventualidadEntry(mountEl, patient, input, atInput) {
  ...
  const next = wasEdit ? updateEventualidad(liveStore, editId, { text, at: atIso }) : appendEventualidad(liveStore, text, "", atIso);
...
// public/js/features/resolve-global-fn.mjs
...
// public/js/features/workbench/mode-frame.mjs
function buildModeFrameHtml(opts) {
  const {
    modeName = "",
    context = "",
    metadata = "",
  ...
  const secondaryHtml = secondaryActions.map(
    (a, i) => `<button type="button" class="wb-btn wb-btn-secondary" data-wb-secondary="${i}"` + (a.title ? ` title="${escAttr(a.title)}"` : "") + `>${escHtml(a.label)}</button>`
  ).join("");
  const contextClickable = !!onContextClick;
  const contextHtml = context ? `<span class="wb-mode-frame-context${contextClickable ? " wb-mode-frame-context--clickable" : ""}"` + (contextClickable ? ' data-wb-context role="button" tabindex="0"' : "") + `>${escHtml(context)}</span>` : "";
  return `<div class="wb-mode-frame"><div class="wb-mode-frame-left"><span class="wb-mode-frame-name">${escHtml(modeName)}</span>` + contextHtml + (metadata ? `<span class="wb-mode-frame-meta">${escHtml(metadata)}</span>` : "") + '</div><div class="wb-mode-frame-actions">' + secondaryHtml + (showShortcut ? `<button type="button" class="wb-btn wb-btn-secondary wb-btn-shortcut" data-wb-shortcut>${escHtml(shortcutLabel)}</button>` : "") + `<button type="button" class="wb-btn wb-btn-primary" data-wb-primary>${escHtml(primaryAction.label)}</button></div></div>`;
}
function mountModeFrame(container, opts) {
...
// public/js/features/workbench/counters-band.mjs
function buildCounterCellHtml(cell) {
  ...
  const detailHtml = detail ? `<span class="wb-counter-detail${isAlert ? " wb-counter-detail--alert" : ""}"> \xB7 ${escHtml(detail)}</span>` : "";
  const percent = progress ? Math.max(0, Math.min(100, Number(progress.percent) || 0)) : null;
  const progressHtml = percent === null ? "" : `<div class="wb-counter-progress"><div class="wb-counter-progress-fill progress-sweep-fill" style="width:${percent}%"></div></div>`;
  return `<div class="${cellClasses.join(" ")}"><div class="wb-counter-text"><span class="${labelClass}">${escHtml(label)}</span><span class="wb-counter-figure">${escHtml(figure)}${detailHtml}</span></div>` + progressHtml + "</div>";
}
function buildCountersBandHtml(cells) {
...
function mountCountersBand(container, cells) {
  if (!container) return void 0;
  container.innerHTML = buildCountersBandHtml(cells);
  return container;
}

// public/js/features/workbench/empty-state.mjs
function buildEmptyStateHtml({ label, missing, whenArrives, exitLabel, exitHref } = {}) {
  const exitHtml = exitLabel ? `<a class="wb-empty-exit" href="${escAttr(exitHref || "#")}" data-wb-empty-exit>${escHtml(exitLabel)}</a>` : "";
  return `<div class="wb-empty-state"><p class="wb-empty-label">${escHtml(label || "")}</p><p class="wb-empty-missing">${escHtml(missing || "")}</p><p class="wb-empty-when">${escHtml(whenArrives || "")}</p>` + exitHtml + "</div>";
}
function mountEmptyState(container, opts = {}) {
  if (!container) return void 0;
...
function importLazyRoutes() {
  return import("/mobile/js/chunks/lazy-feature-routes-IJS5AV5S.js");
}
...
function openCommandPaletteFromShell() {
  void import("/mobile/js/chunks/command-palette-QCNUEF3T.js").then(function(mod) {
...
function renderGuardiaModeFrame() {
...
function renderGuardiaSignosRecibidosPanel() {
  const host = document.getElementById("guardia-signos-recibidos");
  if (!host) return;
  mountEmptyState(host, {
    label: "Signos recibidos",
    missing: "Todav\xEDa no hay una captura de signos de internos conectada a Guardia.",
    whenArrives: "Llega con el Interno m\xF3vil (fase 11), pendiente de las decisiones de Nube y PHI."
  });
}
...
function syncEntregaPhaseChrome(opts = {}) {
  ...
  if (status) {
    if (active && phase?.coveringLabel && !rosterOpen) {
      status.hidden = false;
      status.textContent = `Entregando a ${phase.coveringLabel} \xB7 pulsa "Entregar guardia" para abrir el listado`;
    } else {
      status.hidden = true;
      status.textContent = "";
    }
...
function guardiaCounterCells(summary) {
  const vitalsPercent = summary.vitalsMonitored > 0 ? Math.round(summary.vitalsReceivedToday / summary.vitalsMonitored * 100) : 0;
  const pendientesDetail = summary.pendientesOverdue > 0 ? `${summary.pendientesOverdue} vencido${summary.pendientesOverdue === 1 ? "" : "s"}` : "";
  const ingresosDetail = summary.admissionsEnValoracion > 0 ? `${summary.admissionsEnValoracion} en valoraci\xF3n` : "";
  return [
    {
      label: "Toma de signos \xB7 08:00",
      figure: summary.vitalsMonitored > 0 ? `${summary.vitalsReceivedToday} de ${summary.vitalsMonitored} recibidos` : "Sin plan de signos",
...
function renderGuardiaCensusHead(count, state2) {
  const host = document.getElementById("guardia-census-head");
  if (!host) return;
  const parts = [];
  if (state2.critical > 0) parts.push(`${state2.critical} cr\xEDtico${state2.critical === 1 ? "" : "s"}`);
  if (state2.vitalsOverdue > 0) {
    parts.push(`${state2.vitalsOverdue} signo${state2.vitalsOverdue === 1 ? "" : "s"} vencido${state2.vitalsOverdue === 1 ? "" : "s"}`);
  }
...
function syncGuardiaLearnNudgeChrome() {
  const host = document.getElementById("guardia-census-head");
  if (!host) return;
  void import("/mobile/js/chunks/guardia-v7-progress-HEZKRSIW.js").then(function(progressMod) {
...
function appendGuardiaLearnNudge(host) {
  void Promise.all([
    import("/mobile/js/chunks/guardia-v7-progress-HEZKRSIW.js"),
    import("/mobile/js/chunks/learn-hub-T2DYNN2K.js")
  ]).then(function(mods) {
...
function syncGuardiaBoardChrome(state2) {
  const scroll = document.getElementById("guardia-board-scroll");
  if (scroll) {
    scroll.classList.toggle("guardia-board-scroll--turno", state2.turnoActivo);
    scroll.classList.toggle("guardia-board-scroll--roster", state2.rosterOpen);
  }
  const filterHint = document.getElementById("guardia-census-filter-hint");
  ...
  if (metricsPanel) metricsPanel.hidden = !!state2.rosterOpen;
  if (vitalsSection) vitalsSection.hidden = !state2.turnoActivo || !!state2.rosterOpen;
  if (filterHint) {
    const elevated = hasElevatedTeamPrivileges(clinicalSessionContext.user);
    const alcanceOn = !!clinicalSessionContext.guardiaMode;
    filterHint.textContent = alcanceOn ? "Solo pacientes que te entregaron en este turno." : elevated ? "Censo completo del servicio \u2014 acota con Filtros censo arriba." : state2.turnoActivo ? "Todos los pacientes en tu alcance durante el turno." : "Todos los pacientes en tu alcance cl\xEDnico.";
  ...
  syncEntregaPhaseChrome({ rosterOpen: state2.rosterOpen, turnoActivo: state2.turnoActivo });
  syncGuardiaPhaseBar({
    ...state2,
...
function ensureGuardiaBoardBootstrapped(settings) {
  ...
  void import("/mobile/js/chunks/clinical-rotation-entry-4QY3QSUG.js").then((mod) => {
    mod.syncClinicalRotationEntryChrome?.();
  });
...
function formatCultivosForCenso(labHistory, maxReports) {
  ...
  (labHistory || []).forEach(function(set2) {
...
function setsFromLatestCensoDay(sets) {
  ...
  var daySets = fecha ? sorted.filter(function(s) {
    return censoDayFecha(s) === fecha;
  }) : [sorted[0]];
  return { fecha, daySets: daySets.slice().reverse() };
}
...
var SOPORTE_FROM_TAIL = [
  [/VENTILACI(?:O|Ó)N\s+MEC[AÁ]NICA(?!\s+NO)/i, "Ventilaci\xF3n mec\xE1nica"],
...
function appendCensoSatLine(lines, v, ctx) {
  ...
  var soporte = resolveCensoSoporteLabel(ctx) || (ctx.soporteHint ? toEaSalidaText(ctx.soporteHint) : "") || SOPORTE_LABEL["Aire ambiente"];
...
function sortPatientsForCensus(patients) {
...
// public/js/clinical-access-runtime/scope-ops-user.mjs
...
function buildOpsSessionUser(resolved, sessionUser) {
  return {
    user_id: String(resolved.user_id),
    username: resolved.username ?? sessionUser?.username ?? null,
    rank: normalizeOpsRank(resolved.rank, sessionUser?.rank),
...
function persistOpsUserBinding(nextUser) {
  persistClinicalUserBinding({
...
function applyOpsResolvedUser(snapshot, sessionUser) {
  const settings = readRpcSettings();
  ...
  const nextUser = buildOpsSessionUser(resolved, sessionUser);
  clinicalSessionContext.user = nextUser;
  persistOpsUserBinding(nextUser);
  return true;
}
...
// public/js/clinical-access-runtime/scope-ops.mjs
function isClinicalScopeReadyForPatientApply() {
  const user = clinicalSessionContext.user;
  if (!user?.user_id) return false;
  if (shouldUseElevatedPatientCensus(user)) return true;
  if (!shouldEnforceTeamPatientMirror()) return true;
...
function applyClinicalScopeFromOpsSnapshot(snapshot) {
...
async function countTeamMemberMissingPatients(user, teams, assignments, localIds, now) {
  const { filterJoinedTeams: filterJoinedTeams2 } = await import("/mobile/js/chunks/shared-FR62KMCD.js");
  const { resolvePatientTeamIdFromAssignments: resolvePatientTeamIdFromAssignments2 } = await import("/mobile/js/chunks/clinico-access-AJALTSU5.js");
...
// public/js/clinical-access-runtime/census-nube-pull.mjs
var nubePatientReconcileTimer = null;
var nubePatientReconcileInFlight = null;
async function runNubePatientReconcile(_reason) {
  ...
  nubePatientReconcileInFlight = (async function() {
    try {
      const { getSharedNubeRuntime } = await import("/mobile/js/chunks/panel-conexion-runtime-XRURMEDI.js");
      const { autostartCloudSyncIfConfigured } = await import("/mobile/js/chunks/autostart-2IGWEWI2.js");
...
async function scheduleLanPatientReconcile(_reason, delayMs2) {
  if (!isCloudSyncActive()) return;
  const wait = Number(delayMs2) || 0;
...
async function ensureTeamAssignedPatientsOnDevice(options) {
  ...
  const assignments = Array.isArray(ctx.assignments) ? ctx.assignments : [];
  const now = ctx.now || (/* @__PURE__ */ new Date()).toISOString();
  const localIds = new Set((getPatients() || []).map((p) => String(p?.id || "")));
...
async function refreshClinicalPatientListForScope(options) {
  ...
  setRefreshClinicalPatientListForScopeInFlight(
    (async function() {
      ...
      if (typeof document === "undefined") return;
      try {
        const mod = await import("/mobile/js/chunks/patients-GGIREUSR.js");
...
async function scheduleHostReconcileAfterOpsMerge() {
  await scheduleLanPatientReconcile("assignment-merge", 2e3);
}
...
function wireClinicalOpsSyncRefresh() {
  ...
  document.addEventListener("rpc-clinical-ops-synced", (event) => {
    if (document.body.classList.contains("clinical-directory-open")) return;
...
function scheduleClinicalActivityLanPush() {
  ...
  setClinicalActivityLanPushTimer(
    setTimeout(() => {
      ...
      void import("/mobile/js/chunks/mutate-bridge-clinical-ops-N3N2N3O6.js").then((mod) => {
        if (typeof mod.pushCloudClinicalOpsNow === "function") {
          return mod.pushCloudClinicalOpsNow();
        }
      }).catch(() => {
...
function migrateLocalPatientsClinicalSala() {
  ...
  if (migrated > 0) {
    ...
    if (typeof document !== "undefined") {
      void import("/mobile/js/chunks/patients-GGIREUSR.js").then((mod) => mod.renderPatientList({ silent: true })).catch(() => {
      });
    }
  }
  return migrated;
...
async function refreshClinicalUserProfile() {
  const { ensureLanProfileGateDeviceReset } = await import("/mobile/js/chunks/clinical-settings-7XQJIPLW.js");
...
async function refreshBootstrapScopeAndCensus() {
  ...
  if (typeof document !== "undefined") {
    void import("/mobile/js/chunks/clinical-profile-cloud-stubs-LE2FYXR3.js").then((mod) => mod.flushClinicalProfileToCloud()).catch(() => {
    });
  }
  migrateLocalPatientsClinicalSala();
}
...
function syncUpdateChannelUI() {
  ...
  if (typeof syncTeamSyncHeaderButton === "function") rt3.syncTeamSyncHeaderButton();
}
...
function setUpdateTelemetryEnabled(enabled) {
  ...
  rt3.showToast(value ? "Telemetr\xEDa de actualizaci\xF3n activada." : "Telemetr\xEDa desactivada.", "success");
}
...
function syncHardwareAccelerationUI() {
  ...
  if (!api2 || typeof api2.getPerformancePrefs !== "function") {
    acc.style.display = "none";
    void import("/mobile/js/chunks/settings-dropdown-Y5QUJ4UM.js").then(function(m) {
  ...
  api2.getPerformancePrefs().then(function(prefs) {
...
function onHardwareAccelerationChange(enabled) {
  var api2 = window.electronAPI;
  if (!api2 || typeof api2.setHardwareAcceleration !== "function") {
    rt3.showToast("Solo disponible en la aplicaci\xF3n de escritorio.", "error");
    syncHardwareAccelerationUI();
    return;
  }
  api2.setHardwareAcceleration(!!enabled).then(function() {
    rt3.showToast("Reinicia R+ para aplicar la aceleraci\xF3n por hardware.", "info");
  }).catch(function() {
    rt3.showToast("No se pudo guardar la preferencia.", "error");
    syncHardwareAccelerationUI();
  });
}
...
function checkForRepairUpdate() {
  if (!window.electronAPI || typeof window.electronAPI.reinstallCurrentRelease !== "function") {
    rt4.showToast("Las actualizaciones autom\xE1ticas solo est\xE1n en la app de escritorio.", "error");
    return;
  }
...
function assertClinicalWriteAllowed(patientId, settings) {
  void settings;
  const patient = getPatients().find((p) => String(p.id) === String(patientId)) || (patientId ? { id: patientId } : null);
...
async function refreshDesktopPatientListAfterScopePrune() {
  ...
  try {
    const { renderGuardiaCensusGrid: renderGuardiaCensusGrid2 } = await import("/mobile/js/chunks/guardia-grid-QTBE6UQI.js");
    const { rt: rt5 } = await import("/mobile/js/chunks/patients-runtime-state-FQLJFA6R.js");
    renderGuardiaCensusGrid2(rt5.getSettings());
  } catch {
  }
}
...
export {
  ...
  resolveClinicalTeamsScrollHost,
  ...
  teamsModalEl,
  isClinicalTeamsPanelActive,
  refreshTeamsUiAfterChange,
  openClinicalTeamsPanel,
  openClinicalTeamsPanelModal,
  closeClinicalTeamsPanel,
...
//# sourceMappingURL=/js/chunks/chunk-6P7TSNN6.js.map
