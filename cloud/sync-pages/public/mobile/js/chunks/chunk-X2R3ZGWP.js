...
function appendExitingRows(container, rowsBeforeById, idsStillPresent) {
...
function prefersReducedMotion2() {
  try {
    if (typeof document !== "undefined" && document.documentElement && document.documentElement.classList.contains("motion-sobrio")) {
      return true;
    }
...
function getReleaseVelocity(history, opts) {
  opts = opts || {};
  var axis = opts.axis === "x" ? "x" : "y";
  var windowMs = opts.windowMs == null ? 100 : opts.windowMs;
  var list = Array.isArray(history) ? history : [];
  var now2 = opts.now != null ? opts.now : list.length ? list[list.length - 1].t : 0;
...
function springTo(el, keyframes2, options) {
  if (!el || typeof HTMLElement !== "undefined" && !(el instanceof HTMLElement) || typeof HTMLElement === "undefined" && (!el.style || typeof el.style !== "object")) {
    return null;
  }
  options = options || {};
  ...
  var animOpts = { type: "spring", bounce, duration };
...
function settlePasteSurface(el) {
  if (!el || typeof el.style !== "object") return { stop: function() {
  }, finished: Promise.resolve() };
  if (prefersReducedMotion2()) {
    el.style.opacity = "1";
    el.style.transform = "";
...
function resolvePatientFieldIds(errorMessage, isFromLab) {
...
function swapLabelText(label, nextText, options) {
  ...
  if (prefersReducedMotion2() || options.instant) {
...
function prepareModalBackdropOpen(backdropEl) {
...
function closeModalAnimated(backdropEl, done) {
...
function cancelOverlayClose(backdropEl, opts) {
  if (!backdropEl || !(backdropEl instanceof HTMLElement)) return;
...
function closeOverlayAnimated(backdropEl, hideFn, opts) {
  ...
  function settle() {
    ...
    settled = true;
    backdropEl.removeEventListener("animationend", onEnd);
    if (backdropEl._uiOverlayCloseId !== closeId) return;
    els.forEach(function(el) {
      el.classList.remove(OVERLAY_OUT_CLASS);
    });
    hide();
...
var _asyncElapsedTimers = /* @__PURE__ */ new WeakMap();
...
function setAsyncButtonLoading(btn, loading, opts) {
  ...
  if (loading) {
    ...
    if (!prefersReducedMotion2()) ensureButtonSpinner(btn);
    clearAsyncElapsed(btn);
    if (opts.showElapsed) {
      var started = Date.now();
      var paint = function() {
        swapLabelText(label, loadingText + " \xB7 " + formatButtonElapsed(Date.now() - started));
      };
      paint();
...
//# sourceMappingURL=/js/chunks/chunk-X2R3ZGWP.js.map
