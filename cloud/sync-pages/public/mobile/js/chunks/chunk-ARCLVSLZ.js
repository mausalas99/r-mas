...
function countLabSidecarsInState(state) {
  ...
  return { patients: patients2, sets };
}
...
function summarizeLocalLabHistory(activePatientId) {
  ...
  let totalSets = 0;
  Object.keys(labHistory || {}).forEach(function(pid2) {
    const n = Array.isArray(labHistory[pid2]) ? labHistory[pid2].length : 0;
    if (!n) return;
    patientsWithLabs += 1;
    totalSets += n;
  });
  ...
  const activePatient = pid && patients.find(function(p) {
...
function buildLabSyncDiagnosticsIssues(opts) {
  ...
  if (!ingress.at) {
    if (!networkBlocking) {
      issues.push("A\xFAn no hay un pull Nube registrado en esta sesi\xF3n.");
    }
  } else if (!networkBlocking && Number(ingress.labOpsInPayload || 0) === 0 && Number(ingress.rawSidecars?.sets || 0) === 0) {
    issues.push(
  ...
  if (ingress.at && Number(ingress.rawSidecars?.sets || 0) > Number(ingress.filteredSidecars?.sets || 0)) {
    issues.push(
      "El worker filtr\xF3 " + (Number(ingress.rawSidecars.sets) - Number(ingress.filteredSidecars.sets)) + " estudio(s) por ventana m\xF3vil (" + MOBILE_LAB_HISTORY_DAYS + " d\xEDas)."
    );
  }
...
async function forceLabMobileSyncPull() {
  ...
  try {
    const { getCloudMobileRuntime } = await import("/mobile/js/chunks/runtime-WZ6RASR3.js");
    const runtime = getCloudMobileRuntime();
    if (!runtime?.syncCycle) {
      const { showToast: showToast2 } = await import("/mobile/js/chunks/ui-toast-SWIWD6T6.js");
      showToast2("Sync Nube no est\xE1 activo en esta sesi\xF3n", "error");
      return;
    }
    const { showToast } = await import("/mobile/js/chunks/ui-toast-SWIWD6T6.js");
...
//# sourceMappingURL=/js/chunks/chunk-ARCLVSLZ.js.map
