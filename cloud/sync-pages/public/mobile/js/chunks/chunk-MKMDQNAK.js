...
} from "/mobile/js/chunks/chunk-HNEGYFPT.js";
import {
  hideTourIntroModal,
...
import {
  ...
  loadTourProgress,
  saveTourProgress
} from "/mobile/js/chunks/chunk-XBVMN72A.js";
import {
  getChapterForStep,
...
} from "/mobile/js/chunks/chunk-P6VHYBXO.js";
import {
  DEMO_GARCIA_LAB_REPORT,
  DEMO_SOME_LAB_REPORT,
  OLDER_DEMO_SOME_LAB_REPORT,
  buildTourDemoListadoProblemas
} from "/mobile/js/chunks/chunk-JOF7DXVU.js";
import {
  switchLabInner
} from "/mobile/js/chunks/chunk-UKBXHDKP.js";
import {
  applyAppModeSwitchEffects
} from "/mobile/js/chunks/chunk-VPWSQYDS.js";
import {
  GUIDED_TOUR_LS_KEY,
...
} from "/mobile/js/chunks/chunk-43SWAHG6.js";
import {
...
} from "/mobile/js/chunks/chunk-Z7AG6BZL.js";
import {
  DEMO_PATIENT_ID,
  DEMO_PATIENT_ID_2,
  DEMO_REGISTRO,
...
var TEND_SERIES_CATALOG = [
  ...
  { sectionKey: "QS", fieldKey: "BUN/CR", cardTitle: "Relaci\xF3n BUN/CR" },
...
var LS_GROUP_LEGEND_ORDER = "rpc-tend-group-legend-order";
...
function readLegendOrder(patientId, sectionKey, familyKey) {
  var map = readJson(LS_GROUP_LEGEND_ORDER, {});
  var arr = map[legendOrderKey(patientId, sectionKey, familyKey)];
  return Array.isArray(arr) ? arr.slice() : null;
}
function writeLegendOrder(patientId, sectionKey, familyKey, fieldKeys) {
  var map = readJson(LS_GROUP_LEGEND_ORDER, {});
...
function tendSectionChartSvgIcon() {
  return '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M8 17V9M12 17V5M16 17v-4"/></svg>';
}
function buildPanelToolbar() {
  var toolbar = document.createElement("div");
  toolbar.className = "patient-card-toolbar tend-group-panel-toolbar";
  toolbar.innerHTML = '<div class="patient-card-toolbar-left"><button type="button" class="patient-toolbar-chip patient-toolbar-chip--icon tend-group-panel-copy-svg" title="Copiar gr\xE1fica" aria-label="Copiar gr\xE1fica como imagen">' + tendSectionChartSvgIcon() + '</button><button type="button" class="patient-toolbar-chip patient-toolbar-chip--icon tend-group-panel-eye" title="Ocultar panel" aria-label="Ocultar panel">' + tendPanelEyeSvg() + '</button></div><span class="tend-group-panel-drag-hint" aria-hidden="true" title="Arrastrar para reordenar">\u22EE\u22EE</span>';
  return toolbar;
}
  ...
  try {
    ...
    toolbar.querySelector(".tend-group-panel-copy-svg").onclick = function(ev) {
      ...
      copyChartPng(chart, title, visibleDatasetsForChart(chart), function(ok) {
        if (ctx.deps.showToast) ctx.deps.showToast(ok ? "Gr\xE1fica copiada \u2713" : "No se pudo copiar la gr\xE1fica", ok ? "success" : "error");
      });
    };
  } catch (chartErr) {
...
var TARGETS = {
  ...
  lab_parse: {
    appTab: "lab",
    selector: "#btn-procesar, #lab-input",
    focus: false,
    openLabPasteModal: true
  },
...
function ensureConnectionExpandedForTour(_stepId) {
  ...
  void import("/mobile/js/chunks/panel-conexion-tour-7JYDVXEQ.js").then(function(m) {
...
function applyGuardiaTourLayoutForStep(stepId) {
  void import("/mobile/js/chunks/tour-guards-BTBYLE5G.js").then((guards) => {
    ...
    if (guards.shouldShowGuardiaBoardWithoutEntrega(stepId)) {
      void Promise.all([
        import("/mobile/js/chunks/clinical-entrega-5DW537SJ.js"),
        import("/mobile/js/chunks/entrega-roster-panel-SKG62QBK.js")
      ]).then(([entrega, roster]) => {
    ...
    if (guards.shouldOpenEntregaRosterForTour(stepId)) {
      void import("/mobile/js/chunks/clinical-entrega-5DW537SJ.js").then((entrega) => {
        ...
        void import("/mobile/js/chunks/entrega-roster-panel-SKG62QBK.js").then((roster) => {
...
function seedIcBoardDemoForTour() {
  void import("/mobile/js/chunks/interconsulta-demo-toggle-GOBJNR22.js").then((m) => {
    if (!m.isInterconsultaDemoActive()) m.seedInterconsultaDemoOnMainApp(/* @__PURE__ */ new Date());
  });
}
function seedIcConsultBandDemoForTour() {
  Promise.all([import("/mobile/js/chunks/patients-bridge-LBA365TC.js"), import("/mobile/js/chunks/interconsulta-mode-chrome-YY536KJV.js")]).then(
    ([pb, ic]) => {
...
function applyTourOverlayChromeForStep(id, t) {
  ...
  if (t.showIcBoard) {
    void import("/mobile/js/chunks/interconsulta-mode-chrome-YY536KJV.js").then((m) => {
...
function applyTourTargetForStep(id) {
  ...
  if (id === "gv7_trust_strip") {
    void import("/mobile/js/chunks/guardia-trust-strip-CSCJHMBF.js").then((m) => {
...
function getMapTabsCopy() {
  ...
  if (getUiDensity() !== "normal") {
    return '<p style="margin:0;line-height:1.5;">En <strong>Guardia</strong> el centro es un <strong>resumen</strong> del paciente. Pulsa un bloque o usa <strong>' + mod + "+1\u20264</strong> para abrir el detalle en vista <strong>Normal</strong> (repite el n\xFAmero para ciclar subvistas).</p>";
  }
...
function renderIcBoardMap(bodyEl, nextBtn) {
  bodyEl.innerHTML = '<p style="margin:0;line-height:1.5;">En Interconsultas no hay barra lateral: este <strong>tablero de equipos</strong> es tu punto de partida. Un carril por equipo \u2014 <strong>guardia</strong>, <strong>postguardia</strong> y el resto como <strong>activo</strong> \u2014 m\xE1s un carril <strong>Sin equipo</strong> si hay pacientes sin asignar. Solo el carril de guardia tiene <strong>Preop/Nuevas hoy</strong>; todos tienen <strong>Pendientes</strong> y <strong>Under</strong> por paciente.</p>';
  showNext(nextBtn);
}
function renderIcBoardDrilldown(bodyEl, nextBtn) {
  bodyEl.innerHTML = '<p style="margin:0;line-height:1.5;">Toca una tarjeta para abrir el <strong>Resumen</strong> de ese paciente a pantalla completa. <strong>\u2190 Tablero</strong> (o <strong>Esc</strong>) regresa al tablero. Los pacientes llegan al tablero desde un laboratorio procesado, como en los siguientes pasos.</p>';
  showNext(nextBtn);
}
function renderIcBoardActions(bodyEl, nextBtn) {
  bodyEl.innerHTML = '<p style="margin:0;line-height:1.5;"><strong>+ Agregar</strong> da de alta un paciente nuevo sin esperar un laboratorio. <strong>Actualizar pacientes</strong> refresca el tablero. Tambi\xE9n puedes <strong>arrastrar una tarjeta</strong> de un carril a otro para reasignar el equipo de ese paciente.</p>';
  showNext(nextBtn);
}
function renderIcConsultBand(bodyEl, nextBtn) {
  bodyEl.innerHTML = '<p style="margin:0;line-height:1.5;">Al abrir un paciente, esta tarjeta trae <strong>Servicio solicitante</strong>, <strong>Motivo</strong>, <strong>Seguimiento</strong> y el <strong>equipo asignado</strong> \u2014 todo editable aqu\xED, junto al nombre.</p>';
  showNext(nextBtn);
}
...
function renderWrap(bodyEl, nextBtn) {
  bodyEl.innerHTML = '<p style="margin:0;line-height:1.5;">Listo. Repite el tutorial desde <strong>Mi Perfil</strong> o <strong>Ajustes</strong>. Para el equipo en vivo usa <strong>R+ Cloud</strong> en \u21C4 y, si hace falta, el enlace m\xF3vil.</p>';
  finishNext(nextBtn);
}
...
function maybeMarkFundamentosChapterComplete(stepId) {
  ...
  void import("/mobile/js/chunks/fundamentos-progress-FZY2NJKY.js").then((m) => {
...
function maybeMarkGuardiaV7ChapterComplete(stepId) {
  ...
  if (stepsInChapter.stepInChapter !== stepsInChapter.chapterSteps) return;
  void import("/mobile/js/chunks/guardia-v7-progress-HEZKRSIW.js").then((m) => {
    ...
    if (m.isGuardiaV7TrackComplete()) {
      void import("/mobile/js/chunks/guardia-board-chrome-PJYGEIOP.js").then((chrome) => {
...
async function promptMiRotacionAfterSalaTourIfNeeded(branch) {
  ...
  const { needsTeamOnboarding } = await import("/mobile/js/chunks/clinical-onboarding-P2ZGDIVY.js");
  ...
  const { ensureClinicalPanelSession } = await import("/mobile/js/chunks/clinical-panel-host-HRHE24IW.js");
  ...
  try {
    const { wireClinicalTeamsModalChrome } = await import("/mobile/js/chunks/teams-roster-modal-chrome-ASODRUQG.js");
    ...
    const { openMiRotacion } = await import("/mobile/js/chunks/clinical-rotation-entry-4QY3QSUG.js");
...
//# sourceMappingURL=/js/chunks/chunk-MKMDQNAK.js.map
