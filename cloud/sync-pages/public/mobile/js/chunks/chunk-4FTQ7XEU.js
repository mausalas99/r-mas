...
function formatLabSectionLabel(label, lineIndex) {
  var t2 = String(label || "").trim().replace(/:$/, "");
  if (/^Coag\.?$/i.test(t2)) return "COAG";
  if (lineIndex === 0) return t2;
  return t2;
}
function isLabSectionLabel(label, lineIndex) {
  var t2 = String(label || "").trim().replace(/:$/, "");
  if (/^Coag\.?$/i.test(t2) || /^COAG$/i.test(t2)) return true;
...
function extraerConRangoCoag(nombres, texto, opts) {
  ...
  for (var i = 0; i < nombres.length; i++) {
    ...
    while (true) {
      var idx = t2.indexOf(nombre, start);
      if (idx === -1) break;
      var parsed = tryParseCoagAt_(texto, t2, nombre, idx, maxInr);
...
function collectLcrBlocks_(textoBruto) {
  var t2 = String(textoBruto || "");
  var blocks = [];
  var mChem = t2.match(/CITOQUIMICO\s+DE\s+LCR[\s\S]*?(?=BACTERIOLOGIA|CUADERNILLO|$)/i);
  if (mChem) blocks.push(mChem[0]);
  var mMicro = t2.match(/CITOQUIMICO\s+LIQ\.?\s+LCR[\s\S]*?(?=CUADERNILLO|$)/i);
  if (mMicro) {
    var micro = mMicro[0];
...
function mergeCoagResLabRows_(rows) {
  ...
  (rows || []).forEach(function(row) {
    ...
    body.split(/\s{2,}/).forEach(function(tok) {
      var t2 = tok.trim();
      if (!t2) return;
      var key = t2.split(/\s+/)[0];
      var score = lineRichnessScore_(t2);
      var prev = coagByKey[key];
      if (!prev || score > prev.score) coagByKey[key] = { tok: t2, score };
    });
  });
...
function findMycoStudyResult_(slice, fromIdx) {
  for (var k = fromIdx + 1; k < Math.min(fromIdx + 22, slice.length); k++) {
    ...
    if (/NEGATIVO|POSITIVO|PENDIENTE|EN CURSO|CRECIMIENTO|NO SE AISL/i.test(tUp) && t2.length < 120) {
      return tUp;
    }
  }
  return "NEGATIVO";
...
function isCultivoChromeBodyLine_(line) {
  var t2 = String(line || "").trim();
  if (!t2) return true;
  if (/^USER\b/i.test(t2)) return true;
  if (/\bLabo\s*-?\d+/i.test(t2) && /\b(DJEG|UANL|Campo|Feme)\b/i.test(t2)) return true;
  return false;
}
...
function resultFromSameOrNextLine_(lineas, i) {
  ...
  for (var j = i + 1; j < Math.min(i + 6, lineas.length); j++) {
    var t2 = String(lineas[j] || "").replace(/\t/g, " ").trim();
    if (isLabelOrNoise_(t2)) {
      if (/^GRUPO\s+SANGU|^COOMBS\s+(DIRECTO|INDIRECTO)/i.test(t2)) break;
      continue;
    }
    return t2;
  }
...
function isCultivoStartLineLocal(first) {
  var t2 = String(first || "").trim();
  if (!t2) return false;
  if (CULTIVO_START_RE.test(t2)) return true;
  if (/^CULTIVO\s+DE\s+MICOBACTERIAS\b/i.test(t2)) return true;
  if (PARSED_CULTIVO_HEADER_RE.test(t2)) return true;
  if (PARSED_CULTIVO_DATED_RE.test(t2)) return true;
  if (/^[•\u2022\u00B7]\s*/.test(t2)) return true;
  return false;
}
function isSomeReportChromeLine(line) {
  var t2 = String(line == null ? "" : line).trim();
  if (!t2) return false;
  if (/^USER\b/i.test(t2)) return true;
  if (/\bLabo\s*-?\d+/i.test(t2) && /\b(DJEG|UANL|Campo|Feme)\b/i.test(t2)) return true;
  if (/Sistema\s+SOME|UNIVERSIDAD\s+AUT[OÓ]NOMA|MOP-HU-\d+|REPORTE\s+DE\s+RESULTADOS/i.test(t2)) {
    return true;
  }
  if (/^(Expediente|Solicitud|Nombre|Sexo|Ubicaci[oó]n|Edad|Medico)\s*:/i.test(t2)) return true;
  return false;
}
...
function stripTrailingSomeReportChrome(text) {
  ...
  t2 = t2.replace(/\s*(?:Expediente|Solicitud)\s*:[\s\S]*$/i, "");
  t2 = t2.replace(/\s*^USER\b[\s\S]*$/im, "");
  return t2.trim();
}
...
function readSerolQualFromFollowLines_(lineas, i) {
  ...
  for (var j = i + 1; j < Math.min(i + 12, lineas.length); j++) {
    var t2 = String(lineas[j] || "").replace(/\*/g, "").trim();
    if (!t2 || t2 === ":") continue;
    if (/^ESTUDIO|RESULTADO|UNIDADES|VALOR DE REFERENCIA$/i.test(t2)) continue;
    if (/^S\/CO$/i.test(t2)) continue;
    if (/^(Positivo|Indeterminado|Negativo)\s*[<>=]/i.test(t2)) continue;
    if (/^(Anticuerpos|Ant[ií]geno)\b/i.test(t2)) break;
    var mNum = t2.match(/^(\d+\.\d+|\d+)$/);
    if (mNum && sco === null) {
      sco = mNum[1];
      continue;
    }
    var mQ = t2.match(/^(NEGATIVO|POSITIVO|INDETERMINADO)$/i);
    if (mQ) {
...
function extraerGlucosaSuero_(textoBruto) {
  ...
  if (!t2) return null;
  var gluData = extraerConRangoSuero(["GLUCOSA"], t2);
...
function looksLikeSomeLabReport(textoBruto) {
  ...
  return /Fecha\s+Registro/i.test(t2) || /HEMATOLOG[IÍ]A|QU[IÍ]MICA|BIOMETR[IÍ]A|GASOMETR[IÍ]A|BANCO\s+DE\s+SANGRE|TROPONINA/i.test(t2);
}
...
function saveLabPanelOverlays(arr) {
  memory = (arr || []).slice();
  try {
    if (typeof localStorage !== "undefined") {
...
function upsertLabPanelOverlay(record) {
...
function isQualFollowNoiseLine_(t2) {
  if (!t2 || t2 === ":") return true;
  if (/^ESTUDIO|RESULTADO|UNIDADES|VALOR DE REFERENCIA$/i.test(t2)) return true;
  if (/^S\/CO$/i.test(t2)) return true;
  if (/^(Positivo|Indeterminado|Negativo)\s*[<>=]/i.test(t2)) return true;
  return false;
}
function qualDupTitleDecision_(t2, j, i, sco, qual) {
  if (!/^(Anticuerpos|Ant[ií]geno|Antigeno)\b/i.test(t2) || j <= i + 1) return "none";
...
function readQualFromFollowLines_(lineas, i) {
  ...
  for (var j = i + 1; j < Math.min(i + 12, lineas.length); j++) {
    var t2 = String(lineas[j] || "").replace(/\*/g, "").trim();
    if (isQualFollowNoiseLine_(t2)) continue;
    var dup = qualDupTitleDecision_(t2, j, i, sco, qual);
...
var I18N_ES = {
  ...
  "settings.theme": "Tema",
  "settings.appearanceFoot": "\u2318P alterna Normal/Pase. Tama\xF1o escala toda la interfaz. Mixto equilibra las animaciones.",
  ...
  "settings.uiDensityHint": "Normal: Laboratorio, Expediente, Manejo y Agenda en pesta\xF1as completas (vista Ronda centrada). Pase: resumen del paciente en una columna; pulsa un t\xEDtulo de secci\xF3n para abrir el detalle en Normal. \u2318P o Ctrl+P alterna.",
...
function toggleGuardiaMode() {
  if (isGuardiaMode()) {
    void import("/mobile/js/chunks/entrega-roster-panel-JWXPDVCE.js").then(({ closeEntregaRosterPanel }) => {
      closeEntregaRosterPanel();
    });
    void import("/mobile/js/chunks/clinical-entrega-LVO6RJ7F.js").then(({ endEntregaPhase }) => {
      endEntregaPhase();
    });
    void import("/mobile/js/chunks/guardia-phase-bar-3XCTZZQB.js").then(({ teardownGuardiaPhaseBar }) => {
...
function setUiDensity(mode) {
  ...
  void import("/mobile/js/chunks/clinical-rotation-entry-4N3VIZZF.js").then((mod) => {
...
//# sourceMappingURL=/js/chunks/chunk-4FTQ7XEU.js.map
