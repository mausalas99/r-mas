...
function extractElectrolytesFromResLabs(rows) {
...
var BASAL_INSULIN_RE = /\b(GLARGINA|LANTUS|TOUJEO|DEGLUDEC|TRESIBA|DETEMIR|LEVEMIR|NPH)\b/i;
...
function isInsulinPrandialMedicationItem(item) {
  ...
  var nombre = String(item.nombreRaw || "");
  if (!INSULIN_RE2.test(nombre)) return false;
...
function planLabHistoryDateTimeUpsert(existingSets, incoming) {
  ...
  if (!siblings.length && areDuplicateLabSets(keeper, {
    ...
    return { action: "skip", keeper, siblings: [], matchKind: "datetime" };
  }
  return { action: "merge", keeper, siblings, matchKind: "datetime" };
}
...
// public/js/labs-calcium-corrected.mjs
function parseLabNum_2(str) {
  if (str === "---" || str == null || str === "") return null;
  var n = parseFloat(String(str).replace(",", "."));
  return isNaN(n) ? null : n;
}
...
function computeCorrectedCalciumValue_(caStr, albStr) {
  var ca = parseLabNum_2(caStr);
  var alb = parseLabNum_2(albStr);
  if (ca == null || alb == null) return null;
  return ca + 0.8 * (4 - alb);
}
function computeCorrectedCalcium_(caStr, albStr) {
...
//# sourceMappingURL=/js/chunks/chunk-ZSD6QMCL.js.map
