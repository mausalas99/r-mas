import {
  canAccessCloudAdmin
} from "/mobile/js/chunks/chunk-TVSHPTWN.js";
import {
  getSharedNubeOutbox,
...
} from "/mobile/js/chunks/chunk-HA6KSINZ.js";
import {
...
// public/js/features/cloud-sync/cloud-sync-diagnostics-human-format.mjs
...
var FIX_GUIDES = {
  ...
  toxic_outbox_chunk: {
    ...
    summary: "Hay un push local que excede el tama\xF1o que el servidor acepta (~2 MB por lote).",
    steps: [
...
function resolveCloudErrorFixId(entry) {
  ...
  return matchRevisionStaleFixId(code, explain) || matchInvalidTokenFixId(code) || matchSyncClientNotReadyFixId(explain) || matchOpFixId(entry) || "generic_sync_error";
}
...
// public/js/features/cloud-sync/cloud-sync-diagnostics-human-issues.mjs
...
function buildRecentErrorRows(d, now) {
...
function buildToxicOutboxRows(d) {
  return (Array.isArray(d.outbox?.entries) ? d.outbox.entries : []).filter(isToxicCloudOutboxEntry).sort(function(a, b) {
...
function buildIssues(d, now, status, transport, wsClose, recentErrors, outboxCount, toxicRows) {
  const issues = [];
...
function buildToxicOutboxSummary(toxicRows) {
...
function statusIdentityHtml(cloudUser) {
  ...
  return '<div class="cloud-sync-inset-group cloud-sync-status-identity" aria-label="Cuenta"><div class="cloud-sync-inset-row cloud-sync-inset-row--static cloud-sync-inset-row--identity"><span class="cloud-sync-options-entry-text"><span class="cloud-sync-status-handle">@' + esc(handle || "\u2014") + "</span>" + (display ? '<span class="cloud-sync-status-display">' + esc(display) + "</span>" : "") + '</span></div><button type="button" class="cloud-sync-inset-row cloud-sync-inset-row--action cloud-sync-inset-row--danger" data-cloud-action="logout">Cerrar sesi\xF3n</button></div>';
}
...
function connectedViewsHtml({
  ...
  const sistemaRows = optionsRow("Diagn\xF3stico Nube", "Dashboard de estado y alertas", "nube") + (pendingDeletes.length ? optionsRow(
    "Eliminaciones pendientes",
    pendingDeletes.length + " paciente(s) con eliminaci\xF3n remota rechazada",
    "pendientes"
  ...
  ) + viewBlock("cuenta", "Cuenta", cuentaBodyHtml(cloudUser)) + (showAdmin ? viewBlock("admin", "Administraci\xF3n", adminHost) : "") + (pendingDeletes.length ? viewBlock("pendientes", "Eliminaciones pendientes", pendingRemoteDeletesHtml(pendingDeletes)) : "") + viewBlock(
    "nube",
    "Diagn\xF3stico Nube",
...
// public/js/features/cloud-sync/cloud-sync-diagnostics-human-sections.mjs
...
function buildFacts(d, now, status, transport, outboxCount, roomLabel) {
  return [
...
function buildVerdict(status, transport, issues, recentErrors) {
...
function buildLiveTileFields(d, transport, wsClose) {
  if (transport === "offline" || d.online === false) {
    return { liveValue: "Sin conexi\xF3n", liveStatus: "error", liveHint: "Sin red" };
  }
  if (transport === "ws") {
...
function buildTiles(d, now, transport, wsClose, outboxCount, status, recentErrors) {
...
function overrideSyncPipelineFields(d, status, recentErrors, state, detail) {
  if (recentErrors.length > 0 || d.lastCycleOk === false) {
    return { state: "error", detail: recentErrors[0]?.explain || "Fall\xF3 el \xFAltimo ciclo de sync" };
  }
  if (status === "error") {
...
function buildPipeline(d, status, roomLabel, recentErrors) {
...
function buildDisplayStatusKey(status, issues, recentErrors) {
  const hasWarn = issues.some(function(item) {
    return item.severity === "warn";
  });
  return recentErrors.length > 0 || status === "error" ? "error" : hasWarn || status === "pending" ? "pending" : status === "syncing" ? "syncing" : status === "offline" ? "offline" : "idle";
}
function buildOutboxBreakdown(d, outboxCount) {
  return Object.entries(d.outbox?.byKind || {}).filter(function(pair) {
    return Number(pair[1]) > 0;
  }).map(function(pair) {
    const kind = pair[0];
...
function renderDashHead(v, verdict, chipClass) {
  let html = '<div class="cloud-sync-inset-row cloud-sync-inset-row--static cloud-nube-dash-head"><div class="cloud-nube-dash-head-main"><span class="cloud-sync-status-chip cloud-nube-dash-chip ' + esc(chipClass) + '">' + esc(verdict.headline) + "</span>";
  if (v.roomLabel) {
...
function renderDashTiles(tiles) {
  ...
  return tiles.map(function(tile) {
    ...
    return '<div class="cloud-sync-inset-row cloud-sync-inset-row--kv cloud-nube-dash-kv" data-status="' + esc(tile.status) + '"><dt>' + esc(tile.label) + "</dt><dd>" + dd + "</dd></div>";
  }).join("");
}
...
function renderDashToxicOutbox(toxicOutbox) {
  ...
  let html = '<div class="cloud-sync-inset-row cloud-sync-inset-row--static cloud-nube-dash-toxic-head">Lotes pesados en cola</div>';
...
function renderDashAlerts(issues, recentErrors) {
  const hasAlerts = Array.isArray(issues) && issues.length > 0 || Array.isArray(recentErrors) && recentErrors.length > 0;
  if (!hasAlerts) return "";
...
function renderDashActions() {
  return '<div class="cloud-nube-dash-actions"><button type="button" class="cloud-sync-btn cloud-sync-btn--ghost" data-cloud-diag-action="repair-team-salas">Reempujar censo a salas de equipo</button><button type="button" class="cloud-sync-btn cloud-sync-btn--ghost" data-cloud-diag-action="retry">Reintentar cola</button><button type="button" class="cloud-sync-btn cloud-sync-btn--ghost" data-cloud-diag-action="sync">Forzar sync</button><button type="button" class="cloud-sync-btn cloud-sync-btn--ghost" data-cloud-diag-action="prune-labs">Descartar labs en cola</button></div></div>';
}
...
function runDiagnosticsRepairTeamSalas(host, deps) {
  void import("/mobile/js/chunks/cloud-census-sala-push-NUJHVZDI.js").then(function(mod) {
    ...
    deps?.toast?.(
      parts.length ? parts.join(" \xB7 ") : "Nada pendiente \u2014 censo ya alineado con equipos.",
      parts.length ? "success" : "info"
    );
    refreshCloudSyncDiagnostics(host, deps);
  }).catch(function() {
    deps?.toast?.("Fall\xF3 el reempuje de censo.", "error");
    refreshCloudSyncDiagnostics(host, deps);
  });
}
...
//# sourceMappingURL=/js/chunks/chunk-KLATWUL3.js.map
