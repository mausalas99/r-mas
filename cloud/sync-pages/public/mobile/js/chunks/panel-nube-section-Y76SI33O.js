import {
  ...
  userHasJoinedTeam,
  wireCloudAuthTabs
} from "/mobile/js/chunks/chunk-C7VMU5EN.js";
import {
  buildCloudMobileJoinUrl
} from "/mobile/js/chunks/chunk-OBGB2GI4.js";
import {
  canAccessCloudAdmin
} from "/mobile/js/chunks/chunk-ILAZFLEF.js";
import {
  createNubeRuntime,
  getSharedNubeOutbox,
...
var CLOUD_INVITE_MOBILE_OPEN_KEY = "rpc-cloud-invite-mobile-open";
function appendCloudMobileInviteCollapsible(root, opts) {
  const details = document.createElement("details");
  details.className = "rpc-disclosure lan-invite-collapsible lan-hub-invite-card " + String(opts.extraClass || "");
  try {
  ...
  sum.className = "rpc-disclosure__summary rpc-disclosure__summary--stacked lan-settings-card-summary";
  sum.innerHTML = '<span class="settings-card__title">' + opts.title + "</span>" + (opts.subtitle ? '<span class="settings-card__desc">' + opts.subtitle + "</span>" : "");
...
function renderCloudNubeDashboardHtml(view) {
  ...
  html += '<div class="cloud-nube-dash-actions"><button type="button" class="cloud-sync-btn cloud-sync-btn--ghost" data-cloud-diag-action="retry">Reintentar cola</button><button type="button" class="cloud-sync-btn cloud-sync-btn--ghost" data-cloud-diag-action="sync">Forzar sync</button></div></div>';
  return html;
}
...
function mountAdminShell(section, deps, toast) {
  ...
  async function ensureAdminOpen() {
    ...
    if (!host) return;
    if (!adminMount) {
      const { mountCloudAdminPanel } = await import("/mobile/js/chunks/panel-admin-EHBZZMIW.js");
      host.textContent = "";
...
//# sourceMappingURL=/js/chunks/panel-nube-section-Y76SI33O.js.map
