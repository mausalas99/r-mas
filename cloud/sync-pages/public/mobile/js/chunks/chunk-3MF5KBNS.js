...
function appendEventualidad(store, text, clientId, atIso, kind, transfusionProduct) {
  const normalizedKind = normalizeEventualidadKind(kind);
...
function updateEventualidad(store, entryId, patch) {
  ...
  const patchKind = patch && patch.kind != null ? normalizeEventualidadKind(patch.kind) : normalizeEventualidadKind(cur.kind);
  const text = patch && patch.text != null ? resolveEventualidadEntryText(patch.text, patchKind || cur.kind) : resolveEventualidadEntryText(cur.text, patchKind || cur.kind);
  if (!text) return base;
...
function initAppState() {
  ...
  if (stripLabs.ran && stripLabs.patientsChanged > 0) {
    saveState({ immediate: true });
    try {
      import("/mobile/js/chunks/mutate-bridge-SSVVHAFB.js").then(function(m) {
...
//# sourceMappingURL=/js/chunks/chunk-3MF5KBNS.js.map
