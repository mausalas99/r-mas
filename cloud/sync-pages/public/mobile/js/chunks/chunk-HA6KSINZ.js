...
async function recordLabPullIngress(result) {
  try {
    const labDiag = await import("/mobile/js/chunks/lab-sync-diagnostics-4XJAKVHX.js");
...
async function promptAndApplyRemotePatientDeletes(pending) {
  ...
  if (ok) {
    ...
    if (removed) {
      ...
      try {
        const { renderPatientList } = await import("/mobile/js/chunks/patients-PW5HBT3C.js");
...
function loadLabSyncDiagMod() {
  if (!_labSyncDiagMod) {
    _labSyncDiagMod = import("/mobile/js/chunks/lab-sync-diagnostics-4XJAKVHX.js");
  }
...
async function applyClinicalOpsSnapshot(clinicalOps) {
  ...
  try {
    ...
    const { applyClinicalScopeFromOpsSnapshot } = await import("/mobile/js/chunks/clinical-access-runtime-OFZ4WTIT.js");
    let applied = false;
    ...
    if (applied) {
      const { hydrateClinicalTeamsAfterCloudPull } = await import("/mobile/js/chunks/clinical-ops-hydrate-ZNJRXQBV.js");
...
async function refreshCloudTodoUIs(patientIds) {
  ...
  try {
    const mod = await import("/mobile/js/chunks/todos-refresh-26MDAZZT.js");
...
async function finalizeCloudPullPatientScope() {
  try {
    const access = await import("/mobile/js/chunks/clinical-access-runtime-OFZ4WTIT.js");
...
//# sourceMappingURL=/js/chunks/chunk-HA6KSINZ.js.map
