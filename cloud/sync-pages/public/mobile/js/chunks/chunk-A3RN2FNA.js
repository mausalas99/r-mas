...
} from "/mobile/js/chunks/chunk-7E4ACOES.js";
import {
  removePatientLocally
} from "/mobile/js/chunks/chunk-HHBM77OL.js";
import {
  showConfirmDialog
...
async function recordLabPullIngress(result) {
  try {
    const labDiag = await import("/mobile/js/chunks/lab-sync-diagnostics-64CSW3CR.js");
...
async function promptAndApplyRemotePatientDeletes(pending) {
  ...
  if (ok) {
    ...
    if (removed) {
      ...
      try {
        const { renderPatientList } = await import("/mobile/js/chunks/patients-GGIREUSR.js");
...
function loadLabSyncDiagMod() {
  if (!_labSyncDiagMod) {
    _labSyncDiagMod = import("/mobile/js/chunks/lab-sync-diagnostics-64CSW3CR.js");
  }
...
async function applyClinicalOpsSnapshot(clinicalOps) {
  ...
  try {
    ...
    const { applyClinicalScopeFromOpsSnapshot } = await import("/mobile/js/chunks/clinical-access-runtime-3IIQQT2K.js");
    let applied = false;
    ...
    if (applied) {
      const { hydrateClinicalTeamsAfterCloudPull } = await import("/mobile/js/chunks/clinical-ops-hydrate-PFA7FZBN.js");
...
async function refreshCloudTodoUIs(patientIds) {
  ...
  try {
    const mod = await import("/mobile/js/chunks/todos-refresh-MCV7PXJE.js");
...
async function finalizeCloudPullPatientScope() {
  try {
    const access = await import("/mobile/js/chunks/clinical-access-runtime-3IIQQT2K.js");
...
//# sourceMappingURL=/js/chunks/chunk-A3RN2FNA.js.map
