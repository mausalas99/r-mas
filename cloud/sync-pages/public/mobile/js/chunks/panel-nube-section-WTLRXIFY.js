...
function mountAdminShell(section, deps, toast) {
  ...
  async function ensureAdminOpen() {
    ...
    if (!host) return;
    if (!adminMount) {
      const { mountCloudAdminPanel } = await import("/mobile/js/chunks/panel-admin-X5O254YQ.js");
      host.textContent = "";
...
function createEnsureTurn(deps, ui) {
  ...
  return async function tryAutoEnsureTurnRoom() {
    ...
    const { ensureTurnRoom } = await import("/mobile/js/chunks/ensure-turn-room-3DQPADBT.js");
...
//# sourceMappingURL=/js/chunks/panel-nube-section-WTLRXIFY.js.map
