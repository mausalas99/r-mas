...
function fechaFromDayOffset(dayOffset, today) {
  const d = new Date(today.getTime() + dayOffset * DAY_MS2);
  return String(d.getDate()).padStart(2, "0") + "/" + String(d.getMonth() + 1).padStart(2, "0") + "/" + d.getFullYear();
}
function buildPitchLabHistoryEntry(spec, today) {
  const resLabs = procesarLabs(spec.report).resLabs;
  return {
    id: spec.id,
    fecha: fechaFromDayOffset(spec.dayOffset || 0, today instanceof Date ? today : /* @__PURE__ */ new Date()),
    hora: "",
    resLabs,
...
function buildPitchLabHistoryEntries(today) {
...
function buildPitchDemoPatient(today) {
  return {
    ...
    consultInfo: {
      requestingService: "Cirug\xEDa general",
      reason: "Descontrol gluc\xE9mico posoperatorio, ajuste de esquema de insulina y valoraci\xF3n de nefropat\xEDa.",
      followUpStatus: "en_curso"
    }
  };
}
function fillPitchDemoClinicalMaps(maps, fecha, hora, today) {
  ...
  try {
    labHistory[PITCH_DEMO_PATIENT_ID] = buildPitchLabHistoryEntries(
      today instanceof Date ? today : /* @__PURE__ */ new Date()
    );
  ...
  medRecetaByPatient[PITCH_DEMO_PATIENT_ID] = {
    ...
    items: [
      ...
      // split-count callout has real data to render against — see med-receta-apoyo.mjs.
      {
        ...
        nombreRaw: "OXIGENO \xB7 MASCARILLA RESERVORIO 10 L/MIN",
        viaRaw: "",
        dosisRaw: "",
    ...
    dietas: [
      {
        id: "pitch-dieta-1",
        descripcionRaw: "Dieta renal, restricci\xF3n de K y P",
        detalleRaw: "V\xEDa oral",
        kcal: 1400,
        proteinG: 60,
        suspendido: false
      }
...
//# sourceMappingURL=/js/chunks/chunk-LQDWCDXH.js.map
