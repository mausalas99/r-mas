...
} from "/mobile/js/chunks/chunk-KY3W2VTY.js";
import {
  createOutbox
} from "/mobile/js/chunks/chunk-GUZBLPYB.js";
import {
  bridgeCloudIdentityToLocal
} from "/mobile/js/chunks/chunk-E2YV5EEU.js";
import {
...
import {
  ensureTurnRoom
} from "/mobile/js/chunks/chunk-X4LAKGL3.js";
import {
  CLINICAL_SALAS,
...
async function refreshOnboardingHost() {
  const { refreshMainClinicalOnboardingIfNeeded } = await import("/mobile/js/chunks/clinical-onboarding-main-MZCMSFEI.js");
...
async function claimUsernameIfNeeded(api, sessionUserId, username, sala, settings, errEl) {
  ...
  const { assertLanRoomForUsernameRegister } = await import("/mobile/js/chunks/clinical-profile-lan-sync-L6HSI2M3.js");
...
async function connectShiftPinIfProvided(shiftPin, sala) {
  ...
  if (isLanSkipShiftPin()) {
    const { tryEasyLanShiftPinConnect } = await import("/mobile/js/chunks/lan-shift-pin-connect-IC6DCXRK.js");
    await tryEasyLanShiftPinConnect({ sala, force: true });
    return;
  }
  if (!shiftPin) return;
  const { connectLanWithShiftPin } = await import("/mobile/js/chunks/lan-shift-pin-connect-IC6DCXRK.js");
...
async function pushProfileToLanAndNotify(sala, needsClaim) {
  ...
  } = await import("/mobile/js/chunks/clinical-profile-lan-sync-L6HSI2M3.js");
...
async function renderCompletedOnboarding(host) {
  const { hideMainClinicalOnboarding } = await import("/mobile/js/chunks/clinical-onboarding-main-MZCMSFEI.js");
  hideMainClinicalOnboarding();
  if (host.closest("#clinical-teams-panel-body")) {
    const { renderClinicalTeamsPanel } = await import("/mobile/js/chunks/clinical-teams-JAF3UJCR.js");
...
async function renderNoSessionOnboarding(host) {
  ...
  const { buildOnboardingSessionBlockHtml } = await import("/mobile/js/chunks/clinical-onboarding-main-MZCMSFEI.js");
...
async function renderOnboardingPanelInto(host) {
  ...
  if (isClinicalLocalOnlyMode(settings)) {
    ...
    const { wireOnboardingSessionRecoveryOnce } = await import("/mobile/js/chunks/clinical-onboarding-main-MZCMSFEI.js");
...
//# sourceMappingURL=/js/chunks/chunk-KE5KLMVD.js.map
