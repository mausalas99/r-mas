...
var LAB_SECTION_LEAD_RE = /^(BH|QS|ESC|PFHs?|GASES|COAG|ORINA|EGO|CUANTORINA|PROT12H|PROT24H|PltCit|LIPASA|CULTIVO|LCR|TROP|GS|SEROL|FROTIS|HECES|PIE|INTERPRETACI[OÓ]N|LIQ|ASCITIS|TIR|ENDO|CARD|FE|FEB|INFL|INM|META|NEF|NIVEL|TM|NUT|GI|TOX|HEPB|VIRAL|MICRO)\b/i;
...
function germenFromSameLine_(line) {
  var rest = line.replace(/\r/g, "").replace(/^[\s*:]+/, "").replace(/^MICROORGANISMO/i, "").replace(/^[\s*:]+/, "").trim();
  if (!rest) return null;
...
var ATB_GLUED_ENTRY_RE_ = new RegExp(
  "^([A-Z\xC1\xC9\xCD\xD3\xDA\xD1][A-Z\xC1\xC9\xCD\xD3\xDA\xD10-9/.\\- ]*?)\\s*([<>]=?\\s?\\d[\\d.\\/]*|\\d[\\d.\\/]*)\\s*(" + ATB_INTERP_CODES_ + ")$",
  "i"
);
function parseSensCrudasAntibiogramaGlued(lineasAb) {
  var sensCrudas = [];
  for (var i = 0; i < (lineasAb || []).length; i++) {
    var entry = String(lineasAb[i] || "").replace(/^ANTIBIOGRAMA/i, "").trim();
    if (!entry) continue;
    ...
    if (!med) continue;
    sensCrudas.push({ med, mic: m[2].replace(/\s+/g, ""), interp: m[3].toUpperCase() });
  }
  return sensCrudas;
}
...
function parseSensCrudasAntibiogramaFieldRows(lineasAb) {
  ...
  while (i < tokens.length) {
    ...
    if (ATB_FIELD_INTERP_RE_.test(interp)) {
      sensCrudas.push({ med: name.toUpperCase(), mic, interp });
    }
  }
  return sensCrudas;
}
...
function trendArrowHtml_(info) {
  ...
  return '<span class="lab-trend-arrow ' + cls + '" aria-label="' + label + '" title="' + label + ' vs. toma previa"></span>';
}
function renderToken(tok, trendInfo) {
  if (!tok) return tok;
  if (tok.endsWith("*")) {
    var inner = escTxt(tok.slice(0, -1));
    return '<strong class="lab-value-altered" title="Fuera de rango de referencia">' + inner + trendArrowHtml_(trendInfo) + '</strong><span class="lab-value-star" aria-hidden="true">*</span>';
  }
  return escTxt(tok);
}
function renderEntry(text, trendLookup) {
  ...
  return text.split("\n").map(function(line, li) {
    ...
    if (tabIdx >= 0) {
      ...
      var rh;
      if (isStructured) {
        rh = restTokens.map(function(tok, ti) {
          if (!tok) return "";
          if (tok === "-") return '<span class="lab-row-value lab-row-value-muted">-</span>';
          var trendInfo = null;
          if (trendLookup && labelByValueIdx[ti] && tok.endsWith("*")) {
            var fieldKey = isBhFamily ? bhFieldKeyFromOutputLabel(labelByValueIdx[ti]) : labelByValueIdx[ti];
            trendInfo = trendLookup(sectionKey, fieldKey);
          }
          return '<span class="lab-row-value">' + renderToken(tok, trendInfo) + "</span>";
        }).filter(Boolean).join(" ");
      } else {
        rh = restTokens.map(function(tok) {
          if (!tok) return tok;
          if (tok === "-") return '<span class="text-gray-500">-</span>';
          return renderToken(tok);
        }).join(" ");
      }
      return '<span class="lab-row-label">' + lh + '</span> <span class="lab-row-values' + (isStructured ? " lab-row-values-chips" : "") + '">' + rh + "</span>";
    }
    var plain = line.split(" ").map(function(tok, ti) {
      if (!tok) return tok;
...
//# sourceMappingURL=/js/chunks/chunk-7XJNQXQX.js.map
