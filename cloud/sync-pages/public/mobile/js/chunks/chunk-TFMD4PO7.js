import {
  getUserSala
} from "/mobile/js/chunks/chunk-AA7ORONM.js";
import {
  isCloudMobileClient
...
function renderConnectionPanelFallback(root, kind) {
  const copy = kind === "local-only" ? {
    title: "Solo este equipo",
    body: "Este modo no usa R+ Cloud ni \u21C4 Conexi\xF3n. Para sincronizar censo y equipos, activa guardia en <strong>Ajustes \u2192 Respaldos, sync y recuperaci\xF3n</strong>."
  } : {
    title: "Conexi\xF3n no disponible",
    body: "Tu rotaci\xF3n actual no usa Nube. Completa el registro en la pantalla principal o elige una rotaci\xF3n con sincronizaci\xF3n."
  };
  root.innerHTML = '<section class="cloud-sync-conexion cloud-sync-conexion--fallback" data-cloud-nube-fallback="1"><h4 class="cloud-sync-conexion-title">' + escapeHtml(copy.title) + '</h4><p class="cloud-sync-hint">' + copy.body + "</p></section>";
}
function nubeSectionInDom() {
  return !!(typeof document !== "undefined" && document.querySelector(".cloud-sync-conexion:not([data-cloud-nube-fallback])"));
}
async function mountCloudConnectionPanel(root, deps) {
  ...
  const { shouldShowNubePanel } = await import("/mobile/js/chunks/nube-sync-policy-4LEV3NBQ.js");
  ...
  const settings = await import("/mobile/js/chunks/settings-JOYOHYNE.js");
  const { createCloudSyncApi } = await import("/mobile/js/chunks/api-client-HR7DOKAQ.js");
  const { getSessionAdminKey } = await import("/mobile/js/chunks/panel-admin-WMCC6TVL.js");
  const { mountNubeSection } = await import("/mobile/js/chunks/panel-nube-section-LEJ6VNOM.js");
  const { setCloudRoomConnected } = await import("/mobile/js/chunks/nube-sync-policy-4LEV3NBQ.js");
  ...
  _cloudNubeMount = mountNubeSection(root, {
    ...
    onCloudRoomChange: function(connected) {
      ...
      try {
        void import("/mobile/js/chunks/clinical-rotation-entry-DPKDAZ3H.js").then((m) => m.syncClinicalRotationEntryChrome?.());
      } catch {
      }
    }
  });
...
function defaultConnectionRuntime() {
  return function runtime() {
    return {
      showToast(msg, kind) {
        void import("/mobile/js/chunks/app-shell-6H37RPZ4.js").then(function(m) {
...
async function renderConnectionPanel(opts) {
  ...
  if (!root) return;
  const runtime = typeof opts?.runtime === "function" ? opts.runtime : defaultConnectionRuntime();
...
function getRuntime() {
  return {
    showToast(msg, kind) {
      try {
        import("/mobile/js/chunks/app-shell-6H37RPZ4.js").then((m) => m.showToast?.(msg, kind));
      } catch {
      }
    },
    closeSettingsDropdown() {
      try {
        import("/mobile/js/chunks/app-shell-6H37RPZ4.js").then((m) => m.closeSettingsDropdown?.());
      } catch {
      }
    }
  };
...
function setConnectionDropdownOpen(open) {
  ...
  void renderConnectionPanel({ force: true, runtime: getRuntime }).then(function() {
    return import("/mobile/js/chunks/panel-conexion-tour-Y3YO23AN.js").then(function(m) {
...
//# sourceMappingURL=/js/chunks/chunk-TFMD4PO7.js.map
