...
} from "/mobile/js/chunks/chunk-GYM4L4N4.js";
import {
  getClientId,
  ...
  needsUsernameClaim
} from "/mobile/js/chunks/chunk-LZJH44EB.js";
import {
...
import {
  ensureTurnRoom
} from "/mobile/js/chunks/chunk-47DFSCNL.js";
import {
  bridgeCloudIdentityToLocal
} from "/mobile/js/chunks/chunk-BYJGS6YL.js";
import {
  isCutoverDone,
  isCutoverPending,
  setCutoverFlag
...
async function refreshOnboardingHost() {
  const { refreshMainClinicalOnboardingIfNeeded } = await import("/mobile/js/chunks/clinical-onboarding-main-PT47BMD3.js");
...
async function scheduleOptionalPush() {
  try {
    const lan = await import("/mobile/js/chunks/lan-sync-B5GZBXU3.js");
...
async function claimUsernameIfNeeded(api, sessionUserId, username, sala, settings, errEl) {
  ...
  const { assertLanRoomForUsernameRegister } = await import("/mobile/js/chunks/clinical-profile-lan-sync-UPES6SV2.js");
...
async function connectShiftPinIfProvided(shiftPin, sala) {
  ...
  if (isLanSkipShiftPin()) {
    const { tryEasyLanShiftPinConnect } = await import("/mobile/js/chunks/lan-shift-pin-connect-P2H6BGVU.js");
    await tryEasyLanShiftPinConnect({ sala, force: true });
    return;
  }
  if (!shiftPin) return;
  const { connectLanWithShiftPin } = await import("/mobile/js/chunks/lan-shift-pin-connect-P2H6BGVU.js");
...
async function pushProfileToLanAndNotify(sala, needsClaim) {
  ...
  } = await import("/mobile/js/chunks/clinical-profile-lan-sync-UPES6SV2.js");
  ...
  if (!localOnly && !lanPush.ok && isLanProfileNeedsConnectCode(lanPush.code)) {
    ...
    const rot = await import("/mobile/js/chunks/clinical-rotation-entry-OY24Q6TU.js");
...
async function renderCompletedOnboarding(host) {
  if (needsTeamOnboardingStep()) {
    const { renderTeamOnboardingInto, wireTeamOnboardingInteractions } = await import("/mobile/js/chunks/clinical-onboarding-team-U6PYOM3Y.js");
  ...
  if (host.closest("#clinical-teams-panel-body")) {
    const { renderClinicalTeamsPanel } = await import("/mobile/js/chunks/clinical-teams-H5X4TZS7.js");
...
async function renderNoSessionOnboarding(host) {
  ...
  const { buildOnboardingSessionBlockHtml } = await import("/mobile/js/chunks/clinical-onboarding-main-PT47BMD3.js");
...
async function renderOnboardingPanelInto(host) {
  ...
  if (isClinicalLocalOnlyMode(settings)) {
    ...
    const { wireOnboardingSessionRecoveryOnce } = await import("/mobile/js/chunks/clinical-onboarding-main-PT47BMD3.js");
...
//# sourceMappingURL=/js/chunks/chunk-F22TO3UT.js.map
