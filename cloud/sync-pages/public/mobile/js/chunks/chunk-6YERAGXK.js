...
async function recordLabPullIngress(result) {
  try {
    const labDiag = await import("/mobile/js/chunks/lab-sync-diagnostics-F3GK4S7O.js");
...
function shouldConfirmRemotePatientDelete(opts) {
...
function buildRemoteDeleteConfirmOpts(pending) {
  ...
  if (n === 1) {
    return {
      ...
      question: "Un admin (u otro equipo) lo elimin\xF3 en Nube. Si confirmas, desaparece de este censo.",
      items,
  ...
  return {
    ...
    question: "Un admin (u otro equipo) los elimin\xF3 en Nube. Si confirmas, desaparecen de este censo.",
    items,
    confirmLabel: "Eliminar aqu\xED",
    cancelLabel: "Conservar aqu\xED"
  };
...
async function promptAndApplyRemotePatientDeletes(pending) {
  ...
  if (ok) {
    ...
    if (removed) {
      ...
      try {
        const { renderPatientList } = await import("/mobile/js/chunks/patients-C7M2CDCW.js");
...
function partitionCloudTombstonesForConfirm(tombstones, opts = {}) {
  ...
  for (const patientId of Object.keys(tombstones || {})) {
    ...
    if (shouldConfirmRemotePatientDelete({
      patientId,
      localActorId,
      tombstoneActorId,
      deletedAt,
...
function loadLabSyncDiagMod() {
  if (!_labSyncDiagMod) {
    _labSyncDiagMod = import("/mobile/js/chunks/lab-sync-diagnostics-F3GK4S7O.js");
  }
...
async function recordLabPullDiagnostics(patientSync, raw, filtered) {
  try {
    ...
    try {
      const rt = await import("/mobile/js/chunks/lab-panel-runtime-state-KI6OCGVF.js");
...
async function applyClinicalOpsSnapshot(clinicalOps) {
  ...
  try {
    ...
    if (applied) {
      const { hydrateClinicalTeamsAfterCloudPull } = await import("/mobile/js/chunks/clinical-ops-hydrate-SJBYI5QS.js");
...
async function refreshCloudTodoUIs(patientIds) {
  ...
  try {
    const mod = await import("/mobile/js/chunks/todos-refresh-4LDSGP4W.js");
...
//# sourceMappingURL=/js/chunks/chunk-6YERAGXK.js.map
