...
} from "/mobile/js/chunks/chunk-TYH5ME2D.js";
import {
  isWebClinicalClient
...
function mergePatientMonitoreoFromImported(target, source) {
  ...
  try {
    if ("monitoreo" in s && s.monitoreo != null && typeof s.monitoreo === "object") {
      ...
      var localHasPayload = t.monitoreo != null && typeof t.monitoreo === "object" && monitoreoHasLanPayload(t.monitoreo);
...
function getEventualidadesLabsText(store) {
  if (!store || typeof store !== "object") return "";
  return normalizeEventualidadText(
    /** @type {{ labsText?: unknown }} */
    store.labsText
  );
}
function setEventualidadesLabsText(store, text) {
  const next = cloneStoreShell_(store);
  next.labsText = normalizeEventualidadText(text);
  return touchEventualidadesMeta_(next);
}
function mergeEventualidadesLabsText2(store, text) {
  const next = cloneStoreShell_(store);
...
function initAppState() {
  ...
  if (stripLabs.ran && stripLabs.patientsChanged > 0) {
    saveState({ immediate: true });
    try {
      import("/mobile/js/chunks/lan-sync-BWTC4J5Y.js").then(function(m) {
...
export {
  ...
  mergeEventualidadesLabsText2 as mergeEventualidadesLabsText,
  appendEventualidad,
  updateEventualidad,
...
//# sourceMappingURL=/js/chunks/chunk-CLJUGM4X.js.map
