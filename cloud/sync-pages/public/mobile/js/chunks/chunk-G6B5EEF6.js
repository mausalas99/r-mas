// public/js/clinical-ops-merge-users.mjs
...
function mergeIncomingUserHandleConflict(byUserId, row, handle, uid) {
  ...
  const mergedByUid = prevByUid ? mergeUserProfileFields(prevByUid, row) : { ...row, username: prevByUid ? prevByUid.username : row.username };
...
function mergeIncomingClinicalUser(byUsername, byUserId, row) {
  ...
  const merged = prev ? { ...mergeUserProfileFields(prev, row), username: handle } : { ...row, username: handle };
  ...
  for (const list of [localRows, incomingRows]) {
    for (const row of list || []) {
      ...
      if (!prev || at >= String(prev.assigned_at || "")) {
        byPatient.set(pid, {
          patient_id: pid,
          guardia_id: row?.guardia_id ? String(row.guardia_id) : void 0,
          assigned_at: at
        });
      }
    }
  }
  return [...byPatient.values()].sort((a, b) => String(a.assigned_at).localeCompare(String(b.assigned_at))).slice(-200);
}
...
// public/js/clinical-ops-sync.mjs
...
function clearClinicalOpsTrace() {
...
function isClinicalOpsSyncAvailable() {
  const api = dbApi();
...
async function prepareClinicalOpsForSync() {
...
async function collectClinicalOpsForSync(opts) {
  const api = dbApi();
...
async function flushPendingClinicalOpsSnapshot() {
...
async function applyClinicalOpsSnapshot(snapshot) {
...
export {
  ...
  collectClinicalOpsForSync,
  clinicalOpsMergeHadChanges,
  flushPendingClinicalOpsSnapshot,
  applyClinicalOpsSnapshot,
  mergeClinicalOpsFromSources
};
//# sourceMappingURL=/js/chunks/chunk-G6B5EEF6.js.map
