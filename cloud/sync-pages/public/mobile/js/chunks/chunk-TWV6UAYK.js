...
} from "/mobile/js/chunks/chunk-ARAHUBAM.js";
import {
  isWebClinicalClient
...
function registroForPatientId(patients2, patientId) {
  const pid = String(patientId || "").trim();
  if (!pid || !Array.isArray(patients2)) return "";
  const row = patients2.find((p) => p && String(p.id || "") === pid);
  return String(row?.registro || "").trim();
}
function resolveCloudTodoLocalPatientId(remotePatientId, registro, patients2, idMap) {
  ...
  return resolveLiveSyncLocalPatientId(remotePid, registro, patients2);
}
function stampCloudTodoRow(patientId, todo, patients2) {
  const pid = String(patientId || todo?.patientId || "").trim();
  const registro = String(todo?.registro || registroForPatientId(patients2, pid)).trim();
...
function initAppState() {
  ...
  if (stripLabs.ran && stripLabs.patientsChanged > 0) {
    saveState({ immediate: true });
    try {
      import("/mobile/js/chunks/mutate-bridge-7CJF23JI.js").then(function(m) {
...
//# sourceMappingURL=/js/chunks/chunk-TWV6UAYK.js.map
