...
async function ensureTurnRoomAfterTeamJoin(toast) {
  const [settings, { createCloudSyncApi }] = await Promise.all([
    import("/mobile/js/chunks/settings-JOYOHYNE.js"),
    import("/mobile/js/chunks/api-client-HR7DOKAQ.js")
  ]);
  const api = createCloudSyncApi({
    getBaseUrl: settings.getCloudSyncUrl,
    getToken: settings.getCloudSyncToken
  });
  const { getUserSala } = await import("/mobile/js/chunks/panel-clinical-context-CIEQUOIK.js");
...
//# sourceMappingURL=/js/chunks/chunk-3FIUPWII.js.map
