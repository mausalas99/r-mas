...
describe('handleApiRoute — /rooms app-version gate', () => {
  ...
  it('rejects a request with no X-App-Version header (pre-1.0.0 client) before touching the DB', async () => {
  ...
  it('rejects an old X-App-Version value the same way', async () => {
    const res = await handleApiRoute(roomsRequest({ 'X-App-Version': '0.9.9' }), gateOn);
    assert.equal(res.status, 426);
  });

  it('lets a current version pass the gate through to the room handler', async () => {
    const res = await handleApiRoute(roomsRequest({ 'X-App-Version': '1.0.0' }), gateOn);
  ...
  it('does not block a pre-1.0.0 client when the gate is off (default)', async () => {
