...
export async function handleSyncWorkerRequest(request, env) {
  ...
  if (path === `${API_PREFIX}/ping` && request.method === 'GET') {
    const ping = await withEdgeCache(
      request,
      async () => Response.json({ ok: true, service: 'rplus-sync' }),
      60
    );
    return applyCors(request, ping);
  }

  if (path === `${API_PREFIX}/meta` && request.method === 'GET') {
    const meta = await withEdgeCache(request, async () => handleMeta(env), 300);
...
export default {
  ...
  async scheduled(_event, env, ctx) {
    ctx.waitUntil(
      runScheduledPurge(env).catch((err) => {
        console.error('[rplus-sync] scheduled purge failed:', err?.message || err);
      })
    );
  },
};
