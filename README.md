<p align="center">
  <img src="packages/core/public/icons/icon-192.png" width="112" alt="R+">
</p>

<h1 align="center">R+</h1>

<p align="center">
  Herramienta clínica de escritorio para generación de <b>notas de evolución</b>, <b>indicaciones médicas</b> y visualización de <b>laboratorios</b> con diagramas automáticos.
</p>

<p align="center">
  <a href="https://github.com/mausalas99/r-mas/releases/latest"><img src="https://img.shields.io/github/v/release/mausalas99/r-mas?label=versi%C3%B3n&color=0f766e" alt="Versión"></a>
  <img src="https://img.shields.io/badge/macOS%20%C2%B7%20Windows-555" alt="macOS · Windows">
  <a href="https://github.com/mausalas99/r-mas/releases/latest"><img src="https://img.shields.io/badge/descargar-instalador-0f766e" alt="Descargar"></a>
</p>
...
- **Importante:** tras actualizar, pega una vez la dirección del portal de laboratorio en Ajustes → Laboratorio. La app ya no la trae incluida.
- **Nuevo (labs):** si falta la dirección, R+ avisa y abre Ajustes en ese campo.
- **Arreglado (labs):** algunos reportes PDF del portal no se leían. Ya se leen bien.
- **Privacidad:** se quitaron datos institucionales del código y la documentación.

Notas: `docs/RELEASE_NOTES_8.4.2.txt`.

## R+ 8.4.1 (estable — release 8.4.1)

- **Arreglado (labs):** la fusión automática de labs del mismo día ya no mezcla dos tomas cuando comparten un analito con valores distintos.
- **Arreglado (labs):** un set de labs fusionado con varios reportes ya no pierde su hora guardada.
- **Arreglado:** el panel de electrolitos (ESC) ya no desaparece por completo cuando falta el sodio pero hay otros valores.
- **Nuevo:** telemetría anónima de actualización activada por defecto (solo versión, resultado y plataforma). Se puede desactivar en Ajustes.

Notas: `docs/RELEASE_NOTES_8.4.1.txt`.

## Historial de versiones

Notas de cada versión: [GitHub Releases](https://github.com/mausalas99/r-mas/releases) y `docs/RELEASE_NOTES_*.txt`.
...
**Stack:** Electron 41 · electron-builder 26 · electron-updater 6 · Node doc generators (`lib/doc-generators/`) · Cloudflare Workers (Nube)
...
| Layer | File | Role |
|-------|------|------|
| Electron main | `main.js` | Window (`app://rplus`), auto-updater, IPC |
...
| Node shared logic | `lib/` | SQLCipher store (`lib/db/`), doc generators, entrega |
| Cloud sync | `cloud/sync-worker/` | Nube rooms, Interno MIP, R+ Móvil `/mobile/` |
...
### Telemetría anónima de actualización

- **Activada por defecto** desde 8.4.1. Se desactiva en **Ajustes → Aplicación y actualizaciones → Enviar telemetría anónima de actualización**.
...
- Los errores de red son silenciosos. Los datos llegan al Worker `rmas-update-feed` (`/telemetry`), que solo guarda esos tres campos, sin IP ni identificadores.
