...
| 2026-08-14 | Security | **Intended Nube crypto = client encrypt → CF opaque → client decrypt** (envelope DEKs). Not implemented. V1 deferred E2EE (Worker LWW). Worker AES then dropped (Free CPU). Production = plaintext JSON in D1 `rplus-sync` (WNAM, `rmas-workersdev`). | `docs/core/15-security.md`, north star, Worker README |
...
| 2026-08-02 | Security | ~~Pilot PHI at rest = AES-GCM with Worker secret~~ **Superseded 2026-08-14** — D1 is plaintext JSON; passwords still PBKDF2-SHA-256 50k; not E2EE | See 2026-08-14 row; envelope DEKs still NEXT |
