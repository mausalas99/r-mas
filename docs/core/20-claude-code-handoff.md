...
| Boot speed and debt (teal shell) | `docs/superpowers/plans/2026-08-22-boot-speed-debt.md` — **not started.** CEO-reviewed. Debt score jumped 0 → 384 after teal shell redesign; 200 pts real (8 eager boot imports), 184 pts partly a `score.mjs` scanner bug over-charging test files ~3x. Phase 1 = make 5 of 8 boot imports non-blocking (real speed, no visible change). Phase 2 = fix scanner bug + split long files (score cleanup). Do Phase 1 first. |
