...
| Teal workbench UI redesign | `docs/superpowers/plans/2026-08-17-teal-workbench-ui-redesign.md` — **phase 1 shipped 2026-08-17.** New design handoff replaces Hybrid H / Warm instrument as source of truth (teal accent, not ink). Tokens + fonts + animations + docs done, build/tests verified (see plan's "Phase 1 result"). Pilot screen corrected mid-flight: real target is `patient-dashboard/`, not `expediente/`; it was already 100% token-driven so it recolored for free — new layout features (counters band, alert-tint rows, empty states) are NOT built yet. 11 screens + those layout features queued as follow-up plans — see roadmap in the plan file. |
