...
| Boot debt Phase 3 Stage B (profile-chain decoupling) | `docs/superpowers/plans/2026-08-23-boot-debt-phase3-stage-b.md` — **Stage B.0 gate passed, Stage B.1 shipped 2026-08-23 (commit `10a24fa7`). Stage B.2 not started.** B.0 confirmed 102.6 KB would actually leave the eager bundle. B.1: the 4 profile/medications files (+ a 5th call site found mid-implementation) now resolve `switchAppTab`/`switchInnerTab`/`renderInnerTabs`/`getActiveInnerTab`/`refreshExpedienteForAppModeChange`/`renderNotaEvolucionPrimaryTab`/`invalidateInnerTabRenderCache` via the existing `globalThis` window-handler pattern (new shared `resolveGlobalFn` helper) instead of static imports — 18 new tests, all passing. **B.1 alone does not shrink the bundle** — confirmed by `app-boot-imports.test.mjs` still failing budget (3,473,191 B). Stage B.2 (converting `app.js`/`app-runtimes.mjs`'s own static imports of the 5 boot files to non-blocking, re-applying Phase 1) is what actually realizes the 102.6 KB, and needs the packaged-app cold-launch verification the plan's own gate requires — not done this session, boot-critical code. Also scopes Step 9 (tab-level code split) as a distinct future follow-on, not started. |
