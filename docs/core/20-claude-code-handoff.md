...
| Boot debt Phase 3 Stage B (profile-chain decoupling) | `docs/superpowers/plans/2026-08-23-boot-debt-phase3-stage-b.md` — **CEO-planned 2026-08-23, not started.** Root cause: 4 profile/medications files import `switchAppTab`/`switchInnerTab`/4 other functions from the 5 boot files at module top level, though every call site is handler-only. Plan: re-gate with a fresh metafile check that also cuts these 4 files' edges (Stage B.0, < ~75 KB stop); if it passes, switch the 4 files to the existing `globalThis` window-handler pattern already used by `paste-smart.mjs`/`command-palette.mjs` (Stage B.1), then re-apply the reverted Phase 1 non-blocking-import split (Stage B.2). Two separate commits, each independently revertable. Sits near patient-settings/medication-action code — packaged-launch smoke test required before shipping. Also scopes Step 9 (tab-level code split) as a distinct future follow-on, not started. |
