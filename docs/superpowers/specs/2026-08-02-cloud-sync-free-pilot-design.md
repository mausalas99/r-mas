...
- True E2EE (client encrypt → Cloudflare opaque → client decrypt) — **still the product target**, deferred in V1 because it blocks Worker-side LWW; V1 used Worker AES instead. **Neither shipped:** production D1 is plaintext JSON. See [`docs/core/15-security.md`](../../core/15-security.md).
...
> **Status (2026-08-14):** **Product target** = encrypt on the client, store opaque ciphertext on Cloudflare, decrypt on the peer. V1 deferred that (Worker LWW) and used Worker AES-GCM instead; Worker AES was then **dropped** (Free CPU). Production `room_state` is UTF-8 JSON. See [`docs/core/15-security.md`](../../core/15-security.md).

| Layer | Product target | V1 spec | Production (2026-08) |
|-------|----------------|---------|----------------------|
| In transit | HTTPS carrying ciphertext | HTTPS | HTTPS carrying **plaintext JSON** |
| Cloudflare / D1 | Opaque blobs; no DEK | AES-256-GCM with `WORKER_DATA_KEY` (Worker can decrypt) | **Plaintext JSON** |
| Client | Encrypt before push; decrypt after pull; LWW locally | SQLCipher cache only | SQLCipher cache; **no** Nube payload crypto |
| E2EE | Yes (envelope DEKs) | Explicit non-goal | No |
...
- `WORKER_DATA_KEY` rotation is moot for new writes (plaintext JSON). Re-introduce AES only with Paid CPU or client-side envelope DEKs.
