...
async function exportIcHojaDocx({ patient, payload }) {
  if (!patient || !payload) throw badRequest('Missing patient or payload');
  const buf = await generateIcHojaBuffer({ payload });
  const fileName = `Hoja_IC_${safeName(patient.nombre)}_${safeName(payload.fechaDisplay || '')}.docx`;
  return { buffer: buf, fileName };
}
