...
function applyClinicalUserRowUpdate(db, userId, row, handle) {
  ...
  if (row.last_activity_at) {
    touchClinicalUserActivity(db, userId, String(row.last_activity_at), 'sync');
  }
}
...
function mergeClinicalUserByHandle(db, row, handle, stats) {
  ...
  if (row.last_activity_at) {
    touchClinicalUserActivity(db, targetId, String(row.last_activity_at), 'sync');
  }
  stats.updated += 1;
  return true;
}
 ...
 */
export function ensureStubUsersReferenced(db, incoming, deletedSet = null) {
  const tombstones = deletedSet || new Set(getDeletedUserIds(db));
