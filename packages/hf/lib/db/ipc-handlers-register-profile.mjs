...
import { verifyAdminAccessCode, setAdminAccessCode } from '../admin-access-code.mjs';
...
function registerDbProfileCrudHandlers(ctx) {
  ...
  bindIpcHandler(ipcMain, 'db:admin-access-code-verify', async (payload) => {
    const valid = await dbManager.withTransaction((db) =>
      verifyAdminAccessCode(db, payload?.code)
    );
    return { ok: true, valid };
  });

  bindIpcHandler(ipcMain, 'db:admin-access-code-set', async (payload) => {
    try {
      await dbManager.withTransaction((db) =>
        setAdminAccessCode(db, payload?.currentCode, payload?.newCode)
      );
      return { ok: true };
    } catch (err) {
      return { ok: false, error: err?.message || 'No se pudo cambiar el código.' };
    }
  });

  bindIpcHandler(ipcMain, 'db:clinical-user-touch', async (payload) => {
    const userId = String(payload.userId || '').trim();
    const callerUserId = String(payload.callerUserId || '').trim();
    if (!userId) return { ok: false, error: 'userId requerido' };
    // Only the same user may stamp activity — blocks admin from faking roster "Última".
    if (callerUserId && callerUserId !== userId) {
      return { ok: false, error: 'Solo puedes registrar tu propia actividad.' };
    }
    const source = String(payload.source || 'session').trim() || 'session';
    const touched = await dbManager.withTransaction((db) =>
      touchClinicalUserActivity(db, userId, undefined, source)
    );
