...
import { migrateToV22UserActivityLog } from './schema-migrate-v22-user-activity-log.mjs';
import { migrateToV23ClinicalChangeLog } from './schema-migrate-v23-clinical-change-log.mjs';
import { migrateToV24RankTeam } from './schema-migrate-v24-rank-team.mjs';
import { migrateToV25SingleSala } from './schema-migrate-v25-single-sala.mjs';
import { migrateToV26CardioImages } from './schema-migrate-v26-cardio-images.mjs';
import { migrateToV27ActiveGuardiasIndex } from './schema-migrate-v27-active-guardias-index.mjs';
...
function runMigrationsV11ThroughV19(db) {
  ...
  if (readSchemaVersion(db) < 22) {
    db.transaction(() => migrateToV22UserActivityLog(db))();
  }
  if (readSchemaVersion(db) < 23) {
    db.transaction(() => migrateToV23ClinicalChangeLog(db))();
  }
  if (readSchemaVersion(db) < 24) {
    db.pragma('foreign_keys = OFF');
    try {
      migrateToV24RankTeam(db);
    } finally {
      db.pragma('foreign_keys = ON');
    }
  }
  if (readSchemaVersion(db) < 25) {
    db.pragma('foreign_keys = OFF');
    try {
      migrateToV25SingleSala(db);
    } finally {
      db.pragma('foreign_keys = ON');
    }
  }
  if (readSchemaVersion(db) < 26) {
    db.transaction(() => migrateToV26CardioImages(db))();
  }
  if (readSchemaVersion(db) < 27) {
    db.transaction(() => migrateToV27ActiveGuardiasIndex(db))();
  }
}
