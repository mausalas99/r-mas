...
describe('provisionClinicalUserFromCloudIdentity', () => {
  it('creates clinical user for valid cloud username', async () => {
    ...
    assert.equal(profile.rank, 'Team');
    assert.equal(profile.last_activity_at == null || profile.last_activity_at === '', true);
  });
...
describe('setClinicalUserProfileFromAdmin', () => {
  it('updates rank on existing user without stamping last_activity_at', async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rplus-provision-'));
    dbManager = await createUnlockedDbManager(tmpDir);
    await dbManager.withTransaction((db) =>
      provisionClinicalUserFromCloudIdentity(db, { username: 'r2user', displayName: 'Test', rank: 'Team' })
    );
    const profile = await dbManager.withTransaction((db) =>
      setClinicalUserProfileFromAdmin(db, { username: 'r2user', rank: 'Team' })
    );
    assert.equal(profile.rank, 'Team');
    assert.equal(profile.last_activity_at == null || profile.last_activity_at === '', true);
  });

  it('admin profile and upsert do not advance an existing last_activity_at', async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rplus-provision-'));
    dbManager = await createUnlockedDbManager(tmpDir);
    const stamped = await dbManager.withTransaction((db) => {
      const p = provisionClinicalUserFromCloudIdentity(db, {
        username: 'cindypsc',
        displayName: 'Cindy',
        rank: 'Team',
      });
      touchClinicalUserActivity(db, p.user_id, '2026-06-01T12:00:00.000Z');
      ...
      upsertClinicalProfile(db, {
        userId: p.user_id,
        clinicalName: 'Cindy Admin Edit',
        rank: 'Team',
        sala: 'Sala E',
      });
      return db.prepare('SELECT last_activity_at FROM users WHERE user_id = ?').get(p.user_id);
    });
    assert.equal(stamped.last_activity_at, '2026-06-01T12:00:00.000Z');
  });
});
...
describe('resolveBootstrapClinicalUser', () => {
  it('keeps preferredUserId when preferredUsername differs (no second peer row)', async () => {
    ...
    const bound = await dbManager.withTransaction((db) => {
      const created = ensureClinicalUser(db, { clientId: 'device-a', rank: 'Team' });
