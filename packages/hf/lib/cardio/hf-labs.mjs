...
/**
 * % change in NT-proBNP between two lab snapshots (e.g. `previo`/`actual`
 * from `latestTwoSnapshots`). Mirrors the spreadsheet's "% cambio" column
 * across NT-proBNP tomas — `labSnapshots` already stores one toma per dated
 * entry, so no new field is needed, only this derived comparison.
 * @param {{ values?: { ntProBnp?: unknown } }|null} previo
 * @param {{ values?: { ntProBnp?: unknown } }|null} actual
 * @returns {number|null}
 */
export function ntProBnpPercentChange(previo, actual) {
  var p = Number(previo && previo.values && previo.values.ntProBnp);
  var a = Number(actual && actual.values && actual.values.ntProBnp);
