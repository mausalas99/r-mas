...
test('fenotipoFromFevi classifies by range (2026 ESC: HFrEF <50, HFpEF >=50), never returns HFmrEF/HFimpEF', () => {
  assert.equal(enums.fenotipoFromFevi(20), 'HFrEF');
  assert.equal(enums.fenotipoFromFevi(40), 'HFrEF');
  assert.equal(enums.fenotipoFromFevi(45), 'HFrEF');
  assert.equal(enums.fenotipoFromFevi(49), 'HFrEF');
...
test('FENOTIPOS values are the four selectable codes; only HFmrEF carries an extra label', () => {
  assert.deepEqual(
    enums.FENOTIPOS.map((o) => o.value),
    ['HFrEF', 'HFmrEF', 'HFpEF', 'HFimpEF'],
  );
  for (const opt of enums.FENOTIPOS) {
    if (opt.value === 'HFmrEF') continue;
