...
import { readGroupTableHidden, writeGroupTableHidden, seriesColorKey } from './tend-prefs.mjs';
...
/** Identidad de fila estable entre secciones (evita choques de fieldKey homónimos, p. ej. "Fib" en BH y QS). */
...
function buildTableExportModel(deps, state, rawModel, hidden) {
  ...
  var rows = rawModel.rows.map(function (row) {
    ...
    var cells = rawModel.columns.map(function (set, ci) {
      ...
      var ab = isAbnormal(deps, set, row, val, state.historyDesc);
...
export function renderGroupTable(deps, state, sectionKey, renderTable, opts) {
  ...
  var raw = buildSectionTableModel(state.historyAsc, sectionKey, allSpecs, function (set, sp) {
    return getSetTrendValueForSeries(set, sp.sectionKey || sectionKey, sp.fieldKey);
  });
  state.tableModel = buildTableExportModel(deps, state, raw, hidden);
...
export function createTableExportModel(deps, state, sectionKey, rawModel, hidden) {
  return buildTableExportModel(deps, state, rawModel, hidden);
}
