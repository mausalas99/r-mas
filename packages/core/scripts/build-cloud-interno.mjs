...
/** Cloud-only page flags, shipped as a file: the page CSP has no 'unsafe-inline'. */
const CLOUD_FLAGS_FILE = 'interno-flags.js';
const CLOUD_FLAGS_JS =
  'globalThis.__RPC_CLOUD_INTERNO__=true;try{document.documentElement.dataset.cloudInterno="1";document.documentElement.classList.add("rpc-cloud-interno");}catch(_e){}\n';
...
export function buildInternoIndexHtml(html) {
  ...
  if (!out.includes(CLOUD_FLAGS_FILE)) {
    out = out.replace(/<head>\s*\n/i, `<head>\n<script src="/interno/${CLOUD_FLAGS_FILE}"></script>\n`);
  }
  if (/<script>/i.test(out)) {
    throw new Error('build-cloud-interno: inline <script> found; CSP blocks it');
  }
...
async function main() {
  ...
  writeFile(path.join(DEST, CLOUD_FLAGS_FILE), CLOUD_FLAGS_JS);
