...
function buildMacPublishCmd() {
  return 'npm run prebuild:mac && node scripts/sign-release-bundle.mjs && electron-builder --mac';
}

function buildWinPublishCmd() {
  return 'npm run prebuild:win && node scripts/sign-release-bundle.mjs && electron-builder --win';
}
...
function distFiles(version, { macOnly, winOnly }) {
  ...
  // Module feed assets (docs/superpowers/plans/2026-09-21-live-module-updates.md):
  // platform-independent, uploaded once regardless of macOnly/winOnly. Written
  // to dist/ by scripts/sign-release-bundle.mjs, which build-mac/build-win both
  // run before this list is read. Filenames duplicated from that ESM module's
  // exports — this file is CJS and can't require() it.
  names.push('module-manifest.json', 'module-signature.txt', 'module-bundle.zip');
...
async function cmdPublish(argv) {
  ...
  try {
    ...
    const sharedNotesMd = !skipGh ? writeGithubReleaseNotesFile(version) : null;
    const sharedAssets = !skipGh
...
Publica en GitHub (${REPO}). --no-gh omite GitHub (p. ej. si publicas ahí a mano).
