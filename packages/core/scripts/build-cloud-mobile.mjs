...
export function buildCloudFlagsJs(buildId, appVersion) {
  const v = String(buildId || Date.now());
  return (
    'globalThis.__RPC_CLOUD_MOBILE__=true;globalThis.__RPC_MOBILE_WEB__=true;' +
    ...
    '<button type="button" class="btn-save" data-rpc-reload>Recargar</button></div></div>\';' +
    'var r=g.querySelector("[data-rpc-reload]");r&&r.addEventListener("click",function(){location.reload()});},18000);})();'
  );
}
...
export function rewriteMobileBootScriptPaths(text, buildId) {
  ...
  if (!out.includes('mod.onerror')) {
    out = out.replace(
      ...
      'mod.onerror=function(){try{var g=document.getElementById("rpc-cloud-mobile-gate");if(g){g.innerHTML=\'<div class="rpc-cloud-mobile-modal material-solid-elevated ui-overlay-dialog" role="alert"><div class="rpc-cloud-mobile-modal__head"><h4 class="rpc-cloud-mobile-modal__title">R+ Móvil</h4><p class="rpc-cloud-mobile-modal__sub">Error al cargar la app.</p></div><div class="rpc-cloud-mobile-modal__body"><button type="button" class="btn-save" data-rpc-reload>Recargar</button></div></div>\';var r=g.querySelector("[data-rpc-reload]");r&&r.addEventListener("click",function(){location.reload()});}}catch(_e){}};document.head.appendChild(mod);'
    );
  }
  return out;
}
...
/** Cloud-mobile boot flags + pairing, shipped as js/cloud-mobile-flags.js (CSP: no inline script). */
export const MOBILE_FLAGS_FILE = 'cloud-mobile-flags.js';

/** @param {string} [buildId] @param {string} [appVersion] */
export function buildMobileFlagsJs(buildId, appVersion) {
  return `${buildCloudFlagsJs(buildId, appVersion)}\n${MOBILE_WEB_AND_JOIN_SCRIPTS}\n`;
}

/** Loads the flags file first in <head>, before public/js/early-boot-flags.js reads them. */
export function buildMobileIndexHtml(html) {
  let out = String(html || '');
  if (!out.includes(MOBILE_FLAGS_FILE)) {
    out = out.replace(/<head>\s*\n/i, `<head>\n<script src="/js/${MOBILE_FLAGS_FILE}"></script>\n`);
  }
  if (/<script>/i.test(out)) {
    throw new Error('build-cloud-mobile: inline <script> found in index.html; CSP blocks it');
  }
...
async function main() {
  ...
  writeFile(path.join(jsDest, MOBILE_FLAGS_FILE), buildMobileFlagsJs(buildId, appVersion));
