...
/**
 * Root paths like `data/` and `public/js/` are committed symlinks into
 * packages/*; git refuses a pathspec "beyond a symbolic link", so stage the
 * real target instead.
 */
function realRelPath(root, rel) {
  const real = fs.realpathSync(path.join(root, rel.replace(/\/$/, '')));
  return path.relative(fs.realpathSync(root), real) || rel;
}

function existingReleaseStagePaths(root) {
  const paths = RELEASE_STAGE_PATHS.filter((rel) => releasePathExists(root, rel)).map((rel) =>
    realRelPath(root, rel)
  );
