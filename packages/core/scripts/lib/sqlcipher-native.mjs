...
/**
 * Directory that really holds node_modules.
 *
 * `repoRoot` is this package's own root, which is right for `scripts/` paths
 * and wrong for `node_modules/`: npm hoists workspace dependencies to the
 * monorepo root, so `packages/<pkg>/node_modules/electron` does not always
 * exist. Ask Node where the package actually resolved instead of guessing a
 * depth — the answer is correct both here and in a standalone checkout.
 * @param {string} pkg
 * @returns {string | null}
 */
function installRootFor(pkg) {
  const marker = `${path.sep}node_modules${path.sep}${pkg}${path.sep}`;
  try {
    const resolved = require.resolve(pkg);
    const at = resolved.indexOf(marker);
    return at === -1 ? null : resolved.slice(0, at);
  } catch {
    return null;
  }
}

export const installRoot = installRootFor('electron') || repoRoot;

/**
 * Only this package's own root is redirected. A caller that names some other
 * root (a temp dir, a cross-arch pack staging dir) means that one literally.
 * @param {string} root
 */
function modulesRoot(root) {
  return root === repoRoot ? installRoot : root;
}

/** @param {string} [root] */
export function electronAppBinaryAbs(root = repoRoot) {
  const base = modulesRoot(root);
  if (process.platform === 'win32') {
    return path.join(base, 'node_modules', 'electron', 'dist', 'electron.exe');
  }
  ...
  return path.join(base, 'node_modules', 'electron', 'dist', 'electron');
}
...
export function ensureElectronRuntime(root = repoRoot) {
  ...
  const base = modulesRoot(root);
  const installScript = path.join(base, 'node_modules', 'electron', 'install.js');
  ...
  execSync(`node "${installScript}"`, { cwd: base, stdio: 'inherit', timeout: 300_000 });
...
export function electronBinAbs(root = repoRoot) {
  ...
  return process.platform === 'win32'
    ? path.join(base, 'node_modules', '.bin', 'electron.cmd')
