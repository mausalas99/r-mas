...
import { ALLOWED_ENTITY_TYPES, ALLOWED_RELATIONS } from './schema.mjs';
...
describe('extraction request shape', () => {
  it('puts the stable schema in a cached system prefix and the episode last', () => {
    ...
    const { entities, edges } = a.output_config.format.schema.properties;
    assert.deepEqual(entities.items.properties.type.enum, [...ALLOWED_ENTITY_TYPES]);
    assert.deepEqual(edges.items.properties.relation.enum, [...ALLOWED_RELATIONS]);
