...
describe('buildPatientSummary', () => {
  it('pulls nombre/cama/expediente from fields', () => {
    const entry = {
      id: 'p1',
      fields: { nombre: 'GENERICO CASOBL', cama: '202-1', registro: '1234567-8' },
    };
    const result = buildPatientSummary(entry, {});
    assert.equal(result.nombre, 'GENERICO CASOBL');
  ...
  it('falls back to top-level entry fields when fields is absent', () => {
    const entry = { id: 'p2', nombre: 'SUPUESTO CASOBM', cama: '101', registro: 'NC' };
    const result = buildPatientSummary(entry, {});
    assert.equal(result.nombre, 'SUPUESTO CASOBM');
