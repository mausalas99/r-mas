...
import { applyInternoVitals, checkVitalsRateLimit } from './vitals.js';
...
export async function handleInternoRoutes(request, env, subpath) {
  ...
  if (subpath === '/ping') {
    ...
    return Response.json({ ok: true, interno: true, board: 'v3' });
  }
  ...
  if (subpath === '/board') {
    ...
    const board = await readInternoRelayBoard(env, db, auth.sala);
