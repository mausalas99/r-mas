...
|--------------|-----------|
| **All clinical wards** — Sala 1, Sala 2, Sala E, Torre, Interconsultas, UX, Eme, Área A/Pensionistas | Nube (this worker) when connected |
...
| Column | Type | Purpose |
|--------|------|---------|
| `sala` | TEXT PK | Clinical ward key (e.g. `Sala 1`, `Torre`) |
...
Canonical room for any allowlisted ward (**Sala 1**, **Sala 2**, **Sala E**, **Torre**, **Interconsultas**, **UX**, **Eme**, **Área A/Pensionistas**) + **calendar month** key `YYYY-MM` (America/Mexico_City). One room code lasts the whole month. The desktop client calls this after **Mi rotación** — no manual room code.
...
curl -s -X POST "$BASE/api/sync/v1/rooms/ensure-turn" \
  ...
  -H 'content-type: application/json' \
  -d '{"sala":"Sala 1"}'
# optional explicit month: {"sala":"Torre","turnKey":"2026-08"}
