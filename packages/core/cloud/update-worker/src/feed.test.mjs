...
describe('resolveReleaseTextAsset', () => {
  it('GitHub 200 -> proxies body untouched, off the latest-release URL not raw-main', async () => {
    const fetchFn = fetchByUrl({ [githubLatestYmlUrl('module-manifest.json')]: '{"package":"@rplus/core"}' });
    const result = await resolveReleaseTextAsset(fetchFn, 'module-manifest.json');
    assert.equal(result.origin, 'github');
    assert.equal(result.body, '{"package":"@rplus/core"}');
  });

  it('GitHub 404 + GitLab 200 -> proxies GitLab body', async () => {
    const fetchFn = fetchByUrl({ [gitlabReleaseAssetUrl('module-manifest.json')]: '{"package":"@rplus/core"}' });
    const result = await resolveReleaseTextAsset(fetchFn, 'module-manifest.json');
    assert.equal(result.origin, 'gitlab');
    assert.equal(result.body, '{"package":"@rplus/core"}');
  });
  ...
  it('both fail -> null', async () => {
    const result = await resolveReleaseTextAsset(fetchFail(), 'module-manifest.json');
    assert.equal(result, null);
  });
});

describe('resolveReleaseBinaryAsset', () => {
  function fetchArrayBufferByUrl(map) {
    return async (url) => {
      if (Object.prototype.hasOwnProperty.call(map, url)) {
        return { ok: true, status: 200, arrayBuffer: async () => map[url] };
      }
      return { ok: false, status: 404, arrayBuffer: async () => new ArrayBuffer(0) };
    };
  }

  it('GitHub 200 -> proxies raw bytes, never text()', async () => {
    const zipBytes = new Uint8Array([0x50, 0x4b, 0x03, 0x04]).buffer;
    const fetchFn = fetchArrayBufferByUrl({ [githubLatestYmlUrl('module-bundle.zip')]: zipBytes });
    const result = await resolveReleaseBinaryAsset(fetchFn, 'module-bundle.zip');
    assert.equal(result.origin, 'github');
    assert.equal(result.body, zipBytes);
  });

  it('both fail -> null', async () => {
    const fetchFn = async () => ({ ok: false, status: 404, arrayBuffer: async () => new ArrayBuffer(0) });
    const result = await resolveReleaseBinaryAsset(fetchFn, 'module-bundle.zip');
    assert.equal(result, null);
  });
});
