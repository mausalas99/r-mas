...
| `GET /module-manifest.json` | Module feed (see `docs/superpowers/plans/2026-09-21-live-module-updates.md`). GitHub `releases/latest/download` → GitLab release asset. Body proxied untouched, no yml-style rewrite. |
| `GET /module-signature.txt` | Same resolution as `/module-manifest.json`. |
| `GET /module-bundle.zip` | Same resolution, binary — proxied via `arrayBuffer()`, never `text()`, so the zip's bytes survive untouched. |

The full-app update routes (`latest*.yml`, `min-version.json`,
`stable-versions.json`) and the module feed routes
(`module-manifest.json`, `module-signature.txt`, `module-bundle.zip`) share
this one Worker deployment but are separate filename sets that never
overlap — kept apart by construction so the class of bug that made HF/Neumo
wrongly poll IM's full-app feed (see Revert path below) can't recur between
the two feeds either.

Other than the module bundle zip, the Worker never proxies zip/dmg/exe
bytes for the full-app routes — those only rewrite the yml so
...
`module-manifest.json`, `module-signature.txt` and `module-bundle.zip` are
written to `dist/` and uploaded as release assets by
`scripts/sign-release-bundle.mjs` / `scripts/release.js` — no separate
publish step. They only exist starting with the first release built after
this Worker route was added; older tags don't carry them.
