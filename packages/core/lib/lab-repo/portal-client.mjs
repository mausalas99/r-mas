...
/**
 * @param {{ baseUrl: string, fetch?: typeof fetch, initialCookie?: string }} deps
 *   `baseUrl` is required — the portal address never lives in this repo; the
 *   caller resolves it from user settings, then `RPLUS_LAB_PORTAL_URL`.
 */
export function createLabRepoPortalClient(deps = {}) {
  ...
  async function searchByRegistro(registro) {
    ...
    const resultHtml = await post(baseUrl, {
      ...hidden,
      __EVENTTARGET: '',
      __EVENTARGUMENT: '',
      [controls.modeFieldName]: LAB_REPO_SEARCH_MODE_REGISTRO,
