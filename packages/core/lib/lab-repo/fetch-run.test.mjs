...
test('runLabRepoCheck reports hasStudies from search rows alone, no download', async () => {
  let downloadCalled = false;
  const result = await runLabRepoCheck(
    { registro: '2203912-1' },
    {
      searchByRegistro: async () => ({ rows: [{ folio: '1' }] }),
      downloadPdfForRow: async () => {
        downloadCalled = true;
        return Buffer.from('');
      },
    }
  );
  assert.equal(result.hasStudies, true);
  assert.equal(result.lastFechaSolicitud, null);
  assert.equal(downloadCalled, false);
});

test('runLabRepoCheck reports hasStudies false when the portal returns no rows', async () => {
  const result = await runLabRepoCheck(
    { registro: '2203912-1' },
    { searchByRegistro: async () => ({ rows: [] }) }
  );
  assert.equal(result.hasStudies, false);
  assert.equal(result.lastFechaSolicitud, null);
});

test('runLabRepoCheck picks the newest fechaSolicitud among the search rows', async () => {
  const result = await runLabRepoCheck(
    { registro: '2203912-1' },
    {
      searchByRegistro: async () => ({
        rows: [
          { folio: '1', fechaSolicitud: '2026-06-27 03:35' },
          { folio: '2', fechaSolicitud: '2026-09-01 10:00' },
          { folio: '3', fechaSolicitud: '2026-01-05 00:00' },
        ],
      }),
    }
  );
  assert.equal(result.hasStudies, true);
  assert.equal(result.lastFechaSolicitud, '2026-09-01 10:00');
});
