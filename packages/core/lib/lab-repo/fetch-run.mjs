...
function buildDefaultDeps(baseUrl) {
  const client = createLabRepoPortalClient({ baseUrl });
  let lastPageHtml = '';
...
/**
 * @param {{ registro: string, desde: Date | string, hasta: Date | string, baseUrl?: string }} opts
 * @param {object} [deps]
 * @returns {Promise<{ studies: object[], errors: object[] }>}
 */
export async function runLabRepoFetch(opts, deps) {
  const runDeps = deps || buildDefaultDeps(opts.baseUrl);
 ...
 * @param {{ registro: string, baseUrl?: string }} opts
...
export async function runLabRepoCheck(opts, deps) {
  const client = deps || createLabRepoPortalClient({ baseUrl: opts.baseUrl });
