...
function resolvePortalUrl(relativeUrl, baseUrl) {
...
async function fetchImpresionText(client, selectResponseHtml) {
  ...
  const url = resolvePortalUrl(relative, client.getBaseUrl());
...
export async function fetchReportTextForRow(client, row, pageHtml) {
  ...
  const { contentType, body } = await client.postBinary(client.getBaseUrl(), fields);
