...
Captured from `<URL del portal de laboratorio>` (2026-06-27). The real address
never lives in this repo — it comes from the "Dirección del portal de
laboratorio" field in Ajustes → Laboratorio (local setting, per computer), or
from the `RPLUS_LAB_PORTAL_URL` env var as a fallback preset. Tests and these
fixtures use the fake host `http://portal.example.test/laboratorio/index.aspx`.
...
1. `GET <portal base>/index.aspx` — `Drop1` defaults to `NOMBRE`.
