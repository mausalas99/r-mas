...
const TEMPLATE_HEADER_NAME = 'SINTETICO DE PRUEBA UNO';

/**
 * The template holds the header twice (a text box's mc:Choice and its
 * mc:Fallback copy). Fill each copy in its own slice, so the Fallback never
 * keeps the template's sample patient.
 */
function fillPatientHeader(xml, patient, replaceTBound) {
  const starts = [];
  for (let i = xml.indexOf(TEMPLATE_HEADER_NAME); i !== -1; i = xml.indexOf(TEMPLATE_HEADER_NAME, i + 1)) starts.push(i);
  if (starts.length < 2) return fillPatientHeaderCopy(xml, patient, replaceTBound);
  let out = fillPatientHeaderCopy(xml.slice(0, starts[1]), patient, replaceTBound);
  for (let k = 1; k < starts.length; k += 1) {
    out += fillPatientHeaderCopy(xml.slice(starts[k], starts[k + 1]), patient, replaceTBound);
  }
  return out;
}

function fillPatientHeaderCopy(xml, patient, replaceTBound) {
