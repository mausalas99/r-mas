...
describe('cloud-sync-remember-store', () => {
  ...
  it('round-trips room DEKs (unwrapped, base64) for restart persistence', () => {
    writeCloudSyncRememberStore(dir, {
      remember: true,
      token: 'tok-1',
      roomId: 'room-1',
      revision: 12,
      roomMeta: null,
      deks: { 'room-1': 'YmFzZTY0LWRlay1ieXRlcw==', 'room-2': 'b3RoZXItcm9vbS1kZWs=' },
    });
    const snap = readCloudSyncRememberStore(dir);
    assert.deepEqual(snap.deks, {
      'room-1': 'YmFzZTY0LWRlay1ieXRlcw==',
      'room-2': 'b3RoZXItcm9vbS1kZWs=',
    });
  });

  it('drops empty/blank DEK entries', () => {
    writeCloudSyncRememberStore(dir, {
      remember: true,
      token: 'tok-1',
      deks: { 'room-1': '', '  ': 'value', 'room-2': 'ok' },
    });
    assert.deepEqual(readCloudSyncRememberStore(dir).deks, { 'room-2': 'ok' });
  });
