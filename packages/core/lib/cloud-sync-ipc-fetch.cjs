...
/** @param {{ method?: string, headers?: Record<string, string>, body?: string | null }} payload */
function buildNetFetchInit(payload) {
  return {
...
function isCloudSyncTimeoutError(err) {
  const name = err?.name || '';
  const message = String(err?.message || '');
  return name === 'TimeoutError' || name === 'AbortError' || /aborted/i.test(message);
}

async function runNetFetch(net, url, init) {
  try {
    return await net.fetch(url, init);
  } catch (err) {
    if (isCloudSyncTimeoutError(err)) throw new Error('cloud_sync_timeout');
    throw err;
  }
}
...
async function cloudSyncNetFetch(net, payload) {
  ...
  const res = await runNetFetch(net, url, buildNetFetchInit(payload));
