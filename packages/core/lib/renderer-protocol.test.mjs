...
describe('renderer-protocol', () => {
  ...
  describe('resolveRequestPath', () => {
    function makeRoot() {
      const root = mkdtempSync(join(tmpdir(), 'renderer-protocol-root-'));
      mkdirSync(join(root, 'js'));
      writeFileSync(join(root, 'js', 'app.bundle.mjs'), 'packaged;');
      writeFileSync(join(root, 'index.html'), '<html></html>');
      return root;
    }

    function makeModuleDir() {
      const dir = mkdtempSync(join(tmpdir(), 'renderer-protocol-module-'));
      writeFileSync(join(dir, 'app.bundle.mjs'), 'staged;');
      mkdirSync(join(dir, 'chunks'));
      writeFileSync(join(dir, 'chunks', 'a-1.mjs'), 'export const a = 1;');
      return dir;
    }

    it('serves from root when no active module dir is set', () => {
      const root = makeRoot();
      assert.equal(resolveRequestPath(root, '/js/app.bundle.mjs', null), join(root, 'js', 'app.bundle.mjs'));
    });

    it('prefers the active module dir for js/app.bundle.mjs and js/chunks/*', () => {
      const root = makeRoot();
      const moduleDir = makeModuleDir();
      assert.equal(resolveRequestPath(root, '/js/app.bundle.mjs', moduleDir), join(moduleDir, 'app.bundle.mjs'));
      assert.equal(resolveRequestPath(root, '/js/chunks/a-1.mjs', moduleDir), join(moduleDir, 'chunks', 'a-1.mjs'));
    });

    it('falls back to root when the active module dir lacks the requested file', () => {
      const root = makeRoot();
      const moduleDir = mkdtempSync(join(tmpdir(), 'renderer-protocol-module-empty-'));
      assert.equal(resolveRequestPath(root, '/js/app.bundle.mjs', moduleDir), join(root, 'js', 'app.bundle.mjs'));
    });

    it('never consults the active module dir for non-js/ requests', () => {
      const root = makeRoot();
      const moduleDir = makeModuleDir();
      writeFileSync(join(moduleDir, 'index.html'), '<html>staged</html>');
      assert.equal(resolveRequestPath(root, '/index.html', moduleDir), join(root, 'index.html'));
    });

    it('returns null for a request that would escape the module dir', () => {
      const root = makeRoot();
      const moduleDir = makeModuleDir();
      assert.equal(resolveRequestPath(root, '/js/../../etc/passwd', moduleDir), null);
    });

    it('returns null for a request that would escape root', () => {
      const root = makeRoot();
      assert.equal(resolveRequestPath(root, '/../../etc/passwd', null), null);
    });
  });
});
