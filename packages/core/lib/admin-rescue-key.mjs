...
async function ensureAdminKeyPair({ userDataPath, safeStorage }) {
  ...
  if (existing) {
    if (unwrapSecretFromDisk(existing.wrappedPrivateKeyB64, safeStorage)) return existing;
    // unwrapDek returns null both when the wrap can't decrypt (e.g. the
    // signing identity changed, invalidating the Keychain-scoped wrap) and
    // when the Keychain is simply unavailable right now. Only regenerate in
    // the former case — regenerating during a transient outage would destroy
    // an otherwise-recoverable key.
    if (!safeStorage.isEncryptionAvailable()) return existing;
    console.warn('[admin-rescue-key] llave existente ya no descifra (¿cambió la identidad de firma?); regenerando');
