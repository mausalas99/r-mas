 ...
 * content. All 5 zones (N/V/HD/HI/NM) are always returned — the mockup
 * (#9a) shows a card for every zone regardless of whether it has numeric
 * vitals/labs today, because each zone card also carries a narrative text
 * (see `nota-evolucion-state.mjs`'s `objetivoZonesForRender`); a zone with
 * no real vitals/labs simply gets an empty `items` array — never invented
 * numeric data.
 *
 * @param {{
...
export function buildObjetivoText(zones) {
  ...
  return zones
    .filter((zone) => Array.isArray(zone.items) && zone.items.length)
    .map((zone) => {
