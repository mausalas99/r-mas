...
test('deriveObjetivoZones always returns all 5 zones, even with no real data', () => {
  const { zones, hasAnyData } = deriveObjetivoZones({ vitals: {}, labs: [] });
  assert.deepEqual(
    zones.map((z) => z.id),
    ['N', 'V', 'HD', 'HI', 'NM']
  );
  assert.ok(zones.every((z) => z.items.length === 0));
  assert.equal(hasAnyData, false);
});

test('deriveObjetivoZones builds V and HD from real vitals only, other zones present but empty', () => {
  ...
  assert.deepEqual(ids, ['N', 'V', 'HD', 'HI', 'NM']);
  assert.deepEqual(zones.find((z) => z.id === 'N').items, []);
  assert.deepEqual(zones.find((z) => z.id === 'HI').items, []);
  assert.deepEqual(zones.find((z) => z.id === 'NM').items, []);
...
test('buildObjetivoSnapshot stamps confirmedAt using the injected clock', () => {
  ...
  assert.equal(snapshot.zones.find((z) => z.id === 'HD').items.length, 1);
});
