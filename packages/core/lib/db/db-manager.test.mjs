...
describe('db-manager', () => {
  ...
  it('concurrent ensureUnlocked calls share one in-flight unlock and both resolve ok', async () => {
    const tmpDir = makeUserDataDir();
    const mgr = createManager(tmpDir);
    const [a, b] = await Promise.all([mgr.ensureUnlocked(), mgr.ensureUnlocked()]);
