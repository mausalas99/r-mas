...
function withWriteClock(row) {
  return row ? { ...row, writeClock: String(row.updated_at || row.assigned_at || '') } : row;
}
...
export function mergeActiveGuardias(db, localRows, incomingRows) {
  ...
  for (const patientId of allPatients) {
    // updated_at is the edit clock; assigned_at is kept only for rows written
    // before v28 (it never moves on an edit, so it cannot carry the R1's changes).
    const local = withWriteClock(localByPatient.get(patientId));
    const winner = pickLastWriteRow(local, withWriteClock(incomingByPatient.get(patientId)), 'writeClock');
    // Local already holds the winner: rewriting it would only restamp its clock.
    if (!winner || winner === local) continue;
    try {
      upsertActiveGuardia(db, {
        ...
        updatedAt: winner.updated_at ? String(winner.updated_at) : undefined,
      });
    } catch {
