...
function registerDbProfileCrudHandlers(ctx) {
  ...
  bindIpcHandler(ipcMain, 'db:admin-code-status', async () => {
    const isSet = await dbManager.withTransaction((db) => hasAdminAccessCode(db));
    return { ok: true, isSet };
  });

  bindIpcHandler(ipcMain, 'db:admin-code-verify', async (payload) => {
  ...
  bindIpcHandler(ipcMain, 'db:admin-code-set', async (payload) => {
    await dbManager.withTransaction((db) =>
      setAdminAccessCode(db, {
        userId: String(payload.userId || ''),
