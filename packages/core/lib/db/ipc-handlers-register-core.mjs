...
function registerDbCoreClinicalHandlers(ctx) {
  ...
  bindIpcHandler(ipcMain, 'db:clinical-delete-blobs', async (payload) => {
    const keys = Array.isArray(payload?.keys) ? payload.keys : [];
    const deleted = await dbManager.withTransaction((db) => deleteBlobs(db, keys));
    return { ok: true, deleted: Number(deleted) || 0 };
  });
  ...
  bindIpcHandler(ipcMain, 'db:clinical-save-all', async (payload) => {
    ...
    await dbManager.withTransaction((db, { audit }) => {
      ...
      if (actorUserId) touchClinicalUserActivity(db, actorUserId, undefined, 'save');
    });
    return { ok: true };
  });

  bindIpcHandler(ipcMain, 'db:clinical-ops-export', async (payload) => {
    const sala = String(payload?.sala || '').trim();
    const snapshot = await dbManager.withTransaction((db) =>
      sala ? exportClinicalOpsSnapshotForSala(db, sala) : exportClinicalOpsSnapshot(db)
    );
...
function registerDbCoreAuditHandlers(ctx) {
  ...
  bindIpcHandler(ipcMain, 'db:audit-append', async (payload) => {
    const eventType = String(payload?.eventType || '').trim();
    if (!eventType) return { ok: false, reason: 'missing_event_type' };
    await dbManager.auditOnly(eventType, payload?.meta || {});
    return { ok: true };
  });
}
...
function registerDbCoreBackupHandlers(ctx) {
  ...
  bindIpcHandler(ipcMain, 'db:recover-census-range-export', async () => {
    ...
    try {
      const { buildRecoverCensusRangePayload } = await import('./recover-census-export.mjs');
...
/** @param {import('./ipc-handlers-context.mjs').IpcHandlerContext} ctx */
function registerDbCoreCloudOutboxHandlers(ctx) {
  const { ipcMain, dbManager } = ctx;

  bindIpcHandler(ipcMain, 'db:cloud-outbox-list', async () => {
    const rows = await dbManager.withTransaction((db) => listCloudOutbox(db));
    return { ok: true, rows };
  });

  bindIpcHandler(ipcMain, 'db:cloud-outbox-replace-all', async (payload) => {
    const rows = Array.isArray(payload?.rows) ? payload.rows : [];
    await dbManager.withTransaction((db) => replaceCloudOutbox(db, rows));
    return { ok: true };
  });
}
...
export function registerDbCoreHandlers(ctx) {
  ...
  registerDbCoreCloudOutboxHandlers(ctx);
