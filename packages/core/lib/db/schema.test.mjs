...
describe('schema', () => {
  ...
  it('includes equipos tables, alert photos, push subscriptions, and clinical_change_log', () => {
    const db = new Database(':memory:');
    applyMigrations(db);
    const tables = db.prepare(
      "SELECT name FROM sqlite_master WHERE type='table' ORDER BY name"
    ...
    const changeLogCols = db.prepare('PRAGMA table_info(clinical_change_log)').all().map((c) => c.name);
    for (const name of [
      'id', 'change_id', 'command_type', 'blob_keys', 'patient_id', 'actor_id', 'origin', 'created_at', 'synced_at',
    ]) {
      assert.ok(changeLogCols.includes(name), `missing column ${name}`);
    }
    db.close();
  });
  ...
  it('accepts interconsult_type Under on patients', () => {
    const db = new Database(':memory:');
    applyMigrations(db);
    db.prepare("INSERT INTO patients (id, interconsult_type) VALUES ('p-under', 'Under')").run();
    const row = db.prepare("SELECT interconsult_type FROM patients WHERE id = 'p-under'").get();
    assert.equal(row?.interconsult_type, 'Under');
    db.prepare("UPDATE patients SET interconsult_type = 'Under' WHERE id = 'p-under'").run();
    db.close();
  });

  it('migrateToV25 preserves existing interconsult_type rows and adds Under', () => {
    const db = new Database(':memory:');
    applyMigrations(db);
    // Roll the DB back to a pre-v25 CHECK (simulate an existing installed DB).
    // Include columns the interconsult_type migration has nothing to do with (name, room,
    // a JSON blob) — a rebuild that hand-lists columns instead of reusing the real DDL would
    // silently drop these, so this is here specifically to catch that class of bug.
    db.exec(`
      CREATE TABLE patients_old (
        id TEXT PRIMARY KEY,
        nombre TEXT,
        cama TEXT,
        monitoreo TEXT DEFAULT '{}',
        interconsult_type TEXT DEFAULT 'None' CHECK(interconsult_type IN ('Ephemeral_VPO', 'Follow-up', 'None')),
        interconsult_status TEXT DEFAULT 'Pending' CHECK(interconsult_status IN ('Pending', 'Resolved', 'Active')),
        prognosis_classification TEXT DEFAULT 'Buen Pronóstico',
        negativa_maniobras_firmada INTEGER DEFAULT 0 CHECK(negativa_maniobras_firmada IN (0, 1))
      );
    `);
    db.prepare(
      "INSERT INTO patients_old (id, nombre, cama, monitoreo, interconsult_type) VALUES ('p-old-1', 'Ana', '101', '{\"a\":1}', 'Ephemeral_VPO')"
    ).run();
    db.prepare(
      "INSERT INTO patients_old (id, nombre, cama, interconsult_type) VALUES ('p-old-2', 'Beto', '102', 'Follow-up')"
    ).run();
    db.prepare(
      "INSERT INTO patients_old (id, nombre, cama, interconsult_type) VALUES ('p-old-3', 'Cato', '103', 'None')"
    ).run();
    db.exec('DROP TABLE patients; ALTER TABLE patients_old RENAME TO patients;');
    db.prepare(
      "INSERT INTO app_meta (key, value) VALUES ('schema_version', '24') ON CONFLICT(key) DO UPDATE SET value = excluded.value"
    ).run();

    // Regression: a live connection runs with foreign_keys = ON, and patient_team_assignment
    // has a FK to patients(id). Rebuilding patients via DROP+RENAME (v25) triggers an implicit
    // FK cascade check against child rows — without foreign_keys OFF around the rebuild this
    // throws "FOREIGN KEY constraint failed" and the whole migration transaction rolls back.
    db.pragma('foreign_keys = ON');
    db.prepare(
      "INSERT INTO teams (team_id, name, service, on_call_day_index) VALUES ('t-1', 'Team 1', 'Sala', 0)"
    ).run();
    db.prepare(
      "INSERT INTO patient_team_assignment (patient_id, team_id, effective_at) VALUES ('p-old-1', 't-1', '2026-01-01')"
    ).run();
    ...
    const v = db.prepare("SELECT value FROM app_meta WHERE key = 'schema_version'").get();
    assert.equal(v.value, String(SCHEMA_VERSION));
    const rows = db.prepare('SELECT id, nombre, cama, monitoreo, interconsult_type FROM patients ORDER BY id').all();
    assert.deepEqual(rows, [
      { id: 'p-old-1', nombre: 'Ana', cama: '101', monitoreo: '{"a":1}', interconsult_type: 'Ephemeral_VPO' },
      { id: 'p-old-2', nombre: 'Beto', cama: '102', monitoreo: '{}', interconsult_type: 'Follow-up' },
      { id: 'p-old-3', nombre: 'Cato', cama: '103', monitoreo: '{}', interconsult_type: 'None' },
    ]);
    db.prepare("INSERT INTO patients (id, interconsult_type) VALUES ('p-old-4', 'Under')").run();
    assert.equal(
      db.prepare("SELECT interconsult_type FROM patients WHERE id = 'p-old-4'").get()?.interconsult_type,
      'Under'
    );
    db.close();
  });
    ...
    const insertKind = (kind) => {
      db.prepare(
        `INSERT INTO lan_sync_outbox (room_id, kind, payload_json, enqueued_at)
         VALUES (?, ?, ?, ?)`
      ).run('r1', kind, JSON.stringify({ test: true }), new Date().toISOString());
    };
