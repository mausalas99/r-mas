...
function updateExistingTeamMemberFraction(db, teamId, userId, fraction) {
  db.prepare(
    `UPDATE team_membership SET sub_area_fraction = ?, cycle_set_at = ? WHERE team_id = ? AND user_id = ?`
  ).run(fraction, new Date().toISOString(), teamId, userId);
  return { warnings: [] };
}
...
function insertTeamMemberRow(db, teamId, userId, fraction) {
  ...
  db.prepare(
    `INSERT INTO team_membership (team_id, user_id, sub_area_fraction, cycle_set_at) VALUES (?, ?, ?, ?)`
  ).run(teamId, userId, fraction || null, fraction ? new Date().toISOString() : null);
}
...
/**
 * Changing a user's sala/rotation (self-service "Mi rotación" or admin
 * profile edit) must move them off any active team in the OLD sala too —
 * otherwise they keep showing up on that team's roster after the move.
 * Mirrors addTeamMember's exclusive-move scoping (active teams only).
 * @param {import('better-sqlite3').Database} db
 * @param {string} userId
 * @param {string} newSala
 * @returns {number} removed count
 */
export function reconcileTeamMembershipForSalaChange(db, userId, newSala) {
  const uid = String(userId || '');
  const sala = String(newSala || '').trim();
  if (!uid || !sala) return 0;
  const rows = db
    .prepare(
      `SELECT tm.team_id FROM team_membership tm
       JOIN teams t ON t.team_id = tm.team_id
       WHERE tm.user_id = ? AND t.rotation_active = 1 AND t.sala IS NOT NULL AND t.sala != ?`
    )
    .all(uid, sala);
  for (const row of rows) {
    removeTeamMember(db, String(row.team_id || ''), uid);
