...
import { setAdminAccessCode } from '../admin-access-code.mjs';
import {
...
describe('clinical-access-db', () => {
  ...
  it('countTeamsInEffectiveSala includes teams inferred from creator', () => {
    ...
    createTeam(db, {
      name: 'Ficticio Sintetico Casoay',
      service: 'Sala',
      onCallDayIndex: 0,
      createdBy: leader.userId,
    });
  ...
  it('upsertClinicalProfile requires admin code when enabling program admin', () => {
    ...
    setAdminAccessCode(db, { userId: u.userId, newCode: 'owner-code-1' });
    assert.throws(
      () =>
        upsertClinicalProfile(db, {
          userId: u.userId,
  ...
  it('updateTeam lets an admin manually override rotation_active', () => {
    const r4 = ensureClinicalUser(db, { clientId: 'r4-rotation', rank: 'R4' });
    // Second team in the same sala auto-stages (resolveRotationActiveForNewTeam).
    createTeam(db, { name: 'A', service: 'Sala', onCallDayIndex: 0, sala: 'Sala 1', createdBy: r4.userId });
    const staged = createTeam(db, { name: 'B', service: 'Sala', onCallDayIndex: 0, sala: 'Sala 1', createdBy: r4.userId });
    assert.equal(staged.rotation_active, 0, 'auto-inferred as next rotation since Sala 1 already has an active team');

    const movedToThisRotation = updateTeam(db, staged.team_id, { rotationActive: 1, callerUserId: r4.userId });
    assert.equal(movedToThisRotation.rotation_active, 1);

    const movedBack = updateTeam(db, staged.team_id, { rotationActive: 0, callerUserId: r4.userId });
    assert.equal(movedBack.rotation_active, 0);

    assert.throws(
      () => updateTeam(db, staged.team_id, { rotationActive: 2, callerUserId: r4.userId }),
      /Estado de rotación inválido/
    );

    // Omitting rotationActive on an unrelated edit leaves it untouched.
    const renamedOnly = updateTeam(db, staged.team_id, { name: 'B renombrado', callerUserId: r4.userId });
    assert.equal(renamedOnly.rotation_active, 0);
  });
