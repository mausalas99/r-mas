 ...
 * Packaged with lib/** — do not leave this only under scripts/ (asar would omit it).
 */
...
/** @param {Map<string, object>} map @param {unknown} census */
function collectPatientsFromCensus(map, census) {
  if (!Array.isArray(census)) return;
  for (const p of census) rememberPatient(map, p);
}

/** @param {Map<string, object>} map @param {unknown} snaps */
function collectPatientsFromLegacySnapshots(map, snaps) {
...
function collectOrphanPatientsFromSidecars(map, blobs) {
...
/** @param {Map<string, object>} map @param {Record<string, string|null>} blobs */
export function collectPatientsFromBlobs(map, blobs) {
  collectPatientsFromCensus(map, parseJson(blobs.patients, []));
  collectPatientsFromLegacySnapshots(map, parseJson(blobs.lanRoomSnapshots, {}));
  collectOrphanPatientsFromSidecars(map, blobs);
}
...
/**
 * @param {ReturnType<import('./db-manager.mjs').createDbManager>} dbManager
 */
...
/** @param {{ app: import('electron').App, dbManager: object }} deps */
