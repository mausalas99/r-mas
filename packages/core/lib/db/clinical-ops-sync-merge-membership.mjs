...
export function mergeTeamMembership(db, incomingRows) {
  ...
  // The newer cycle letter wins (cycle_set_at); a peer's older copy must not
  // revert a resident's own «Mi ciclo» change. Same rule as the Worker's
  // mergeClinicalOpsLww and the renderer's pull fold.
  const newer = `excluded.sub_area_fraction IS NOT NULL AND (team_membership.sub_area_fraction IS NULL
       OR COALESCE(excluded.cycle_set_at, '') > COALESCE(team_membership.cycle_set_at, ''))`;
  const stmt = db.prepare(
    `INSERT INTO team_membership (team_id, user_id, sub_area_fraction, cycle_set_at) VALUES (?, ?, ?, ?)
     ON CONFLICT(team_id, user_id) DO UPDATE SET
       sub_area_fraction = CASE WHEN ${newer} THEN excluded.sub_area_fraction ELSE team_membership.sub_area_fraction END,
       cycle_set_at = CASE WHEN ${newer} THEN excluded.cycle_set_at ELSE team_membership.cycle_set_at END`
  );
  let inserted = 0;
  for (const row of incomingRows) {
    ...
    try {
      stmt.run(
        ...
        row.cycle_set_at ? String(row.cycle_set_at) : null
      );
