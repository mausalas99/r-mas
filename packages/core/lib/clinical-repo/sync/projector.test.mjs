...
/** Runs the projector with a plain array-collecting enqueue, actorId 'u1' by default. */
function projectAndCollect(db, opts) {
  const enqueued = [];
...
/** projectAndCollect, then assert the single expected op landed for `res.changeId`. */
function assertProjectedOnce(db, res, nowIso) {
  const { out, enqueued } = projectAndCollect(db, { nowIso });
...
describe('clinical-repo sync projector', () => {
  it('drains change_log into outbox ops and marks synced_at', () => {
    ...
    const { enqueued } = assertProjectedOnce(db, res, '2026-08-11T15:00:00.000Z');
  ...
  it('skips sync-apply origin rows (no echo enqueue)', () => {
    ...
    const { out, enqueued } = projectAndCollect(db, {
      actorId: 'local',
  ...
  it('patient.delete drains to tombstone op', () => {
    ...
    const { out, enqueued } = projectAndCollect(db, { nowIso: '2026-08-11T15:00:00.000Z' });
    assert.equal(out.projected, 1);
  ...
  it('patient.delete tombstone includes registro when on command payload', () => {
    ...
    const res = executeClinicalCommand(
      db,
      { type: 'patient.delete', patientId: 'p1', registro: '9000013-4' },
      { actorId: 'u1', source: 'ui' }
    );
    assert.equal(res.ok, true);

    const { enqueued } = projectAndCollect(db, { nowIso: '2026-08-11T15:00:00.000Z' });
    assert.equal(enqueued[0].ops[0].path, 'tombstones/p1');
    assert.equal(enqueued[0].ops[0].value.registro, '9000013-4');
    db.close();
  });
  ...
  it('unknown command types land in skipIds without crashing', () => {
    ...
    const { out, enqueued } = projectAndCollect(db, { nowIso: '2026-08-11T16:00:00.000Z' });
    assert.equal(out.projected, 0);
    assert.equal(enqueued.length, 0);
  ...
  it('changeIds projects only the requested row', () => {
    const db = openDbWithPatient();
    const first = executeClinicalCommand(
      db,
      {
        type: 'eventualidad.upsert',
        patientId: 'p1',
        entry: { text: 'one', kind: 'note', at: '2026-08-05T18:00:00.000Z' },
      },
      { actorId: 'u1', source: 'ui' }
    );
    const second = executeClinicalCommand(
      db,
      {
        type: 'eventualidad.upsert',
        patientId: 'p1',
        entry: { text: 'two', kind: 'note', at: '2026-08-05T19:00:00.000Z' },
      },
      { actorId: 'u1', source: 'ui' }
    );
    ...
    const { out, enqueued } = projectAndCollect(db, {
      ...
      changeIds: [second.changeId],
      markSynced: false,
    });
    assert.equal(out.projected, 1);
    assert.equal(enqueued.length, 1);
    assert.equal(enqueued[0].clientMutationId, second.changeId);

    const stillOpen = db
      .prepare('SELECT COUNT(*) AS n FROM clinical_change_log WHERE synced_at IS NULL')
      .get();
    assert.equal(stillOpen.n, 2);
    db.close();
  });
});
