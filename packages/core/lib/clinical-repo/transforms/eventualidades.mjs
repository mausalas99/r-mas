...
function appendNewEntry(store, entry) {
  ...
  const forcedId = entry && entry.id != null ? String(entry.id).trim() : '';
...
function upsertIntoStore(store, entry) {
  ...
  if (entryId) {
    const exists = (store.entries || []).some(function (e) {
      return e && String(e.id) === entryId;
    });
    if (exists) return updateExistingEntry(store, entry, entryId);
