...
const CODE_RE = /^(Digit[1-5]|Numpad[1-5]|Key[ETDMGIPSKNC]|Slash|Comma|BracketLeft|BracketRight)$/;
const KEYS = '12345etdmgipsnc/,[]';
...
function isReservedShellShortcutInput(input) {
  ...
  // ⌘C / Ctrl+C alone must reach Chromium as a native copy; only ⌘⇧C is a shell shortcut.
  if ((code === 'KeyC' || key === 'c') && !input.shift) return false;
