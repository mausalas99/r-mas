...
const routineFullMetricsPendientesJson = () =>
...
const buildSinglePatientBoard = (guardiaOverrides) => {
...
test('buildInternoBoardDto hides explicit routine vitals plan without pendientes', () => {
  const board = buildSinglePatientBoard({
    pendientes_json: routineFullMetricsPendientesJson(),
  });
...
test('buildInternoBoardDto marks signos pending without estudios', () => {
  ...
  const board = buildInternoBoardDto(
    'Sala 1',
    [{ id: 'p1', nombre: 'Modelo Simulado Casobb', servicio: 'Sala', area: 'A1' }],
    guardias
  );
...
test('buildInternoBoardDto keeps routine entrega when estudios pendientes exist', () => {
  ...
  const board = buildSinglePatientBoard({ pendientes_json: pendientesJson });
  assert.equal(board.patients.length, 1);
...
test('buildInternoBoardDto reads freshness from monitoreo.historial, not the dead last_vitals_check', () => {
  const pendientesJson = serializePendientesJson({
    version: 2,
    vitalsPlan: {
  ...
  const guardias = new Map([
    [
      'p1',
      {
        ...
        pendientes_json: pendientesJson,
        vitals_frequency: '4h',
        // Stale/never-bumped clinicalOps field — the phone's vitals aren't reflected here.
    ...
    [
      {
        id: 'p1',
        nombre: 'Test',
        monitoreo: { historial: [{ recordedAt: new Date().toISOString() }] },
      },
    ],
    guardias
  );
  // A stale last_vitals_check alone would read as breached; the fresh
  // historial entry should keep it nominal instead.
  assert.notEqual(board.patients[0].vitals.cls, 'breached');
});
