...
test('5 real teams: activo returns the full non-guardia/postguardia list (lane-slotting is the board\'s job, not role computation\'s)', () => {
  const E = team('E');
  const roles = getInterconsultaTeamRoles([A, B, C, D, E], DAY_25);
  assert.equal(roles.guardia, A);
  assert.equal(roles.postguardia, D);
  assert.equal(roles.activo.length, 3);
  assert.deepEqual(roles.activo, [B, C, E]);
});
