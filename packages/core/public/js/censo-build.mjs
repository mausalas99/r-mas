...
import { serviceById } from './features/patient-dashboard/interconsult-catalog.mjs';
...
export function formatPacienteMetaForCenso(patient, settings, teamLabel) {
  ...
  var ic = (Array.isArray(patient.interconsultServiceIds) ? patient.interconsultServiceIds : [])
    .map(function (id) { return serviceById(id)?.name; })
    .filter(Boolean);
  if (ic.length) lines.push('IC: ' + ic.join(', '));
