...
export function isPatientVisibleOnMobileTeamMirror(user, patient, scopeContext, activeGuardia) {
  ...
  if (isPatientVisibleViaInterconsultasOnCall(user, patient, scopeContext)) return true;
  return isUnassignedStructuralMatchOnJoinedTeam(user, patient, scopeContext);
}

/**
 * Same on-call rule desktop uses (evaluateScopeInterconsultas) — a patient on the
 * Interconsultas board has no team assignment row, so without this branch it's
 * invisible on the iPad/PWA mirror until someone manually assigns it a team.
 * @param {object} user
 * @param {{ id?: string, servicio?: string, service?: string, area?: string, sub_area?: string, interconsult_type?: string }} patient
 * @param {object|null|undefined} scopeContext
 */
function isPatientVisibleViaInterconsultasOnCall(user, patient, scopeContext) {
  if (!isInterconsultasPatient(patient)) return false;
  const joinedTeams = getJoinedTeamsForUser(scopeContext?.teams || [], user);
  const rank = user?.rank;
  const now = scopeContext?.now || new Date().toISOString();
  const userId = user.user_id;
  return (
    userOffCallFromInterconsultasRotationServices(userId, joinedTeams, rank, now) ||
    userOnCallForInterconsultasTeam(userId, joinedTeams, rank, now)
  );
}
