...
Expediente:	9000016-9	Solicitud:	9000000021
Nombre:	SINTETICO EJEMPLO MODELO CASOAJ	Fecha Registro:	May 2 2026 5:11PM
Sexo:	MASCULINO	Ubicación:	SERVICIO CLÍNICO 2
Edad:	59	Medico:	FICTICIO SINTETICO EJEMPLO A
...
test('normalizarProteinasFluidoGdl — convierte con la unidad que trae el reporte', () => {
  assert.equal(normalizarProteinasFluidoGdl_('6000', 'mg/dL'), 6);
  assert.equal(normalizarProteinasFluidoGdl_('1176', 'mg/dL'), 1.176);
  assert.equal(normalizarProteinasFluidoGdl_('5.4', 'g/dL'), 5.4);
  assert.equal(normalizarProteinasFluidoGdl_('60', 'g/L'), 6);
  assert.equal(normalizarProteinasFluidoGdl_('6000', 'mg/dL\t15 - 45'), 6);
  assert.equal(normalizarProteinasFluidoGdl_('6000', 'MG/DL'), 6);
});

test('normalizarProteinasFluidoGdl — la banda 100-999 ya no infla 10x', () => {
  // 900 mg/dL son 0.9 g/dL. La heurística vieja devolvía 9 y volvía exudado
  // cualquier trasudado de ese rango.
  assert.equal(normalizarProteinasFluidoGdl_('900', 'mg/dL'), 0.9);
  assert.equal(normalizarProteinasFluidoGdl_('300', 'mg/dL'), 0.3);
  assert.equal(normalizarProteinasFluidoGdl_('600', 'mg/dL'), 0.6);
});

test('normalizarProteinasFluidoGdl — sin unidad no adivina', () => {
  assert.equal(normalizarProteinasFluidoGdl_('300'), null);
  assert.equal(normalizarProteinasFluidoGdl_('6000'), null);
  assert.equal(normalizarProteinasFluidoGdl_('300', ''), null);
  assert.equal(normalizarProteinasFluidoGdl_('300', 'no-es-unidad'), null);
  assert.equal(normalizarProteinasFluidoGdl_('', 'mg/dL'), null);
  assert.equal(normalizarProteinasFluidoGdl_('---', 'mg/dL'), null);
});

test('parsearCitoquimicoLiquidos — la unidad del reporte llega hasta la línea Liq', () => {
  const seisMil = String(parsearCitoquimicoLiquidos(MUESTRA_PLEURAL) || '');
  assert.match(seisMil, /Prot 6\b/, '6000 mg/dL son 6 g/dL');

  const novecientos = String(
    parsearCitoquimicoLiquidos(MUESTRA_PLEURAL.replace('6000', '900')) || ''
  );
  assert.match(novecientos, /Prot 0\.9\b/, '900 mg/dL son 0.9 g/dL, no 9');

  // Sin la línea de unidad no se convierte: se muestra el valor tal cual y el
  // criterio de proteína de Light se queda sin evaluar.
  const sinUnidad = String(
    parsearCitoquimicoLiquidos(MUESTRA_PLEURAL.replace('6000\nmg/dL\t', '900\n')) || ''
  );
  assert.match(sinUnidad, /Prot 900\b/);
});

test('Light — un trasudado con proteína de tres dígitos ya no sale exudado', () => {
  // Prot LP 0.9 g/dL (900 mg/dL), Prot suero 7.0, LDH LP 90, LDH suero 200,
  // LSN 250: ningún criterio se cumple.
  const prot = normalizarProteinasFluidoGdl_('900', 'mg/dL');
  assert.equal(prot, 0.9);
  const v = evaluarCriteriosLight_(prot, 90, 7.0, 200, 250);
  assert.match(v, /TRASUDADO/);
  assert.ok(!/EXUDADO/.test(v), 'no debe llamar exudado a un trasudado: ' + v);
});
...
Expediente:	9000004-8	Solicitud:	9000000027
Nombre:	SIMULADO GENERICO SUPUESTO CASOAE	Fecha Registro:	May 11 2026 6:24PM
Sexo:	FEMENINO	Ubicación:	MEDICINA INTERNA 1
Edad:	73	Medico:	FICTICIO SINTETICO EJEMPLO A
...
Expediente:	9000037-2	Solicitud:	9000000036
Nombre:	GENERICO SUPUESTO CASOBD	Fecha Registro:	Aug 1 2026 9:00AM
Sexo:	FEMENINO	Ubicación:	REUMATOLOGIA
Edad:	52	Medico:	FICTICIO SINTETICO EJEMPLO A
