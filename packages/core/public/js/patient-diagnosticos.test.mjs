...
test('stampCensoFieldsClock marca lanUpdatedAt para el push de fields', () => {
  ...
  stampCensoFieldsClock(patient, '2026-08-14T12:00:00.000Z', 'censoAtbText');
  assert.deepEqual(patient.fieldClocks, { censoAtbText: '2026-08-14T12:00:00.000Z' });
});
...
test('los editores de censo marcan el reloj antes de persistir', () => {
  ...
  for (var i = 0; i < handlers.length; i += 1) {
    ...
    // stampCensoAndPush = clock + Nube push; without the push peers never see the edit.
    assert.match(body, /stampCensoAndPush\(patient, /, handlers[i] + ' sin reloj/push');
  }
});
