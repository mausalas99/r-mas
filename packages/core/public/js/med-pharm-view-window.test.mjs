...
function buildCrossMonthWindowAndRows() {
...
describe('cellValueAtColumn', () => {
  it('lee día del bucket mensual correcto', () => {
    const { profile, w, rows } = buildCrossMonthWindowAndRows();
...
function buildWindowAndMedGroups(profile) {
  const w = buildPharmViewWindow({
    profile,
    viewYear: 2026,
    viewMonthIndex: 5,
...
describe('groupUnifiedRowsByMed', () => {
  it('colapsa presentaciones SOME distintas del mismo principio activo', () => {
    ...
    const { unified, groups } = buildWindowAndMedGroups(profile);
