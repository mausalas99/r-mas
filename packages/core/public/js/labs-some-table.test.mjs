...
const MUESTRA_LUNA = `
Expediente:	9000012-1	Solicitud:	9000000020
Nombre:	FICTICIO SINTETICO EJEMPLO CASOAI	Fecha Registro:	10/06/2025 03:41:29 p. m.
Sexo:	MASCULINO	Ubicación:	EMERGENCIAS SHOCK TRAUMA CONSULTA
Edad:	43	Medico:	FICTICIO SINTETICO EJEMPLO A
...
test('parseSomeReportTables — cultivos: CUENTA con COMENTARIO vacío y polimicrobiano', () => {
  const raw = `Expediente:\t9000096-4\tSolicitud:\t2605291063
BACTERIOLOGIA
Estudio\t\tResultado\tUnidades\tValor de Referencia
ASPIRADO TRAQUEAL
PRODUCTO\t
...
277.0
mmol/L	275 - 295
Expediente:	9000005-1	Solicitud:	9000000029
`;
  const parsed = parseSomeReportTables(raw);
  const rows = parsed.departments[0].groups[0].rows;
  assert.equal(rows.length, 2);
...
*
277.0 Expediente:
9000005-1 Solicitud: 9000000029
`;
  const parsed = parseSomeReportTables(raw);
