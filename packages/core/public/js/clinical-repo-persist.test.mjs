...
describe('clinical-repo-persist', () => {
  ...
  it('resolves { ok: false } when the save throws, instead of rejecting', async () => {
    const seen = [];
    appState.setSaveStateHooks({
      before: null,
      after: null,
      onSaveResult: (r) => seen.push(r),
    });
    globalThis.window.electronAPI.dbClinicalCommand = async () => {
    ...
    // A rejection here would surface as an unhandledRejection after the caller
    // has moved on, because every production caller is fire-and-forget.
    const res = await persistClinicalState({ immediate: true, source: 'throw-test' });

    assert.equal(res.ok, false);
    assert.match(res.error, /ipc exploded/);
