...
describe('applySomePasteToProfile', () => {
  const metamizolFields = { med: 'METAMIZOL', dosis: '1 G //', freq: 'Q8H', via: 'VIA IV' };

  /** A '2026-05' profile with one METAMIZOL row, day 5 indicated, plus `rowExtra` overrides. */
  function makeMetamizolMonthProfile(rowExtra) {
    return {
      months: {
        '2026-05': {
          monthKey: '2026-05',
          ...
          rows: [
            {
              rowKey: buildMedPharmRowKey(metamizolFields),
              ...metamizolFields,
              days: { 5: 1 },
              notAdmin: {},
              cat: '',
  ...
  function parseMetamizolSomePaste() {
    return parseSomePharmMonthPaste(
      'Medicamento\tDosis\tFreq\tVia\t01\t02\t03\t04\t05\n' +
  ...
  it('conserva notAdmin en días que siguen indicados', () => {
    const profile = makeMetamizolMonthProfile({ notAdmin: { 5: true } });
    const next = applySomePasteToProfile(profile, parseMetamizolSomePaste());
  ...
  it('conserva filas ocultas al reimportar SOME', () => {
    const key = buildMedPharmRowKey(metamizolFields);
    const profile = makeMetamizolMonthProfile({ hidden: true });
    const next = applySomePasteToProfile(profile, parseMetamizolSomePaste());
    const row = next.months['2026-05'].rows.find(function (r) {
      return r.rowKey === key;
    });
