...
export function renderPatientCardToolbarHtml(p, pinOn, archOn) {
  ...
  var deleteBtn = canDelete
    ? '<button type="button" class="btn-delete-card" data-onclick="deletePatient" data-onclick-args=\'' +
      escAttr(JSON.stringify([p.id])) +
      '\' data-onclick-pass="event" aria-label="Eliminar">×</button>'
    : '';
  ...
  return (
    ...
    '" aria-label="' +
    archTitle +
    '" data-onclick="togglePatientArchived" data-onclick-args=\'' +
    escAttr(JSON.stringify([p.id])) +
    '\' data-onclick-pass="event">' +
    archiveIcon +
    '</button>' +
...
export function renderArchivedToggleHtml(collapsed, count) {
  return (
    '<button type="button" class="patient-list-section-toggle" data-onclick="toggleArchivedSection" data-onclick-pass="event" aria-expanded="' +
