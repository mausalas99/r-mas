...
function seedPatientWithHistorialRow(patientId, rowId) {
...
function activateRuntimeFor(patientId) {
...
test('editarEstadoActualMedicion precarga la fila y registrar la reemplaza en sitio', () => {
  ...
  seedPatientWithHistorialRow('p4', 'row-1');
  activateRuntimeFor('p4');
...
test('editarEstadoActualMedicion refresca la fecha visible del picker, no la de hoy', () => {
  if (typeof document === 'undefined') return;
  document.body.innerHTML =
    '<div id="ea-registro-backdrop"><div id="ea-registro-modal-body"></div></div>';
  document.getElementById('ea-registro-modal-body').innerHTML = buildRegistroFormMarkup();
  refreshRpcDateFields(document.getElementById('ea-form'));
  seedPatientWithHistorialRow('p5', 'row-2');
  activateRuntimeFor('p5');

  editarEstadoActualMedicion('row-2');
  var label = document.querySelector('.rpc-datetime-field .rpc-date-field__label');
  assert.equal(label.textContent, '02/09/2026');
  setEaFormOpenPatientId(null);
});
