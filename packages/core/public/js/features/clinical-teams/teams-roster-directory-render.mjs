...
export function renderDirectoryUsersTopButtonHtml(user) {
  ...
  return `<button type="button" class="wb-btn wb-btn-secondary clinical-teams-open-lan-users-btn" id="btn-open-lan-users-directory">Directorio LAN</button>`;
}
  ...
  return `<article class="clinical-lan-user-card clinical-lan-user-row" data-user-id="${userId}" data-user-rank="${rankRaw}" data-preferred-cycle="${escapeAttr(placement?.cycle || '')}" data-sala="${salaAttr}" data-has-team="${placement?.teamId ? '1' : '0'}" data-activity-tier="${escapeAttr(activityTier)}" data-search="${searchHaystack}">
    ...
    <div class="clinical-lan-user-card-assign">
      ...
      <span class="clinical-directory-assign-actions" role="group" aria-label="Acciones">
        <button type="button" class="wb-btn wb-btn-primary wb-btn-lg clinical-directory-assign-btn" data-user-id="${userId}">Asignar</button>
        <button type="button" class="wb-btn wb-btn-danger-outline clinical-directory-delete-user-btn${deleteBtnClass}" data-user-id="${userId}" data-user-label="${escapeAttr(String(u.clinical_name || normalizeUsername(u.username || '') || rawUserId))}" title="Quitar de la base clínica (se publica por R+ Cloud)"${deleteBtnAttrs}>Quitar</button>
      </span>
    </div>
  </article>`;
}
  ...
  return `
    <div class="clinical-directory-head">
      <p class="clinical-directory-users-modal-lead">Asigna residentes a equipos activos en esta Mac. <strong>Actualizar desde ⇄</strong> trae usuarios de todas las salas (Sala 1–E, Interconsultas, UX, Eme, etc.).
        <button type="button" class="wb-btn wb-btn-secondary clinical-directory-refresh-btn">Actualizar desde ⇄</button>
      </p>
