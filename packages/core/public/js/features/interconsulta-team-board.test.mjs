...
// Renders with no patients and returns the section count plus the parsed activo lane blocks.
function renderAndActivoLanes(teams) {
  ...
  return { html, sectionCount, activoLanes };
}
...
test('fewer than 2 real activo teams: still 4 sections, empty slot shows placeholder with no drop target', () => {
  ...
  const { sectionCount, activoLanes } = renderAndActivoLanes(teams);
  assert.equal(sectionCount, 4);
