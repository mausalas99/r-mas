 ...
 */
/** A registro with at least one turn value or turn event (a vitals-only row has none). */
function hasIoTurnoData(rowIo) {
  var filled = function (arr) {
    return Array.isArray(arr) && arr.some(function (v) { return v != null && String(v).trim() !== ''; });
  };
  return filled(rowIo.ingTurnos) || filled(rowIo.egrTurnos) || (Array.isArray(rowIo.eventos) && rowIo.eventos.length > 0);
}
...
export function absorbIoRow(rowIo, state) {
  ...
  if (!state.turnosSeen && hasIoTurnoData(rowIo)) {
    state.turnosSeen = { ingTurnos: rowIo.ingTurnos, egrTurnos: rowIo.egrTurnos, eventos: rowIo.eventos };
  }
}
...
export function deriveIoFromHistorial_(sortedAsc) {
  var state = {
    ...
    turnosSeen: /** @type {null | Record<string, unknown>} */ (null),
  };
  ...
  if (state.turnosSeen) snapIo.turnos = state.turnosSeen;
  return snapIo;
}
