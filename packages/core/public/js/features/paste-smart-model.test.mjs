...
const CENSUS = [
  { id: 'p1', nombre: 'SIMULADO GENERICO CASOAM', registro: '9000017-4', cuarto: '412' },
  { id: 'p2', nombre: 'SINTETICO EJEMPLO CASOAR', registro: '9000107-1', cuarto: '410' },
  { id: 'p3', nombre: 'EJEMPLO MODELO CASOAS', registro: '1111111-1', cuarto: '401' },
];

describe('paste-smart-model', () => {
  it('extractSomeNombreFromReport lee Nombre:', () => {
    assert.equal(extractSomeNombreFromReport(GASO_VENOSA_SOLO), 'SIMULADO GENERICO CASOAM');
  });
  ...
  it('scoreNombreAgainstPatient: exacto y tokens reordenados', () => {
    assert.equal(scoreNombreAgainstPatient('SIMULADO GENERICO CASOAM', 'SIMULADO GENERICO CASOAM'), 1000);
    assert.ok(
      scoreNombreAgainstPatient('SIMULADO GENERICO CASOAM', 'CASOAM SIMULADO GENERICO') > 15
    );
    assert.equal(scoreNombreAgainstPatient('ANA', 'PEDRO'), -Infinity);
  });

  it('matchPatientByRegistro encuentra expediente', () => {
    assert.equal(matchPatientByRegistro('9000017-4', CENSUS).id, 'p1');
    assert.equal(matchPatientByRegistro('no-existe', CENSUS), null);
  });

  it('matchPatientsByNombre rankea y distingue', () => {
    const hits = matchPatientsByNombre('SIMULADO GENERICO CASOAM', CENSUS);
  ...
  it('registroFromPaste acepta solo un registro suelto', () => {
    assert.equal(registroFromPaste(' 1234567-8\n'), '1234567-8');
    assert.equal(registroFromPaste('1234567'), '');
    assert.equal(registroFromPaste('1234567-8 7654321-0'), '');
    assert.equal(registroFromPaste('Reg 1234567-8'), '');
  });
  ...
  it('planSmartPaste: 2 expedientes distintos en un bloque → mixed-expediente, no procesa nada', () => {
    var other = String(GASO_VENOSA_SOLO).replace('9000017-4', '5555555-5');
    ...
    assert.match(plan.message, /9000017-4/);
...
Nombre:\tSIMULADO GENERICO\tFecha Registro:\tMay 7 2026 6:43AM
Sexo:\tMASCULINO
...
`;
  ...
  it('enrichBlockWithNombreMatches usa nombre del reporte', () => {
    const block = {
      status: 'no-patient',
      okReportCount: 1,
      reports: [{ ok: true, nombre: 'SIMULADO GENERICO CASOAM', reportText: GASO_VENOSA_SOLO }],
    };
