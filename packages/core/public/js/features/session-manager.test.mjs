...
describe('BackgroundVitalsMonitorLoop', () => {
  ...
  function makeNotifyCollector() {
    const notifications = [];
    return { notifications, notify: (title, body) => notifications.push({ title, body }) };
  }

  function dbWithRow(row) {
    return { all: async () => [row] };
  }

  it('fires overdue notification when check is breached', async () => {
    const { notifications, notify } = makeNotifyCollector();
    const db = dbWithRow({
      patient_id: 'pat-1',
      last_vitals_check: new Date(Date.now() - 2 * 3600000).toISOString(),
      vitals_frequency: '1h',
  ...
  it('skips routine vitalsPlan even when DB frequency is set', async () => {
    ...
    const loop = new BackgroundVitalsMonitorLoop(db, 'user-1', { notify });
    await loop.scan();
