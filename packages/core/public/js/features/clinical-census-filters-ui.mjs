 ...
 * @param {Storage|undefined} storage
 */
export function resolveElevatedTeamFilterId(user, _teams, storage = globalThis.localStorage) {
  const pref = readElevatedTeamFilterPreference(storage);
  if (pref.pinned) return pref.teamId;
  // R4 / Admin / program admin: default to Todos; narrow via the Equipo dropdown.
  return '';
}
