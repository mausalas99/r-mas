...
/**
 * True si dos sets del cluster comparten un analito (misma sección+campo) con
 * valores distintos — p. ej. un K de las 01:00 y otro de las 02:00. El pase
 * automático de consolidación (día + tipo homogéneo, ventana ≤2 h) agrupa por
 * tipo sin mirar el analito, a diferencia del pase de "fragmentos
 * complementarios" (ver sectionsAreComplementary_) — sin este chequeo,
 * mergeLabHistorySetsCluster pisaba la toma más vieja en vez de dejar las dos
 * tomas separadas.
 */
function clusterHasFieldConflict_(arr) {
  for (var i = 0; i < arr.length; i++) {
    var pa = arr[i] && arr[i].parsedBySection;
    if (!pa) continue;
    for (var j = i + 1; j < arr.length; j++) {
      var pb = arr[j] && arr[j].parsedBySection;
      if (!pb) continue;
      var conflict = Object.keys(pa).some(function (k) {
        var rowA = pa[k];
        var rowB = pb[k];
        if (!rowA || !rowB) return false;
        return Object.keys(rowA).some(function (fk) {
          return fk in rowB && Number(rowA[fk]) !== Number(rowB[fk]);
        });
      });
...
function mergeLabHistorySetsCluster(patientId, setsToMerge, tipoGrupo, opts) {
  var removedIds = [];
  if (!setsToMerge || setsToMerge.length < 2) return removedIds;
  var skipConflicting = !opts || opts.skipConflicting !== false;
  if (skipConflicting && clusterHasFieldConflict_(setsToMerge)) return removedIds;
...
function executeLabConsolidationMergeJobs(patientId, jobs, opts) {
  ...
  jobs.forEach(function (job) {
    ...
    var removed = mergeLabHistorySetsCluster(patientId, job.sets, tipoGrupo, opts);
...
function runManualLabConsolidationForPatient(patientId, groups) {
  ...
  // El usuario eligió estos grupos a mano (revisó los sets) — a diferencia del
  // pase automático, aquí un analito compartido con valor distinto no bloquea
  // la fusión.
  var result = executeLabConsolidationMergeJobs(patientId, jobs, { skipConflicting: false });
  if (result.merged) rt.rebuildEstudiosFromLabHistory(patientId);
  return result;
}
