...
function syncInterconsultaModeChrome() {
  // Fire-and-forget, so the rejection has nowhere to go but unhandledRejection;
  // the caller's own catch has already returned by the time this settles.
  import("./interconsulta-mode-chrome.mjs")
    .then(function (mod) {
      mod.syncInterconsultaModeChrome();
    })
    .catch(function (err) {
      console.error("[R+] syncInterconsultaModeChrome:", err);
    });
}
