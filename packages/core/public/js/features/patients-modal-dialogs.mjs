...
export function showDuplicateWarning(existing, onConfirm) {
  ...
  backdrop.innerHTML =
    ...
    '<button id="dup-confirm-cancel-btn" style="background:#F3F4F6;border:none;border-radius:6px;padding:8px 16px;font-size:13px;font-weight:600;font-family:inherit;cursor:pointer;color:#1f2937;">Cancelar</button>' +
  ...
  document.getElementById('dup-confirm-cancel-btn').onclick = function () {
...
export function showExpedienteAdvice(onConfirm) {
  ...
  b.innerHTML =
    ...
    '<button type="button" class="wb-btn wb-btn-secondary wb-btn-lg" id="exp-advice-cancel">' +
    escTxtSafe(advice.cancelLabel) +
    '</button>' +
    '<button type="button" class="wb-btn wb-btn-primary wb-btn-block" id="exp-advice-confirm">' +
