...
export function applyDietProposalFromRecetaBlock(monitoreo, recetaBlock, opts) {
  ...
  if (candidates.length > 1) {
    ...
    if (!mergedDietHasContent(mergedCandidate)) return false;
    if (tryAutoConfirmMatchingDiet(monitoreo, mergedCandidate)) {
      clearDietOptions(monitoreo);
      return true;
    }
    if (shouldSkipDietProposal(monitoreo, opts, mergedCandidate)) return false;
