...
function validateUsernameForm(fields, errEl) {
  if (!isValidUsernameFormat(fields.username)) {
    showOnboardError(errEl, USERNAME_INVALID_MSG);
    return false;
  }
  if (!fields.name) {
...
/** Per-question check for the stepped profile form; the full check still runs on submit. */
function validateUsernameSubstep(index) {
  const fields = readUsernameFormFields();
  if (index === 0 && !isValidUsernameFormat(fields.username)) {
    return USERNAME_INVALID_MSG;
  }
  if (index === 1 && !fields.name) return 'Escribe tu nombre en guardia.';
  return '';
}
...
export async function wireOnboardingInteractions() {
  ...
  if (form && !form._rpcOnboardWired) {
    ...
    // Must run first: it holds back submit until the last question.
    wireOnboardingSubsteps(form, validateUsernameSubstep);
