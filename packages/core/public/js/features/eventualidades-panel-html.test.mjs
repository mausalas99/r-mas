...
function buildOneDayPanelHtml(entries) {
  return buildEventualidadesPanelHtml(
    [{ day: '2026-08-03', label: 'Hoy', isToday: true, entries }],
    true,
    null,
    { entries: [], labsText: '' },
    'note',
...
test('buildEventualidadesPanelHtml: timeline + compose, no Labs tab', () => {
  const html = buildOneDayPanelHtml([
...
test('removeEventualidadCardEl drops one card and keeps the other', { skip: typeof document === 'undefined' }, () => {
  ...
  mount.innerHTML = buildOneDayPanelHtml([
    { id: 'ev_1', at: '2026-08-03T10:00:00.000Z', text: 'NAUSEA' },
