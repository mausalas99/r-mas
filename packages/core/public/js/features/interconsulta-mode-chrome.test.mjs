...
/** Mounts the interconsulta bar with render tracking, then finds+asserts the toggle button starts unpressed. */
function mountToggleBar(selector) {
...
describe('"Solo guardia de hoy" filter toggle', () => {
  it('toggles isInterconsultaGuardiaOnlyFilterActive and re-renders the patient list', () => {
    ...
    var { renderCalls, btn } = mountToggleBar('[data-wb-ic-guardia-filter]');
...
describe('"Ocultar post-guardia" toggle', () => {
  it('toggles isInterconsultaPostguardiaHidden and updates aria-pressed', () => {
    if (typeof document === 'undefined') return;
    var { btn } = mountToggleBar('[data-wb-ic-hide-postguardia]');
