...
import { enqueueCloudLabSidecarsForPatient, scheduleCloudSyncPush } from './cloud-sync/mutate-bridge.mjs';
import { isPlaceholderPatientName } from './sync-apply/patient-entries.mjs';
...
/**
 * A chart can arrive from Nube with registro but no name. The SOME report for
 * that registro carries the name — fill the placeholder and push it back.
 * @param {{ nombre?: string }|null|undefined} patient @param {unknown} labName
 */
function fillPlaceholderNameFromLab(patient, labName) {
  var name = String(labName || '').trim().toUpperCase();
  if (!patient || !name || isPlaceholderPatientName(name) || !isPlaceholderPatientName(patient.nombre)) {
    return false;
  }
  patient.nombre = name;
...
function storeBulkLabBlocks(blocks, processable, opts) {
  ...
  processable.forEach(function (block) {
    ...
    var named = block.reports.find(function (r) {
      return r.ok && r.nombre && (!patientReg || r.expediente === patientReg);
    });
    if (named) fillPlaceholderNameFromLab(block.patient, named.nombre);
