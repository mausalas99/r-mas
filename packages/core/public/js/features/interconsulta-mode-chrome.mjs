...
function ensureIcEscHandler() {
  ...
  // Capture phase, so an open dialog is still open here: its Esc closes the dialog only.
  document.addEventListener('keydown', function (ev) {
    if (ev.key !== 'Escape' || ev.defaultPrevented) return;
    if (!isInterconsultaModeActive() || !isIcPatientViewOpen()) return;
    var dialogs = document.querySelectorAll('[role="dialog"][aria-modal="true"], [role="alertdialog"]');
    for (var i = 0; i < dialogs.length; i += 1) {
      if (dialogs[i].getClientRects().length) return;
    }
    showInterconsultaBoardView();
  }, true);
}

function restorePatientViewAfterInterconsulta() {
...
function deactivateInterconsultaModeChrome(boardMount) {
  ...
  if (_wasInterconsultaActive) {
    restorePatientViewAfterInterconsulta();
  }
  _wasInterconsultaActive = false;
}

/** @param {HTMLElement | null} barMount */
function mountInterconsultaBarIfNeeded(barMount) {
  if (!barMount || barMount.dataset.wbIcMounted) return;
...
export function syncInterconsultaModeChrome() {
  ...
  if (!active) {
    deactivateInterconsultaModeChrome(boardMount);
    return;
  }
  mountInterconsultaBarIfNeeded(barMount);
  ensureIcEscHandler();
  if (!_wasInterconsultaActive) {
