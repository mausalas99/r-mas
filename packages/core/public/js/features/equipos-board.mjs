...
function renderDeviceRow(dev, isAdmin) {
  ...
  const purgeBtn = isAdmin
    ? `<button type="button" class="wb-btn wb-btn-danger-outline" data-purge="${esc(dev.device_type)}">Purgar cola</button>`
    : '';
  return (
...
function buildEquiposBoardHtml(board, cloud, isAdmin) {
  return (
    ...
    `<a href="${esc(cloud.url)}" target="_blank" rel="noopener" class="wb-btn wb-btn-secondary equipos-board-open-mobile">Abrir móvil</a>` +
    `</div>` +
    ...
    (isAdmin
      ? `<div class="equipos-board-admin-actions">` +
        `<button type="button" class="wb-btn wb-btn-secondary" id="btn-equipos-history">Historial de uso</button>` +
        `<button type="button" class="wb-btn wb-btn-danger-outline" id="btn-equipos-purge-all">Purgar todo</button>` +
        `</div>`
      : '')
  );
}
