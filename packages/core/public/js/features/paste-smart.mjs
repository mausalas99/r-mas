...
function onDocumentPaste(ev) {
  ...
  var registro = isPasteTargetEditable(ev.target) ? '' : registroFromPaste(text);
  if (registro) {
    ev.preventDefault();
    ev.stopPropagation();
    openPatientByRegistro(registro);
    return;
  }
...
function openPatientResumen(patient) {
  if (!patient || patient.id == null) return;
  selectPatient(patient.id);
  if (typeof window.switchInnerTab === 'function') window.switchInnerTab('resumen');
}

/** Registro on census → Resumen; else alta por registro (fetches labs), then Resumen. */
function openPatientByRegistro(registro) {
  var existing = findPatientByRegistro(registro);
  if (existing) {
    openPatientResumen(existing);
    return;
  }
  void import('../patient-registro-tunnel.mjs').then(function (m) {
    return m.admitPatientViaRegistroTunnel(registro, { onAdmitted: openPatientResumen });
  });
}
...
function ensureConfirmDom() {
  ...
  cancel.type = 'button';
  cancel.className = 'wb-btn wb-btn-secondary';
