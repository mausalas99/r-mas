...
describe('sweepRoomForPlaintextContent — per-entity (small-piece) pushes', () => {
  ...
  it('stops the whole sweep on a 403 instead of pushing every patient', async () => {
    const state = {
      revision: 500,
      entries: [
        { id: 'p1', note: 'plano p1' },
    ...
    const api = makeFakeApi({
      ...
      pushImpl: () => {
        pushes += 1;
        throw Object.assign(new Error('No eres miembro de esta sala.'), { status: 403 });
      },
    });
    await assert.rejects(sweepRoomForPlaintextContent(api, 'room-1', 'device-owner'), { status: 403 });
