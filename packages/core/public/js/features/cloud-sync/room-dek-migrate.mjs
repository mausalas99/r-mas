...
const ROOM_WIDE_REFUSAL = new Set([401, 403, 426]);
...
async function pushEntityOps(api, roomId, entityKey, entityOps, revisionRef) {
  try {
    ...
    // Auth, membership or version refusals hold for the whole room, not this
    // one entity: stop the sweep. Without this, 8.4.0 re-pushed every patient
    // (100-180KB each, ~5/s) on each reconnect, all 403, and saturated D1.
    if (ROOM_WIDE_REFUSAL.has(Number(err?.status))) throw err;
    return false;
  }
}
