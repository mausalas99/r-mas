...
// Shared test scaffolding: every test below wires createSyncRuntimeCycle
// with the same room/revision/status defaults, and most only supply a
// custom api.push. Pulling that out keeps near-identical setup blocks from
// tripping the jscpd duplication check.
const defaultPull = async () => ({ revision: 1, ops: [] });

function makeRuntime({ api, outbox, getRevision = () => 0, setRevision = () => {}, onStatus = () => {} }) {
  return createSyncRuntimeCycle({
    api: { pull: defaultPull, ...api },
    outbox,
    getRoomId: () => 'room-1',
    getRevision,
    setRevision,
...
function makeCensusOps(count = 20) {
