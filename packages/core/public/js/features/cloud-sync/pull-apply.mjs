...
import { applyLanPatientEntries, applyLanLabSetsToExistingPatients } from '../sync-apply/patient-entries.mjs';
...
/** Full snapshot: the cloud agenda replaces the local one. @param {Record<string, unknown>} agendaMap @param {Record<string, string>} [idMap] */
...
/**
 * Incremental ops: only the events in this batch change; newer local edits win.
 * @param {Record<string, any>} agendaMap @param {Record<string, string>} [idMap]
 */
function mergeCloudAgendaOps(agendaMap, idMap) {
  const incoming = Object.values(agendaMap || {}).filter((item) => item && typeof item === 'object' && item.id);
  if (!incoming.length) return;
  const byId = new Map(storage.getScheduledProcedures().map((ev) => [String(ev.id), ev]));
  for (const item of incoming) {
    const id = String(item.id);
    const cur = byId.get(id);
    if (cur && String(cur.updatedAt || '') > String(item.updatedAt || '')) continue;
    if (item._deleted) byId.delete(id);
    else byId.set(id, remapAgendaPatientIds([item], idMap || {})[0]);
  }
  storage.saveScheduledProcedures([...byId.values()]);
}
...
async function applyFoldedCloudPull(fold, labCounts) {
  ...
  const patientSync = entries.length
    ...
    : { added: 0, updated: 0 };
  const labOnly = {};
  for (const pid of Object.keys(fold.labSidecars)) {
    if (!fold.entries.has(pid)) labOnly[pid] = assembleLabHistoryFromSidecars(fold.labSidecars[pid]);
  }
  patientSync.updated += applyLanLabSetsToExistingPatients(labOnly);
  const todoPatients = applyCloudTodosMap(fold.todos, idMap);
  mergeCloudAgendaOps(fold.agenda, idMap);
