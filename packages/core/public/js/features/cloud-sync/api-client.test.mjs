...
describe('createCloudSyncApi expired-session prompt', () => {
  ...
  /** @param {() => { status?: number, body: object }} respond */
  function setup(respond) {
    const els = { 'nube-session-banner': { hidden: true }, 'profile-nube-session': { hidden: true } };
    globalThis.document = { getElementById: (id) => els[id] || null };
    stubFetch(respond);
    return { els, api: createCloudSyncApi({ getBaseUrl: () => 'https://x', getToken: () => 'tok' }) };
  }

  it('shows the Nube login prompt on auth_required and hides it after a good response', async () => {
    let status = 403;
    const { els, api } = setup(() => (status === 403 ? { status, body: { error: 'auth_required' } } : { body: { rooms: [] } }));

    await assert.rejects(api.listRooms());
    assert.equal(els['nube-session-banner'].hidden, false);
    assert.equal(els['profile-nube-session'].hidden, false);

    status = 200;
    await api.listRooms();
    assert.equal(els['nube-session-banner'].hidden, true);
    assert.equal(els['profile-nube-session'].hidden, true);
  });

  it('leaves the prompt alone for a 403 that is not an expired session', async () => {
    const { els, api } = setup(() => ({ status: 403, body: { error: 'not_member' } }));
    await assert.rejects(api.getRoom(ROOM_ID));
    assert.equal(els['nube-session-banner'].hidden, true);
  });
});
