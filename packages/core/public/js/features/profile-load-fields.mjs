...
export function populateProfileIdentityFields(st) {
  ...
  // Older profiles kept censo names apart; fall back to them until saved into the one list.
  var legacy = { r2: st.residenteR2, r1a: st.residenteR1a || st.residenteR1, r1b: st.residenteR1b };
  ["profesor", "r4", "r2", "r1a", "r1b"].forEach(function (k) {
    var el = document.getElementById("settings-medico-" + k);
    if (el) el.value = medTpl[k] || legacy[k] || "";
  });
...
export function populateProfileOutputFields(st) {
  ...
  var labPortalEl = document.getElementById("settings-lab-portal-url");
  if (labPortalEl) labPortalEl.value = st.labPortalUrl || "";
}
