...
function kclKphosHartmannItems() {
  return [
    {
      id: 'kcl',
...
function syncParacetamolProposal(m) {
  ...
  return syncRecetaProposalsFromSoapSelection(
    'p1',
    m,
    medRecetaByPatient,
    { p1: { a: true } },
...
test('syncRecetaProposalsFromSoapSelection poda y propone desde nuevo SOME', () => {
  ...
  const ok = syncParacetamolProposal(m);
...
test('bucketsFromRecetaItems fusiona KCl + KPO4 en un solo REPOSICIÓN DE POTASIO, oculta HARTMANN', () => {
  const items = kclKphosHartmannItems();
