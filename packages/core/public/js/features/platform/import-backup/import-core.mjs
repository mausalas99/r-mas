...
async function importEntriesWithConflicts(entries, actionLabel) {
  ...
  for (var i = 0; i < entries.length; i += 1) {
    ...
    if (exists) {
      var action = await askConflictAction(entry.patient.nombre || reg || 'sin nombre');
      if (action === 'cancel') {
        out.cancelled = true;
        break;
      }
...
async function importPatientExportPayloads(payloads, sourceLabel) {
  ...
  if (payloads.length > 1) {
    ...
    var res = await importEntriesWithConflicts(entries, 'backup-patient-import');
    if (res.cancelled) {
