...
async function exportSyncBundlePrompt() {
  ...
  var passphrase = await promptText({
    title: 'Passphrase opcional para cifrar',
    message: 'Deja vacío para exportar sin cifrado.',
...
function onSyncBundleFileChosen(ev) {
  ...
  reader.onload = async function() {
    try {
      ...
      if (data.encrypted) {
        var passphrase = await promptText({
          title: 'Este paquete está cifrado. Ingresa la passphrase:',
          inputLabel: 'Passphrase',
          inputType: 'password',
      ...
      var res = await importEntriesWithConflicts(data.entries, 'sync-import');
