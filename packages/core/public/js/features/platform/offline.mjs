...
import { promptText } from '../workbench/confirm.mjs';
import {
...
async function promptForIdleLockPinSetup(reason) {
  ...
  var p1 = await promptText({ title: label, inputLabel: 'PIN', inputType: 'password', confirmLabel: 'Siguiente' });
  ...
  var p2 = await promptText({ title: 'Confirma el PIN:', inputLabel: 'Confirmar PIN', inputType: 'password', confirmLabel: 'Guardar' });
...
async function changeIdleLockPin() {
  ...
  if (existing) {
    var current = await promptText({
      title: 'Ingresa el PIN actual para continuar:',
