...
export function maybeShowGuardiaV7UpgradeCard({ delayMs = 0 } = {}) {
  ...
  const run = () => {
    ...
    el.innerHTML =
      ...
      '<button type="button" class="wb-btn wb-btn-primary wb-btn-lg" id="guardia-v7-upgrade-start">' +
      startLabel +
      '</button>' +
      '<button type="button" class="wb-btn wb-btn-secondary" id="guardia-v7-upgrade-later">Ver después</button>' +
      '</div>';
    main.prepend(el);
