...
const GV7_ACTION_HINT = {
  gv7_guardia_toggle:
    '<p style="margin:10px 0 0;font-size:13px;color:var(--text-muted);">Pulsa el chip resaltado; aparece <strong>Siguiente</strong> al activar el filtro.</p>',
  gv7_lan_wifi:
...
function buildGv7CensoR1Copy(rank) {
  ...
  if (rank === 'R1') {
    return (
      ...
      'En guardia, el chip <strong>Con pendiente</strong> puede acotar aún más.</p>'
    );
...
function getGuardiaV7StepBody(stepId) {
  ...
  const bodies = {
    ...
    gv7_guardia_toggle:
      '<p style="margin:0;line-height:1.5;">El chip <strong>Con pendiente</strong> filtra la grilla a los pacientes que aún tienen algo por resolver este turno.</p>',
    gv7_guardia_exit:
...
export function getGuardiaV7StepHtml(stepId) {
  ...
  return (
    ...
    '<button type="button" class="help-tour-btn" data-onclick="openQuickHelp" data-onclick-args=\'["' +
    articleId +
