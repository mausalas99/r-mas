...
function isGuardiaEntregasFilterActiveForTour() {
  var chip = document.querySelector('.guardia-census-table [data-wb-chip-id="pendiente"]');
...
function applyGuardiaTourLayoutForStep(stepId) {
  void import('../../tour-guards.mjs').then((guards) => {
    ...
    if (guards.shouldShowGuardiaBoardWithoutEntrega(stepId)) {
      // The entrega-phase / roster panel state this used to reset before
      // showing the plain board was removed 2026-09-24 (dead since 7.2.6) —
      // there is nothing left to clear, only the board to refresh.
