...
import { applyBatchStudyGroupNow, openBatchReviewPreview } from './lab-repo-batch-bulk-apply.mjs';
import {
...
function applyFetchKindToTotals(kind, studies, errors, totals, row) {
  ...
  var now = applyBatchStudyGroupNow({ row: row, studies: studies || [], errors: errors || [] }, rt);
  totals.importedPatients += now.imported;
  if (now.reviewText) totals.reviewTexts.push(now.reviewText);
}
...
async function runBatchFetches(selected, range) {
  var totals = { reviewTexts: [], importedPatients: 0, empty: 0, failed: 0 };
  for (var i = 0; i < selected.length; i++) {
    ...
    if (one.kind === 'missing-portal-url') {
      totals.failed += 1;
...
export async function confirmLabRepoBatchImport() {
  ...
  try {
    ...
    // Each patient was saved as it arrived; only the ones needing review wait for the end.
    var applied = { needsReview: totals.reviewTexts.length > 0, importedPatients: totals.importedPatients };
    if (applied.needsReview) openBatchReviewPreview(totals.reviewTexts, rt);
