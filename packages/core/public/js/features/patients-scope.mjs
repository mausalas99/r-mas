...
export function patientMatchesSearch(p) {
  ...
  var q = foldText(patientSearchFilter);
  return [p.nombre, p.registro, p.cuarto, p.cama, p.servicio, p.area].some(function (v) {
    return foldText(v).indexOf(q) !== -1;
  });
}
...
/**
 * Resolve elevatedPatientFilters.sala from storage/profile — cheap, no DOM.
 * Must run before the patient list reads patientsVisibleInSidebar(), even
 * when the Filtros DOM mount isn't in the current tab (e.g. Resumen/Sala),
 * or the census silently falls back to Todas on that tab.
 */
export function syncCensusFilterSalaState() {
  const user = clinicalSessionContext.user;
  try {
  ...
  try {
    resolveCensusTeamFilterState(user);
  } catch (_e) { void _e; }
}
