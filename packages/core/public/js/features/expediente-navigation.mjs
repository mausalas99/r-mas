...
export function switchConsolidatedTab(compositeTab) {
  ...
  if (currentComposite === compositeTab) {
    // Pendientes lives inside the Paciente composite: «Resumen» must still leave it.
    if ((compositeTab === "clinico" || compositeTab === "paciente") && current !== targetGranular) {
...
function scheduleInnerTabPaint(tab, settings, opts, prevInner, prevComposite, nextComposite) {
  ...
  if (needsContentRender) {
    ...
    // Never let the pane paint the previous patient's rows, even for one frame.
    if (isInnerTabRenderedForOtherPatient(targetTab, settings)) {
      renderGranularInnerTab(targetTab, { force: true });
