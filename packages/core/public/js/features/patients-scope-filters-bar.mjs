...
/**
 * Resolve elevatedPatientFilters.teamId from prefs/profile — cheap, no DOM.
 * @param {object} user
 */
export function resolveCensusTeamFilterState(user) {
  ...
  return { teamsForCatalog, teamFilterId };
}
...
export function syncCensusTeamFilterSelect(user) {
  ...
  if (!teamSel) return;
  const { teamsForCatalog, teamFilterId } = resolveCensusTeamFilterState(user);
