...
/** Balance separado por turno del último registro, en el mismo modal. */
export function openEaTurnosBalanceModal() {
  var backdrop = getBackdrop();
  var body = getBodyEl();
  var title = getTitleEl();
  var patient = findActivePatient();
  if (!backdrop || !body || !title || !patient) return;
  ensureMonitoreo(patient);
  var turnos = deriveSnapshot(patient.monitoreo).io.turnos;
  title.textContent = 'Balance por turno';
  body.innerHTML = renderTurnosBalanceHtml(turnos);
  backdrop.classList.add('open');
  backdrop.setAttribute('aria-hidden', 'false');
}
