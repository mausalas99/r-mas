 ...
 * sección BH) del MISMO día que `fecha`. Para calcular RetC cuando Ret y Hto
 * no comparten toma. Solo mismo día (decisión del owner 2026-09-23): un Ret o
 * Hto de otro día no describe la toma actual. Sin `fecha` no hay préstamo.
 * @param {Array<{ fecha?: string, parsedBySection?: { BH?: { [field: string]: number } } }>} history
 * @param {string} fecha DD/MM/YYYY de la toma actual
 * @returns {{ [field: string]: number }}
 */
export function collectPriorBhValuesFromHistory(history, fecha) {
  var out = Object.create(null);
  var day = String(fecha || '').trim();
  if (!history || !history.length || !day) return out;
  for (var i = 0; i < history.length; i++) {
    if (!history[i] || String(history[i].fecha || '').trim() !== day) continue;
    var bh = history[i].parsedBySection && history[i].parsedBySection.BH;
