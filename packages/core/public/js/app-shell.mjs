...
function onLabPortalUrlBlur() {
  var el = document.getElementById('settings-lab-portal-url');
  if (!el) return;
  var v = (el.value || '').trim();
  if (v && !/^https?:\/\/.+/i.test(v)) {
    showToast('Escribe una dirección http(s) válida', 'error');
    return;
  }
  el.value = v;
  var settings = shellCtx.getSettings();
  if (v === (settings.labPortalUrl || '')) return;
  if (v) settings.labPortalUrl = v;
  else delete settings.labPortalUrl;
  localStorage.setItem('rpc-settings', JSON.stringify(settings));
  if (v) showToast('Dirección del portal guardada ✓', 'success');
}
