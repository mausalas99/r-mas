...
describe('app-shell-tab-shortcuts', () => {
  ...
  it('resolveExpedienteCompositeCycle en Interconsulta (sin salida: no es el equipo que da de alta)', () => {
    assert.equal(resolveExpedienteCompositeCycle('paciente', INTER), 'clinico');
    assert.equal(resolveExpedienteCompositeCycle('clinico', INTER), 'paciente');
  });
