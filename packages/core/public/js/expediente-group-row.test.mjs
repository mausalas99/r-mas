...
test('groupSections: clinico follows mode', () => {
  assert.deepEqual(groupSections('clinico', SALA), ['estadoActual', 'eventualidades', 'medAdmin']);
...
test('groupSections: resultados and salida come from the existing maps', () => {
  ...
  assert.deepEqual(groupSections('salida', SALA), ['listado', 'vpo']);
...
test('buildGroupRowModel: paciente is active for datos (todo has its own pill), no sub-pills', () => {
  const todoModel = buildGroupRowModel('todo', SALA);
  const pacTodo = todoModel.find((g) => g.id === 'paciente');
  assert.equal(pacTodo.active, false);
