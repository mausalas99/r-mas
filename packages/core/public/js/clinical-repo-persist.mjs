...
async function runPersistNow(opts = {}) {
  ...
  // Contract above is to resolve { ok: false }, never to reject. Callers are
  // fire-and-forget, so a throw here lands as an unhandledRejection instead of
  // reaching notifySaveResultHook.
  const settled = promise.catch((err) => ({
    ok: false,
    error: String((err && err.message) || err || 'persist_threw'),
  }));

  _persistInFlight = settled;
  try {
    ...
    if (_persistInFlight === settled) _persistInFlight = null;
  }
}
