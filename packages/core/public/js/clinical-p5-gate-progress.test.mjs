...
/** Asserts `hits` is empty, else fails with `message` plus up to 40 file:line hits. */
function assertNoHits(hits, message) {
  assert.equal(
    hits.length,
    0,
    message +
...
describe('clinical P5 gate progress — saveState demotion', () => {
  it('production sources have no saveState( outside app-state allowlist', () => {
    ...
    assertNoHits(hits, 'Unexpected saveState( in production:\n');
  });
});
...
describe('clinical P5 gate progress — export let clinical demotion', () => {
  it('production sources have no export let clinical bindings', () => {
    ...
    assertNoHits(hits, 'Unexpected export let clinical bindings in production:\n');
  });
});
