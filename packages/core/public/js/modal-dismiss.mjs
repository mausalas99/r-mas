...
export function bindBackdropDismiss(backdropEl, requestClose, panelSelector) {
  ...
  backdropEl.addEventListener('click', function (ev) {
    // A button that re-renders its own panel on click is detached by the time
    // the click bubbles here; it was inside the panel, so it is not an outside click.
...
export function focusFirstFocusable(panel) {
  ...
  // The shell × is first in DOM order; start on the content instead.
  var content = focusables.filter(function (el) {
    return !(el.classList && el.classList.contains('wb-modal-close'));
  });
  if (content.length) content[0].focus();
