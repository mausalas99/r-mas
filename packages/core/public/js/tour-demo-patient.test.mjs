...
test('isTourDemoRegistro reconoce registros del tour', () => {
  assert.equal(isTourDemoRegistro('9000095-7'), true);
  assert.equal(isTourDemoRegistro('9000094-3'), true);
...
test('getTourDemoAdmitDefaults sugiere cuarto y cama por registro demo', () => {
  ...
  assert.deepEqual(getTourDemoAdmitDefaults('9000094-3'), {
    servicio: 'MEDICINA INTERNA',
    cuarto: '214',
    cama: '4',
  });
...
test('shouldSelectTourPrimaryAfterLabCommit solo tras alta de García', () => {
  ...
  var list = [
    ...
    { id: DEMO_PATIENT_ID_2, registro: '9000094-3' },
  ];
