...
/** FROTIS is split into one row per part (Cal / Plaq / PlaqObs): dedupe per part, not per section. */
export function frotisPartKey_(key, rowText) {
  if (key !== 'FROTIS') return key;
  return key + ':' + (normalizeLabLine_(rowText).split(' ')[1] || '');
}

export function dedupeSingletonSections_(rows) {
  var singleton = {
    BH: 1, QS: 1, ESC: 1, PFHS: 1, LIPASA: 1, TROP: 1, GASES: 1, PIE: 1, 'LCR:': 1, 'LIQ:': 1,
    HECES: 1, FROTIS: 1, EGO: 1, EU: 1, SEROL: 1, GS: 1, PROT12H: 1, PROT24H: 1, 'INTERPRETACIÓN GASOMETRÍA:': 1,
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (!singleton[labSectionKey_(rowText)]) {
      keep.push(raw);
      continue;
    }
    var key = frotisPartKey_(labSectionKey_(rowText), rowText);
