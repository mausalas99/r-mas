...
const LETTERHEAD_COLLAPSED =
  ...
  'AV. MADERO Y AV. GONZALITOS, COL. MITRAS CENTRO, MONTERREY, N.L. CP. 90001 ' +
  'REPORTE DE RESULTADOS DE LABORATORIO Expediente: 9000006-7 Solicitud: 9000000030 ' +
  'Nombre: FICTICIO SINTETICO DE PRUEBA Fecha Registro: Jun 27 2026 5:43AM ' +
  'Sexo: FEMENINO Ubicación: MEDICINA INTERNA 2 Edad: 52 ' +
  'HEMATOLOGIA HGB 7.85 g/dL 12.0 - 16.0 Campo 90001234 Labo -647* LABX 90001 LABY -647* Feme 1';

const MANGLED_FOOTER =
  'USER Estu 2.53* 89 31 2932 Unid 2.53* UL 11 MD 60 UL 22 ML 98 Campo 90001234 ' +
  'Labo -647* LABX 90001 1017 24.5* RS -647* LABY -647* Feme 1';
...
test('isLikelyLabDataLine ya no acepta sopa digit+letra ni tabs sueltos', () => {
  ...
  assert.equal(isLikelyLabDataLine('Expediente:\t9000006-7\tSolicitud:\t9000000030'), false);
...
test('sanitizeResLabsChunks recorta chrome pegado al final de un panel', () => {
  var out = sanitizeResLabsChunks([
    'BH\tHb 7.85*  Hto 24.5* Expediente: 9000006-7 Solicitud: 9000000030 Nombre: JUAN',
  ]);
...
test('sanitizeResLabsChunks descarta membrete USER/Labo tras cultivo', () => {
  var out = sanitizeResLabsChunks([
    ...
    'USER CP1 -647 Labo -647 LABX 90001 LABY -647 RS -647 MH60 90001 Feme 74',
  ]);
  assert.equal(out.length, 1);
  assert.match(out[0], /KLEBSIELLA/);
  assert.doesNotMatch(out[0], /USER|Labo|LABX|Feme/i);
});
...
test('formatLabsForCensoCompact no muestra blobs no clínicos aunque estén en resLabs viejos', () => {
  ...
  assert.doesNotMatch(text, /UNIVERSIDAD|FORM-XX-|Campo\s+\d+|LABY|LABX|Sistema\s+SOME/i);
});
