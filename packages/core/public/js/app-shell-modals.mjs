...
// Shared modal shell ×: click the dialog's own cancel button when it has one
// (its close logic lives there), else close the topmost registered layer.
// Capture phase: many panels stop click propagation (onclick="event.stopPropagation()").
function wireModalShellCloseHandler() {
  document.addEventListener('click', function (ev) {
    var btn = ev.target && ev.target.closest ? ev.target.closest('.wb-modal-close[data-wb-close]') : null;
    if (!btn) return;
    var dialog = btn.closest('[role="dialog"]');
    var cancel = dialog && dialog.querySelector('[data-wb-cancel]');
    if (cancel) cancel.click();
    else modalDismiss.closeTopmost(ev);
  }, true);
}

/** True while any registered modal, overlay or popover is open. */
export function isAnyModalOpen() {
  return modalDismissInited && modalDismiss.isAnyOpen();
}
