...
export function springTo(el, keyframes, options) {
  ...
  // Motion's controls object is a thenable: returned bare from .then() the
  // promise adopts it and resolves to undefined, so box it.
  var controls = import('motion').then(function (mod) {
    return { c: mod.animate(el, kf, animOpts) };
  });
  return {
    stop: function () {
      controls.then(function (box) {
        try { box.c.stop(); } catch (_e) { void _e; }
      }, function () {});
    },
    finished: controls.then(function (box) { return box.c.finished; }),
  };
}
