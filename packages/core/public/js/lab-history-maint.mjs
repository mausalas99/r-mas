...
import { getPatients, getNotes, getIndicaciones, getLabHistory, getMedRecetaByPatient, getListadoProblemas, getVpoByPatient, getMedPharmProfileByPatient, persistClinicalState } from './app-state.mjs';
...
function syncSetRefsAndHora(set) {
  ...
  // sourceText de un set fusionado (2+ reportes) trae más de un "Fecha Registro":
  // extractLabReportHora solo ve el primero, así que su hora es ambigua para el
  // set combinado — no debe pisar una hora ya vacía a propósito (ver
  // mergeLabHistorySetsCluster en lab-panel-history-dedupe.mjs).
  var reportCount = (String(set.sourceText).match(/Fecha\s+Registro\s*:/gi) || []).length;
  if (reportCount <= 1) {
