...
/** Own trap while open, so the form that opened it stops pulling focus back. */
var focusTrap = null;
...
export function openTodoDueModal(opts) {
  ...
  if (focusTrap) focusTrap.unwire();
  else focusBeforeOpen = document.activeElement;
  focusTrap = wireFocusTrap(backdrop.querySelector('.todo-due-modal') || backdrop);
