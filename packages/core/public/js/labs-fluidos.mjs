...
/**
 * Proteína de líquido → g/dL, convertida con la unidad que imprime el reporte.
 *
 * Antes se adivinaba por el tamaño del número (`>=1000` ÷1000, `>=100` ÷100).
 * Las dos bandas suponen convenciones distintas y chocan entre 100 y 999: 900
 * mg/dL son 0.9 g/dL, no 9. Ese es justo el rango del trasudado, así que un
 * derrame por insuficiencia cardíaca salía como exudado por Light.
 *
 * Sin unidad no hay conversión segura y se devuelve `null`: el criterio de
 * Light se queda sin evaluar y el resultado sale como incompleto, que es
 * preferible a un veredicto equivocado.
 *
 * @param {string|number} valStr
 * @param {string} [unidad] Unidad tal como la imprime el laboratorio.
 * @returns {number|null}
 */
export function normalizarProteinasFluidoGdl_(valStr, unidad) {
  var n = toNum_(String(valStr || '').replace(/[A-Z*]$/i, ''));
  if (n == null) return null;
  var u = String(unidad || '').toUpperCase().replace(/\s/g, '');
  if (u.indexOf('MG/DL') === 0) return n / 1000;
  if (u.indexOf('G/DL') === 0) return n;
  if (u.indexOf('G/L') === 0) return n / 10;
  return null;
}
...
function extraerSueroParaLight_(textoBruto, bloqueCito) {
  ...
  return {
    // La proteína total sérica se reporta en g/dL (rango 6.1 - 7.9). No pasa
    // por el escáner del citoquímico, así que la unidad se declara aquí.
    protGdl: normalizarProteinasFluidoGdl_(protData.valor, 'g/dL'),
...
function fmtProteinaFluido_(valStr, unidad) {
  var g = normalizarProteinasFluidoGdl_(valStr, unidad);
...
function buildLightPleural_(bloque, pleuralProtRaw, pleuralLdhRaw, textoBruto, protUnit) {
  var pleuralProt = normalizarProteinasFluidoGdl_(pleuralProtRaw, protUnit);
...
export function parseCitoquimicoLiquidosParsed(textoBruto, serumOpts) {
  ...
  var pmnInfo = parsePmnField_(fields.pmn, leuNum, fields.pmnUnit);
  var lightTxt = esPleural
    ? buildLightPleural_(bloque, fields.prot, fields.ldh, textoBruto, fields.protUnit)
    : '';
  var gasaVal = null;
  var serumAlb = null;
  var asciticAlb = null;
  ...
  return {
    ...
    serumAlb: serumAlb,
    gasaVal: gasaVal,
    protGdl: normalizarProteinasFluidoGdl_(fields.prot, fields.protUnit),
