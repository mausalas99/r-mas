...
/** Builds a fake doc plus a single-focusable outer modal (trigger, panel, backdrop). */
function setupOuterModal() {
...
test('el trap de foco sigue al modal anidado más arriba y restaura en cascada', () => {
  var priorDocument = globalThis.document;
  try {
    var setup = setupOuterModal();
    var fakeDoc = setup.fakeDoc;
    var trigger = setup.trigger;
    var outerFirst = setup.outerFirst;
    var outerBackdrop = setup.outerBackdrop;
