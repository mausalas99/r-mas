...
test('shouldShowConsolidatedTab: Paciente inner groups exclude Resultados', () => {
  ...
  assert.equal(shouldShowConsolidatedTab('itab-salida', INTER), false);
...
test('defaultGranularForConsolidatedTab returns sensible defaults per mode', () => {
  ...
  assert.equal(defaultGranularForConsolidatedTab('salida', INTER), 'listado');
...
test('getSalidaSections only in sala', () => {
  assert.deepEqual(getSalidaSections(SALA), ['listado', 'vpo']);
...
test('sala salida sections are listado and vpo', () => {
  assert.deepEqual(getSalidaSections(SALA), ['listado', 'vpo']);
});
