...
function idsOf(cluster) {
  return cluster.map(byId);
}
function isTipoGaso(x) {
  return x.tipo === 'gaso';
}
function hasGaso(x) {
  return !!x.hasGaso;
...
describe('lab-consolidation-cluster', () => {
  it('clusterByTimeWindow une tomas consecutivas ≤2 h', () => {
    ...
    var clusters = clusterByTimeWindow(items, byMs);
    assert.equal(clusters.length, 2);
    assert.deepEqual(idsOf(clusters[0]), ['a', 'b']);
    assert.deepEqual(idsOf(clusters[1]), ['c']);
  });
  ...
  it('clusterLabworkByTimeWindow une gaso solitaria con labs aunque >2 h', () => {
    ...
    var clusters = clusterLabworkByTimeWindow(items, byMs, isTipoGaso);
    assert.equal(clusters.length, 1);
    assert.deepEqual(idsOf(clusters[0]), ['gaso', 'labs']);
  });
  ...
  it('clusterLabworkByTimeWindow gaso solitaria elige el labs más cercano', () => {
    ...
    var clusters = clusterLabworkByTimeWindow(items, byMs, isTipoGaso);
    assert.equal(clusters.length, 2);
    assert.deepEqual(idsOf(clusters[0]), ['labsAm']);
    assert.deepEqual(idsOf(clusters[1]), ['gaso', 'labsPm']);
  });
  ...
  it('clusterLabworkByTimeWindow no une gaso seriada lejana (>2 h del labs)', () => {
    ...
    assert.deepEqual(clusters.map(idsOf), [['labs', 'gaso1'], ['gaso2'], ['gasoFar']]);
  });
  ...
  it('clusterLabworkByTimeWindow une labs+gaso pero no gaso+gaso', () => {
    ...
    var clusters = clusterLabworkByTimeWindow(items, byMs, isTipoGaso);
    assert.equal(clusters.length, 2);
    assert.deepEqual(idsOf(clusters[0]), ['labs', 'gaso1']);
    assert.deepEqual(idsOf(clusters[1]), ['gaso2']);
  });
  ...
  it('clusterLabworkByTimeWindow empareja la gaso más cercana al labs', () => {
    ...
    var clusters = clusterLabworkByTimeWindow(items, byMs, isTipoGaso);
    assert.equal(clusters.length, 2);
    assert.deepEqual(idsOf(clusters[0]), ['gasoEarly']);
    assert.deepEqual(idsOf(clusters[1]), ['labs', 'gasoNear']);
  });
  ...
  it('clusterLabworkByTimeWindow no absorbe 2ª gaso en set que ya trae GASES', () => {
    ...
    var clusters = clusterLabworkByTimeWindow(items, byMs, hasGaso);
    assert.equal(clusters.length, 2);
    assert.deepEqual(clusters.map(idsOf), [['labsGaso'], ['gaso2']]);
  });
  ...
  it('clusterLabworkByTimeWindow une clones con los mismos GASES', () => {
    ...
    var clusters = clusterLabworkByTimeWindow(items, byMs, hasGaso, undefined, byGasKey);
    assert.equal(clusters.length, 1);
    assert.deepEqual(idsOf(clusters[0]), ['a', 'b', 'c']);
  });
  ...
  it('clusterLabworkByTimeWindow une GASES lean vs rich (misma toma)', () => {
    ...
    var clusters = clusterLabworkByTimeWindow(items, byMs, hasGaso, undefined, function (x) {
  ...
  it('clusterByDayTipoAndTimeWindow agrupa labs y gaso del mismo día', () => {
    ...
    var clusters = clusterByDayTipoAndTimeWindow(items, byDay, byTipo, byMs);
    assert.equal(clusters.length, 3);
    assert.equal(clusters[0].length, 2);
