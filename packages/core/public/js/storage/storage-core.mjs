...
/**
 * Write one clinical blob: SQLCipher (desktop) through the blob cache, else localStorage.
 * @param {string} blobKey @param {string} lsKey @param {unknown} value @param {string} eventType
 */
function writeClinicalBlob(blobKey, lsKey, value, eventType) {
  if (isSessionScopedWebClient()) return;
  const json = JSON.stringify(value);
  if (_blobCache) {
    _blobCache[blobKey] = json;
    if (isDbMode()) {
      void persistSaveAll({ [blobKey]: value }, { eventType, meta: { source: 'storage.' + blobKey } });
      invalidateParsed(blobKey);
      return;
    }
  }
  ...
  safeLocalStorageSet(lsKey, json);
  invalidateParsed(blobKey);
}

function writeTodosMap(map) {
  writeClinicalBlob('todos', 'rpc-todos', map, 'clinical.todos_save');
}
export {
  invalidateParsed,
