...
// Shared accessors + small test helpers — kept local, not exported.
function getDay(s) {
  return s.day;
}
function getTipo(s) {
...
function isGasoOnlyResLabs(s) {
  return isGasometriaOnlyResLabs(s.resLabs);
}
function hasGasoFlag(s) {
  return !!s.hasGaso;
}

function outlierGroups(sets) {
  return findOutlierLabConsolidationGroups(sets, getDay, getTipo, getMs);
}
function autoMergeJobs(sets, outlierKeys, getHasGaso) {
  return buildLabConsolidationMergeJobs(sets, getDay, getTipo, getMs, outlierKeys || null, getHasGaso);
}
function sameDateTimeJobs(sets) {
  return buildSameDateTimeLabMergeJobs(sets, getTipo, isGasoOnlyResLabs);
}
function candidates(sets) {
  return listLabConsolidationCandidates(sets, getDay, getTipo);
}
function assertSingleAutoJob(jobs) {
...
describe('lab-consolidation-plan — outliers y same-datetime', () => {
  it('findOutlierLabConsolidationGroups detecta mismo día con >2 h entre clusters', () => {
    ...
    var outliers = outlierGroups(sets);
  ...
  it('buildSameDateTimeLabMergeJobs une BH+GASES con QS+GASES misma hora', () => {
    ...
    var jobs = sameDateTimeJobs(sets);
    assert.equal(jobs.length, 1);
  ...
  it('buildLabConsolidationMergeJobs auto solo une ≤2 h', () => {
    ...
    var jobs = autoMergeJobs(sets);
    assertSingleAutoJob(jobs);
...
describe('lab-consolidation-plan — auto merge y manual', () => {
  ...
  it('buildLabConsolidationMergeJobs no fusiona 2ª gaso con labs+gaso ya pareados', () => {
    ...
    var jobs = autoMergeJobs(sets, null, hasGasoFlag);
  ...
  it('buildLabConsolidationMergeJobs outlier une día completo', () => {
    ...
    var jobs = autoMergeJobs(sets, [gk]);
    assert.equal(jobs.length, 1);
  ...
  it('listLabConsolidationCandidates omite mixtos y sin día', () => {
    ...
    var out = candidates(sets);
    assert.deepEqual(getIds(out), ['a', 'd']);
  });
  ...
  it('buildManualLabConsolidationJobs solo fusiona grupos elegidos', () => {
    ...
    assert.deepEqual(getIds(jobs[0].sets), ['a', 'b']);
    assert.deepEqual(getIds(jobs[1].sets), ['c', 'd']);
  });
