...
test('formatPatientNameForCenso conserva nombre completo', () => {
  assert.equal(formatPatientNameForCenso('SINTETICO EJEMPLO CASOAZ CARLOS'), 'SINTETICO EJEMPLO CASOAZ CARLOS');
...
test('buildCensusPayload usa nombre completo del paciente', () => {
  var payload = buildCensusPayload({
    settings: {},
    patients: [{ id: '1', nombre: 'FICTICIO SINTETICO CASOBG', archived: false }],
    includeArchived: false,
    labHistoryByPatient: { 1: [] },
    medRecetaByPatient: {},
    todosByPatient: { 1: [] },
  });
  assert.equal(payload.rows[0].pacienteNombre, 'FICTICIO SINTETICO CASOBG');
});
...
test('formatPacienteMetaForCenso lista servicios interconsultantes', () => {
  var meta = formatPacienteMetaForCenso(
    { registro: '7', interconsultServiceIds: ['card', 'unknown-svc', 'nef'] },
    {},
    'Equipo Azul'
  );
  var lines = meta.split('\n');
  assert.equal(lines[1], 'IC: Cardiología, Nefrología');
  assert.equal(lines[2], 'Equipo: Equipo Azul');
  assert.doesNotMatch(formatPacienteMetaForCenso({ registro: '7', interconsultServiceIds: [] }), /IC:/);
});
...
function buildPendientesPayload(todos) {
  return buildCensusPayload({
    settings: {},
    patients: [{ id: '1', nombre: 'T', archived: false, diagnosticosList: ['X'] }],
    includeArchived: false,
    labHistoryByPatient: { 1: [] },
    medRecetaByPatient: {},
    todosByPatient: { 1: todos },
  });
}

test('buildCensusPayload pendientes — max 3 alta, sin mezclar media', () => {
  var payload = buildPendientesPayload([
