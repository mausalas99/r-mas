...
- [x] Click a patient card to open their Resumen, "← Tablero"/Esc to return {#ic-drilldown}
  ...
  by: claude
- [x] Wire the "Terminar guardia y repartir pacientes" button to the rollover logic {#ic-rollover-button}
  tech: this row was stale — a later uncommitted pass in the same session dropped the per-lane rollover button from interconsulta-team-board.mjs (test added asserting it's gone) without updating this row, so it shipped 8.2.2 with no UI trigger at all (only the backend IPC handler survived). Caught during 8.2.2 release prep; owner asked to restore it. Rebuilt as one always-visible button top-left of the board header (interconsulta-mode-chrome.mjs's renderInterconsultaBoardView, .ic-board-header) instead of per-lane — calls dbClinicalInterconsultaRollover via window.rplusDb/electronAPI with today's on-call team (getInterconsultaTeamRoles) as postCallTeamId and the rest as remainingTeamIds; toasts instead of hiding when there's no guardia team today or demo mode is active.
  by: claude
...
- 2026-08-26, owner: restore the "Terminar guardia y repartir pacientes" rollover button, always visible top-left of the board header (not conditional on there being a guardia team today — explains itself via toast instead of hiding). It had been silently dropped from the board mid-session (test added asserting its removal, PLAN.md not updated) and was about to ship in 8.2.2 with zero UI trigger.
