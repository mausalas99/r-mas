...
- 2026-09-19, claude: sub-phase B of the monorepo split closed on two Jev calls. The "IM-only screens" sort waits for Phase C (0.94) — a file missing from `packages/hf` is not proof it is IM-only, since HF’s content stops at 2026-08-24 and its 81-file drift dropped import lines from its copies of the shared importers; moving first risks putting files in `packages/im` that HF imports, which `im-not-to-hf` then forbids. The electron-builder config stays in the root `package.json` (0.96) — its paths were already re-rooted to `packages/...` by A3/A4, the root manifest already is the IM app manifest, and the paid-HF boundary rests on `im-not-to-hf` plus the `files` allowlist, not on where the config lives.
- 2026-09-19, claude: per-file Jev screens do not sort this codebase, measured twice. The 114 sub-phase B candidates came back in one flat band (0.13-0.61, nothing at or above 0.80), and Phase C’s HF drift sort is mostly below the 0.5 floor (52 of 79 lib rows, 438 of 600 public rows), leaving only 5 confident “belongs in core” files. Import-graph structure separates the files instead; use Jev for the single framing decision over that structure, not for N per-file calls.
