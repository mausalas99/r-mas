...
- [x] Keep the waiting-changes queue safe if the app closes {#cloud-sync-outbox-sqlcipher}
  ...
  from: agent
- [x] Stop Nube saying "al día" when a change never arrived {#cloud-sync-idle-stall-fix}
  by: claude
  tech: three paths that left status idle with work undone. (1) applyRoomWsOpsMessage (sync-runtime-cycle.mjs) bumped the local revision even when decryptOpsFromPull left an op as ciphertext (no room DEK yet) and pull-apply had dropped it — flushSignal's `rev > local` check then went no-op and the next `since` pull skipped it forever. New hasLockedOpValue (cloud-sync-crypto-wire.mjs) checks the whole value AND the PATIENT_LOCKED_FIELD_KEYS sub-keys, which api-client.mjs's old top-level-only isEncryptedEnvelope scan missed; api-client.pull now sets data.locked and reconcileServerRevision (sync-runtime-pull-push.mjs) holds the revision back on it, so the ops stay in the next pull's window and land once the DEK loads. (2) interno/vitals.js committed a phone's vital signs and bumped rooms.revision with no notifyRoomRevision call at all — sync.js:551 was the only production caller — so desktop waited out its 90s safety poll; it now broadcasts like handleMutations. (3) room-sync-notify.js fired the DO subrequest with `void`, so the Worker could cancel it at request teardown and the committed push reached nobody; now awaited (ctx.waitUntil is unreachable — worker-app.mjs's fetch(request, env) takes no ExecutionContext). 136 client + 189 worker tests pass via test:one, build:ui clean, metrics:check debt back to 0. Not live-verified.
files: [cloud/sync-worker/src/room-sync-hub.js, cloud/sync-worker/src/room-live.js, public/js/live-sync-room.mjs, public/js/patient-team-assign-ui.mjs, public/js/features/cloud-sync/**, public/js/features/cloud-mobile/outbox-memory.mjs, cloud/sync-worker/src/d1-errors.js, cloud/sync-worker/src/errors.js, cloud/sync-worker/src/worker-app.mjs, cloud/sync-worker/src/mutation-guard.mjs, cloud/sync-worker/src/routes.js, lib/db/cloud-outbox.mjs, cloud/sync-worker/src/room-sync-notify.js, cloud/sync-worker/src/interno/vitals.js]
needs: [db]
