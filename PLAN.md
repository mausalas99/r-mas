...
- [x] Stop the debt gate measuring the vendored HF app and the plan archive {#shrink-gate-scope}
  by: claude
  tech: new GATE_EXCLUDED_RE in scripts/metrics/constants.mjs (/^(packages\/hf\/|docs\/)/), consumed by scripts/metrics/tracked-size.mjs and scripts/ci/no-duplicate-files.mjs so both gates cut the same tree. The monorepo split's HF subtree-add put 477,537 lines and 1,170 byte-identical pairs inside the gate, and un-gitignoring docs/superpowers added 81,164 more; neither is product code. trackedLoc had no code filter at all (only moduleCount's MODULE_RE does), so it counted markdown like code. After the filter trackedLoc measures 346,176 against the untouched 353,700 baseline and moduleCount 1,269 against 1,272, so baseline.json needed no raise at all. Full metrics:check chain green.
  from: agent
...
- 2026-09-19, Jev (routed by claude): the debt gate was red on main (`tracked 912214 > baseline 353700`). Jev chose scoping the gate to `packages/core` and `packages/im` and skipping `packages/hf` until Phase C cleans it (0.61) over raising the baseline to 912,214 and blessing all 1,170 duplicate pairs (0.00). A second call found the non-HF overage was not code at all: non-HF measured 434,677 against the 353,700 baseline, a gap of 80,977, and `docs/superpowers` is 81,164 lines of plan markdown un-gitignored the same day in `c19e871`. `trackedLoc` counts every tracked non-binary file with no code filter, so Jev also chose excluding `docs/superpowers` from the counter over baselining it in (0.97). Excluding both put the count under the baseline but left only 69 lines of headroom, because this change's own required handoff and plan entries are 94 lines of markdown; a third call chose excluding all of `docs/` over leaving the exclusion at `docs/superpowers` (0.95), since CLAUDE.md and AGENTS.md mandate a 50-to-90-line handoff entry per finished job and the ratchet exists to catch product-code growth. Implemented as #shrink-gate-scope. Outcome neither original option predicted: `baseline.json` was left untouched and needs no owner sign-off, with 7,524 lines of headroom.
