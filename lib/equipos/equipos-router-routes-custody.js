...
const { scheduleLanPush, scheduleLanWaitlistHeadPush } = require('./equipos-router-routes-push.js');
...
function mountEquiposCustodyRoutes(r, ctx) {
  ...
  r.post('/return', postLimiter, express.json({ limit: '3mb' }), ctx.authEquipos, async (req, res) => {
    try {
      ...
      const kind = req.body.deviceType === 'lumify' ? 'lumify_return' : 'device_available';
      void scheduleLanPush(req.equiposDb, kind, {
        deviceType: req.body.deviceType,
