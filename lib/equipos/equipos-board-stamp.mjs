...
function wlStampRow(wl) {
  return [wl?.c ?? 0, wl?.max_j || ''];
}

function alertsStampRow(alerts) {
  return [alerts?.c ?? 0, alerts?.max_c || ''];
}

function evStampRow(ev) {
  return [ev?.max_ev || ''];
}
...
export function formatBoardStamp(dev, wl, alerts, ev) {
  return [...devStampRow(dev), ...wlStampRow(wl), ...alertsStampRow(alerts), ...evStampRow(ev)].join(
    '|'
  );
}
