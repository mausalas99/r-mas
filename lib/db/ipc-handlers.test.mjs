...
describe('db IPC handlers — patient assign', () => {
  ...
  it('db:clinical-interconsulta-rollover moves a post-call team patient to a remaining team', async () => {
    const boot = await harness.ipc.invoke('db:clinical-access-bootstrap', {
      clientId: 'ipc-test-client-rollover',
      rank: 'R4',
      preferredUsername: 'rollover_host',
    });
    const postCall = await harness.ipc.invoke('db:clinical-teams-create', {
      name: 'Equipo A',
    ...
    const remaining = await harness.ipc.invoke('db:clinical-teams-create', {
      name: 'Equipo B',
      service: 'Interconsultas',
      sala: 'Interconsultas',
      createdBy: boot.user.userId,
    ...
    await harness.ipc.invoke('db:clinical-save-all', {
      blobs: { patients: JSON.stringify([{ id: 'p-ipc-roll-1', nombre: 'PACIENTE ROLLOVER' }]) },
      auditMeta: { eventType: 'test.seed_patients' },
    });
    await harness.ipc.invoke('db:clinical-assign-patient-to-team', {
      patientId: 'p-ipc-roll-1',
      teamId: postCall.team.team_id,
      effectiveAt: '2026-06-12T00:00:00.000Z',
    });
    const rollover = await harness.ipc.invoke('db:clinical-interconsulta-rollover', {
      postCallTeamId: postCall.team.team_id,
      remainingTeamIds: [remaining.team.team_id],
      nowIso: '2026-06-13T00:00:00.000Z',
    });
    assert.equal(rollover.ok, true);
    assert.equal(rollover.result.movedCount, 1);
    const active = await harness.ipc.invoke('db:patient-active-team-id', {
      patientId: 'p-ipc-roll-1',
      nowIso: '2026-06-13T12:00:00.000Z',
    });
    assert.equal(active.teamId, remaining.team.team_id);
  });
});
