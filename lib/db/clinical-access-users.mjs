...
import { CLINICAL_RANKS } from '../clinical-ranks.mjs';
import {
 ...
 */
export function ensureClinicalUser(db, { clientId, rank = 'Team', clinicalName, sala }) {
  const username = String(clientId || 'local-device').slice(0, 64);
  const safeRank = CLINICAL_RANKS.includes(rank) ? rank : 'Team';
...
function assertAdminChangeAllowed(db, userId, adminFlag, adminAccessCode) {
  if (adminFlag === null) return;
  const currentAdmin = db
    .prepare('SELECT is_program_admin FROM users WHERE user_id = ?')
    .get(userId);
  if ((currentAdmin?.is_program_admin === 1) === (adminFlag === 1)) return;
  if (!verifyAdminAccessCode(db, adminAccessCode)) {
...
export function upsertClinicalProfile(
  ...
  if (existing) {
    assertAdminChangeAllowed(db, userId, fields.adminFlag, adminAccessCode);
...
function parseCloudIdentityProvisionOpts(opts) {
  ...
  return {
    ...
    safeRank: CLINICAL_RANKS.includes(String(opts?.rank || '')) ? String(opts.rank) : 'Team',
...
/**
 * Admin updates rank/profile for a Nube @usuario on this Mac.
 *
 * @param {import('better-sqlite3').Database} db
...
export function setClinicalUserProfileFromAdmin(db, opts) {
  ...
  const safeRank = CLINICAL_RANKS.includes(String(opts?.rank || '')) ? String(opts.rank) : 'Team';
