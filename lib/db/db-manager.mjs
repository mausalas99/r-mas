...
export function createDbManager({ userDataPath, safeStorage, getClientId }) {
  ...
  async function openDatabaseConnection({ keyHex } = {}) {
    ...
    db = conn;
    state = 'unlocked';
    activeKeyHex = keyHex || null;
  }

  async function unlockWithKeyHex(keyHex) {
    await openDatabaseConnection({ keyHex });
  }

  async function unlockPlain() {
  ...
  /**
   * @returns {Promise<{ recoveryCode: string|null, isNew: boolean }>}
   */
  async function setupRecoveryKey(keyHex, { forceRotate = false } = {}) {
  ...
  async function unwrapRecoveryKeyFromCode(bootstrap, code) {
  ...
  async function unlockWithPassphrase(passphrase, { remember, setup } = {}) {
    ...
    try {
      const recoverySetup = await setupRecoveryKey(keyHex);
  ...
  async function unlockWithRecoveryCode(code) {
    ...
    for (let i = 0; i < candidates.length; i += 1) {
      ...
      const attempt = await unwrapRecoveryKeyFromCode(bootstrap, candidate);
      if (attempt) {
        keyHex = attempt;
        usedLegacy = candidate === LEGACY_RECOVERY_CODE;
        break;
      }
    }
    ...
    if (usedLegacy || Number(bootstrap.recovery_version) < 2) {
      try {
        const rotated = await setupRecoveryKey(keyHex, { forceRotate: true });
  ...
  /**
   * Opens the clinical DB without a user passphrase (encryption deferred).
   * @returns {Promise<{ ok: true }>}
   */
  async function ensureUnlocked() {
    if (state === 'unlocked') return { ok: true };
    loadNativeDatabase();
    if (await tryUnlockRemembered()) return { ok: true };
