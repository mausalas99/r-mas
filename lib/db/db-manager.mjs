...
export function createDbManager({ userDataPath, safeStorage, getClientId }) {
  ...
  async function setupRecoveryKey(keyHex) {
    const bootstrap = readUnlockMeta(userDataPath);
    const recoverySalt = newSalt();
    const wrappingKeyHex = await deriveRecoveryWrappingKeyHex(recoverySalt);
  ...
  async function unlockWithRecoveryCode(code) {
    if (typeof code !== 'string' || code !== 'r+123') {
      recordUnlockFail();
    ...
    try {
      ...
      recordUnlockFail();
      const err = new Error('Invalid recovery data');
      err.code = 'DB_RECOVERY_NOT_CONFIGURED';
      throw err;
    }
    let keyHex;
    try {
      const wrappingKeyHex = await deriveRecoveryWrappingKeyHex(recoverySalt);
      keyHex = unwrapKeyForRecovery(wrapped, wrappingKeyHex);
    } catch {
      recordUnlockFail();
      const err = new Error('Invalid recovery code');
      err.code = 'DB_UNLOCK_FAILED';
