...
function mergeTeamsPhase(db, local, incoming, localTeams) {
  ...
  mergeTeams(db, localTeams, incoming.teams || []);
...
export function mergeClinicalOpsSnapshot(db, incoming, localSnapshot = null) {
  ...
  const archivedTeamIds = mergeTeamsPhase(db, local, incoming, localTeams);
