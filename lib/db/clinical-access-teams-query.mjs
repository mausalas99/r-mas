...
import {
  ...
  validateSelfJoinTeamMembership,
...
export function resolveTeamByInviteCode(db, code) {
  ...
  const matches = listRotationVisibleTeams(db).filter((team) => {
...
export function listTeamsBySala(db, { sala, forUserId, allSalas } = {}) {
  ...
  // Includes staged (rotation_active=0) teams so R4/Admin can build and staff
  // next month's rosters before "Iniciar nueva rotación" — validateSelfJoinTeamMembership
  // below still blocks residents from self-joining a team that isn't live yet.
...
export function getClinicalScopeContext(db, userId) {
  ...
  const teams = listTeamsForRenderer(db, uid).map((team) => ({
    ...withEffectiveTeamSala(db, team),
    members: listTeamMembers(db, team.team_id),
  }));
  const teams_archived = listArchivedTeams(db).map((team) => withEffectiveTeamSala(db, team));
