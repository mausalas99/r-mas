...
function normalizeTeamInput(teamInput) {
  const input = teamInput && typeof teamInput === 'object' ? teamInput : {};
  return {
    name: String(input.name),
    service: String(input.service),
    subAreaFraction: input.subAreaFraction ?? null,
    onCallDayIndex: Number(input.onCallDayIndex),
    sala: input.sala ?? null,
    teamLeaderName: input.teamLeaderName ?? null,
    createdBy: input.createdBy ?? null,
    leaderUserId: input.leaderUserId ?? input.createdBy ?? null,
  };
}

export function createTeam(db, teamInput) {
  const n = normalizeTeamInput(teamInput);
  const teamId = crypto.randomUUID();
  const rotationActive = resolveRotationActiveForNewTeam(db, n.sala);
  db.prepare(
  ...
  return {
    ...
    name: n.name,
    service: n.service,
    sub_area_fraction: n.subAreaFraction,
    on_call_day_index: n.onCallDayIndex,
    sala: n.sala,
    team_leader_name: n.teamLeaderName,
    created_by: n.createdBy,
    leader_user_id: n.leaderUserId,
 ...
 */
function resolveNextTeamName(name, team) {
  return name != null ? String(name).trim() : String(team.name || '').trim();
}

function resolveNextTeamSala(db, sala, team) {
  return sala != null
    ? String(sala).trim()
    : String(team.sala || effectiveTeamSala(db, team) || '').trim();
}

function resolveNextSucceedsTeamId(db, tid, team, succeedsTeamId) {
  if (succeedsTeamId === undefined) return team.succeeds_team_id ?? null;
...
/**
 * Manual override for the auto-inferred `rotation_active` (see
 * `resolveRotationActiveForNewTeam`) — an admin moves a team between "esta
 * rotación" and "próxima rotación" by hand, since the auto rule (sala already
 * has an active team → new team stages next) doesn't hold for every real
 * case (late joiners, teams created for the current cycle after others).
 * @param {number|undefined} rotationActive
 * @param {{rotation_active: number}} team
 */
function resolveNextRotationActive(rotationActive, team) {
  if (rotationActive === undefined) return Number(team.rotation_active);
  const next = Number(rotationActive);
  if (next !== 0 && next !== 1) throw new Error('Estado de rotación inválido.');
  return next;
}
...
export function updateTeam(db, teamId, { name, sala, succeedsTeamId, rotationActive, callerUserId }) {
  ...
  const nextName = resolveNextTeamName(name, team);
  const nextSala = resolveNextTeamSala(db, sala, team);
