...
export function hasCurrentRotationTeams(db) {
  return !!db
    .prepare(
...
export function resolveRotationActiveForNewTeam(db) {
  return hasCurrentRotationTeams(db) ? 0 : 1;
}

export function createTeam(db, teamInput) {
  const input = teamInput && typeof teamInput === 'object' ? teamInput : {};
  const teamId = crypto.randomUUID();
  const rotationActive = resolveRotationActiveForNewTeam(db);
  db.prepare(
  ...
  const staged = db
    .prepare(
      `SELECT t.team_id, t.name, t.service, t.sub_area_fraction, t.on_call_day_index, t.created_by,
              t.archived_at, t.sala, t.team_leader_name, t.leader_user_id, t.rotation_active
       FROM teams t
