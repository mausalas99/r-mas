...
function openDb() {
  ...
  applyMigrations(db);
  db.pragma('foreign_keys = ON'); // production runs FK ON
  return db;
}
...
describe('mergeTeams — skipped rows are logged, not silent', () => {
  it('warns when a team row cannot satisfy a DB constraint', () => {
    ...
    try {
      ...
      const skipped = mergeTeams(db, [], [incoming]);
      ...
      assert.equal(skipped, 1, 'the skip count must surface to the caller for the sync trace');
    } finally {
...
describe('mergeTeams — a dangling succeeds_team_id must not drop the team', () => {
  it('keeps the team row and nulls the missing predecessor link', () => {
    const db = openDb();
    try {
      mergeTeams(db, [], [{
        team_id: 'new-team', name: 'Dra. Melissa', service: 'Sala', on_call_day_index: 1,
        sala: 'Sala 1', rotation_active: 0, succeeds_team_id: 'absent-predecessor',
      }]);
      const row = db.prepare('SELECT team_id, succeeds_team_id FROM teams WHERE team_id = ?').get('new-team');
      assert.ok(row, 'the team must survive a predecessor that this peer has never seen');
