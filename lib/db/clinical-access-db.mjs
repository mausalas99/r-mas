...
/**
 * Registered LAN handles (excludes legacy device usernames).
 * @param {import('better-sqlite3').Database} db
 */
export function listLanDirectoryUsers(db) {
  return listClinicalUsers(db).filter((row) =>
    isValidUsernameFormat(normalizeUsername(row?.username || ''))
  );
}
...
export function getClinicalScopeContext(db, userId) {
  ...
  let assignments = [];
  try {
    assignments = fetchIncomingAssignments(db, nowIso);
  } catch (err) {
    console.error(
      '[clinical-scope] fetchIncomingAssignments failed:',
...
export function updateTeam(db, teamId, { name, sala, callerUserId }) {
  ...
  db.prepare(
    `UPDATE teams SET name = ?, sala = ?, team_leader_name = ? WHERE team_id = ?`
  ).run(nextName, nextSala, nextName, tid);
