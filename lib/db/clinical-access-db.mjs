...
export function upsertClinicalProfile(db, { userId, clinicalName, rank, sala, username }) {
 ...
 */
export function listTeamsBySala(db, { sala, forUserId } = {}) {
  const salaFilter = String(sala || '').trim();
  const uid = String(forUserId || '');
  return listActiveTeams(db)
    .filter((team) => !salaFilter || String(team.sala || '') === salaFilter)
    .map((team) => {
