...
import { migrateToV24ActiveGuardiasIndex } from './schema-migrate-v24-active-guardias-index.mjs';
import { migrateToV25InterconsultUnder } from './schema-migrate-v25-interconsult-under.mjs';
...
function runMigrationsV11ThroughV19(db) {
  ...
  if (readSchemaVersion(db) < 24) {
    db.transaction(() => migrateToV24ActiveGuardiasIndex(db))();
  }
  if (readSchemaVersion(db) < 25) {
    db.pragma('foreign_keys = OFF');
    try {
      db.transaction(() => migrateToV25InterconsultUnder(db))();
    } finally {
      db.pragma('foreign_keys = ON');
    }
  }
  if (readSchemaVersion(db) < 26) {
    db.transaction(() => migrateToV26TeamsSucceedsTeamId(db))();
  }
  if (readSchemaVersion(db) < 27) {
    db.transaction(() => migrateToV27CloudOutbox(db))();
  }
}
