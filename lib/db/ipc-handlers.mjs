...
function migrationPending(userDataPath) {
  const hostJson = path.join(userDataPath, 'lan-squad-host-state.json');
  const hostMigrated = path.join(userDataPath, 'lan-squad-host-state.json.migrated.backup');
  if (fs.existsSync(hostJson) && !fs.existsSync(hostMigrated)) return true;
  const lsExport = path.join(userDataPath, 'clinical-localStorage-export.migrated.backup.json');
  const legacyLs = path.join(userDataPath, 'clinical-localStorage-export.json');
  if (fs.existsSync(legacyLs) && !fs.existsSync(lsExport)) return true;
  return false;
}
...
export function registerDbIpcHandlers({
  ...
  ipcMain.handle('db:status', async () => {
    ...
    return {
      ...
      migrationPending: migrationPending(userDataPath()),
