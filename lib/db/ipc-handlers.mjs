...
import { canManageInternoQr, canViewLanUserDirectory } from './clinical-privileges.mjs';
...
export function registerDbIpcHandlers({
  ...
  ipcMain.handle('db:guardia-resolve', async (_e, payload = {}) => {
    try {
  ...
  ipcMain.handle('db:clinical-teams-update', async (_e, payload = {}) => {
    try {
      const result = await dbManager.withTransaction((db) =>
  ...
  ipcMain.handle('db:clinical-teams-archive', async (_e, payload = {}) => {
    try {
      const result = await dbManager.withTransaction((db) =>
  ...
  ipcMain.handle('db:clinical-team-resolve-code', async (_e, payload = {}) => {
    try {
  ...
  ipcMain.handle('db:clinical-user-lookup', async (_e, payload = {}) => {
    try {
  ...
  ipcMain.handle('db:clinical-users-list', async (_e, payload = {}) => {
    try {
  ...
  ipcMain.handle('db:clinical-user-delete', async (_e, payload = {}) => {
    try {
      const result = await dbManager.withTransaction((db, { audit }) => {
  ...
  ipcMain.handle('db:clinical-user-touch', async (_e, payload = {}) => {
    try {
      const userId = String(payload.userId || '').trim();
      if (!userId) return { ok: false, error: 'userId requerido' };
      const touched = await dbManager.withTransaction((db) =>
        touchClinicalUserActivity(db, userId)
      );
  ...
  ipcMain.handle('db:patient-active-team-id', async (_e, payload = {}) => {
    try {
  ...
  ipcMain.handle('db:interno-access-list', async (_e, payload = {}) => {
    try {
      const rows = await dbManager.withTransaction((db) => {
        const profile = getClinicalProfile(db, String(payload.userId || ''));
  ...
  ipcMain.handle('db:interno-access-rotate', async (_e, payload = {}) => {
    try {
      const row = await dbManager.withTransaction((db, { audit }) => {
        const userId = String(payload.userId || '');
        const profile = getClinicalProfile(db, userId);
        if (!canManageInternoQr(profile)) {
          throw new Error('Sin permisos para gestionar QR de internos.');
        }
  ...
  ipcMain.handle('db:interno-access-set-active', async (_e, payload = {}) => {
    try {
      const row = await dbManager.withTransaction((db, { audit }) => {
        const userId = String(payload.userId || '');
        const profile = getClinicalProfile(db, userId);
  ...
  ipcMain.handle('db:entrega-template-list', async (_e, payload = {}) => {
    try {
  ...
  ipcMain.handle('db:entrega-template-save-user', async (_e, payload = {}) => {
    try {
  ...
  ipcMain.handle('db:entrega-template-save-team', async (_e, payload = {}) => {
    try {
      const template = await dbManager.withTransaction((db) =>
  ...
  ipcMain.handle('db:entrega-template-delete', async (_e, payload = {}) => {
    try {
  ...
  ipcMain.handle('db:lan-outbox-enqueue', async (_e, payload = {}) => {
    try {
  ...
  ipcMain.handle('db:lan-outbox-drain', async (_e, payload = {}) => {
    try {
  ...
  ipcMain.handle('db:lan-outbox-count', async (_e, payload = {}) => {
    try {
