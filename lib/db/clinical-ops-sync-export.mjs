...
export function filterClinicalOpsSnapshotForSala(db, snapshot, sala) {
  ...
  // Removals/rejoins need clinical_users for peer @username → user_id remap.
  ...
  // — invisible in every sala-scoped list.
  for (const team of teams) {
    for (const uid of [team?.created_by, team?.leader_user_id]) {
      const id = String(uid || '').trim();
      if (id) memberUserIds.add(id);
    }
  }

  const active_guardias = (snapshot.active_guardias || []).filter((row) =>
    memberUserIds.has(String(row.covering_user_id || '').trim())
  );
  const activeCoveringIds = new Set(
    active_guardias.map((row) => String(row.covering_user_id || '').trim()).filter(Boolean)
  );

  const clinical_users = (snapshot.clinical_users || []).filter((row) => {
    const uid = String(row.user_id || '').trim();
    return memberUserIds.has(uid) || activeCoveringIds.has(uid);
  });
  ...
  return {
    ...
    active_guardias,
    active_guardias_resolved: filterRowsByTeamId(snapshot.active_guardias_resolved, teamIds),
    clinical_users,
  };
}
...
export function exportClinicalOpsSnapshot(db) {
  ...
  return {
    ...
    team_membership,
    active_guardias,
    active_guardias_resolved: getResolvedGuardiasMeta(db),
    clinical_users,
