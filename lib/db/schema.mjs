...
export function applyMigrations(db) {
  ...
  if (readSchemaVersion(db) < 15) {
    const runV15 = db.transaction(() => {
      migrateToV15LanHostTables(db);
    });
    runV15();
  }
  if (readSchemaVersion(db) < 16) {
    const runV16 = db.transaction(() => {
      migrateToV16UserLastActivity(db);
    });
    runV16();
  }
  if (readSchemaVersion(db) < 17) {
    const runV17 = db.transaction(() => {
