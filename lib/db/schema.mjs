...
export function applyMigrations(db) {
  ...
  const migrate = db.transaction(() => {
    ...
    // Task 1: Add clinical access control tables and columns
    db.exec(`
    ...
    // Ensure patients table exists (for test compatibility)
    const patientTableExists = db.prepare(
    ...
    if (!patientCols.includes('interconsult_status')) {
      db.exec("ALTER TABLE patients ADD COLUMN interconsult_status TEXT DEFAULT 'Pending' CHECK(interconsult_status IN ('Pending', 'Resolved', 'Active'))");
    }
