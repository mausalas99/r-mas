...
export function exportClinicalOpsSnapshot(db) {
  ...
  const clinical_users = db
    ...
    .filter(
      (row) =>
        !deletedIds.has(String(row?.user_id || '')) &&
...
export function mergeClinicalOpsSnapshot(db, incoming, localSnapshot = null) {
  ...
  mergeTeamMembership(
    db,
    filterIncomingTeamMembership(incoming.team_membership || [], deletedSet, removalKeys)
  );
 ...
 */
function filterIncomingTeamMembership(rows, deletedSet, removalKeys) {
