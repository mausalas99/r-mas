...
/** Blocks self-join to staged (incoming) teams; R4 can still assign via addTeamMember. */
export function validateSelfJoinTeamMembership(db, { userId, teamId, teamSala }) {
  ...
  if (team && !team.archived_at && Number(team.rotation_active) === 0) {
    errors.push(
      'Este equipo aún no está activo. Espera a que R4 inicie la nueva rotación o pide que te asignen desde el directorio.'
    );
  }
  return errors;
}
 ...
 * @returns {string[]}
 */
export function getSalaTeamMembershipWarnings(db, { userId: _userId, teamId, teamSala, rank: _rankOverride }) {
  const warnings = [];
  const sala = resolveTeamSalaForMembership(db, teamId, teamSala);

  // Soft hint only on number of teams in a sala — not on member counts.
