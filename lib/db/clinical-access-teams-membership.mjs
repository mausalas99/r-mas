...
export function validateSalaTeamMembership(db, { userId, teamId, teamSala: _teamSala }) {
  ...
  // No per-user / per-rank team caps — any number of members may share a team.
  void db;
  return errors;
}

/** Blocks self-join to staged (incoming) teams; Admin can still assign via addTeamMember. */
export function validateSelfJoinTeamMembership(db, { userId, teamId, teamSala }) {
  const errors = validateSalaTeamMembership(db, { userId, teamId, teamSala });
  if (errors.length) return errors;
  const team = getTeamById(db, teamId);
  if (team && !team.archived_at && Number(team.rotation_active) === 0) {
    errors.push(
      'Este equipo aún no está activo. Espera a que Admin inicie la nueva rotación o pide que te asignen desde el directorio.'
    );
  }
  return errors;
}
...
function resolveMemberSubAreaFraction(_db, _team, _userId, opts) {
  return { fraction: opts.subAreaFraction ? String(opts.subAreaFraction).trim() : '' };
}
 ...
 */
export function removeOtherTeamMemberships(db, userId, keepTeamId) {
  const uid = String(userId || '');
  const keep = String(keepTeamId || '');
  if (!uid) return 0;
  const rows = db
    .prepare(
      `SELECT team_id FROM team_membership WHERE user_id = ? AND team_id != ?`
    )
    .all(uid, keep);
...
export function addTeamMember(db, teamId, userId, opts = {}) {
  ...
  // Default: one team per user — leave others so members can be moved/reassigned.
  let movedFrom = 0;
  if (opts.exclusive !== false) {
    movedFrom = removeOtherTeamMemberships(db, uid, tid);
  }
  ...
  const { fraction } = resolveMemberSubAreaFraction(db, team, uid, opts);
