...
test('searchByRegistro GETs index then POSTs Buscar with REGISTRO mode', async () => {
  const calls = [];
  ...
  assert.equal(calls[1].url, LAB_REPO_BASE_URL);

  const postParams = new URLSearchParams(calls[1].body);
  assert.equal(postParams.get('Drop1'), LAB_REPO_SEARCH_MODE_REGISTRO);
  assert.equal(postParams.get('TextBox2'), '2203912-1');
  assert.equal(postParams.get('Button1'), 'Buscar');
  assert.ok(postParams.get('__VIEWSTATE'));
  assert.ok(postParams.get('__EVENTVALIDATION'));
  assert.equal(postParams.get('__VIEWSTATEENCRYPTED'), '');
