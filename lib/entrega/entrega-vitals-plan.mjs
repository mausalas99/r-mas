...
export function normalizeFrequencySpec(raw) {
  if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
    const o = /** @type {{ mode?: string, hours?: number, timesPerShift?: number, untilTime?: string|null }} */ (
      raw
    );
  ...
  if (STRUCTURED_DB.has(t) && t.endsWith('h')) {
    return { mode: 'interval', hours: clampHours(Number(t.replace('h', ''))) };
  }
...
/** @param {VitalsFrequencySpec|unknown} spec */
export function frequencyDisplayLabel(spec) {
  const norm = normalizeFrequencySpec(spec);
  if (isVitalsFrequencyPaused(norm)) {
    return `Finalizado${norm.untilTime ? ` (${norm.untilTime})` : ''}`;
  }
  if (norm.mode === 'routine') return 'Rutina / según evolución';
