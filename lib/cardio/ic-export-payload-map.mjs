...
/**
 * Same narrow NT-proBNP regex as `consulta-ic-data.mjs`'s
 * `extractNtProBnpFromRow` / `dashboard-cardio-glance.mjs`'s copy — kept
 * local here (rather than a shared import) since none of those modules
 * export it and this file already owns the `p.labHistory` raw-rows parsing
 * (`pushLabHistorySet` above).
 * @param {unknown} row
 * @returns {string|null}
 */
function extractNtProBnpFromRow(row) {
  const s = String(row || '');
  const m = /NT[\s-]?PRO\s*BNP\D{0,12}([\d.,]+)/i.exec(s);
  return m ? m[1] : null;
}

/**
 * Último NT-proBNP importado (Part C, Phase 7) — same "último valor" the
 * Resumen dashboard chip shows, scanned from `p.labHistory`'s raw resLabs
 * rows (kept in chronological order, oldest first; last match wins).
...
export function resolveNtProBnpUltimo(labHistory) {
  ...
  for (const set of sets) {
    const rows = set && typeof set === 'object' && Array.isArray(set.resLabs) ? set.resLabs : [];
    for (const row of rows) {
      const val = extractNtProBnpFromRow(row);
...
/**
 * One printable line per `cardio.eventualidadesSeguimiento[]` entry for the
 * docx template's fixed `ORIG_EVENTOS` slots — headlines date + VExUS grado
 * + FEVI + impresión (truncated), since the template has room for one line
 * per entry, not the full structured record (still fully visible in-app).
 * @param {any[]} entries
 */
export function collectEventualidadesSeguimientoLines(entries) {
  const list = Array.isArray(entries) ? entries.slice() : [];
  list.sort((a, b) => String((a && a.date) || '').localeCompare(String((b && b.date) || '')));
  return list
    .map((e) => {
      if (!e) return '';
      const prefix = e.date ? formatShortDate(e.date) : '';
      const vexus = e.vexusInicial && e.vexusInicial.grado != null && e.vexusInicial.grado !== '' ? `VExUS ${e.vexusInicial.grado}` : '';
      const fevi = e.feviEstimadaInicial != null && e.feviEstimadaInicial !== '' ? `FEVI ${e.feviEstimadaInicial}%` : '';
      const impresion = e.impresionDiagnostica ? `Impresión: ${truncate(e.impresionDiagnostica, 80)}` : '';
      const parts = [vexus, fevi, impresion].filter(Boolean);
