...
describe('buildIcExportPayload', () => {
  it('maps Ma.Elena-shaped fixture: identity, days, VExUS/congestion as-of, events', () => {
    const patient = {
      ...
      cardio: {
        ...
        eventualidadesSeguimiento: [
          { date: '2026-03-14', vexusInicial: { grado: 2 }, impresionDiagnostica: 'Respuesta a diurético; IC Neumología.' },
          { date: '2026-03-19', vexusInicial: { grado: 0 }, impresionDiagnostica: 'Alta planificada; cita IC en 7 días.' },
        ],
        pocusByDay: [
          {
            date: '2026-03-13',
  ...
  it('empty patient yields blank-safe defaults without throwing', () => {
    ...
    assert.equal(payload.dispositivo, '');
    assert.equal(payload.nyhaActual, '');
    assert.equal(payload.ntProBnpUltimo, '');
  });

  it('Part C, Phase 7: maps dispositivo/NYHA actual/NT-proBNP último, in sync with the Resumen chips', () => {
    const patient = {
      cardio: {
        device: { colocado: true, tipo: 'TRC-D' },
        scores: [
          { date: '2026-03-13', nyha: 'II' },
          { date: '2026-03-19', nyha: 'III' },
        ],
      },
      labHistory: [{ resLabs: ['CARD: NT-PROBNP 2100 pg/mL'] }],
    };
    const payload = buildIcExportPayload(patient, { asOfDate: '2026-03-19' });
    assert.equal(payload.dispositivo, 'TRC-D');
    assert.equal(payload.nyhaActual, 'III');
    assert.equal(payload.ntProBnpUltimo, '2100');
  });
});
