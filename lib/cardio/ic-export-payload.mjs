...
import {
  ...
  collectEventualidadesSeguimientoLines,
  collectLabsLines,
...
function assembleIcPayload(p, cardio, asOfDate, ingresoDate, computed) {
  ...
  return Object.assign(
    ...
    medEventLabFields(p, cardio, diureticSegs, activeDiuretics),
  );
}

/**
 * Compact dispositivo label — mirrors `dashboard-cardio-glance.mjs`'s
 * `deviceChipLabel()` and `seguimiento-sheets.mjs`'s `resolveDispositivoLabel()`
 * so the docx export and the Resumen chip never diverge.
 * @param {any} cardio
 */
function resolveDispositivoLabel(cardio) {
  const d = normalizeDevice(cardio.device);
...
function identityFields(p, cardio, asOfDate, ingresoDate) {
  const scoreActual = latestTwoScores(cardio.scores).actual;
  return {
    asOfDate,
    ...
    dispositivo: resolveDispositivoLabel(cardio),
    nyhaActual: scoreActual && scoreActual.nyha ? String(scoreActual.nyha) : '',
    ntProBnpUltimo: resolveNtProBnpUltimo(p.labHistory),
  };
}
 ...
 */
function medEventLabFields(p, cardio, diureticSegs, activeDiuretics) {
  return {
    ...
    eventosLines: collectEventualidadesSeguimientoLines(cardio.eventualidadesSeguimiento),
