...
test('buildSeguimientoFullTsv produces cohort + diario blocks', () => {
  const patient = {
    id: 'p2',
    nombre: 'Demo',
    fimiFecha: '2026-03-13',
    cardio: {
      inicioDescongestion: '2026-03-13',
      overrides: {},
      pocusByDay: [{ date: '2026-03-13', vexus: 1, congestionScore: 2, note: 'test' }],
      diureticSegments: [],
      medSegments: [],
      fantasticos: [],
      medCatalog: [],
  ...
  const tsv = buildSeguimientoFullTsv(patient, { asOfDate: '2026-03-13' });
  assert.match(tsv, /SEGUIMIENTO COHORTE/);
  assert.match(tsv, /SEGUIMIENTO HOSPITALIZACIÓN DIARIO/);
  assert.match(tsv, /VExUS/);
});
