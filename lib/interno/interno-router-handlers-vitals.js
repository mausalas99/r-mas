 ...
 * @param {string} sala
 */
async function applyInternoVitalsToStore(ctx, patientId, body, sala) {
 ...
 * @param {string} patientId
 * @param {object} nextPatient
 * @param {boolean} hasAlterations
 */
function emitInternoVitalsSync(ctx, sala, patientId, nextPatient, hasAlterations) {
  const syncPayload = {
...
function createInternoVitalsPostHandler(ctx) {
  return async function handleInternoVitalsPost(req, res) {
    try {
      ...
      const applied = await applyInternoVitalsToStore(ctx, patientId, req.body, req.internoSala);
...
module.exports = { createInternoVitalsPostHandler, emitInternoVitalsSync };
