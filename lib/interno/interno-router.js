...
function createInternoRouter(deps) {
  ...
  async function readBoard(sala) {
    ...
    const activeGuardias = db
      ...
      .all();
    const r1Ids = mod.resolveSalaR1GuardiaUserIds(scope, normalized);
    const state = deps.store.getState();
    const patients = mod.filterInternoScopePatients(
      state.patients || [],
      activeGuardias,
      normalized,
      r1Ids
  ...
  r.post('/vitals', postLimiter, express.json({ limit: '64kb' }), authInterno, async (req, res) => {
    try {
      ...
      const state = deps.store.getState();
      const cur = (state.patients || []).find((p) => String(p.id) === patientId);
      if (!cur) return res.status(404).json({ error: 'patient_not_found' });
      ...
      const updated = deps.store.upsertPatient(nextPatient, Number(cur.version || 0));

      const db = deps.getDb?.();
      if (db) mod.touchActiveGuardiaVitalsCheck(db, patientId);

      if (typeof deps.broadcastSync === 'function') {
        deps.broadcastSync('sync', { type: 'patients-updated' });
      }
