...
export function compareInternoBoardRowsByVitalsFrequency(a, b) {
  const aSignos = !!a.signosPending;
  const bSignos = !!b.signosPending;
  if (aSignos !== bSignos) return aSignos ? -1 : 1;

  const aMs = aSignos ? a.vitals?.frequencyMs ?? Number.POSITIVE_INFINITY : Number.POSITIVE_INFINITY;
  const bMs = bSignos ? b.vitals?.frequencyMs ?? Number.POSITIVE_INFINITY : Number.POSITIVE_INFINITY;
  if (aMs !== bMs) return aMs - bMs;

  const aCls = VITALS_CLS_SORT_RANK[a.vitals?.cls || ''] ?? 2;
  const bCls = VITALS_CLS_SORT_RANK[b.vitals?.cls || ''] ?? 2;
