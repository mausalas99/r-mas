...
/**
 * Board rows read vitals recency straight from monitoreo now (the phone already
 * decrypted it) instead of clinicalOps' last_vitals_check, which the Worker can no
 * longer bump server-side once clinicalOps is E2EE — and which had no callers even
 * before that, so this is a working replacement, not a regression.
...
function buildInternoBoardRow(p, g) {
  ...
  const vitals = calcVitalsBannerForSpec(resolveLastVitalsCheck(p, g), vitalsFreqSpec);
  const bed =
