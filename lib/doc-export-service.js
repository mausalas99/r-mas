...
async function exportRecetaHuPdf({ patient, receta, doctorName, cedulaProfesional, outputDir }, paths) {
  if (!patient) throw badRequest('Missing patient');
  const dest = resolveExportDir(outputDir, paths);
  const payload = Object.assign({}, receta || {}, {
    patient,
    doctorName: doctorName || '',
    cedulaProfesional: cedulaProfesional || '',
  });
  const buf = await fillRecetaHuPdf(payload, getScriptsDir());
